...
def check_shape(interpolator_cls, x_shape, y_shape, deriv_shape=None, axis=0, extra_args=None):
...
class TestKrogh:
    ...
    def test_wrapper(self):
        ...
        assert_almost_equal(
            P.derivatives(self.test_xs, 2), ki(self.xs, self.ys, self.test_xs, der=[0, 1])
        )
    ...
    def test_int_inputs(self):
        ...
        xp_assert_close(
            abs((f(x) - offset_cdf) / f.derivative(x, 1)), np.zeros_like(offset_cdf), atol=1e-10
        )
...
class TestBarycentric:
    ...
    def test_wrapper(self):
        ...
        xp_assert_close(
            P.derivative(self.test_xs, 2), bi(self.xs, self.ys, self.test_xs, der=2, rng=1)
        )
        xp_assert_close(
            P.derivatives(self.test_xs, 2), bi(self.xs, self.ys, self.test_xs, der=[0, 1], rng=1)
        )
...
class TestPCHIP:
    ...
    def test_cast(self):
        ...
        data = np.array(
            [[0, 4, 12, 27, 47, 60, 79, 87, 99, 100], [-33, -33, -19, -2, 12, 26, 38, 45, 53, 55]]
        )
        xx = np.arange(100)
...
class TestCubicSpline:
    ...
    def check_correctness(S, bc_start="not-a-knot", bc_end="not-a-knot", tol=1e-14):
        ...
        xp_assert_close(
            c[2, 1:], 3 * c[0, :-1] * dxi**2 + 2 * c[1, :-1] * dxi + c[2, :-1], rtol=tol, atol=tol
        )
        ...
        if bc_end == "not-a-knot":
            ...
            xp_assert_close(S(x[-1], 2), np.zeros_like(S(x[-1], 2)), rtol=2 * tol, atol=2 * tol)
        else:
            order, value = bc_end
