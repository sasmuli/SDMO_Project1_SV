...
class NDFrame(PandasObject, indexing.IndexingMixin):
        ...
        if dtype is not None:
            ...
            if dtype.kind == "V":
                raise NotImplementedError(
                    "compound dtypes are not implemented " f"in the {cls.__name__} constructor"
                )
    ...
    def squeeze(self, axis: Axis | None = None):
        ...
        result = self.iloc[
            tuple(0 if i in axes and len(a) == 1 else slice(None) for i, a in enumerate(self.axes))
        ]
    ...
    def bool(self) -> bool_t:
        ...
        if isinstance(v, (bool, np.bool_)):
            ...
            raise ValueError(
                "bool cannot act on a non-boolean single element " f"{type(self).__name__}"
            )
    ...
    def _is_label_reference(self, key: Level, axis: Axis = 0) -> bool_t:
        ...
        return (
            key is not None and is_hashable(key) and any(key in self.axes[ax] for ax in other_axes)
        )
    ...
    def to_latex(
        ...
        if isinstance(formatters, list):
            formatters_ = {
                c: partial(_wrap, alt_format_=formatters[i]) for i, c in enumerate(self.columns)
            }
        ...
        render_kwargs_ = {
            ...
            "clines": (
                "skip-last;data" if (multirow and isinstance(self.index, MultiIndex)) else None
            ),
            "bold_rows": bold_rows,
        }
    ...
    def take(self, indices, axis: Axis = 0, **kwargs) -> Self:
        ...
        **kwargs
        ...
        if not isinstance(indices, slice):
            ...
            raise TypeError(
                f"{type(self).__name__}.take requires a sequence of integers, " "not slice."
            )
        else:
            warnings.warn(
                # GH#51539
            ...
            indices = np.arange(indices.start, indices.stop, indices.step, dtype=np.intp)
    ...
    def _getitem_slice(self, key: slice) -> Self:
        ...
        if isinstance(slobj, np.ndarray):
            ...
            indexer = lib.maybe_indices_to_slice(slobj.astype(np.intp, copy=False), len(self))
    ...
    def astype(self, dtype, copy: bool_t | None = None, errors: IgnoreRaise = "raise") -> Self:
        ...
        if is_dict_like(dtype):
            ...
            results = [ser.astype(dtype, copy=copy, errors=errors) for _, ser in self.items()]
    ...
    @final
    @doc(klass=_shared_doc_kwargs["klass"])
    def tz_convert(self, tz, axis: Axis = 0, level=None, copy: bool_t | None = None) -> Self:
        """
