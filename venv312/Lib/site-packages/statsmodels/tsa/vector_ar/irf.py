        ...
        if plot_stderr is False:
            ...
            if stderr_type == "sz1":
                stderr = self.err_band_sz1(
                    orth=orth, svar=svar, repl=repl, signif=signif, seed=seed, component=component
                )
            if stderr_type == "sz2":
...
class IRAnalysis(BaseIRAnalysis):
    ...
    def errband_mc(self, orth=False, svar=False, repl=1000, signif=0.05, seed=None, burn=100):
        ...
        if svar:
            return model.sirf_errband_mc(
                orth=orth, repl=repl, steps=periods, signif=signif, seed=seed, burn=burn, cum=False
            )
        else:
    ...
    def err_band_sz1(
        self, orth=False, svar=False, repl=1000, signif=0.05, seed=None, burn=100, component=None
    ):
        """
    ...
    def err_band_sz3(
        ...
        for p in range(repl):
            ...
            for j in range(neqs):
                for i in range(neqs):
                    gamma[p, 1:, i, j] = (
                        W[j, k[j], i * periods : (i + 1) * periods] * irf_resim[p, 1:, i, j]
                    )
                    if i == neqs - 1:
    ...
    def cum_errband_mc(self, orth=False, repl=1000, signif=0.05, seed=None, burn=100):
        ...
        periods = self.periods
        return model.irf_errband_mc(
            orth=orth, repl=repl, steps=periods, signif=signif, seed=seed, burn=burn, cum=True
        )
