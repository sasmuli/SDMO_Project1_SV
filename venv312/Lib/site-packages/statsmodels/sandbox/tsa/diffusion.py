...
class OUprocess(AffineDiffusion):
    ...
    def exact(self, xzero, t, normrvs):
        ...
        return (
            xzero * expnt
            + self.mu * (1 - expnt)
            + self.sigma * np.sqrt((1 - expnt * expnt) / 2.0 / self.lambd) * normrvs
        )
    ...
    def exactprocess(self, xzero, nobs, ddt=1.0, nrepl=2):
        ...
        inc = (
            self.mu * (1 - expddt)
            + self.sigma * np.sqrt((1 - expddt * expddt) / 2.0 / self.lambd) * normrvs
        )
...
class SchwartzOne(ExactDiffusion):
    ...
    def fitls(self, data, dt):
        ...
        const, slope = parest
        errvar = res / (nobs - 2.0)  # check denominator estimate, of sigma too low
