...
class StylerRenderer:
    ...
    def format_index(
        ...
        if not isinstance(formatter, dict):
            ...
            formatter = {
                obj._get_level_number(level): formatter_ for level, formatter_ in formatter.items()
            }
...
def maybe_convert_css_to_tuples(style: CSSProperties) -> CSSList:
    ...
    if isinstance(style, str):
        ...
        try:
            return [
                (x.split(":")[0].strip(), x.split(":")[1].strip()) for x in s if x.strip() != ""
            ]
        except IndexError:
            raise ValueError(
        ...
        if self.table_styles:
            ...
            for row in d["body"]:
                for item in row:
                    if item["type"] == "td":
                        item["display_value"] = (
                            str(item["display_value"]) + f'<span class="{self.class_name}"></span>'
                        )
