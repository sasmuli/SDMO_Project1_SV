...
def _min_max_band(args):
    ...
    if have_de_optim and not use_brute:
        ...
        max_ = brute(
            _curve_constrained, ranges=bounds, finish=fmin, args=(idx, -1, band, pca, ks_gaussian)
        )
...
def fboxplot(
    data, xdata=None, labels=None, depth=None, method="MBD", wfactor=1.5, ax=None, plot_opts=None
):
    """
    ...
    ax.fill_between(
        xdata, lower_nonout, upper_nonout, color=plot_opts.get("c_outer", (0.75, 0.75, 0.75))
    )
    ...
    # Plot median curve
    ax.plot(
        xdata, median_curve, color=plot_opts.get("c_median", "k"), lw=plot_opts.get("lw_median", 2)
    )
