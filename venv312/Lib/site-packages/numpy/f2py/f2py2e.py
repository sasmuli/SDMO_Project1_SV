...
def scaninputline(inputline):
    ...
    if signsfile and os.path.isfile(signsfile) and "h-overwrite" not in options:
        errmess(f'Signature file "{signsfile}" exists!!! Use --overwrite-signature to overwrite.\n')
        sys.exit()
            ...
            if "use" in module:
                for u in module["use"].keys():
                    if u in isusedby and u in mnames:
                        ...
                        outmess(
                            f'\tModule "{name}" uses nonexisting "{u}" ' "which will be ignored.\n"
                        )
            ret[name] = {}
...
def run_main(comline_list):
    ...
    for plist in postlist:
        if plist["block"] != "python module":
            ...
            raise TypeError(
                "All blocks must be python module blocks but got %s" % (repr(plist["block"]))
            )
...
def f2py_parser():
    ...
    parser.add_argument(
        "--freethreading-compatible", dest="ftcompat", action=argparse.BooleanOptionalAction
    )
    return parser
