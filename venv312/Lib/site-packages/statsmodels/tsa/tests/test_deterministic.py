...
def test_time_trend_smoke(index, forecast_index):
    ...
    if (
        ...
        or (type(index) is pd.Index and max(index) > 2**63 and forecast_index is None)
    ):
        warn = UserWarning
...
def test_additional_terms(time_index):
    ...
    with pytest.raises(ValueError, match="One or more terms in additional_terms"):
        DeterministicProcess(time_index, constant=True, order=1, additional_terms=add_terms)
