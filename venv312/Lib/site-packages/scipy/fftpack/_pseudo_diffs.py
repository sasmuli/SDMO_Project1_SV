...
def cs_diff(x, a, b, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return cs_diff(tmp.real, a, b, period, _cache) + 1j * cs_diff(
            tmp.imag, a, b, period, _cache
        )
    if period is not None:
        a = a * 2 * pi / period
        b = b * 2 * pi / period
...
def sc_diff(x, a, b, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return sc_diff(tmp.real, a, b, period, _cache) + 1j * sc_diff(
            tmp.imag, a, b, period, _cache
        )
    if period is not None:
        a = a * 2 * pi / period
...
def ss_diff(x, a, b, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return ss_diff(tmp.real, a, b, period, _cache) + 1j * ss_diff(
            tmp.imag, a, b, period, _cache
        )
    if period is not None:
        a = a * 2 * pi / period
...
def cc_diff(x, a, b, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return cc_diff(tmp.real, a, b, period, _cache) + 1j * cc_diff(
            tmp.imag, a, b, period, _cache
        )
    if period is not None:
        a = a * 2 * pi / period
