...
def tfqmr(A, b, x0=None, *, rtol=1e-5, atol=0.0, maxiter=None, M=None, callback=None, show=False):
    """
    ...
    for iter in range(maxiter):
        ...
        if tau * np.sqrt(iter + 1) < atol:
            if show:
                print("TFQMR: Linear solve converged due to reach TOL " f"iterations {iter+1}")
            return (postprocess(x), 0)
    ...
    if show:
        print("TFQMR: Linear solve not converged due to reach MAXIT " f"iterations {iter+1}")
