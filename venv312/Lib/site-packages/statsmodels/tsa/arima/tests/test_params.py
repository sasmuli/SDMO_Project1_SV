...
def test_init():
    ...
    assert_equal(p.param_names, ["a", "ar.L1", "ma.L1", "ar.S.L4", "ma.S.L4", "sigma2"])
...
def test_set_params_single_nonconsecutive():
    ...
    exog = pd.DataFrame([[0]], columns=["a"])
    spec = specification.SARIMAXSpecification(
        exog=exog, order=([0, 1], 1, [0, 1]), seasonal_order=([0, 1], 1, [0, 1], 4)
    )
    p = params.SARIMAXParams(spec=spec)
...
def test_set_params_multiple():
    ...
    assert_equal(
        p.reduced_ar_poly.coef, [1, 3, -4, 0, 7, (-3 * -7), (4 * -7), 0, -8, (-3 * 8), 4 * 8]
    )
    # (1 + a_1 L + a_2 L^2) (1 + b_1 L^4 + b_2 L^8) =
    #     (1 + b_1 L^4 + b_2 L^8) +
    #     (a_1 L + a_1 b_1 L^5 + a_1 b_2 L^9) +
    ...
    #     a_2 b_1 L^6 + b_2 L^8 + a_1 b_2 L^9 + a_2 b_2 L^10
    assert_equal(
        p.reduced_ma_poly.coef, [1, -5, 6, 0, -9, (-5 * -9), (6 * -9), 0, 10, (-5 * 10), (6 * 10)]
    )
...
def test_set_poly_short_lags():
    ...
    assert_raises(ValueError, p.__setattr__, "seasonal_ar_poly", [2, 0, 0, 0, -0.2])
    assert_raises(ValueError, p.__setattr__, "seasonal_ma_poly", [2, 0, 0, 0, 0.1])
    # Too short
    ...
    assert_raises(ValueError, p.__setattr__, "seasonal_ar_poly", [1, 0, 0, 0, 0.1, 0])
    assert_raises(ValueError, p.__setattr__, "seasonal_ma_poly", [1, 0, 0, 0, 0.1, 0])
    # Number in invalid location (only for seasonal polynomials)
    assert_raises(ValueError, p.__setattr__, "seasonal_ar_poly", [1, 1, 0, 0, 0, -0.2])
    assert_raises(ValueError, p.__setattr__, "seasonal_ma_poly", [1, 1, 0, 0, 0, 0.1])
...
def test_repr_str():
    ...
    assert_equal(
        repr(p),
        "SARIMAXParams(exog=[nan nan], ar=[nan], ma=[nan],"
    ...
    assert_equal(
        repr(p),
        "SARIMAXParams(exog=[1. 2.], ar=[nan], ma=[nan],"
        " seasonal_ar=[nan], seasonal_ma=[nan], sigma2=nan)",
    )
    ...
    assert_equal(
        repr(p),
        "SARIMAXParams(exog=[1. 2.], ar=[0.5], ma=[nan],"
        " seasonal_ar=[nan], seasonal_ma=[nan], sigma2=nan)",
    )
    ...
    assert_equal(
        repr(p),
        "SARIMAXParams(exog=[1. 2.], ar=[0.5], ma=[0.2],"
        " seasonal_ar=[nan], seasonal_ma=[nan], sigma2=nan)",
    )
