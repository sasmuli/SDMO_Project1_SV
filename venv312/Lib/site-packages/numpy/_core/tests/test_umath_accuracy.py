        ...
        with np.errstate(all="ignore"):
            ...
            for filename in files:
                ...
                with open(filepath) as fid:
                    file_without_comments = (r for r in fid if r[0] not in ("$", "#"))
                    data = np.genfromtxt(
                    ...
                    for datatype in np.unique(data["type"]):
                        ...
                        inval = np.array(
                            str_to_float(
                                data_subset["input"].astype(str), data_subset["type"].astype(str)
                            ),
                            dtype=eval(datatype),
                        )
                        outval = np.array(
                            str_to_float(
                                data_subset["output"].astype(str), data_subset["type"].astype(str)
                            ),
                            dtype=eval(datatype),
                        )
