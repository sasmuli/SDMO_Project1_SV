...
class MultiIndex(Index):
    ...
    def _engine(self):
        ...
        sizes = np.ceil(
            np.log2([len(level) + libindex.multiindex_nulls_shift for level in self.levels])
        )
    ...
    def _get_level_number(self, level) -> int:
        ...
        try:
            ...
            # Note: levels are zero-based
                ...
                raise IndexError(
                    f"Too many levels: Index has only {self.nlevels} levels, " f"not {level + 1}"
                ) from err
        return level
    ...
    def append(self, other):
        ...
        if all((isinstance(o, MultiIndex) and o.nlevels >= self.nlevels) for o in other):
            ...
            for i in range(self.nlevels):
                ...
                level_codes = [
                    recode_for_categories(mi.codes[i], mi.levels[i], level_values, copy=False)
