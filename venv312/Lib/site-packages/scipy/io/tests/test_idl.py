...
class TestStructures:
    ...
    def test_arrays_replicated(self):
        ...
        for i in range(5):
            ...
            assert_array_identical(
                s.arrays_rep.c[i], np.array([np.complex64(1 + 2j), np.complex64(7 + 8j)])
            )
            assert_array_identical(
                s.arrays_rep.d[i], np.array([b"cheese", b"bacon", b"spam"], dtype=object)
            )
        ...
        for i in range(4):
            for j in range(3):
                for k in range(2):
                    ...
                    assert_array_identical(
                        s.arrays_rep.b[i, j, k], np.array([4.0, 5.0, 6.0, 7.0], dtype=np.float32)
                    )
                    assert_array_identical(
...
class TestPointers:
    ...
    def test_pointers(self):
        ...
        assert_identical(
            s.c64_pointer1, np.complex128(1.1987253647623157e112 - 5.1987258887729157e307j)
        )
        assert_identical(
            s.c64_pointer2, np.complex128(1.1987253647623157e112 - 5.1987258887729157e307j)
        )
    ...
    def test_arrays_replicated_3d(self):
        ...
        for i in range(4):
            for j in range(3):
                for k in range(2):
                    assert_array_identical(
                        s.arrays_rep.g[i, j, k], np.repeat(np.float32(4.0), 2).astype(np.object_)
                    )
                    assert_array_identical(
                        s.arrays_rep.h[i, j, k], np.repeat(np.float32(4.0), 3).astype(np.object_)
                    )
...
def test_invalid_pointer():
    ...
    assert_(
        str(w[0].message)
        == ("Variable referenced by pointer not found in " "heap: variable will be set to None")
    )
