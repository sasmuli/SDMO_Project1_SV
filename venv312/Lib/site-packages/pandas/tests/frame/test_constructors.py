...
class TestDataFrameConstructors:
    ...
    def test_dict_nocopy(
        ...
        def check_views(c_only: bool = False):
            ...
            assert (
                sum(get_base(x) is a for x in df._mgr.arrays if isinstance(x.dtype, np.dtype)) == 1
            )
            assert (
                sum(get_base(x) is b for x in df._mgr.arrays if isinstance(x.dtype, np.dtype)) == 1
            )
