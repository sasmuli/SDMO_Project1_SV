...
class FactorModelUnivariate:
    ...
    def fit_find_nfact(self, maxfact=None, skip_crossval=True, cv_iter=None):
        ...
        if (maxfact + hasconst) < 1:
            raise ValueError(
                "nothing to do, number of factors (incl. constant) should " + "be at least 1"
            )
        ...
        for k in range(1, maxfact + hasconst):  # k includes now the constnat
            ...
            if not skip_crossval:
                ...
                for inidx, outidx in cv_iter:
                    ...
                    prederr2 += (
                        y0[outidx] - res_l1o.model.predict(res_l1o.params, fact[outidx, :])
                    ) ** 2.0
            else:
                prederr2 = np.nan
        ...
        self.best_nfact = np.r_[
            (
                np.argmin(results[:, 1:3], 0),
                np.argmax(results[:, 3], 0),
