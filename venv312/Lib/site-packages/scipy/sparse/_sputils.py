...
def getdtype(dtype, a=None, default=None):
    ...
    if newdtype not in supported_dtypes:
        ...
        raise ValueError(
            f"scipy.sparse does not support dtype {newdtype.name}. "
    ...
    if current_shape is None:
        ...
        if not negative_indexes:
            ...
            if new_size != current_size:
                raise ValueError(
                    f"cannot reshape array of size {current_size}" f" into shape {new_shape}"
                )
            ...
            if remainder != 0:
                ...
                raise ValueError(
                    f"cannot reshape array of size {current_size}" f" into shape {err_shape}"
                )
...
def check_reshape_kwargs(kwargs):
    ...
    if kwargs:  # Some unused kwargs remain
        raise TypeError(
            "reshape() got unexpected keywords arguments: " f"{', '.join(kwargs.keys())}"
        )
    return order, copy
