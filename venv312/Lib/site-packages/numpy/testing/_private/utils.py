    ...
    *,
    ...
    try:
        ...
        if isnumber(x) and isnumber(y):
            ...
            if equal_nan and dt == y.dtype and hasattr(dt, "na_object"):
                ...
                if is_nan or bool_errors:
                    # nan-like NA object
                    flagged = func_assert_same_pos(x, y, func=isnan, hasval=x.dtype.na_object)
        ...
        if cond != True:
            ...
            remarks = [
                f"Mismatched elements: {n_mismatch} / {n_elements} " f"({percent_mismatch:.3g}%)"
            ]
            ...
            with errstate(all="ignore"):
                ...
                with contextlib.suppress(TypeError):
                    ...
                    if getattr(error, "dtype", object_) == object_:
                        remarks.append(
                            "Max absolute difference among violations: " + str(max_abs_error)
                        )
                    else:
                        remarks.append(
                    ...
                    if all(~nonzero_and_invalid):
                        ...
                        max_rel_error = max(nonzero_invalid_error / abs(nonzero_invalid_y))

                    if getattr(error, "dtype", object_) == object_:
                        remarks.append(
                            "Max relative difference among violations: " + str(max_rel_error)
                        )
                    else:
                        remarks.append(
            ...
            err_msg += "\n" + "\n".join(remarks)
            msg = build_err_msg(
                [ox, oy], err_msg, verbose=verbose, header=header, names=names, precision=precision
            )
            raise AssertionError(msg)
    except ValueError:
        import traceback
        ...
        msg = build_err_msg(
            [x, y], err_msg, verbose=verbose, header=header, names=names, precision=precision
        )
...
def assert_allclose(
    actual, desired, rtol=1e-7, atol=0, equal_nan=True, err_msg="", verbose=True, *, strict=False
):
    """
    Raises an AssertionError if two objects are not equal up to desired
    tolerance.
...
def run_threaded(
    func, max_workers=8, pass_count=False, pass_barrier=False, outer_iterations=1, prepare_args=None
):
