...
def test_influence_glm_bernoulli():
    ...
    res = GLM(
        df["constrict"], df[["const", "log_rate", "log_volumne"]], family=families.Binomial()
...
class InfluenceCompareExact:
    ...
    def test_basics(self):
        ...
        assert_allclose(infl0.resid_studentized, infl1.resid_studentized, rtol=1e-12, atol=1e-7)

        cd_rtol = getattr(self, "cd_rtol", 1e-7)
        assert_allclose(
            infl0.cooks_distance[0], infl1.cooks_distance[0], rtol=cd_rtol, atol=1e-14
        )  # very small values possible
        ...
        assert_allclose(infl0.d_fittedvalues, infl1.d_fittedvalues, rtol=5e-9, atol=1e-14)
        assert_allclose(
            infl0.d_fittedvalues_scaled, infl1.d_fittedvalues_scaled, rtol=5e-9, atol=1e-14
        )
...
class TestInfluenceGaussianGLMMLE(InfluenceCompareExact):
    ...
    def setup_class(cls):
        ...
        data = pd.DataFrame(
            np.column_stack((endog, exog)), columns="y const var1 var2".split(), index=labels
        )
...
class TestInfluenceGaussianGLMOLS(InfluenceCompareExact):
    ...
    def test_basics(self):
        ...
        assert_allclose(infl0.resid_studentized, infl1.resid_studentized, rtol=1e-12, atol=1e-7)
        assert_allclose(
            infl0.cooks_distance, infl1.cooks_distance, rtol=1e-7, atol=1e-14
        )  # very small values possible
        ...
        assert_allclose(infl0.d_fittedvalues_scaled, infl1.dffits_internal[0], rtol=1e-9)
...
class TestInfluencePoissonCompare(InfluenceCompareExact):
    ...
    def setup_class(cls):
        df = data_bin
        mod = GLM(
            df["constrict"], df[["const", "log_rate", "log_volumne"]], family=families.Poisson()
        )
