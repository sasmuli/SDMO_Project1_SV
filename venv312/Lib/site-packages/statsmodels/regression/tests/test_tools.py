...
class TestMinimalWLS:
    ...
    def test_equivalence_with_wls(self, check):
        ...
        minres = _MinimalWLS(
            self.endog1, self.exog1, weights=self.weights1, check_endog=check, check_weights=check
        ).fit()
        assert_allclose(res.params, minres.params)
        assert_allclose(res.resid, minres.resid)
        ...
        minres = _MinimalWLS(
            self.endog2, self.exog2, weights=self.weights2, check_endog=check, check_weights=check
        ).fit()
        assert_allclose(res.params, minres.params)
        assert_allclose(res.resid, minres.resid)
    ...
    def test_inf_nan(self, bad_value):
        with pytest.raises(ValueError, match=r"detected in endog, estimation infeasible"):
            endog = self.endog1.copy()
            endog[0] = bad_value
            _MinimalWLS(
                endog, self.exog1, self.weights1, check_endog=True, check_weights=True
            ).fit()

        with pytest.raises(ValueError, match=r"detected in weights, estimation infeasible"):
            weights = self.weights1.copy()
            weights[-1] = bad_value
            _MinimalWLS(
                self.endog1, self.exog1, weights, check_endog=True, check_weights=True
            ).fit()
