...
def _map_cyclic(x, lbound, ubound):
    ...
    if lbound >= ubound:
        raise ValueError(
            "Invalid argument: lbound (%r) should be " "less than ubound (%r)." % (lbound, ubound)
        )
...
class CubicRegressionSpline(object):
    ...
    def memorize_finish(self):
        ...
        if args["df"] is not None:
            ...
            if args["df"] < min_df:
                raise ValueError(
                    "'df'=%r must be greater than or equal to %r." % (args["df"], min_df)
                )
...
def test_te_1smooth():
    ...
    assert np.allclose(cr(x, df=3, constraints="center"), te(cr(x, df=4), constraints="center"))
