...
class TestGEE:
    ...
    def test_missing_formula(self):
        ...
        data0 = pd.DataFrame(
            {"endog": endog, "exog1": exog1, "exog2": exog2, "exog3": exog3, "groups": groups}
        )
    ...
    def test_logistic(self):
        ...
        se = [
            [0.127291720283049, 0.166725808326067, 0.192430061340865, 0.173141068839597],
            [0.127045031730155, 0.165470678232842, 0.192052750030501, 0.173174779369249],
            [0.127240302296444, 0.170554083928117, 0.191045527104503, 0.169776150974586],
        ]
    ...
    def test_linear(self):
        ...
        cf = [
            ...
            [-0.0182920577154767, 0.814898414022467, -1.56194040106201, 0.793499517527478],
        ]
        se = [
            [0.0440733554189401, 0.0479993639119261, 0.0496045952071308, 0.0479467597161284],
            [0.0440369906460754, 0.0480069787567662, 0.049519758758187, 0.0479760443027526],
        ]
    ...
    def test_nested_pandas(self):
        ...
        assert_allclose(
            smry.Variance, np.r_[1.437299, 4.421543, 8.905295, 0.258480], atol=1e-5, rtol=1e-5
        )
    ...
    def test_stationary_grid(self):
        ...
        assert_allclose(cs.covariance_matrix(np.r_[1, 1, 1], 0)[0].sum(), 6.4633538285149452)
    ...
    def test_stationary_nogrid(self):
        ...
        model = gee.GEE(
            endog, exog, group, time=time, cov_struct=cov_struct.Stationary(max_lag=4, grid=False)
        )
        model.fit()
    ...
    def test_predict_exposure(self):
        ...
        data = pd.DataFrame(
            {"Y": Y, "X1": X1, "X2": X2, "groups": groups, "offset": offset, "exposure": exposure}
        )

        fml = "Y ~ X1 + X2"
        model = gee.GEE.from_formula(
            fml, groups, data, family=families.Poisson(), offset="offset", exposure="exposure"
        )
        result = model.fit()
        assert_equal(result.converged, True)
        ...
        pred5 = result.predict(
            exog=data[-10:], offset=data["offset"][-10:], exposure=data["exposure"][-10:]
        )
        # without patsy
    ...
    def test_predict_exposure_lists(self):
        ...
        endog = [
            np.random.poisson(0.1 * (exog_i[1] + exog_i[2]) + offset_i + np.log(exposure_i))
    ...
    def test_offset_formula(self):
        ...
        model5 = gee.GEE.from_formula(
            fml, groups, data, family=families.Poisson(), exposure="exposure", offset="offset"
        )
        result5 = model5.fit()
    ...
    def test_sensitivity(self):
        ...
        # Regression test
        assert_almost_equal(
            [np.asarray(x.params)[0] for x in ps], [0.1696214707458818, 0.17836097387799127]
        )
...
def test_quasipoisson(reg):
    ...
    if not reg:
        ...
        assert_allclose(
            result2.cov_naive / result1.cov_naive, result2.scale * np.ones_like(result2.cov_naive)
        )
