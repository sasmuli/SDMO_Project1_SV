...
def _check_args_2(endog, n_factor, corr, nobs, k_endog):
    ...
    if corr.shape[0] != corr.shape[1]:
        raise ValueError(
            "Correlation matrix corr must be a square " "(rows %d != cols %d)" % corr.shape
        )
...
class Factor(Model):
    ...
    **Experimental**
    ...
    def fit(self, maxiter=50, tol=1e-8, start=None, opt_method="BFGS", opt=None, em_iter=3):
        """
    ...
    def _fit_pa(self, maxiter=50, tol=1e-8):
        ...
        if tol <= 0 or tol > 0.01:
            raise ValueError(
                "tolerance must be larger than 0 and smaller than" " 0.01! Got %f instead" % (tol)
            )
    ...
    def _unpack(self, par):
        return (par[0 : self.k_endog] ** 2, np.reshape(par[self.k_endog :], (-1, self.k_endog)).T)
