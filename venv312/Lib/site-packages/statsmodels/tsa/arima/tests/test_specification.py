...
def check_attributes(
    spec, order, seasonal_order, enforce_stationarity, enforce_invertibility, concentrate_scale
):
    p, d, q = order
...
def check_methods(
    ...
    params = np.r_[
        exog_params, ar_params, ma_params, seasonal_ar_params, seasonal_ma_params, sigma2
    ]
    ...
    if not concentrate_scale:
        params = np.r_[
            exog_params, ar_params, ma_params, seasonal_ar_params, seasonal_ma_params, 0.0
        ]
        assert_raises(ValueError, spec.validate_params, params)
        params = np.r_[
            exog_params, ar_params, ma_params, seasonal_ar_params, seasonal_ma_params, -1
        ]
        assert_raises(ValueError, spec.validate_params, params)
...
@pytest.mark.parametrize(
    ("endog,exog,p,d,q,P,D,Q,s," "enforce_stationarity,enforce_invertibility," "concentrate_scale"),
    [
    ...
    properties_kwargs.update(
        {
            ...
            "ar_names": ["ar.L%d" % i for i in (p if isinstance(p, list) else range(1, p + 1))],
            "ma_names": ["ma.L%d" % i for i in (q if isinstance(q, list) else range(1, q + 1))],
            "seasonal_ar_names": [
                "ar.S.L%d" % (i * s) for i in (P if isinstance(P, list) else range(1, P + 1))
            ],
            "seasonal_ma_names": [
                "ma.S.L%d" % (i * s) for i in (Q if isinstance(Q, list) else range(1, Q + 1))
            ],
        }
    )
    ...
    methods_kwargs.update(
        {
            ...
            "ar_params": ([] if k_ar_params == 0 else constrain(np.arange(k_ar_params) / 10)),
            "ma_params": (
                [] if k_ma_params == 0 else constrain((np.arange(k_ma_params) + 10) / 100)
            ),
            "seasonal_ar_params": (
                [] if k_seasonal_ar_params == 0 else constrain(np.arange(k_seasonal_ar_params) - 4)
            ),
            "seasonal_ma_params": (
                []
...
def test_invalid():
    ...
    assert_raises(
        ValueError, specification.SARIMAXSpecification, order=(1, 0, 0), endog=np.zeros((10, 2))
    )
...
@pytest.mark.parametrize(
    "order,seasonal_order,enforce_stationarity," "enforce_invertibility,concentrate_scale,valid",
    [
        # Different orders
        (
            (0, 0, 0),
