...
class _MergeOperation:
    ...
    def _indicator_pre_merge(
        ...
        for i in ["_left_indicator", "_right_indicator"]:
            if i in columns:
                raise ValueError(
                    "Cannot use `indicator=True` option when " f"data contains a column named {i}"
                )
                ...
                if left_indexer is not None or right_indexer is not None:
                    if name in self.left:
                        if left_has_missing is None:
                            left_has_missing = (
                                False if left_indexer is None else (left_indexer == -1).any()
                            )
                        ...
                        if right_has_missing is None:
                            right_has_missing = (
                                False if right_indexer is None else (right_indexer == -1).any()
                            )
...
def get_join_indexers(
    ...
    if len(left_keys) > 1:
        ...
        mapped = (
            _factorize_keys(left_keys[n], right_keys[n], sort=sort) for n in range(len(left_keys))
        )
        zipped = zip(*mapped)
...
def _factorize_keys(
    ...
    if (isinstance(lk.dtype, DatetimeTZDtype) and isinstance(rk.dtype, DatetimeTZDtype)) or (
        lib.is_np_dtype(lk.dtype, "M") and lib.is_np_dtype(rk.dtype, "M")
    ):
        ...
        if (isinstance(lk.dtype, ArrowDtype) and is_string_dtype(lk.dtype)) or (
            ...
            llab, rlab, count = (
                pc.fill_null(dc.indices[slice(len_lk)], -1).to_numpy().astype(np.intp, copy=False),
...
def _get_join_keys(
    ...
    nlev = next(
        lev for lev in range(len(shape), 0, -1) if not is_int64_overflow_possible(shape[:lev])
    )
