...
@_handle_tensorly_backends_dataset("factor_matrix1", None)
@_handle_tensorly_backends_dataset("factor_matrix2", None)
def get_factor_matrix_permutation(
    factor_matrix1, factor_matrix2, ignore_sign=True, allow_smaller_rank=False
):
    ...
    return _get_linear_sum_assignment_permutation(
...
def factor_match_score(
    ...
    if consider_weights:
        ...
        ) / np.maximum(weights1[:, np.newaxis], weights2[np.newaxis, :])
...
def get_cp_permutation(
    cp_tensor, reference_cp_tensor=None, consider_weights=True, allow_smaller_rank=False
):
    ...
    if reference_cp_tensor is not None:
        ...
        if (
            rank > target_rank
        ):  # There are more components in the tensor to permute than the reference tensor
...
def check_cp_tensor_equal(cp_tensor1, cp_tensor2, ignore_labels=False):
    ...
    for mode in range(num_modes):
        if not check_factor_matrix_equal(
            cp_tensor1[1][mode], cp_tensor2[1][mode], ignore_labels=ignore_labels
        ):
            return False
    return True
...
@_handle_tensorly_backends_dataset("factor_matrix1", None)
@_handle_tensorly_backends_dataset("factor_matrix2", None)
def check_factor_matrix_close(
    factor_matrix1, factor_matrix2, rtol=1e-5, atol=1e-8, ignore_labels=False
):
    """Check that all entries in a factor matrix are close, if labelled, then label equality is also checked.
