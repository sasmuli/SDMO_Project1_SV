...
class HoltWintersResults(Results):
    ...
    def simulate(
        ...
        n_params = 2 + 2 * self.model.has_trend + (m + 1) * self.model.has_seasonal + damped
        ...
        if isinstance(random_errors, np.ndarray):
            ...
            eps = np.random.choice(resid, size=(nsimulations, repetitions), replace=True)
        ...
        for t in range(nsimulations):
            ...
            if error == "add":
                ...
                kappa_b = kappa_l / lvl[t - 1, :] if mul_trend else kappa_l + lvl[t - 1, :]
        ...
        _, _, _, index = self.model._get_prediction_index(start_idx, start_idx + nsimulations - 1)
        if repetitions == 1:
