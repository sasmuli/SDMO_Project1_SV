...
def epps_singleton_2samp(x, y, t=(0.4, 0.8)):
    ...
    if (nx < 5) or (ny < 5):
        raise ValueError(
            "x and y should have at least 5 elements, but len(x) " f"= {nx} and len(y) = {ny}."
        )
...
def _pval_cvm_2samp_exact(s, m, n):
    ...
    for u in range(n + 1):
        ...
        for v, g in enumerate(gs):
            ...
            tmp = np.concatenate(
                [np.stack([vi, tmp[1, i0] + g[1, i1]]), np.delete(tmp, i0, 1), np.delete(g, i1, 1)],
                1,
            )
            res = (a * v - b * u) ** 2
...
def tukey_hsd(*args):
    ...
    mse = np.sum([np.var(arg, ddof=1) for arg in args] * (nsamples_treatments - 1)) / (
        nobs - ntreatments
    )
