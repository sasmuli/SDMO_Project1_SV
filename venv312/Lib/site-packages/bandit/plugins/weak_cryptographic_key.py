    ...
    for size, level in key_sizes[key_type]:
        if key_size < size:
            return bandit.Issue(
                ...
                text="%s key sizes below %d bits are considered breakable. " % (key_type, size),
            )
...
def weak_cryptographic_key(context, config):
    return _weak_crypto_key_size_cryptography_io(context, config) or _weak_crypto_key_size_pycrypto(
        context, config
    )
