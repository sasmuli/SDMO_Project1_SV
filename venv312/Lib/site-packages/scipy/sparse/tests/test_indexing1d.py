...
class TestSlicingAndFancy1D:
    ...
    def test_fancy_indexing_2darray(self, spcreator):
        ...
        assert_equal(toarray(A[np.array([-1, -3]), np.array([2, -4])]), B[[-1, -3], [2, -4]])

        # [[[1],[2]],[1,2]]
        assert_equal(A[[[1], [3]], [2, 4]].toarray(), B[[[1], [3]], [2, 4]])
        assert_equal(A[[[-1], [-3], [-2]], [2, -4]].toarray(), B[[[-1], [-3], [-2]], [2, -4]])
        assert_equal(
            A[np.array([[-1], [-3], [-2]]), [2, -4]].toarray(), B[[[-1], [-3], [-2]], [2, -4]]
        )
        assert_equal(
            A[[[-1], [-3], [-2]], np.array([2, -4])].toarray(), B[[[-1], [-3], [-2]], [2, -4]]
        )
        assert_equal(
        ...
        assert_equal(A[[-1, -3], :][:, [2, -4]].toarray(), B[[-1, -3], :][:, [2, -4]])
        assert_equal(
            A[np.array([-1, -3]), :][:, np.array([2, -4])].toarray(), B[[-1, -3], :][:, [2, -4]]
        )
    ...
    def test_fancy_indexing_set(self, spcreator):
        ...
        # [1:2]
        for j in [[2, 3, 4], slice(None, 10, 4), np.arange(3), slice(5, -2), slice(2, 5)]:
            A = spcreator(M)
            B = np.zeros(M)
            with np.testing.suppress_warnings() as sup:
                sup.filter(
