...
def test_kalman_filter_pickle(data):
    ...
    initial_state_cov = np.dot(
        np.dot(model.transition[:, :, 0], initial_state_cov), model.transition[:, :, 0].T
    )
    ...
    assert_allclose(
        results.llf_obs[true["start"] :].sum(), pkl_results.llf_obs[true["start"] :].sum()
    )
    assert_allclose(
        results.filtered_state[0][true["start"] :], pkl_results.filtered_state[0][true["start"] :]
    )
    assert_allclose(
        results.filtered_state[1][true["start"] :], pkl_results.filtered_state[1][true["start"] :]
    )
    assert_allclose(
        results.filtered_state[3][true["start"] :], pkl_results.filtered_state[3][true["start"] :]
    )
