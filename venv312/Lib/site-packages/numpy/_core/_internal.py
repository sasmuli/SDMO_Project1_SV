...
def _commastring(astr):
    ...
    while startindex < len(astr):
        ...
        if order2 == "":
            ...
            if order1 != order2:
                raise ValueError(f"inconsistent byte-order specification {order1} and {order2}")
            order = order1
...
class _ctypes:
    ...
    def get_data(self):
        ...
        """
        warnings.warn(
            '"get_data" is deprecated. Use "data" instead', DeprecationWarning, stacklevel=2
        )
        return self.data
    ...
    def get_shape(self):
        ...
        """
        warnings.warn(
            '"get_shape" is deprecated. Use "shape" instead', DeprecationWarning, stacklevel=2
        )
        return self.shape
    ...
    def get_strides(self):
        ...
        """
        warnings.warn(
            '"get_strides" is deprecated. Use "strides" instead', DeprecationWarning, stacklevel=2
        )
        return self.strides
...
def _gcd(a, b):
    ...
    if not (math.isfinite(a) and math.isfinite(b)):
        raise ValueError(
            "Can only find greatest common divisor of " f'finite arguments, found "{a}" and "{b}"'
        )
    while b:
        a, b = b, a % b
    return a
