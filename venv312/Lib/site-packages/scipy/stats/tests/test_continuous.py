...
class Test_RealDomain:
    ...
    def test_contains(self, shapes, inclusive_a, inclusive_b, data):
        ...
        a_elements = dict(allow_nan=False, allow_infinity=False, min_value=-1e3, max_value=1)
        b_elements = dict(allow_nan=False, allow_infinity=False, min_value=2, max_value=1e3)
        ...
        d = b - a
        x = np.concatenate(
            [np.linspace(a - d, a, 10), np.linspace(a, b, 10), np.linspace(b, b + d, 10)]
        )
        # Domain is defined by two parameters, 'a' and 'b'
        domain = _RealDomain(endpoints=("a", "b"), inclusive=(inclusive_a, inclusive_b))
        domain.define_parameters(
            _RealParameter("a", domain=_RealDomain()), _RealParameter("b", domain=_RealDomain())
        )
    ...
    @pytest.mark.parametrize(
        "case", [(-np.inf, np.pi, False, True, r"(-\infty, \pi]"), ("a", 5, True, False, "[a, 5)")]
    )
...
def draw_distribution_from_family(family, data, rng, proportions, min_side=0):
    ...
    if n_parameterizations > 0:
        ...
        shapes, result_shape = data.draw(
            npst.mutually_broadcastable_shapes(num_shapes=n_parameters, min_side=min_side)
        )
        dist = family._draw(shapes, rng=rng, proportions=proportions, i_parameterization=i)
    else:
...
families = [StandardNormal, Normal, Uniform, _LogUniform]
...
def check_cdf2(dist, log, x, y, result_shape, methods):
    ...
    for method in methods:
        res = np.exp(dist.logcdf(x, y, method=method)) if log else dist.cdf(x, y, method=method)
...
def check_ccdf2(dist, log, x, y, result_shape, methods):
    ...
    for method in methods:
        res = np.exp(dist.logccdf(x, y, method=method)) if log else dist.ccdf(x, y, method=method)
        np.testing.assert_allclose(res, ref, atol=1e-14)
        np.testing.assert_equal(res.dtype, ref.dtype)
        np.testing.assert_equal(res.shape, result_shape)
        if result_shape == tuple():
...
def check_nans_and_edges(dist, fname, arg, res):
    ...
    valid_parameters, *classified_args = np.broadcast_arrays(valid_parameters, *classified_args)
...
def check_moment_funcs(dist, result_shape):
    ...
    for i in range(6):
        ...
        if not (dist.__class__ == stats.Uniform and i == 5):
            ...
            check(i, "central", "transform", ref, success=has_formula(i, "raw") or (i <= 1))
    ...
    for i in range(3, 6):
        ref = dist.moment(i, "central", method="quadrature")
        check(i, "central", "normalize", ref, success=has_formula(i, "standardized"))
...
def get_valid_parameters(dist):
    ...
    for name, value in parameter_values.items():
        ...
        # within the domain.
        a, b = parameter.domain.get_numerical_endpoints(parameter_values=parameter_values)
        ...
        assert_equal(valid, parameter.domain.contains(value, parameter_values=parameter_values))
...
def test_input_validation():
    ...
    message = "The `Test2` distribution family requires parameters, but none " "were provided."
    with pytest.raises(ValueError, match=message):
        Test2()
    ...
    def test_make_distribution(self, i, distdata):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            if distname not in skip_kurtosis:
                assert_allclose(X.kurtosis(convention="excess"), k, rtol=rtol, atol=atol)
            ...
            for order in range(5):
                if distname not in skip_raw.get(order, {}):
                    assert_allclose(
                        X.moment(order, kind="raw"), Y.moment(order), rtol=rtol, atol=atol
                    )
    ...
    def test_loc_scale(self, data, seed):
        ...
        with np.errstate(invalid="ignore", divide="ignore"):
            ...
            for i in range(1, 5):
                ...
                assert_allclose(dist.moment(i, "central"), dist0.moment(i, "central") * scale**i)
                assert_allclose(
        ...
        with np.errstate(invalid="ignore", divide="ignore"):
            ...
            for i in range(1, 5):
                ...
                assert_allclose(dist.moment(i, "standardized"), dist0.moment(i, "standardized"))
...
class TestFullCoverage:
    ...
    def test_ContinuousDistribution_set_invalid_nan(self):
        ...
        class TestDist(ContinuousDistribution):
            _variable = _RealParameter("x", domain=_RealDomain(endpoints=(0.0, 1.0)))
    ...
    def test_ContinuousDistribution__repr__(self):
        ...
        if np.__version__ < "2":
            ...
            assert repr(X * 3 + 2) == (
                "np.float64(3.0)*Uniform(a=np.float64(0.0), b=np.float64(1.0))" " + np.float64(2.0)"
            )
...
class TestMixture:
    ...
    def test_basic(self, shape):
        ...
        X = Mixture((Normal(mu=-0.25, sigma=1.1), Normal(mu=0.5, sigma=0.9)), weights=(0.4, 0.6))
        Y = MixedDist()
        x = rng.random(shape)
        ...
        for kind in ["raw", "central", "standardized"]:
            for order in range(5):
                assert_allclose(X.moment(order, kind=kind), Y.moment(order, kind=kind), atol=1e-15)
