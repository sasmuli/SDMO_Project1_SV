...
class VertexBase(ABC):
    ...
    def __getattr__(self, item):
        if item not in ["x_a"]:
            raise AttributeError(f"{type(self)} object has no attribute " f"'{item}'")
        if item == "x_a":
    ...
    def connect(self, v):
        raise NotImplementedError(
            "This method is only implemented with an " "associated child of the base class."
        )
...
class VertexScalarField(VertexBase):
    ...
    def __init__(
        self, x, field=None, nn=None, index=None, field_args=(), g_cons=None, g_cons_args=()
    ):
        """
        Parameters
        ----------
