...
class ContrastResults:
    ...
    def __init__(
        self, t=None, F=None, sd=None, effect=None, df_denom=None, df_num=None, alpha=0.05, **kwds
    ):
    ...
    def summary_frame(self, xname=None, alpha=0.05):
        ...
        if self.effect is not None:
            ...
            summ = summary_params_frame(
                (self, self.effect, self.sd, self.statistic, self.pvalue, self.conf_int(alpha)),
                yname=yname,
                xname=xname,
                use_t=use_t,
                alpha=alpha,
...
def _get_pairs_labels(k_level, level_names):
    ...
    labels = [f"{level_names[name[1]]}-{level_names[name[0]]}" for name in zip(*idx_pairs_all)]
    return labels
...
def t_test_pairwise(result, term_name, method="hs", alpha=0.05, factor_labels=None, ignore=False):
    ...
    res_df = t_test_multi(
        result, contrasts, method=method, ci_method=None, alpha=alpha, contrast_names=labels
    )
    res = MultiCompResult(
        result_frame=res_df,
        contrasts=contrasts,
