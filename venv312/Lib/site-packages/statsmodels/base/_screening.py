...
class VariableScreening:
    ...
    def screen_exog(self, exog, endog=None, maxiter=100, method="bfgs", disp=False, fit_kwds=None):
        ...
        for it in range(maxiter):
            ...
            mod_pen = model_class(
                endog, x[:, idx], penal=self.penal, pen_weight=self.pen_weight, **self.init_kwds
            )
        ...
        mod_final = model_class(
            endog, x[:, idx_nonzero], penal=penal, pen_weight=self.pen_weight, **self.init_kwds
        )
    ...
    def screen_exog_iterator(self, exog_iterator):
        ...
        exog_winner_names = [
            "var%d_%d" % (bidx, idx) for bidx, batch in enumerate(exog_idx) for idx in batch
        ]
