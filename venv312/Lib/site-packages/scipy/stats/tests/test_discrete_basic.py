...
def check_discrete_chisquare(distfn, arg, rvs, alpha, msg):
    ...
    npt.assert_(
        pval > alpha, f"chisquare - test for {msg} at arg = {str(arg)} with pval = {str(pval)}"
    )
...
def test_methods_with_lists(method, distname, args):
    ...
    npt.assert_allclose(result, [dist_method(*v) for v in zip(z, *p2, loc)], rtol=1e-15, atol=1e-15)
