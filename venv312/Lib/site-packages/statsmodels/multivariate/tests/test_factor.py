...
X = pd.DataFrame(
    [
        ["Minas Graes", 2.068, 2.070, 1.580, 1, 0],
...
def test_direct_corr_matrix():
    ...
    a = np.array(
        [
            [0.965392158864, 0.225880658666255],
...
def test_example_compare_to_R_output():
    ...
    a = np.array(
        [
            [0.98935598, 0.98242714, 0.94078972, 0.33442284],
...
def test_getframe_smoke():
    ...
    res.get_loadings_frame(
        style="display", decimals=3, threshold=0.45, highlight_max=False, sort_=False
    )
...
def test_factor_scoring():
    ...
    assert_allclose(f_reg * [1, -1], f_s[["f1", "f2"]].values, atol=1e-4, rtol=1e-3)
    f_bart = res.factor_scoring()
    assert_allclose(f_bart * [1, -1], f_s[["f1b", "f2b"]].values, atol=1e-4, rtol=1e-3)
    ...
    f_reg = res.factor_scoring(method="reg")
    assert_allclose(f_reg * [1, -1], f_s[["f1o", "f2o"]].values, atol=1e-4, rtol=1e-3)
    f_bart = res.factor_scoring()
    assert_allclose(f_bart * [1, -1], f_s[["f1ob", "f2ob"]].values, atol=1e-4, rtol=1e-3)
