...
class ComplexityVisitor(CodeVisitor):
    ...
    def total_complexity(self):
        ...
        """
        return (
            self.complexity + self.functions_complexity + self.classes_complexity + (not self.off)
        )
    ...
    def generic_visit(self, node):
        ...
        # Note: Lambda functions are not counted anymore, see #68
            ...
            contain_underscore = any(
                (case for case in node.cases if getattr(case.pattern, "pattern", False) is None)
            )
    ...
    def visit_ClassDef(self, node):
        ...
        for child in node.body:
            ...
            body_complexity += (
                visitor.complexity + visitor.functions_complexity + len(visitor.functions)
            )
    ...
    def dispatch(meth):
        ...
        def aux(self, node):
            ...
            for operand in results[3]:
                new_operand = getattr(operand, self.types.get(type(operand), ""), operand)
