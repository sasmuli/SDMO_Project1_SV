...
class BoundConstraints:
    ...
    def __init__(self, bounds):
        ...
        self.is_feasible = (
            np.all(self.xl <= self.xu) and np.all(self.xl < np.inf) and np.all(self.xu > -np.inf)
        )
...
class LinearConstraints:
    ...
    def __init__(self, constraints, n, debug):
        ...
        for constraint in constraints:
            is_equality = np.abs(constraint.ub - constraint.lb) <= get_arrays_tol(
            ...
            if np.any(is_equality):
                ...
                self._b_eq = np.concatenate(
                    (
                        self.b_eq,
                        0.5 * (constraint.lb[is_equality] + constraint.ub[is_equality]),
                    )
                )
...
class Problem:
    ...
    def __init__(
        ...
        if linear.a_ub.shape[1] != n:
            raise ValueError(
                f"The left-hand side matrices of the linear constraints must " f"have {n} columns."
            )
        ...
        self._linear = LinearConstraints(
            [
                LinearConstraint(
                    ...
                    -np.inf,
                    linear.b_ub - linear.a_ub[:, self._fixed_idx] @ self._fixed_val,
                ),
            ...
            self._linear = LinearConstraints(
                [
                    LinearConstraint(
                        ...
                        -np.inf,
                        self._linear.b_ub - self._linear.a_ub @ self._scaling_shift,
                    ),
                    LinearConstraint(
    ...
    def __call__(self, x, penalty=0.0):
        ...
        if np.isnan(fun_val) and np.isnan(maxcv_val):
            ...
            include_point = all(
                np.isnan(fun_filter) and maxcv_val < maxcv_filter or np.isnan(maxcv_filter)
            ...
            include_point = all(
                np.isnan(maxcv_filter) and fun_val < fun_filter or np.isnan(fun_filter)
                for fun_filter, maxcv_filter in zip(
                    self._fun_filter,
                    self._maxcv_filter,
                )
    ...
    def best_eval(self, penalty):
        ...
        if np.any(finite_idx):
            ...
            if np.any(feasible_idx) and not np.all(np.isnan(fun_filter[feasible_idx])):
                ...
                fun_min_idx = feasible_idx & (fun_filter <= np.nanmin(fun_filter[feasible_idx]))
                if np.count_nonzero(fun_min_idx) > 1:
                    fun_min_idx &= maxcv_filter <= np.min(maxcv_filter[fun_min_idx])
