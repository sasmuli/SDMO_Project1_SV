...
class UnobservedComponents(MLEModel):
    ...
    **Regression effects**
        ...
        **kwargs,
        ...
        if freq_seasonal:
            ...
            self.freq_seasonal_harmonics = [
                d.get("harmonics", int(np.floor(d["period"] / 2))) for d in freq_seasonal
            ]
        else:
        ...
        if not (
            self.irregular
            or (self.level and self.stochastic_level)
            or (self.trend and self.stochastic_trend)
            or (self.seasonal and self.stochastic_seasonal)
            or (self.freq_seasonal and any(self.stochastic_freq_seasonal))
            or (self.cycle and self.stochastic_cycle)
            or self.autoregressive
        ):
            warn(
        ...
        k_posdef = (
            ...
            + (
                (
                    sum(
                        2 * h if self.stochastic_freq_seasonal[ix] else 0
        ...
        super().__init__(endog, k_states, k_posdef=k_posdef, exog=exog, **kwargs)
        self.setup()
    ...
    def _get_init_kwds(self):
        ...
        kwds["freq_seasonal"] = [
            {"period": p, "harmonics": self.freq_seasonal_harmonics[ix]}
            for ix, p in enumerate(self.freq_seasonal_periods)
        ]
    ...
    def setup(self):
        ...
        if self.seasonal:
            ...
            self.ssm["design", 0, i] = 1.0
            self.ssm["transition", i : i + n, i : i + n] = companion_matrix(
                np.r_[1, [1] * n]
            ).transpose()
        ...
        if self.autoregressive:
            ...
            self.ssm["transition", i : i + self.ar_order, i : i + self.ar_order] = companion_matrix(
                self.ar_order
            ).T
        ...
        if self.regression:
            if self.mle_regression:
                ...
                self.ssm["transition", i : i + self.k_exog, i : i + self.k_exog] = np.eye(
                    self.k_exog
                )
    ...
    def transform_params(self, unconstrained):
        ...
        if self.autoregressive:
            constrained[offset : offset + self.ar_order] = constrain_stationary_univariate(
    ...
    def untransform_params(self, constrained):
        ...
        # Autoregressive coefficients must be stationary
        if self.autoregressive:
            unconstrained[offset : offset + self.ar_order] = unconstrain_stationary_univariate(
    ...
    def _validate_can_fix_params(self, param_names):
        ...
        if "ar_coeff" in self.parameters:
            ...
            if fix_any_ar and not fix_all_ar:
                raise ValueError(
                    "Cannot fix individual autoregressive."
        ...
        if self.regression:
            if self.mle_regression:
                self.ssm["obs_intercept"] = np.dot(
                    self.exog, params[offset : offset + self.k_exog]
                )[None, :]
            offset += self.k_exog
...
class UnobservedComponentsResults(MLEResults):
    ...
    def freq_seasonal(self):
        ...
        if spec.freq_seasonal:
            previous_states_offset = int(
                spec.trend + spec.level + self._k_states_by_type["seasonal"]
            )
            ...
            for ix, h in enumerate(spec.freq_seasonal_harmonics):
                ...
                filtered_state = np.sum(
                    [self.filtered_state[offset + j] for j in states_in_sum], axis=0
                )
                filtered_cov = np.sum(
                    [self.filtered_state_cov[offset + j, offset + j] for j in states_in_sum], axis=0
                )
                ...
                if self.smoothed_state is not None:
                    item.smoothed = np.sum(
                        [self.smoothed_state[offset + j] for j in states_in_sum], axis=0
                    )
                if self.smoothed_state_cov is not None:
                    item.smoothed_cov = np.sum(
                        [self.smoothed_state_cov[offset + j, offset + j] for j in states_in_sum],
                        axis=0,
                    )
                out.append(item)
    ...
    def regression_coefficients(self):
        ...
        if spec.regression:
            if spec.mle_regression:
                ...
                warnings.warn(
                    "Regression coefficients estimated via maximum"
    ...
    def summary(self, alpha=0.05, start=None):
        ...
        return super().summary(
            alpha=alpha, start=start, title="Unobserved Components Results", model_name=model_name
        )
...
wrap.populate_wrapper(UnobservedComponentsResultsWrapper, UnobservedComponentsResults)  # noqa:E305
