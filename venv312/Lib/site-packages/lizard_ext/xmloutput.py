    ...
    for source_file in result:
        if source_file:
            ...
            for func in source_file.function_list:
                ...
                measure.appendChild(_create_function_item(doc, number, file_name, func, verbose))
            ...
            if number != 0:
                ...
                measure.appendChild(
                    _create_labeled_value_item(doc, "average", "CCN", str(total_func_ccn / number))
                )
    return measure
...
def _create_file_measure(doc, result, all_result):
    ...
    measure.appendChild(_create_labels(doc, ["Nr.", "NCSS", "CCN", "Functions"]))
    ...
    for source_file in result:
        ...
        measure.appendChild(_create_file_node(doc, source_file, file_nr))

    if file_nr != 0:
        file_summary = [
            ("NCSS", all_in_one.nloc / file_nr),
            ("CCN", file_total_ccn / file_nr),
            ("Functions", file_total_funcs / file_nr),
        ]
        for key, val in file_summary:
            measure.appendChild(_create_labeled_value_item(doc, "average", key, val))

    summary = [("NCSS", all_in_one.nloc), ("CCN", file_total_ccn), ("Functions", file_total_funcs)]
    for key, val in summary:
        measure.appendChild(_create_labeled_value_item(doc, "sum", key, val))

    if file_total_funcs != 0:
        summary = [
            ("NCSS", all_in_one.average_nloc),
