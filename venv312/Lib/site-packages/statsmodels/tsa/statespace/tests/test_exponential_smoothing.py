...
class CheckKnownInitialization:
    ...
    def test_estimated_params(self):
        ...
        fit_res1 = self.mod.fit_constrained(
            self.init_params.to_dict(), start_params=self.start_params, includes_fixed=True, disp=0
        )
...
class TestHoltWintersDampedKnownInitialization(CheckKnownInitialization):
    ...
    def setup_class(cls):
        ...
        start_params = pd.Series(
            [0.0005, 0.0004, 0.0005, 0.95, 17.0, 1.5, -0.2, 0.1, 0.4], index=mod.param_names
        )
        super().setup_class(mod, start_params)
...
class TestHoltWintersDampedHeuristicInitialization(CheckHeuristicInitialization):
    ...
    def setup_class(cls):
        mod = ExponentialSmoothing(
            aust, trend=True, damped_trend=True, seasonal=4, initialization_method="heuristic"
        )
        super().setup_class(mod)
...
class CheckConcentratedInitialization:
    ...
    def setup_class(cls, mod, start_params=None, atol=0, rtol=1e-7):
        ...
        cls.params = pd.Series(
            [0.5, 0.2, 0.2, 0.95],
            index=["smoothing_level", "smoothing_trend", "smoothing_seasonal", "damping_trend"],
        )
...
def test_invalid():
    ...
    with pytest.raises(
        ...
        mod = ExponentialSmoothing(aust, trend=True, initialization_method="known", initial_level=0)
    ...
    with pytest.raises(
        ...
        mod = ExponentialSmoothing(aust, seasonal=4, initialization_method="known", initial_level=0)
    ...
    with pytest.raises(
        ...
        mod = ExponentialSmoothing(
            aust, seasonal=4, initialization_method="known", initial_level=0, initial_seasonal=0
        )

    with pytest.raises(NotImplementedError, match="ExponentialSmoothing does not support `exog`."):
