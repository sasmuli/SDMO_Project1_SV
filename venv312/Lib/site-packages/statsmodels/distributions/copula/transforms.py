...
class TransfFrank(Transforms):
    ...
    def deriv4_inverse(self, phi, theta):
        ...
        d4 = (
            (et - 1)
            * ept
            * (-4 * ept + np.exp(2 * (p + b)) + 4 * np.exp(p + 2 * b) - 2 * et + np.exp(2 * b) + 1)
        ) / (b * (ept - et + 1) ** 4)
...
class TransfGumbel(Transforms):
    ...
    def deriv2(self, t, theta):
        tmp1 = np.log(t)
        d2 = (
            theta * (-1) ** (1 + theta) * tmp1 ** (theta - 1) * (1 - theta)
            + theta * (-1) ** (1 + theta) * tmp1**theta
        ) / (t**2 * tmp1)
    ...
    def deriv3_inverse(self, phi, theta):
        ...
        b = theta
        d3 = (
            -(p ** (3 / b)) + (3 - 3 * b) * p ** (2 / b) + ((3 - 2 * b) * b - 1) * p ** (1 / b)
        ) / (p * b) ** 3
        d3 *= np.exp(-(p ** (1 / b)))
        return d3
