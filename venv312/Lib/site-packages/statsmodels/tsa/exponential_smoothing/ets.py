...
class ETSResults(base.StateSpaceMLEResults):
    ...
    def _relative_forecast_variance(self, steps):
        ...
        if model == "ANN":
            ...
            return (
                1
                + (h - 1) * (alpha**2 + alpha * beta * h + (beta**2) / 6 * h * (2 * h - 1))
    ...
    def simulate(
        ...
        if isinstance(random_errors, np.ndarray):
            if random_errors.shape != (nsimulations, repetitions):
                raise ValueError(
                    "If random is an ndarray, it must have shape " "(nsimulations, repetitions)!"
                )
            eps = random_errors
    ...
    **simulate_kwargs :
    ...
    def __init__(
        ...
        **simulate_kwargs,
        ...
        self.model = Bunch(
            data=results.model.data.__class__(endog=self.endog, predict_dates=self.row_labels)
        )
    ...
    def summary_frame(self, endog=0, alpha=0.05):
        ...
        res = pd.DataFrame(to_include, index=self.row_labels, columns=list(to_include.keys()))
        return res
