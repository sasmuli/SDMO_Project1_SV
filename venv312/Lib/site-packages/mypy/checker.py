...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi):
    ...
    def check_method_override_for_base_with_name(
        ...
        if isinstance(original_type, AnyType) or isinstance(typ, AnyType):
            ...
            is_subtype(typ, orig_typ, ignore_pos_arg_names=True) for orig_typ in original_type.items
        ):
        ...
        if self.is_stub and isinstance(rvalue, EllipsisExpr):
            ...
            if (
                ...
                if not is_valid_inferred_type(rvalue_type, self.options) and inferred is not None:
                    self.msg.need_annotation_for_var(inferred, context, self.options.python_version)
    ...
    def infer_context_dependent(
        ...
        with self.msg.filter_errors(
            ...
            with self.local_type_map as type_map:
                typ = get_proper_type(
                    self.expr_checker.accept(expr, type_ctx, allow_none_return=allow_none_func_call)
                )
        ...
        if isinstance(s.expr, IndexExpr):
            ...
            for elt in flatten(s.expr):
                if isinstance(elt, NameExpr):
                    self.binder.assign_type(elt, DeletedType(source=elt.name), get_declaration(elt))
