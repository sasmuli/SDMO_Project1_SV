...
from statsmodels.tsa.statespace.kalman_filter import MEMORY_CONSERVE, MEMORY_NO_LIKELIHOOD
...
def check_output(res_chand, res_orig, memory_conserve=False):
    ...
    assert_allclose(
        res_chand.model.score_obs(params), res_orig.model.score_obs(params), rtol=5e-5, atol=5e-6
    )
    ...
    for name in res_chand.model.ssm.shapes:
        if name == "obs":
            continue
        assert_allclose(
            getattr(res_chand.filter_results, name), getattr(res_orig.filter_results, name)
        )
...
def test_invalid():
    ...
    with pytest.raises(
        RuntimeError, match=("Cannot use Chandrasekhar" " recursions with missing data.")
    ):
        mod.filter([0.5, 1.0])
    ...
    with pytest.raises(
        RuntimeError, match=("Cannot use Chandrasekhar" " recursions with filtered" " timing.")
    ):
        mod.filter([0.5, 1.0])
