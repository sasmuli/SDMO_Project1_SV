    ...
    def __new__(cls, *system):
        ...
        if cls is lti:
            ...
            if N == 2:
                ...
                raise ValueError(
                    "`system` needs to be an instance of `lti` " "or have 2, 3 or 4 arguments."
                )
    ...
    def __new__(cls, *system, **kwargs):
        ...
        if cls is dlti:
            ...
            if N == 2:
                ...
                raise ValueError(
                    "`system` needs to be an instance of `dlti` " "or have 2, 3 or 4 arguments."
                )
        # __new__ was called from a subclass, let it call its own functions
        return super().__new__(cls)
    ...
    *system: arguments
    ...
    def to_discrete(self, dt, method="zoh", alpha=None):
        ...
        return TransferFunction(
            *cont2discrete((self.num, self.den), dt, method=method, alpha=alpha)[:-1], dt=dt
        )
    ...
    *system : arguments
    ...
    def to_discrete(self, dt, method="zoh", alpha=None):
        ...
        return ZerosPolesGain(
            *cont2discrete((self.zeros, self.poles, self.gain), dt, method=method, alpha=alpha)[
                :-1
            ],
            dt=dt,
        )
...
class StateSpaceContinuous(StateSpace, lti):
    ...
    *system: arguments
    ...
    def to_discrete(self, dt, method="zoh", alpha=None):
        ...
        """
        return StateSpace(
            *cont2discrete((self.A, self.B, self.C, self.D), dt, method=method, alpha=alpha)[:-1],
            dt=dt,
        )
...
def _valid_inputs(A, B, poles, method, rtol, maxiter):
    ...
    if len(poles) > A.shape[0]:
        raise ValueError(
            "maximum number of poles is %d but you asked for %d" % (A.shape[0], len(poles))
        )
    if len(poles) < A.shape[0]:
        raise ValueError(
...
def _YT_complex(ker_pole, Q, transfer_matrix, i, j):
    ...
    m = np.dot(
        np.dot(np.conj(ker_pole_ij.T), np.dot(u, np.conj(u).T) - np.dot(np.conj(u), u.T)),
        ker_pole_ij,
    )
    ...
    transfer_matrix_i_j = np.dot(
        np.dot(ker_pole_mu, np.conj(ker_pole_mu.T)), transfer_matrix_j_mo_transfer_matrix_j
    )
