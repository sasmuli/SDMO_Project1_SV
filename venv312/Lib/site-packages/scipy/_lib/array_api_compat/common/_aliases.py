...
def empty_like(
    x: ndarray, /, xp, *, dtype: Optional[Dtype] = None, device: Optional[Device] = None, **kwargs
) -> ndarray:
    _check_device(xp, device)
...
def unique_counts(x: ndarray, /, xp) -> UniqueCountsResult:
    kwargs = _unique_kwargs(xp)
    res = xp.unique(x, return_counts=True, return_index=False, return_inverse=False, **kwargs)
...
def clip(
    ...
    *,
    ...
    if out is None:
        out = wrapped_xp.asarray(xp.broadcast_to(x, result_shape), copy=True, device=device(x))
    if min is not None:
