...
class ExponentialSmoothing(MLEModel):
    ...
    **Seasonality**
    ...
    def __init__(
        ...
        super().__init__(
            endog, k_states=k_states, k_posdef=k_posdef, initialization=init, dates=dates, freq=freq
        )
        ...
        if self.initialization_method == "simple":
            initial_level, initial_trend, initial_seasonal = es_init._initialization_simple(
                self.endog[:, 0],
            ...
            initial_level, initial_trend, initial_seasonal = es_init._initialization_heuristic(
                self.endog[:, 0],
                trend="add" if self.trend else None,
                seasonal="add" if self.seasonal else None,
                seasonal_periods=self.seasonal_periods,
    ...
    def _res_classes(self):
        return {"fit": (ExponentialSmoothingResults, ExponentialSmoothingResultsWrapper)}
        ...
        if self.initialization_method == "estimated":
            ...
            if self.seasonal:
                param_names += ["initial_seasonal"] + [
                    "initial_seasonal.L%d" % i for i in range(1, self.seasonal_periods - 1)
                ]
    ...
    def k_params(self):
        ...
        if self.initialization_method == "estimated":
            k_params += 1 + int(self.trend) + int(self.seasonal) * (self._seasonal_periods - 1)
        return k_params
...
class ExponentialSmoothingResults(MLEResults):
    ...
    def summary(self, alpha=0.05, start=None):
        ...
        summary = super().summary(
            alpha=alpha, start=start, title="Exponential Smoothing Results", model_name=model_name
        )
...
wrap.populate_wrapper(ExponentialSmoothingResultsWrapper, ExponentialSmoothingResults)  # noqa:E305
