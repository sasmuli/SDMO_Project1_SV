...
class QAPCommonTests:
    ...
    def test_accuracy_1(self):
        ...
        res = quadratic_assignment(A, B, method=self.method, options={"rng": rng, "maximize": True})
        ...
        quadratic_assignment(A, B, method=self.method, options={"rng": rng, "maximize": True})
    ...
    def test_accuracy_3(self):
        ...
        empty = np.empty((0, 0))
        res = quadratic_assignment(empty, empty, method=self.method, options={"rng": rng})
        assert_equal(res.nit, 0)
    ...
    def test_deprecation_future_warnings(self):
        ...
        with pytest.warns(DeprecationWarning, match="Use of `RandomState`*"):
            ...
            quadratic_assignment(A, B, method=self.method, options={"rng": rng, "maximize": False})
        ...
        with pytest.warns(FutureWarning, match="The behavior when the rng option*"):
            quadratic_assignment(A, B, method=self.method, options={"rng": 0, "maximize": False})
...
class TestQAPOnce:
    ...
    def test_common_input_validation(self):
        ...
        with pytest.raises(ValueError, match="`partial_match` must have two columns"):
            quadratic_assignment(
                np.identity(3), np.identity(3), options={"partial_match": _range_matrix(2, 3)}
            )
        # test that seed has no more than two dimensions
        with pytest.raises(ValueError, match="`partial_match` must have exactly two"):
            quadratic_assignment(
                np.identity(3), np.identity(3), options={"partial_match": np.random.rand(3, 2, 2)}
            )
        # seeds cannot be negative valued
        with pytest.raises(ValueError, match="`partial_match` must contain only pos"):
            quadratic_assignment(
                np.identity(3), np.identity(3), options={"partial_match": -1 * _range_matrix(2, 2)}
            )
        # seeds can't have values greater than number of nodes
        with pytest.raises(ValueError, match="`partial_match` entries must be less than number"):
            quadratic_assignment(
                np.identity(5), np.identity(5), options={"partial_match": 2 * _range_matrix(4, 2)}
            )
        # columns of seed matrix must be unique
        with pytest.raises(ValueError, match="`partial_match` column entries must be unique"):
            quadratic_assignment(
                np.identity(3), np.identity(3), options={"partial_match": np.ones((2, 2))}
            )
