...
def test_mv_mean():
    ...
    res_stata = Holder(
        p_F=1.25062334808e-09, df_r=22, df_m=2, F=59.91609589041116, T2=125.2791095890415
    )
    ...
    res_stata = Holder(
        T2=9.698067632850247, df=10, k=2, N=12, F=4.4082126, p_F=0.0424  # not in return List
    )  # not in return List
    ...
    res_stata = Holder(
        p_F=0.0423949782937231, df_r=10, df_m=2, F=4.408212560386478, T2=9.69806763285025
    )
...
def test_mvmean_2indep():
    ...
    res_stata = Holder(
        p_F=0.6686659171701677, df_r=43, df_m=4, F=0.594263378678938, T2=2.5428944576028973
    )
...
def test_confint_simult():
    ...
    cii = [
        confint_mvmean_fromstats(m, cov, nobs, lin_transf=np.eye(3)[i], simult=True)[:2]
        for i in range(3)
    ]
...
def test_cov_oneway():
    ...
    cov_m = np.array(
        [
            ...
            [4.545362903225806, 13.184475806451612, 6.76008064516129, 6.266129032258064],
            [6.522177419354839, 6.76008064516129, 28.673387096774192, 14.46774193548387],
            [5.25, 6.266129032258064, 14.46774193548387, 16.64516129032258],
        ]
    )
    cov_f = np.array(
        [
            [9.13608870967742, 7.549395161290322, 4.86391129032258, 4.151209677419355],
            [7.549395161290322, 18.60383064516129, 10.224798387096774, 5.445564516129032],
            [4.86391129032258, 10.224798387096774, 30.039314516129032, 13.493951612903226],
            [4.151209677419355, 5.445564516129032, 13.493951612903226, 27.995967741935484],
        ]
    )
