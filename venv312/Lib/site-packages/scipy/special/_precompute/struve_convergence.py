...
def do_plot(is_h=True):
    ...
    lp = plt.contour(
        vs, zs, np.log10(err_est_p).T, levels=levels, colors=["r", "r"], linestyles=["-", "-"]
    )
    la = plt.contour(
        vs, zs, np.log10(err_est_a).T, levels=levels, colors=["b", "b"], linestyles=["-", "-"]
    )
    lb = plt.contour(
        vs, zs, np.log10(err_est_b).T, levels=levels, colors=["g", "g"], linestyles=["-", "-"]
    )
