...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
        ...
        if issparse(arg1):
            ...
            if isshape(arg1, allow_nd=self._allow_nd):
                ...
                if len(arg1) == 2:
                    ...
                    raise ValueError(
                        f"unrecognized {self.__class__.__name__} " f"constructor input: {arg1}"
                    )
    ...
    def _minor_index_fancy(self, idx):
        ...
        res_data = np.empty(nnz, dtype=self.dtype)
        csr_column_index2(
            col_order, col_offsets, len(self.indices), indices, self.data, res_indices, res_data
        )
        return self.__class__((res_data, res_indices, res_indptr), shape=new_shape, copy=False)
    ...
    def tocoo(self, copy=True):
        ...
        return self._coo_container((self.data, coords), self.shape, copy=copy, dtype=self.dtype)
    ...
    def resize(self, *shape):
        ...
        if hasattr(self, "blocksize"):
            ...
            if rm or rn:
                raise ValueError(
                    f"shape must be divisible into {self.blocksize}" f" blocks. Got {shape}"
                )
