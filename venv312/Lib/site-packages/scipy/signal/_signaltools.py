...
def _reject_objects(arr, name):
    ...
    if not (
        ...
        or dt in [np.bool_, np.float16, np.float32, np.float64, np.complex64, np.complex128]
    ):
        msg = (
...
def order_filter(a, domain, rank):
    ...
    for dimsize in domain.shape:
        if (dimsize % 2) != 1:
            raise ValueError(
                "Each dimension of domain argument " "should have an odd number of elements."
            )

    a = np.asarray(a)
    if not (np.issubdtype(a.dtype, np.integer) or a.dtype in [np.float32, np.float64]):
...
def medfilt(volume, kernel_size=None):
    ...
    if not (np.issubdtype(volume.dtype, np.integer) or volume.dtype in [np.float32, np.float64]):
    ...
    if any(k > s for k, s in zip(kernel_size, volume.shape)):
        warnings.warn(
            "kernel_size exceeds volume extent: the volume will be " "zero-padded.", stacklevel=2
        )
...
def envelope(
    ...
    * The frequency spacing ``1 / (n*T)`` for corner frequencies of the bandpass filter
    ...
    z_res = fak * (sp_fft.ifft(Z, n=n_out) if np.iscomplexobj(z) else sp_fft.irfft(Z, n=n_out))
...
def resample(x, num, t=None, axis=0, window=None, domain="time"):
    ...
    if domain not in ("time", "freq"):
        raise ValueError("Acceptable domain flags are 'time' or" f" 'freq', not domain={domain}")
...
def resample_poly(x, up, down, axis=0, window=("kaiser", 5.0), padtype="constant", cval=None):
    ...
    if padtype in funcs:
        ...
        raise ValueError(
            "padtype must be one of: maximum, mean, median, minimum, " + ", ".join(_upfirdn_modes)
        )
...
def _validate_pad(padtype, padlen, x, axis, ntaps):
    ...
    if x.shape[axis] <= edge:
        raise ValueError(
            "The length of the input vector x must be greater " "than padlen, which is %d." % edge
        )
