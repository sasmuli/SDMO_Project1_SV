    ...
    def _smooth(
        ...
        **kwargs,
        ...
        prefix, dtype, create_smoother, create_filter, create_statespace = (
            self._initialize_smoother(smoother_output, smooth_method, prefix=prefix, **kwargs)
        )
...
class SmootherResults(FilterResults):
    ...
    def update_smoother(self, smoother):
            ...
            if name == "smoother_output":
                ...
                if name in ["smoothing_error", "smoothed_measurement_disturbance"]:
                    ...
                    if vector is not None and has_missing:
                        vector = np.array(
                            reorder_missing_vector(vector, self.missing, prefix=self.prefix)
                        )
                    else:
        ...
        if "scaled_smoothed_estimator_cov" in attributes:
            self.scaled_smoothed_estimator_cov_presample = self.scaled_smoothed_estimator_cov[
                :, :, 0
            ]
    ...
    def _smoothed_state_autocovariance(self, shift, start, end, extend_kwargs=None):
        ...
        if n_postsample > 0:
            ...
            mod.initialize_known(
                self.predicted_state[..., self.nobs], self.predicted_state_cov[..., self.nobs]
            )
            res = mod.smooth()
    ...
    def news(
        ...
        if self.smoothed_state_cov is None:
            raise ValueError(
                "Cannot compute news without having applied the" " Kalman smoother first."
            )
        ...
        if self._smoothed_forecasts is None:
            ...
            for t in range(self.nobs):
                ...
                self._smoothed_forecasts_error_cov[:, :, t] = (
                    np.dot(
                        np.dot(self.design[:, :, design_t], self.smoothed_state_cov[:, :, t]),
    ...
    def get_smoothed_decomposition(self, decomposition_of="smoothed_state", state_index=None):
        ...
        weights, state_intercept_weights, prior_weights = tools._compute_smoothed_state_weights(
        ...
        if decomposition_of == "smoothed_signal":
            ...
            # of m)
            weights = np.nansum(weights[..., None] * ZT[:, None, :, None, :], axis=2).transpose(
                0, 1, 3, 2
            )
