...
class CheckWeight:
    def test_basic(self):
        ...
        if hasattr(res2, "normalized_cov_params"):
            assert_allclose(
                res1.normalized_cov_params, res2.normalized_cov_params, atol=1e-8, rtol=2e-6
            )
        if isinstance(
            self,
            (
    ...
    def test_pearson_chi2(self):
        if hasattr(self.res2, "chi2"):
            assert_allclose(self.res1.pearson_chi2, self.res2.deviance_p, atol=1e-6, rtol=1e-6)
...
class TestGlmGammaAwNr(CheckWeight):
    ...
    def test_r_llf(self):
        ...
        ll = self.res1.family.loglike(
            self.res1.model.endog, self.res1.mu, freq_weights=self.res1._var_weights, scale=scale
        )
            ...
            for binom_version in 0, 1:
                ...
                for max_start_irls, start_params in ((0, rslt_irls.params), (3, None)):
                    ...
                    rslt_gradient = mod_gradient.fit(
                        max_start_irls=max_start_irls, start_params=start_params, method=method
                    )
...
class TestTweedieRepeatedvsAverage(CheckWeight):
    ...
    def setup_class(cls):
        ...
        avg_endog = agg_endog["endog"] / agg_wt
        mod2 = sm.GLM(
            avg_endog, agg_exog, family=family(link=link(), var_power=1.5), var_weights=agg_wt
        )
...
def test_warnings_raised():
    ...
    with pytest.warns(SpecificationWarning):
        res1 = GLM(
            cpunish_data.endog, cpunish_data.exog, family=sm.families.Poisson(), var_weights=weights
        ).fit(cov_type="cluster", cov_kwds=cov_kwds)
        res1.summary()
...
def check_weights_as_formats(weights):
    res = GLM(
        cpunish_data.endog, cpunish_data.exog, family=sm.families.Poisson(), freq_weights=weights
    ).fit()
