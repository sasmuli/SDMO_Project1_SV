...
def test_concentrated_loglike_sarimax():
    ...
    assert_allclose(
        out.mod_conc.loglikeobs(out.params_conc), out.mod_orig.loglikeobs(out.params_orig)
    )
    ...
    assert_allclose(out.mod_conc.loglike(out.params_conc), out.mod_orig.loglike(out.params_orig))
    assert_allclose(
        out.mod_conc.loglikeobs(out.params_conc)[2:], out.mod_orig.loglikeobs(out.params_orig)[2:]
    )
...
def test_concentrated_predict_sarimax():
    ...
    assert_allclose(
        out.res_conc.predict(start=0, end=45, dynamic=10),
...
def test_fixed_scale_sarimax():
    ...
    assert_raises(
        AssertionError, assert_allclose, mod_conc.loglike(params[:-1]), mod_orig.loglike(params)
    )
