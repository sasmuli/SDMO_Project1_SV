...
class TrustRegion:
    ...
    def get_second_order_correction_step(self, step, options):
        ...
        if options[Options.DEBUG]:
            ...
            if np.any(soc_step + tol < xl) or np.any(xu < soc_step - tol):
                warnings.warn(
                    "The second-order correction step does not " "respect the bound constraints.",
                    RuntimeWarning,
                    2,
                )
    ...
    def enhance_resolution(self, options):
        ...
        """
        if (
            self._constants[Constants.LARGE_RESOLUTION_THRESHOLD] * options[Options.RHOEND]
            < self.resolution
        ):
            self.resolution *= self._constants[Constants.DECREASE_RESOLUTION_FACTOR]
        elif (
            self._constants[Constants.MODERATE_RESOLUTION_THRESHOLD] * options[Options.RHOEND]
            < self.resolution
        ):
    ...
    def _get_low_penalty(self):
        r_val_ub = np.c_[
            (self.models.interpolation.x_base[np.newaxis, :] + self.models.interpolation.xpt.T)
