...
def _zeros_like_dispatcher(a, dtype=None, order=None, subok=None, shape=None, *, device=None):
    return (a,)
...
def zeros_like(a, dtype=None, order="K", subok=True, shape=None, *, device=None):
    ...
    """
    res = empty_like(a, dtype=dtype, order=order, subok=subok, shape=shape, device=device)
...
def _ones_like_dispatcher(a, dtype=None, order=None, subok=None, shape=None, *, device=None):
    return (a,)
...
def _full_dispatcher(shape, fill_value, dtype=None, order=None, *, device=None, like=None):
    return (like,)
...
def isclose(a, b, rtol=1.0e-5, atol=1.0e-8, equal_nan=False):
    ...
    x, y, atol, rtol = (
        a if isinstance(a, (int, float, complex)) else asanyarray(a) for a in (a, b, atol, rtol)
    )
...
def array_equal(a1, a2, equal_nan=False):
    ...
    cannot_have_nan = _dtype_cannot_hold_nan(a1.dtype) and _dtype_cannot_hold_nan(a2.dtype)
    if cannot_have_nan:
...
def astype(x, dtype, /, *, copy=True, device=None):
    ...
    if not (isinstance(x, np.ndarray) or isscalar(x)):
        raise TypeError("Input should be a NumPy array or scalar. " f"It is a {type(x)} instead.")
    if device is not None and device != "cpu":
        raise ValueError('Device not understood. Only "cpu" is allowed, but received:' f" {device}")
