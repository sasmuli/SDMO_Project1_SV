...
class Quadratic:
    ...
    def __init__(self, interpolation, values, debug):
        ...
        if interpolation.npt < interpolation.n + 1:
            raise ValueError(
                f"The number of interpolation points must be at least " f"{interpolation.n + 1}."
            )
    ...
    def solve_systems(interpolation, rhs):
        ...
        assert rhs.ndim == 2 and rhs.shape[0] == npt + n + 1, "The shape of `rhs` is not valid."
...
class Models:
    ...
    def _check_interpolation_conditions(self):
        ...
        if error_fun > tol * np.max(np.abs(self.fun_val), initial=1.0):
            warnings.warn(
                "The interpolation conditions for the objective function are " "not satisfied.",
                RuntimeWarning,
                2,
            )
