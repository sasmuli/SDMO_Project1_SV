...
class TestDIRECT:
    ...
    def test_direct_callback(self, locally_biased):
        ...
        res_callback = direct(
            self.sphere, self.bounds_sphere, locally_biased=locally_biased, callback=callback
        )
    ...
    def test_vol_tol(self, vol_tol, locally_biased):
        bounds = 4 * [(-10.0, 10.0)]
        res = direct(
            self.sphere, bounds=bounds, vol_tol=vol_tol, len_tol=0.0, locally_biased=locally_biased
        )
        assert res.status == 4
        assert res.success
        assert_allclose(res.x, np.zeros((4,)))
        message = (
            "The volume of the hyperrectangle containing the lowest "
    ...
    def test_f_circle_with_args(self, locally_biased):
        ...
        res = direct(
            self.circle_with_args, bounds, args=(1, 1), maxfun=1250, locally_biased=locally_biased
        )
    ...
    def test_no_segmentation_fault(self, locally_biased):
        ...
        bounds = [(-5.0, 20.0)] * 100
        result = direct(
            self.sphere, bounds, maxfun=10000000, maxiter=1000000, locally_biased=locally_biased
        )
    ...
    def test_fmin_rtol_validation(self, f_min_rtol):
        ...
        with pytest.raises(ValueError, match=error_msg):
            direct(
                self.styblinski_tang, self.bounds_stylinski_tang, f_min_rtol=f_min_rtol, f_min=0.0
            )
