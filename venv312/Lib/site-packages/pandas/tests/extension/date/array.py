...
class DateArray(ExtensionArray):
    def __init__(
        self,
        dates: dt.date | Sequence[dt.date] | tuple[np.ndarray, np.ndarray, np.ndarray] | np.ndarray,
    ) -> None:
        ...
        if isinstance(dates, list):
            ...
            # populate them
            for i, (y, m, d) in enumerate((date.year, date.month, date.day) for date in dates):
                self._year[i] = y
                self._month[i] = m
                self._day[i] = d
