...
LinksISD = [inverse_power, sqrt, inverse_squared, identity, logc, cauchy, probit, loglog]
...
def test_deriv():
    ...
    for link in Links:
        for k in range(10):
            ...
            if not isinstance(link, (type(inverse_power), type(inverse_squared), type(logc))):
...
def test_deriv2():
    ...
    for link in Links:
        for k in range(10):
            ...
            assert_allclose(d, da, rtol=5e-6, atol=1e-6, err_msg=str(link))
