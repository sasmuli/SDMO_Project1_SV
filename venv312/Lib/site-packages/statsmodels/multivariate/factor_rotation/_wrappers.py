...
def rotate_factors(A, method, *method_args, **algorithm_kwargs):
    ...
    if "algorithm" in algorithm_kwargs:
        ...
        algorithm = "gpa"
    assert not (
        "rotation_method" in algorithm_kwargs
    ...
    if method == "orthomax":
        ...
        assert len(method_args) == 2, (
            "Both %s family parameter and " "rotation_method should be " "provided" % method
        )
        ...
        if algorithm == "gpa":
            ...
            ff = lambda L=None, A=None, T=None: oblimin_objective(
                L=L, A=A, T=T, gamma=gamma, rotation_method=rotation_method, return_gradient=False
            )
        else:
            raise ValueError(
                "Algorithm %s is not possible for %s " "rotation" % (algorithm, method)
        ...
        if algorithm == "gpa":
            vgQ = lambda L=None, A=None, T=None: CF_objective(
                L=L, A=A, T=T, kappa=kappa, rotation_method=rotation_method, return_gradient=True
            )
        elif algorithm == "gpa_der_free":
            ff = lambda L=None, A=None, T=None: CF_objective(
                L=L, A=A, T=T, kappa=kappa, rotation_method=rotation_method, return_gradient=False
            )
        else:
            raise ValueError(
                "Algorithm %s is not possible for %s " "rotation" % (algorithm, method)
