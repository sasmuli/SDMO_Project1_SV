...
class TestLegendre:
    ...
    @pytest.mark.parametrize("zr", [0.5241717, 12.80232, -9.699001, 0.5122437, 0.1714377])
    @pytest.mark.parametrize("zi", [9.766818, 0.2999083, 8.24726, -22.84843, -0.8792666])
...
class TestAssocLegendreP:
    ...
    def test_specific(self, shape, norm):
        ...
        np.testing.assert_allclose(p_jac[1, -3], 0)
        np.testing.assert_allclose(p_jac[1, -2], 0)
        np.testing.assert_allclose(p_jac[1, -1], assoc_legendre_p_1_m1_jac(x, norm=norm))
        ...
        np.testing.assert_allclose(p_jac[2, -4], 0)
        np.testing.assert_allclose(p_jac[2, -3], 0)
        np.testing.assert_allclose(p_jac[2, -2], assoc_legendre_p_2_m2_jac(x, norm=norm))
        np.testing.assert_allclose(p_jac[2, -1], assoc_legendre_p_2_m1_jac(x, norm=norm))
        ...
        np.testing.assert_allclose(p_jac[3, -4], 0)
        np.testing.assert_allclose(p_jac[3, -3], assoc_legendre_p_3_m3_jac(x, norm=norm))
        np.testing.assert_allclose(p_jac[3, -2], assoc_legendre_p_3_m2_jac(x, norm=norm))
        np.testing.assert_allclose(p_jac[3, -1], assoc_legendre_p_3_m1_jac(x, norm=norm))
        ...
        np.testing.assert_allclose(p_jac[4, -4], assoc_legendre_p_4_m4_jac(x, norm=norm))
        np.testing.assert_allclose(p_jac[4, -3], assoc_legendre_p_4_m3_jac(x, norm=norm))
        np.testing.assert_allclose(p_jac[4, -2], assoc_legendre_p_4_m2_jac(x, norm=norm))
        np.testing.assert_allclose(p_jac[4, -1], assoc_legendre_p_4_m1_jac(x, norm=norm))
    ...
    def test_all_limits(self, m_max, n_max, x):
        ...
        np.testing.assert_allclose(
            p_jac[:, 2], np.where(n >= 2, -pow(x, n + 1) * (n + 2) * (n + 1) * n * (n - 1) / 4, 0)
        )
...
class TestMultiAssocLegendreP:
    ...
    def test_specific(self, shape, branch_cut, z_min, z_max, norm):
        ...
        p, p_jac = assoc_legendre_p_all(4, 4, z, branch_cut=branch_cut, norm=norm, diff_n=1)
...
class TestSphLegendreP:
    ...
    def test_ode(self, shape):
        ...
        np.testing.assert_allclose(
            np.sin(theta) * p_hess,
            -np.cos(theta) * p_jac - (n * (n + 1) * np.sin(theta) - m * m / np.sin(theta)) * p,
            rtol=1e-05,
            atol=1e-08,
        )
...
class TestLegendreFunctions:
    ...
    def test_clpmn_close_to_real_2(self):
        ...
        assert_array_almost_equal(
            np.array([clp_plus, clp_minus]),
            np.array([special.lpmv(m, n, x), special.lpmv(m, n, x)]),
            7,
        )
    ...
    def test_lqn(self):
        lqf = special.lqn(2, 0.5)
        assert_array_almost_equal(
            lqf, (np.array([0.5493, -0.7253, -0.8187]), np.array([1.3333, 1.216, -0.8427])), 4
        )
    ...
    @pytest.mark.parametrize("m,n", [(0, 1), (1, 2), (1, 4), (3, 8), (11, 16), (19, 32)])
    @pytest.mark.parametrize("z_inexact", [False, True])
    @pytest.mark.parametrize("input_shape", [(), (1,), (2,), (2, 1), (1, 2), (2, 2), (2, 2, 1)])
...
def assoc_legendre_p_2_m1(z, *, branch_cut=2, norm=False):
    ...
    return -branch_cut_sign * fac * z * assoc_legendre_p_1_1(z, branch_cut=branch_cut) / 2
...
def assoc_legendre_p_3_1(z, *, branch_cut=2, norm=False):
    ...
    return 3 * fac * (5 * z * z - 1) * assoc_legendre_p_1_1(z, branch_cut=branch_cut) / 2
...
def assoc_legendre_p_3_3(z, *, branch_cut=2, norm=False):
    ...
    return 15 * branch_cut_sign * fac * (1 - z * z) * assoc_legendre_p_1_1(z, branch_cut=branch_cut)
...
def assoc_legendre_p_3_m1(z, *, branch_cut=2, norm=False):
    ...
    return (
        branch_cut_sign * fac * (1 - 5 * z * z) * assoc_legendre_p_1_1(z, branch_cut=branch_cut) / 8
    )
...
def assoc_legendre_p_4_1(z, *, branch_cut=2, norm=False):
    ...
    return 5 * fac * (7 * z * z - 3) * z * assoc_legendre_p_1_1(z, branch_cut=branch_cut) / 2
...
def assoc_legendre_p_1_m1_jac(z, *, branch_cut=2, norm=False):
    ...
    return -branch_cut_sign * fac * z * assoc_legendre_p_1_1_jac_div_z(z, branch_cut=branch_cut) / 2
...
def assoc_legendre_p_2_1_jac(z, *, branch_cut=2, norm=False):
    ...
    return 3 * fac * (2 * z * z - 1) * assoc_legendre_p_1_1_jac_div_z(z, branch_cut=branch_cut)
...
def assoc_legendre_p_3_m3_jac(z, *, branch_cut=2, norm=False):
    ...
    return fac * (z * z - 1) * z * assoc_legendre_p_1_1_jac_div_z(z, branch_cut=branch_cut) / 16
...
def sph_legendre_p_4_0(theta):
    ...
    return fac * (35 * np.square(np.square(np.cos(theta))) - 30 * np.square(np.cos(theta)) + 3) / 8


def sph_legendre_p_4_1(theta):
    fac = sph_legendre_factor(4, 1)

    return -5 * fac * (7 * np.square(np.cos(theta)) - 3) * np.cos(theta) * np.abs(np.sin(theta)) / 2


def sph_legendre_p_4_2(theta):
    fac = sph_legendre_factor(4, 2)

    return -15 * fac * (7 * np.square(np.cos(theta)) - 1) * (np.square(np.cos(theta)) - 1) / 2
...
def sph_legendre_p_4_m2(theta):
    ...
    return -fac * (7 * np.square(np.cos(theta)) - 1) * (np.square(np.cos(theta)) - 1) / 48


def sph_legendre_p_4_m1(theta):
    fac = sph_legendre_factor(4, -1)

    return fac * (7 * np.square(np.cos(theta)) - 3) * np.cos(theta) * np.abs(np.sin(theta)) / 8
...
def sph_legendre_p_4_0_jac(theta):
    ...
    return -5 * fac * (7 * np.square(np.cos(theta)) - 3) * np.sin(theta) * np.cos(theta) / 2
...
def sph_legendre_p_4_1_jac(theta):
    ...
    return (
        5
        * fac
        * (-3 + 27 * np.square(np.cos(theta)) - 28 * np.square(np.square(np.cos(theta))))
        * (2 * np.heaviside(np.sin(theta), 1) - 1)
        / 2
    )
...
def sph_legendre_p_4_3_jac(theta):
    ...
    return -105 * fac * (4 * np.square(np.cos(theta)) - 1) * np.abs(np.sin(theta)) * np.sin(theta)
...
def sph_legendre_p_4_m3_jac(theta):
    ...
    return fac * (4 * np.square(np.cos(theta)) - 1) * np.abs(np.sin(theta)) * np.sin(theta) / 48
