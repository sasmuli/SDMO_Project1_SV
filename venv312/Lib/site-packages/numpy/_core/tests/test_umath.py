...
class TestDivisionIntegerOverflowsAndDivideByZero:
    ...
    @pytest.mark.parametrize(
        "operation",
        [np.remainder, np.fmod, np.divmod, np.floor_divide, operator.mod, operator.floordiv],
    )
...
class TestPower:
    ...
    def test_power_complex(self):
        ...
        assert_almost_equal(
            x**14 * norm,
            [i * norm for i in [-76443 + 16124j, 23161315 + 58317492j, 5583548873 + 2465133864j]],
        )
...
class TestLog:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (1.000000000000001 + 3e-08j, 1.5602230246251546e-15 + 2.999999999999996e-08j),
            (0.9999995000000417 + 0.0009999998333333417j, 7.831475869017683e-18 + 0.001j),
            (0.9999999999999996 + 2.999999999999999e-08j, 5.9107901499372034e-18 + 3e-08j),
            (0.99995000042 - 0.009999833j, -7.015159763822903e-15 - 0.009999999665816696j),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (np.complex64(0.9999999 + 1e-06j), np.complex64(-1.192088e-07 + 1.0000001e-06j)),
        ],
    )
...
class TestSpecialFloats:
    ...
    @pytest.mark.skipif(IS_WASM, reason="fp errors don't work in wasm")
    @pytest.mark.xfail(
        sys.platform.startswith("darwin"), reason="underflow is triggered for scalar 'sin'"
    )
    ...
    @pytest.mark.skipif(IS_WASM, reason="fp errors don't work in wasm")
    @pytest.mark.skipif(
        "bsd" in sys.platform, reason="fallback implementation may not raise, see gh-2487"
    )
    def test_cosh(self):
        in_ = [np.nan, -np.nan, np.inf, -np.inf]
        out = [np.nan, np.nan, np.inf, np.inf]
...
class TestFPClass:
    ...
    def test_fpclass(self, stride):
        arr_f64 = np.array(
            [np.nan, -np.nan, np.inf, -np.inf, -1.0, 1.0, -0.0, 0.0, 2.2251e-308, -2.2251e-308],
            dtype="d",
        )
        arr_f32 = np.array(
            [np.nan, -np.nan, np.inf, -np.inf, -1.0, 1.0, -0.0, 0.0, 1.4013e-045, -1.4013e-045],
            dtype="f",
        )
        nan = np.array(
    ...
    def test_avx_based_ufunc(self):
        ...
        for func, prop in avx_ufuncs.items():
            ...
            for size in range(1, 32):
                ...
                x_f32 = np.random.uniform(low=minval, high=maxval, size=size).astype(np.float32)
...
def assert_hypot_isnan(x, y):
    with np.errstate(invalid="ignore"):
        assert_(np.isnan(ncu.hypot(x, y)), f"hypot({x}, {y}) is {ncu.hypot(x, y)}, not nan")


def assert_hypot_isinf(x, y):
    with np.errstate(invalid="ignore"):
        assert_(np.isinf(ncu.hypot(x, y)), f"hypot({x}, {y}) is {ncu.hypot(x, y)}, not inf")
...
class TestMaximum(_FilterInvalids):
    ...
    def test_strided_array(self):
        ...
        assert_equal(
            np.maximum(arr1[:6:2], arr2[::3], out=out[::3]), np.array([-2.0, 10.0, np.nan])
        )
...
class TestAbsoluteNegative:
    ...
    def test_noncontiguous(self, dtype, big):
        data = np.array(
            [-1.0, 1.0, -0.0, 0.0, 2.2251e-308, -2.5, 2.5, -6, 6, -2.2251e-308, -8, 10], dtype=dtype
        )
        expect = np.array(
            [1.0, -1.0, 0.0, -0.0, -2.2251e-308, 2.5, -2.5, 6, -6, 2.2251e-308, 8, -10], dtype=dtype
        )
...
class TestSpecialMethods:
    ...
    def test_ufunc_override_methods(self):
        ...
        assert_equal(
            res[4], {"dtype": "dtype0", "out": ("out0",), "keepdims": "keep0", "axis": "axis0"}
        )
        ...
        assert_equal(
            res[4], {"axis": 0, "dtype": None, "keepdims": False, "initial": 2, "where": True}
        )
        ...
        assert_equal(
            res[4], {"axis": 0, "dtype": None, "keepdims": False, "initial": None, "where": True}
        )
    ...
    def test_ufunc_docstring(self):
        ...
        new_doc = "new docs"
        expected_dict = {} if IS_PYPY else {"__module__": "numpy", "__qualname__": "add"}
...
class TestRoundingFunctions:
    ...
    @pytest.mark.parametrize("dtype", [np.bool, np.float64, np.float32, np.int64, np.uint32])
