...
class TestODR:
    ...
    def explicit_fjb(self, B, x):
        eBx = np.exp(B[2] * x)
        res = np.vstack(
            [np.ones(x.shape[-1]), np.power(eBx - 1.0, 2), B[1] * 2.0 * (eBx - 1.0) * eBx * x]
        )
        return res
    ...
    def multi_fcn(self, B, x):
        ...
        r = (B[0] - B[1]) * np.power(
            np.sqrt(np.power(1.0 + omega * ctheta, 2) + np.power(omega * stheta, 2)), -B[4]
        )
    ...
    def lorentz(self, beta, x):
        return (
            ...
            * beta[1]
            * beta[2]
            / np.sqrt(np.power(x * x - beta[2] * beta[2], 2.0) + np.power(beta[1] * x, 2.0))
        )
    ...
    @pytest.mark.skipif(
        True, reason="Fortran I/O prone to crashing so better " "not to run this test, see gh-13127"
    )
