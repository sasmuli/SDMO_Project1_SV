...
def test_basic():
    ...
    p, res = statespace(
        endog, exog=exog, order=(1, 0, 0), include_constant=True, concentrate_scale=False
    )
    ...
    p, res = statespace(
        endog, exog=exog, order=(1, 0, 0), include_constant=False, concentrate_scale=False
    )
...
def test_start_params():
    ...
    p, _ = statespace(
        endog, order=(1, 0, 0), start_params=[0, 1.0, 1.0], enforce_stationarity=False
    )
    p, _ = statespace(
        endog, order=(0, 0, 1), start_params=[0, 1.0, 1.0], enforce_invertibility=False
    )
