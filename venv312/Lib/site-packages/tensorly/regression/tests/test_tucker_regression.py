...
def test_TuckerRegressor():
    ...
    X = T.tensor(
        np.random.normal(size=(1200, image_height, image_width, n_channels), loc=0, scale=1)
    )
    ...
    estimator = TuckerRegressor(
        weight_ranks=ranks, tol=10e-8, reg_W=1, n_iter_max=200, verbose=True
    )
