    ...
    def __iter__(self):
        ...
        if weight is None:
            ...
            for n in self._nodes:
                ...
                ) + sum(d.get(weight, 1) for key_dict in preds.values() for d in key_dict.values())
                yield (n, deg)
...
class OutEdgeDataView(EdgeViewABC):
    ...
    def __iter__(self):
        return (
            self._report(n, nbr, dd) for n, nbrs in self._nodes_nbrs() for nbr, dd in nbrs.items()
        )
...
class InEdgeDataView(OutEdgeDataView):
    ...
    def __iter__(self):
        return (
            self._report(nbr, n, dd) for n, nbrs in self._nodes_nbrs() for nbr, dd in nbrs.items()
        )
