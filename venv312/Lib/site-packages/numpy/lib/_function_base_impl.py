...
class vectorize:
    ...
    def __init__(
        self, pyfunc=np._NoValue, otypes=None, doc=None, excluded=None, cache=False, signature=None
    ):
    ...
    def _vectorize_call_with_signature(self, func, args):
        ...
        for index in np.ndindex(*broadcast_shape):
            ...
            if nout != n_results:
                raise ValueError(
                    "wrong number of outputs from pyfunc: expected %r, got %r" % (nout, n_results)
                )
...
def _cov_dispatcher(
    m, y=None, rowvar=None, bias=None, ddof=None, fweights=None, aweights=None, *, dtype=None
):
...
def corrcoef(x, y=None, rowvar=True, bias=np._NoValue, ddof=np._NoValue, *, dtype=None):
    ...
    if bias is not np._NoValue or ddof is not np._NoValue:
        # 2015-03-15, 1.10
        warnings.warn(
            "bias and ddof have no effect and are deprecated", DeprecationWarning, stacklevel=2
        )
...
def _quantile(
    ...
    if weights is None:
        ...
        try:
            ...
            raise ValueError(
                f"{method!r} is not a valid method. Use one of: " f"{_QuantileMethods.keys()}"
            ) from None
...
def delete(arr, obj, axis=None):
    ...
    if single_value:
        ...
        if obj < -N or obj >= N:
            raise IndexError(f"index {obj} is out of bounds for axis {axis} with " f"size {N}")
        if obj < 0:
            obj += N
        newshape[axis] -= 1
        new = empty(newshape, arr.dtype, arrorder)
...
def insert(arr, obj, values, axis=None):
    ...
    if isinstance(obj, slice):
        ...
        if indices.dtype == bool:
            if obj.ndim != 1:
                raise ValueError("boolean array argument obj to insert " "must be one dimensional")
