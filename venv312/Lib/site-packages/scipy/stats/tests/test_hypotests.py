...
class TestCvm:
    ...
    def test_cdf_4(self):
        assert_allclose(
            _cdf_cvm([0.02983, 0.04111, 0.12331, 0.94251], 4), [0.01, 0.05, 0.5, 0.999], atol=1e-4
        )

    def test_cdf_10(self):
        assert_allclose(
            _cdf_cvm([0.02657, 0.03830, 0.12068, 0.56643], 10), [0.01, 0.05, 0.5, 0.975], atol=1e-4
        )
    ...
    def test_cdf_inf(self):
        assert_allclose(
            _cdf_cvm([0.02480, 0.03656, 0.11888, 1.16204]), [0.01, 0.05, 0.5, 0.999], atol=1e-4
        )
...
class TestMannWhitneyU:
    ...
    @pytest.mark.parametrize("kwargs_update", [{"x": []}, {"y": []}, {"x": [], "y": []}])
    ...
    def test_exact_distribution(self):
        ...
        for n, table in p_tables.items():
            for m, p in table.items():
                ...
                assert_allclose(
                    _mwu_state.s.cdf(k=u2) + _mwu_state.s.sf(k=u2) - _mwu_state.s.pmf(k=u2), 1
                )
...
class TestBoschlooExact:
    ...
    def test_raises(self):
        ...
        # Test value error on wrong alternative param
        error_msg = (
            r"`alternative` should be one of \('two-sided', 'less', " r"'greater'\), found .*"
        )
        with assert_raises(ValueError, match=error_msg):
...
class TestTukeyHSD:
    ...
    def test_compare_r(self):
        ...
        """
        res_expect = np.asarray(str_res.replace(" - ", " ").split()[5:], dtype=float).reshape(
            (3, 6)
        )
        data = (
