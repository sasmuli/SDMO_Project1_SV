...
class TestExpmActionSimple:
    ...
    def test_complex(self):
        ...
        observed = expm_multiply(A, B)
        expected = np.array(
            [1j * np.exp(1j) + 1j * (1j * np.cos(1) - np.sin(1)), 1j * np.exp(1j)], dtype=complex
        )
        assert_allclose(observed, expected)
        observed = estimated(expm_multiply)(aslinearoperator(A), B)
        assert_allclose(observed, expected)
...
class TestExpmActionInterval:
    ...
    def test_sparse_expm_multiply_interval_dtypes(self):
        ...
        assert_allclose(
            np.diag(expm_multiply(A, scipy.sparse.csr_array(BI), 0, 1)[-1]), Aexpm.dot(B)
        )
    ...
    def _help_test_specific_expm_interval_status(self, target_status):
        ...
        for num in [14, 13, 2] * nrepeats:
            ...
            status = _expm_multiply_interval(
                A, B, start=start, stop=stop, num=num, endpoint=endpoint, status_only=True
            )
