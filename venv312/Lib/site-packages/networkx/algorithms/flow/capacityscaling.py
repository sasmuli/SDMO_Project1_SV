...
def _build_residual_network(G, demand, capacity, weight):
    ...
    if G.is_multigraph():
        ...
        edge_list = [
            (u, v, 0, e) for u, v, e in G.edges(data=True) if u != v and e.get(capacity, inf) > 0
        ]
    ...
    inf = (
        max(
            ...
            2
            * sum(e[capacity] for u, v, k, e in edge_list if capacity in e and e[capacity] != inf),
        )
        or 1
    )
...
def _build_flow_dict(G, R, capacity, weight):
    ...
    if G.is_multigraph():
        ...
        for u in G:
            ...
            flow_dict[u].update(
                (v, e["flow"]) for v, es in R[u].items() for e in es.values() if e["flow"] > 0
            )
    return flow_dict


@nx._dispatchable(node_attrs="demand", edge_attrs={"capacity": float("inf"), "weight": 0})
