...
from numpy.testing import assert_almost_equal, assert_equal, assert_allclose, assert_
...
class TestMixedLM:
    ...
    def test_vcomp_3(self):
        ...
        assert_allclose(
            result.resid.iloc[0:4], np.r_[-1.180753, 0.279966, 0.578576, -0.667916], rtol=1e-3
        )
        assert_allclose(
    ...
    def test_dietox_slopes(self):
        ...
        #  as.numeric(VarCorr(r)[[1]])
        assert_allclose(
            result.cov_re.values.ravel(), np.r_[19.190922, 0.293568, 0.293568, 0.409695], rtol=1e-2
        )
    ...
    def test_vcomp_formula(self):
        ...
        model2 = MixedLM.from_formula(
            "y ~ x1", groups=groups, re_formula="0 + re1 + re2", vc_formula=vc_formula, data=df
        )
        result2 = model2.fit()
...
def test_summary_col():
    ...
    out = summary_col(
        [results1, results2], stars=True, regressor_order=["Group Var", "Intercept", "X", "Y"]
    )
    s = (
        "\n=============================\n              Y         X    \n"
...
def test_get_distribution():
    ...
    m = MixedLM.from_formula(
        "y ~ 0 + x1 + x2", groups="grp", re_formula="0 + z0 + z1", vc_formula=vcf, data=df
    )
