    ...
    for group_id in groups:
        ...
        for other_group, group_edge_types in neighbor_info[next(iter(group_set))].items():
            if group_edge_types:
                ...
                edge_types = [
                    dict(zip(edge_attributes, edge_type)) for edge_type in group_edge_types
                ]
...
def snap_aggregation(
    ...
    """
    edge_types = {
        edge: tuple(attrs.get(attr) for attr in edge_attributes) for edge, attrs in G.edges.items()
    }
    ...
    group_lookup = {
        node: tuple(attrs[attr] for attr in node_attributes) for node, attrs in G.nodes.items()
    }
    groups = defaultdict(set)
    for node, node_type in group_lookup.items():
        groups[node_type].add(node)

    eligible_group_id, nbr_info = _snap_eligible_group(G, groups, group_lookup, edge_types)
