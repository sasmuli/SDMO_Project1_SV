    ...
    **kwargs
    ...
    def prepare_data(self):
        ...
        if self.simple_differencing and (self.orig_k_diff > 0 or self.orig_k_seasonal_diff > 0):
            ...
            endog = diff(
                endog.copy(), self.orig_k_diff, self.orig_k_seasonal_diff, self.seasonal_periods
            )
            if exog is not None:
                exog = diff(
                    exog.copy(), self.orig_k_diff, self.orig_k_seasonal_diff, self.seasonal_periods
                )
    ...
    def initial_design(self):
        ...
        if self.state_regression:
            if self._k_order > 0:
                design = np.c_[
                    np.reshape(np.repeat(design, self.nobs), (design.shape[0], self.nobs)).T,
                    self.exog,
                ].T[None, :, :]
            else:
        ...
        if k_ar + k_ma + k_trend > 0:
            try:
                ...
                if len(endog) == 0:
                    ...
                    residuals = np.r_[
                        np.zeros(k_params_ma * 2, dtype=endog.dtype), endog - np.mean(endog)
                    ]
    ...
    def start_params(self):
        ...
        (params_trend, params_ar, params_ma, params_variance) = self._conditional_sum_squares(
            endog,
            self.k_ar,
            self.polynomial_ar,
            self.k_ma,
        ...
        invalid_ar = (
            self.k_ar > 0 and self.enforce_stationarity and not is_invertible(np.r_[1, -params_ar])
        )
        if invalid_ar:
            warn(
        ...
        invalid_ma = (
            self.k_ma > 0 and self.enforce_invertibility and not is_invertible(np.r_[1, params_ma])
        )
        if invalid_ma:
            warn(
        ...
        invalid_seasonal_ar = (
            ...
            and not is_invertible(np.r_[1, -params_seasonal_ar])
        )
        if invalid_seasonal_ar:
            warn(
                "Non-stationary starting seasonal autoregressive"
                " Using zeros as starting parameters."
            )
        ...
        invalid_seasonal_ma = (
            ...
            and not is_invertible(np.r_[1, params_seasonal_ma])
        )
        if invalid_seasonal_ma:
            warn(
                "Non-invertible starting seasonal moving average"
                " Using zeros as starting parameters."
            )
    ...
    def endog_names(self, latex=False):
        ...
        if self.k_seasonal_diff > 0:
            if self.k_seasonal_diff == 1:
                seasonal_diff = (r"\Delta_%d" if latex else "DS%d") % (self.seasonal_periods)
            else:
                seasonal_diff = (r"\Delta_%d^%d" if latex else "D%dS%d") % (
                    self.k_seasonal_diff,
    ...
    def model_orders(self):
        ...
        return {
            ...
            "exog_variance": (
                self._k_exog if (self.state_regression and self.time_varying_regression) else 0
            ),
    ...
    def _get_model_names(self, latex=False):
        ...
        if self.state_regression and self.time_varying_regression:
            if not self.concentrate_scale:
                ...
                exog_var_template = (
                    "$\\sigma_\\text{%s}^2 / \\sigma_\\zeta^2$" if latex else "snr.%s"
                )
            names["exog_variance"] = [
        ...
        if self.measurement_error:
            if not self.concentrate_scale:
                ...
                meas_var_tpl = (
                    "$\\sigma_\\eta^2 / \\sigma_\\zeta^2$" if latex else "snr.measurement_error"
                )
    ...
    def untransform_params(self, constrained):
        ...
        if self.k_ma_params > 0:
            ...
            if self.enforce_invertibility:
                unconstrained[start:end] = unconstrain_stationary_univariate(
    ...
    def _validate_can_fix_params(self, param_names):
        ...
        items = [
            ("ar", "autoregressive", self.enforce_stationarity, "`enforce_stationarity=True`"),
            (
                "seasonal_ar",
                "seasonal autoregressive",
            ...
            ("ma", "moving average", self.enforce_invertibility, "`enforce_invertibility=True`"),
            (
                "seasonal_ma",
                "seasonal moving average",
        ...
        for name, title, condition, condition_desc in items:
            ...
            if condition and fix_any and not fix_all:
                raise ValueError(
                    "Cannot fix individual %s parameters when"
