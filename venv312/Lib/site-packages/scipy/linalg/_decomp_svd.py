...
def svd(
    ...
    lwork = _compute_lwork(
        gesXd_lwork, a1.shape[0], a1.shape[1], compute_uv=compute_uv, full_matrices=full_matrices
    )

    # perform decomposition
    u, s, v, info = gesXd(
        a1, compute_uv=compute_uv, lwork=lwork, full_matrices=full_matrices, overwrite_a=overwrite_a
    )
...
def subspace_angles(A, B):
    ...
    if len(B) != len(QA):
        raise ValueError(
            "A and B must have the same number of rows, got " f"{QA.shape[0]} and {B.shape[0]}"
        )
    QB = orth(B)
    del B
