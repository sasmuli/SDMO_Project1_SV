...
class ImageFileDirectory_v2(_IFDv2Base):
    ...
    def __init__(
        ...
        self.reset()
        self.next = (
            self._unpack("Q", ifh[8:])[0] if self._bigtiff else self._unpack("L", ifh[4:])[0]
        )
    ...
    def _setitem(self, tag: int, value: Any, legacy_api: bool) -> None:
        ...
        if not is_ifd:
            values = tuple(
                info.cvt_enum(value) if isinstance(value, str) else value for value in values
            )
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def seek(self, frame: int) -> None:
        ...
        if self._im is not None and (
            self.im.size != self._tile_size or self.im.mode != self.mode or self.readonly
        ):
            self._im = None
