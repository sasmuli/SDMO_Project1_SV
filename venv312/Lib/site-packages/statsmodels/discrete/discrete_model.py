...
class MultinomialModel(BinaryModel):
    ...
    def _derivative_predict(self, params, exog=None, transform="dydx"):
        ...
        F1[:, other_idx] = (-eXB.T[:, :, None] * X * repeat_eXB / (sum_eXB**2)).transpose(
            (1, 0, 2)
        )[:, other_idx]
...
class CountModel(DiscreteModel):
    def __init__(
        self, endog, exog, offset=None, exposure=None, missing="none", check_rank=True, **kwargs
    ):
        self._check_kwargs(kwargs)
        super().__init__(
            endog, exog, check_rank, missing=missing, offset=offset, exposure=exposure, **kwargs
        )
...
class Poisson(CountModel):
    ...
    def fit(
        ...
        **kwargs,
        ...
        cntfit = super(CountModel, self).fit(
            start_params=start_params,
            method=method,
            maxiter=maxiter,
            full_output=full_output,
    ...
    def fit_constrained(self, constraints, start_params=None, **fit_kwds):
        ...
        **fit_kwds : keyword arguments
        ...
        res = self.fit(
            maxiter=0, method="nm", disp=0, warn_convergence=False
    ...
    def predict(
        ...
        if which in ["mean", "linear"]:
            return super().predict(
                params, exog=exog, exposure=exposure, offset=offset, which=which, linear=linear
            )
...
class GeneralizedPoisson(CountModel):
    ...
    def score_obs(self, params):
        ...
        dparams = dmudb * (
            -a4 / a1 + a3 * a2 / (a1**2) + (1 + a4) * ((y - 1) / a2 - 1 / a1) + 1 / mu
        )
    ...
    def hessian_factor(self, params):
        ...
        dbb = mu * (
            ...
            * (
            ...
            + (-a4 / a1 + a3 * a2 / a1**2 + (y - 1) * (1 + a4) / a2 - (1 + a4) / a1 + 1 / mu)
        )
...
class NegativeBinomial(CountModel):
    ...
    def _score_nbin(self, params, Q=0):
        ...
        if Q == 1:  # nb1
            ...
            dalpha = (
                (alpha * (y - mu * np.log(prob) - mu * (dgpart + 1)) - mu * (np.log(prob) + dgpart))
                / (alpha**2 * (alpha + 1))
            ).sum()
        ...
        for i in range(dim):
            for j in range(dim):
                ...
                hess_arr[i, j] = np.squeeze(
                    np.sum(-exog[:, i, None] * exog[:, j, None] * const_arr, axis=0)
                )
        tri_idx = np.triu_indices(dim, k=1)
        hess_arr[tri_idx] = hess_arr.T[tri_idx]
        return hess_arr
    ...
    def _hessian_nb1(self, params):
        ...
        mu2 = mu**2
        dada = (
            alpha3 * mu * (2 * log_alpha + 2 * dgpart + 3)
            - 2 * alpha3 * y
        ...
        for i in range(dim):
            for j in range(dim):
                if j > i:
                    continue
                hess_arr[i, j] = np.sum(
                    -exog[:, i, None] * exog[:, j, None] * const_arr, axis=0
                ).squeeze()
        tri_idx = np.triu_indices(dim, k=1)
        hess_arr[tri_idx] = hess_arr.T[tri_idx]
    ...
    def predict(
        ...
        exog, offset, exposure = self._get_predict_arrays(
            exog=exog, offset=offset, exposure=exposure
        )
...
class NegativeBinomialP(CountModel):
    ...
    def hessian(self, params):
        ...
        coeff = mu**2 * (
            ...
            + (-(1 + a4) * a3 / a2 + y / mu + a4 * (lprob + dgpart + 1)) / mu
        )
        ...
        da2 = (
            a1
            * (2 * lprob + 2 * dgpart + 3 - 2 * a3 / a2 + a1 * pgpart - 2 * prob + prob * a3 / a2)
            / alpha**2
        )
    ...
    def hessian_factor(self, params):
        ...
        coeff = mu**2 * (
            ...
            + (-(1 + a4) * a3 / a2 + y / mu + a4 * (np.log(a1) - np.log(a2) + dgpart + 1)) / mu
        )
        ...
        hfba = (
            ...
            * (
                ...
                + p * (a3 / mu + a4) / a2
                + a4 * (polygamma(1, a1) - polygamma(1, a3))
            )
            / alpha
        )
    ...
    def llnull(self):
        ...
        if optim_kwds:
            ...
            res_null = mod_null.fit(
                start_params=sp_null, method="nm", warn_convergence=False, maxiter=10000, disp=0
            )
            res_null = mod_null.fit(
...
class L1CountResults(DiscreteResults):
    __doc__ = _discrete_results_docs % {
        "one_line_description": "A results class for count data fit by l1 regularization",
...
class PoissonResults(CountResults):
    ...
    def predict_prob(self, n=None, exog=None, exposure=None, offset=None, transform=True):
        ...
        mu = self.predict(
            exog=exog, exposure=exposure, offset=offset, transform=transform, which="mean"
        )[:, None]
        # uses broadcasting
...
class L1MultinomialResults(MultinomialResults):
    __doc__ = _discrete_results_docs % {
        "one_line_description": "A results class for multinomial data fit by l1 regularization",
        "extra_attr": _l1_results_attr,
    }
