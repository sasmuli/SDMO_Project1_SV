...
def check_irf(test, mod, results, params=None):
    ...
    for i in range(3):
        ...
        assert_allclose(
            res.impulse_responses(10, i, orthogonalized=True), results_var_R_output[columns]
        )
...
def test_var_ct_as_exog0():
    ...
    columns = [f"{test}.fcast.{name}.fcst" for name in endog.columns]
    assert_allclose(
        res.forecast(10, exog=exog0_fcast[:, :2]), results_var_R_output[columns].iloc[:10]
    )
...
def test_var_ct_as_exog1():
    ...
    columns = [f"{test}.fcast.{name}.fcst" for name in endog.columns]
    assert_allclose(
        res.forecast(10, exog=exog1_fcast[:, :2]), results_var_R_output[columns].iloc[:10]
    )
...
def test_var_ct_exog():
    ...
    mod = varmax.VARMAX(endog, order=(2, 0), exog=exog, trend="ct", loglikelihood_burn=2)
    res = mod.smooth(results["params"])
