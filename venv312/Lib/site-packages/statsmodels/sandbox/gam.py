...
def default_smoother(x, s_arg=None):
    ...
    if n < 500:
        ...
        if n < 200:
            nknots = 2 ** (a1 + (a2 - a1) * (n - 50) / 150.0)
        elif n < 800:
            nknots = 2 ** (a2 + (a3 - a2) * (n - 200) / 600.0)
        elif n < 3200:
            nknots = 2 ** (a3 + (a4 - a3) * (n - 800) / 2400.0)
        else:
            nknots = 200 + (n - 3200.0) ** 0.2
...
class Results:
    ...
    def predict(self, exog):
        ...
        if exog_smoothed.shape[0] == self.k_vars:
            ...
            warnings.warn("old orientation, colvars, will go away", FutureWarning)
    ...
    def smoothed(self, exog):
        ...
        return np.array(
            [
                self.smoothers[i].predict(exog[:, i]) + self.offset[i]
...
class AdditiveModel:
    ...
    def df_resid(self):
        ...
        return (
            self.results.Y.shape[0]
            - np.array([self.smoothers[i].df_fit() for i in range(self.exog.shape[1])]).sum()
        )
...
class Model(GLM, AdditiveModel):
    ...
    def next(self):
        ...
        Z = _results.predict(self.exog) + self.family.link.deriv(_results.mu) * (
            Y - _results.mu
        )  # - _results.alpha #?added alpha

        m = AdditiveModel(
            self.exog, smoothers=self.smoothers, weights=self.weights, family=self.family
        )
