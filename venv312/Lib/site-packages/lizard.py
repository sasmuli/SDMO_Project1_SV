...
class FunctionInfo(Nesting):  # pylint: disable=R0902
    ...
    location = property(
        lambda self: f" {self.name}@{self.start_line}-{self.end_line}@{self.filename}"
    )
    ...
    def parameters(self):
        ...
        matches = [re.search(r"(\w+)(\s=.*)?(\s:.*)?$", f) for f in self.full_parameters]
...
class FileInformation(object):  # pylint: disable=R0903
    ...
    ND = property(  # pylint: disable=C0103
        lambda self: sum(fun.max_nesting_depth for fun in self.function_list)
    )
...
class FileInfoBuilder(object):
    ...
    def pop_nesting(self):
        ...
        if isinstance(nest, FunctionInfo):
            ...
            self.current_function = self._nesting_stack.last_function or self.global_pseudo_function
    ...
    def try_new_function(self, name):
        self.current_function = FunctionInfo(
            self.with_namespace(name), self.fileinfo.filename, self.current_line
        )
    ...
    for file_info in module_infos:
        if file_info:
            for fun in file_info.function_list:
                if any(getattr(fun, attr) > limit for attr, limit in option.thresholds.items()):
                    yield fun
...
def whitelist_filter(warnings, script=None, whitelist=None):
    def _get_whitelist_item(script):
        ...
        if len(pieces) > 1:
            ...
            script = pieces[1]
        white["function_names"] = [x.strip().replace("##", "::") for x in script.split(",")]
        return white

    def _in_list(warning):
        return any(_match_whitelist_item(white, warning) for white in whitelist)

    def _match_whitelist_item(white, warning):
        return warning.name in white["function_names"] and warning.filename == white.get(
    ...
    whitelist = [_get_whitelist_item(line.split("#")[0]) for line in script.splitlines()]
...
class OutputScheme(object):
    ...
    def __init__(self, ext):
        ...
        self.items = [
            ...
            {"caption": caption, "value": part, "avg_caption": average}
    ...
    def average_captions(self):
        return "".join([e["avg_caption"] for e in self.items if e.get("avg_caption", None)])

    def average_formatter(self):
        return "".join(
            [
                "{{module.average_{ext[value]}:{size}.1f}}".format(
                    ext=e, size=len(e["avg_caption"])
                )
                for e in self.items
    ...
    def msvs_warning_format(self):
        return "{f.filename}({f.start_line}): warning: {f.name} ({f.long_name}) has " + ", ".join(
            [
                "{{f.{ext[value]}}} {caption}".format(ext=e, caption=e["caption"].strip())
...
def print_warnings(option, scheme, warnings):
    ...
    warn_str = "!!!! Warnings ({0}) !!!!".format(
        " or ".join("{0} > {1}".format(k, val) for k, val in option.thresholds.items())
    )
    for warning in warnings:
        if warning_count == 0:
            print("\n" + "=" * len(warn_str) + "\n" + warn_str)
...
class AllResult(object):
    def __init__(self, result):
        ...
        self.all_fun = list(
            itertools.chain(*(file_info.function_list for file_info in self.result))
        )
    ...
    def as_fileinfo(self):
        return FileInformation("", sum([f.nloc for f in self.result]), self.all_fun)


def print_total(warning_count, warning_nloc, all_result, scheme):
    print("=" * 90)
    print(
        "Total nloc  " + scheme.average_captions() + "  Fun Cnt  Warning" " cnt   Fun Rt   nloc Rt"
    )
    print("-" * 90)
    print(
        (
            ...
            + "{function_rate:10.2f}{nloc_rate:8.2f}"
        ).format(
    ...
    for module_info in saved_fileinfos:
        print(
            (
                ...
                + "{function_count:10d}"
                + "     {module.filename}"
            ).format(module=module_info, function_count=len(module_info.function_list))
        )
    return saved_fileinfos


def get_warnings(code_infos, option):
    warnings = whitelist_filter(warning_filter(option, code_infos), whitelist=option.whitelist)
    if isinstance(option.sorting, list) and option.sorting:
        warnings = sorted(warnings, reverse=True, key=lambda x: getattr(x, option.sorting[0]))
    return warnings
...
def get_all_source_files(paths, exclude_patterns, lans):
    ...
    def _support(reader):
        return not lans or set(lans).intersection(reader.language_names)
    ...
    def _validate_file(pathname):
        if gitignore_spec is not None and base_path is not None:
            ...
            if gitignore_spec.match_file(rel_path):
                return False
        return pathname in paths or (
            get_reader_for(pathname)
            and _support(get_reader_for(pathname))
            and all(not fnmatch(pathname, p) for p in exclude_patterns)
...
def get_extensions(extension_names):
    ...
    def expand_extensions(existing):
        for name in extension_names:
            ...
            existing.insert(
                len(existing) if not hasattr(ext, "ordering_index") else ext.ordering_index, ext
            )
        return existing
