...
class _coo_base(_data_matrix, _minmax_mixin):
        ...
        if self.nnz > 0:
            for i, idx in enumerate(self.coords):
                if idx.max() >= self.shape[i]:
                    raise ValueError(
                        f"axis {i} index {idx.max()} exceeds " f"matrix dimension {self.shape[i]}"
                    )
                if idx.min() < 0:
    ...
    def transpose(self, axes=None, copy=False):
        ...
        return self.__class__((self.data, permuted_coords), shape=permuted_shape, copy=copy)
    ...
    def resize(self, *shape) -> None:
        ...
        if is_truncating:
            mask = np.logical_and.reduce([idx < size for idx, size in zip(self.coords, shape)])
            if not mask.all():
    ...
    def toarray(self, order=None, out=None):
        ...
        if self.ndim == 1:
            coo_todense_nd(
                np.array([1]), self.nnz, self.ndim, self.coords[0], self.data, B.ravel("A"), fortran
            )
        elif self.ndim == 2:
            M, N = self.shape
            ...
            coo_todense_nd(strides, self.nnz, self.ndim, coords, self.data, B.ravel("A"), fortran)
    ...
    def todia(self, copy=False):
        ...
        if len(diags) > 100:
            ...
            warn(
                f"Constructing a DIA matrix with {len(diags)} diagonals " "is inefficient",
    ...
    def _add_dense(self, other):
        ...
        if self.ndim == 1:
            ...
            coords = np.concatenate(self.coords)
            coo_todense_nd(
                strides, self.nnz, self.ndim, coords, self.data, result.ravel("A"), fortran
            )
    ...
    def _matmul_vector(self, other):
        if self.ndim > 2:
            result = np.zeros(
                math.prod(self.shape[:-1]), dtype=upcast_char(self.dtype.char, other.dtype.char)
            )
            shape = np.array(self.shape)
            strides = np.append(np.cumprod(shape[:-1][::-1])[::-1][1:], 1)
            coords = np.concatenate(self.coords)
            coo_matvec_nd(self.nnz, len(self.shape), strides, coords, self.data, other, result)
    ...
    def _matmul_dispatch(self, other):
        ...
        if other.__class__ is np.ndarray:
            ...
            if other.shape[-2] == N:
                ...
                raise ValueError(f"{err_prefix} (n,..,k={N}),(k={other.shape[-2]},..,m)->(n,..,m)")
    ...
    def _matmul_multivector(self, other):
        ...
        if self.ndim == 2:
            ...
            row = np.zeros_like(col)
        result = np.zeros(result_shape, dtype=result_dtype)
        coo_matmat_dense(self.nnz, other.shape[-1], row, col, self.data, other.ravel("C"), result)
    ...
    def dot(self, other):
        ...
        if isdense(other):
            ...
            if self.shape[0] != other.shape[0]:
                raise ValueError(
                    f"shapes {self.shape} and {other.shape}" " are not aligned for inner product"
                )
            return self @ other
            ...
            if self.shape[1] != other.shape[0]:
                raise ValueError(
                    f"shapes {self.shape} and {other.shape}" " are not aligned for matmul"
                )
            return self @ other
        else:
    ...
    def _sparse_tensordot(self, other, axes_self, axes_other):
        ...
        # Get the shape of the non-reduced axes
        self_nonreduced_shape = tuple(
            self.shape[ax] for ax in range(ndim_self) if ax not in axes_self
        )
        other_nonreduced_shape = tuple(
            other.shape[ax] for ax in range(ndim_other) if ax not in axes_other
        )

        # Create 2D coords arrays
        ravel_coords_shape_self = (
            math.prod(self_nonreduced_shape),
            math.prod([self.shape[ax] for ax in axes_self]),
        )
        ravel_coords_shape_other = (
            math.prod([other.shape[ax] for ax in axes_other]),
    ...
    def _broadcast_to(self, new_shape, copy=False):
        ...
        if len(new_shape) < len(old_shape):
            raise ValueError(
                "New shape must have at least as many dimensions" " as the current shape"
            )
        ...
        if any((o != 1 and o != n) for o, n in zip(shape, new_shape)):
            raise ValueError(
                f"current shape {old_shape} cannot be " "broadcast to new shape {new_shape}"
            )
