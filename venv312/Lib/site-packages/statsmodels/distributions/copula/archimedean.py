...
class ArchimedeanCopula(Copula):
    ...
    def _handle_u(self, u):
        ...
        if u.shape[-1] != self.k_dim:
            ...
            warnings.warn(
                "u has different dimension than k_dim. "
    ...
    def cdf(self, u, args=()):
        ...
        cdfv = np.clip(cdfv, 0.0, 1.0, out=out)  # inplace if possible
        return cdfv
...
class FrankCopula(ArchimedeanCopula):
    ...
    def theta_from_tau(self, tau):
        ...
        result = optimize.least_squares(
            _theta_from_tau, start, bounds=(MIN_FLOAT_LOG, MAX_FLOAT_LOG)
        )
        theta = result.x[0]
        return theta
...
class GumbelCopula(ArchimedeanCopula):
    ...
    def rvs(self, nobs=1, args=(), random_state=None):
        ...
        v = stats.levy_stable.rvs(
            1.0 / th, 1.0, 0, np.cos(np.pi / (2 * th)) ** th, size=(nobs, 1), random_state=rng
        )
