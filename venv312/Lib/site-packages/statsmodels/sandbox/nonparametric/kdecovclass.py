...
def plotkde(covfact):
    ...
    plt.plot(
        ind,
        alpha * stats.norm.pdf(ind, loc=mlow) + (1 - alpha) * stats.norm.pdf(ind, loc=mhigh),
        color="r",
...
def test_kde_1d():
    ...
    assert_almost_equal(
        gkde.integrate_gaussian(xnmean, xnstd**2), (kdepdf * normpdf).sum() * intervall, decimal=2
    )
...
if __name__ == "__main__":
    ...
    xn = np.concatenate(
        [
            mlow + np.random.randn(alpha * n_basesample),
