...
def Tukeythreegene2(genes):  # Performing the Tukey HSD post-hoc test for three genes
    ...
    mserrorden = (
        len(genes[0]) + len(genes[1]) + len(genes[2])
    ) - 3  # denominator for mean square error
    mserror = mserrornum / mserrorden  # mean square error
...
def mcfdr(nrepl=100, nobs=50, ntests=10, ntrue=6, mu=0.5, alpha=0.05, rho=0.0):
    ...
    for i in range(nrepl):
        ...
        results.append(
            ...
            + [np.sum(tpval[:ntrue] < alpha / ntests), np.sum(tpval[ntrue:] < alpha / ntests)]
        )
...
class TukeyHSDResults:
    ...
    def plot_simultaneous(
        ...
        if comparison_name is None:
            ...
            ax1.plot(
                [minrange[midx]] * 2, [-1, self._multicomp.ngroups], linestyle="--", color="0.7"
            )
            ax1.plot(
                [maxrange[midx]] * 2, [-1, self._multicomp.ngroups], linestyle="--", color="0.7"
            )
