...
def tangential_byrd_omojokun(grad, hess_prod, xl, xu, delta, debug, **kwargs):
    ...
    if kwargs.get("improve_tcg", True) and boundary_reached:
        ...
        while np.count_nonzero(free_bd) > 0:
            ...
            sd[~free_bd] = 0.0
            if grad_sd >= -1e-8 * reduct or np.any(grad_sd >= -TINY * np.abs(sd[free_bd])):
                break
            sd[free_bd] /= -grad_sd
            ...
            all_reduct = sin_values * (
                ...
                - grad_sd
                - t_samples * curv_step
                + sin_values * (t_samples * curv_step_sd - 0.5 * (curv_sd - curv_step))
            )
    ...
    **kwargs,
    ...
    if kwargs.get("improve_tcg", True) and boundary_reached and n_act < n:
        ...
        while n_act < n:
            ...
            if grad_sd >= -1e-8 * reduct or np.any(grad_sd >= -TINY * np.abs(sd)):
                break
            sd /= -grad_sd
            ...
            resid = np.maximum(
                0.0,
                resid - (cos_value - 1.0) * aub_step - sin_values[i_max] * aub_sd,
            )
            reduct += all_reduct[i_max]
...
def qr_tangential_byrd_omojokun(aub, aeq, free_xl, free_xu, free_ub):
    ...
    n_act = np.count_nonzero(
        np.abs(np.diag(r))
        >= 10.0 * EPS * n * np.linalg.norm(r[: np.min(r.shape), : np.min(r.shape)], axis=0)
    )
    return n_act, q
