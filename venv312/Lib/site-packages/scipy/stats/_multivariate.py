...
class _PSD:
    ...
    def __init__(
        self, M, cond=None, rcond=None, lower=True, check_finite=True, allow_singular=True
    ):
...
class multivariate_normal_gen(multi_rv_generic):
    ...
    def _process_parameters_psd(self, dim, mean, cov):
        ...
        if cov.ndim == 0:
            ...
            raise ValueError(
                "Array 'cov' must be at most two-dimensional," " but cov.ndim = %d" % cov.ndim
            )
...
class matrix_normal_gen(multi_rv_generic):
    ...
    def _process_quantiles(self, X, dims):
        ...
        if X.shape[-2:] != dims:
            raise ValueError(
                "The shape of array `X` is not compatible " "with the distribution parameters."
            )
        return X
...
def _dirichlet_check_parameters(alpha):
    ...
    if np.min(alpha) <= 0:
        ...
        raise ValueError(
            "Parameter vector 'a' must be one dimensional, " f"but a.shape = {alpha.shape}."
        )
    return alpha
...
class wishart_gen(multi_rv_generic):
    ...
    def _process_parameters(self, df, scale):
        ...
        if scale.ndim == 0:
            ...
            raise ValueError(
                "Array 'scale' must be at most two-dimensional," " but scale.ndim = %d" % scale.ndim
            )
...
class ortho_group_gen(multi_rv_generic):
    ...
    def _process_parameters(self, dim):
        ...
        if dim is None or not np.isscalar(dim) or dim <= 1 or dim != int(dim):
            raise ValueError(
                "Dimension of rotation must be specified," "and must be a scalar greater than 1."
            )
...
class multivariate_t_frozen(multi_rv_frozen):
    ...
    def cdf(self, x, *, maxpts=None, lower_limit=None, random_state=None):
        ...
        return self._dist._cdf(
            x, self.loc, self.shape, self.df, self.dim, maxpts, lower_limit, random_state
        )
    ...
    def rvs(self, size=1, random_state=None):
        return self._dist.rvs(
            loc=self.loc, shape=self.shape, df=self.df, size=size, random_state=random_state
        )
...
class uniform_direction_gen(multi_rv_generic):
    ...
    def _process_parameters(self, dim):
        ...
        if dim is None or not np.isscalar(dim) or dim < 1 or dim != int(dim):
            raise ValueError(
                "Dimension of vector must be specified, " "and must be an integer greater than 0."
            )
...
for name in ["logpmf", "pmf", "mean", "var", "cov"]:
    ...
    method_frozen.__doc__ = doccer.docformat(method.__doc__, dirichlet_mn_docdict_noparams)
