...
class _Parameter(ABC):
    ...
    def draw(
        self, size=None, *, rng=None, region="domain", proportions=None, parameter_values=None
    ):
...
def _set_invalid_nan(f):
    ...
    endpoints = {"icdf": (0, 1), "iccdf": (0, 1), "ilogcdf": (-np.inf, 0), "ilogccdf": (-np.inf, 0)}
    replacements = {
        "logpdf": (-inf, -inf),
        "pdf": (0, 0),
        "_logcdf1": (-inf, 0),
    ...
    def filtered(self, x, *args, **kwargs):
        ...
        mask_low = x < low if (method_name in replace_strict and left_inc) else x <= low
        mask_high = x > high if (method_name in replace_strict and right_inc) else x >= high
        ...
        if method_name in replace_exact:
            ...
            any_endpoint = mask_endpoint if mask_endpoint.shape == () else np.any(mask_endpoint)
        ...
        if any_endpoint:
            ...
            replace_low_endpoint = (
                b[mask_low_endpoint] if method_name.endswith("ccdf") else a[mask_low_endpoint]
            )
            replace_high_endpoint = (
                a[mask_high_endpoint] if method_name.endswith("ccdf") else b[mask_high_endpoint]
            )
...
def _generate_example(dist_family):
    ...
    if n_parameters:
        ...
        name_values = [f"{name}={value}" for name, value in zip(parameter_names, parameter_values)]
...
class ContinuousDistribution(_ProbabilityDistribution):
    ...
    def __init__(self, *, tol=_null, validation_policy=None, cache_policy=None, **parameters):
        self.tol = tol
        ...
        parameters = {key: val for key, val in sorted(parameters.items()) if val is not None}
    ...
    def _mode_optimization(self, **params):
        ...
        f, args = _kwargs2args(
            lambda x, **params: -self._pdf_dispatch(x, **params), args=(), kwargs=params
        )
    ...
    def _logcdf2_logexp_safe(self, x, y, **params):
        ...
        if np.any(mask):
            params_mask = {
                key: np.broadcast_to(val, mask.shape)[mask] for key, val in params.items()
            }
            out = np.asarray(out)
    ...
    def _logcdf2_quadrature(self, x, y, **params):
        logres = self._quadrature(self._logpdf_dispatch, limits=(x, y), log=True, params=params)
        return logres
    ...
    def _logcdf_quadrature(self, x, **params):
        ...
        return self._quadrature(self._logpdf_dispatch, limits=(a, x), params=params, log=True)
    ...
    def _cdf_quadrature(self, x, **params):
        a, _ = self._support(**params)
        return self._quadrature(self._pdf_dispatch, limits=(a, x), params=params)
    ...
    def _logccdf_quadrature(self, x, **params):
        ...
        return self._quadrature(self._logpdf_dispatch, limits=(x, b), params=params, log=True)
    ...
    def _ccdf_quadrature(self, x, **params):
        _, b = self._support(**params)
        return self._quadrature(self._pdf_dispatch, limits=(x, b), params=params)
    ...
    def sample(self, shape=(), *, method=None, rng=None):
        ...
        res = self._sample_dispatch(
            sample_shape, full_shape, method=method, rng=rng, **self._parameters
        )
    ...
    def _moment_central_dispatch(self, order, *, methods, **params):
        ...
        if moment is None and "quadrature" in methods:
            mean = self._moment_raw_dispatch(self._one, **params, methods=self._moment_methods)
    ...
    def _moment_central_normalize(self, order, **params):
        methods = {"cache", "formula", "general"}
        standard_moment = self._moment_standardized_dispatch(order, **params, methods=methods)
    ...
    def _moment_standardized_normalize(self, order, use_quadrature, **params):
        methods = {"quadrature"} if use_quadrature else {"cache", "formula", "transform"}
    ...
    def _moment_integrate_icdf(self, order, center, **params):
        ...
        return self._quadrature(integrand, limits=(0.0, 1.0), args=(order, center), params=params)
    ...
    def _logmoment_quad(self, order, logcenter, **params):
        ...
        return self._quadrature(logintegrand, args=(order, logcenter), params=params, log=True)
    ...
    def plot(self, x="x", y="pdf", *, t=("cdf", 0.0005, 0.9995), ax=None):
        ...
        message = f'Argument `t` of `{self.__class__.__name__}.plot` "' f"must be one of {valid_t}"
        if t_name not in valid_t:
            raise ValueError(message)

        message = f'Argument `x` of `{self.__class__.__name__}.plot` "' f"must be one of {valid_xy}"
        if x_name not in valid_xy:
            raise ValueError(message)

        message = f'Argument `y` of `{self.__class__.__name__}.plot` "' f"must be one of {valid_xy}"
        ...
        try:
            ...
            message = "`matplotlib` must be installed to use " f"`{self.__class__.__name__}.plot`."
        ...
        if len(self._parameters):
            ...
            param_arrays = [np.atleast_1d(self._parameters[pname]) for pname in param_names]
...
def make_distribution(dist):
    ...
    for shape_info in dist._shape_info():
        domain = _RealDomain(endpoints=shape_info.endpoints, inclusive=shape_info.inclusive)
    ...
    def _overrides(method_name):
        return getattr(dist.__class__, method_name, None) is not getattr(
            stats.rv_continuous, method_name, None
        )
...
class TruncatedDistribution(TransformedDistribution):
    ...
    _lb_param = _RealParameter("lb", symbol=r"b_l", domain=_lb_domain, typical=(0.1, 0.2))

    _ub_domain = _RealDomain(endpoints=("lb", inf), inclusive=(False, True))
    _ub_param = _RealParameter("ub", symbol=r"b_u", domain=_ub_domain, typical=(0.8, 0.9))
    ...
    def __repr__(self):
        with np.printoptions(threshold=10):
            return f"truncate({repr(self._dist)}, " f"lb={repr(self.lb)}, ub={repr(self.ub)})"

    def __str__(self):
        with np.printoptions(threshold=10):
            return f"truncate({str(self._dist)}, " f"lb={str(self.lb)}, ub={str(self.ub)})"
...
class ShiftedScaledDistribution(TransformedDistribution):
    ...
    _loc_param = _RealParameter("loc", symbol=r"\mu", domain=_loc_domain, typical=(1, 2))
    ...
    def __repr__(self):
        with np.printoptions(threshold=10):
            ...
            elif np.any(self.loc != 0) or not np.can_cast(self.loc.dtype, self.scale.dtype):
    ...
    def _moment_standardized_dispatch(self, order, *, loc, scale, sign, methods, **params):
    ...
    def _sample_dispatch(
        self, sample_shape, full_shape, *, rng, loc, scale, sign, method, **params
    ):
    ...
    def __mul__(self, scale):
        return ShiftedScaledDistribution(self._dist, loc=self.loc * scale, scale=self.scale * scale)
...
class OrderStatisticDistribution(TransformedDistribution):
    ...
    def __repr__(self):
        with np.printoptions(threshold=10):
            return f"order_statistic({repr(self._dist)}, r={repr(self.r)}, " f"n={repr(self.n)})"

    def __str__(self):
        with np.printoptions(threshold=10):
            return f"order_statistic({str(self._dist)}, r={str(self.r)}, " f"n={str(self.n)})"
...
class Mixture(_ProbabilityDistribution):
    ...
    def entropy(self, *, method=None):
        self._raise_if_method(method)
        return _tanhsinh(lambda x: -self.pdf(x) * self.logpdf(x), *self.support()).integral
    ...
    def _moment_central(self, order):
        ...
        out = self._full(0)
        for var, weight in zip(self._components, self._weights):
            moment_as = [var.moment(order, kind="central") for order in range(order + 1)]
...
def log(X, /):
    ...
    if np.any(X.support()[0] < 0):
        ...
        raise NotImplementedError(message)
    return MonotonicTransformedDistribution(X, g=np.log, h=np.exp, dh=np.exp, logdh=lambda u: u)
