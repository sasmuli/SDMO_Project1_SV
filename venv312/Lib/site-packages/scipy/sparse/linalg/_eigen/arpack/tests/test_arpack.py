...
def _get_test_tolerance(type_char, mattype=None, D_type=None, which=None):
    ...
    if isinstance(mattype, type) and issubclass(mattype, csr_array) and type_char in ("f", "F"):
...
def generate_matrix(N, complex_=False, hermitian=False, pos_definite=False, sparse=False, rng=None):
    M = rng.random((N, N))
    if complex_:
...
def eval_evec(
    symmetric, d, typ, k, which, v0=None, sigma=None, mattype=np.asarray, OPpart=None, mode="normal"
):
    general = "bmat" in d
...
class SymmetricParams:
    def __init__(self):
        ...
        Ar = generate_matrix(N, hermitian=True, pos_definite=True, rng=rng).astype("f").astype("d")
        M = generate_matrix(N, hermitian=True, pos_definite=True, rng=rng).astype("f").astype("d")
        Ac = (
            generate_matrix(N, hermitian=True, pos_definite=True, complex_=True, rng=rng)
            .astype("F")
            .astype("D")
        )
        Mc = (
