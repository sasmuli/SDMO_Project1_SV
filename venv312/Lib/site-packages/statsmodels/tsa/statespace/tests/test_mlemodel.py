...
def test_score_analytic_ar1():
    ...
    harvey_fd_centered = mod.score(
        params, transformed=True, method="harvey", approx_complex_step=False, approx_centered=True
    )
    ...
    analytic_score = np.dot(
        np.diag(np.r_[partial_transform_phi(uparams[0]), partial_transform_sigma2(uparams[1])]),
    ...
    assert_allclose(harvey_fd, analytic_score, atol=1e-5)
    harvey_fd_centered = mod.score(
        uparams, transformed=False, method="harvey", approx_complex_step=False, approx_centered=True
    )
    assert_allclose(harvey_fd_centered, analytic_score, atol=1e-5)
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        assert_allclose(mod._hessian_complex_step(params) * 2, analytic_hessian, atol=1e-1)
        assert_allclose(mod._hessian_finite_difference(params) * 2, analytic_hessian, atol=1e-1)
