...
def combine_pvalues_weighted(*args, **kwargs):
    return stats.combine_pvalues(args[0], *args[2:], weights=args[1], method="stouffer", **kwargs)
...
axis_nan_policy_cases = [
    ...
    (stats.cramervonmises, ("norm",), dict(), 1, 2, False, lambda res: (res.statistic, res.pvalue)),
    (
    ...
    (stats.chatterjeexi, tuple(), dict(), 2, 2, True, lambda res: (res.statistic, res.pvalue)),
]
...
def test_keepdims(
    ...
    for axis in axis_cases:
        ...
        nan_res = unpacker(
            hypotest(*nan_data, *args, axis=axis, keepdims=True, nan_policy=nan_policy, **kwds)
        )
        nan_res_base = unpacker(
            hypotest(*nan_data, *args, axis=axis, keepdims=False, nan_policy=nan_policy, **kwds)
        )
        ...
        for axis in range(-max_axis, max_axis):
            ...
            try:
                ...
                if expected.size and hypotest not in too_small_special_case_funcs:
                    message = too_small_1d_not_omit if max_axis == 1 else too_small_nd_not_omit
                ...
                assert not _check_arrays_broadcastable(samples, None if paired else axis)
...
def test_non_broadcastable(hypotest, args, kwds, n_samples, n_outputs, paired, unpacker, rng, axis):
    # test for correct error message when shapes are not broadcastable

    get_samples = True
    while get_samples:
        samples = [rng.random(size=rng.integers(2, 100, size=2)) for i in range(n_samples)]
