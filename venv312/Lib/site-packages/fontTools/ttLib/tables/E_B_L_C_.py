...
def _createOffsetArrayIndexSubTableMixin(formatStringForDataType):
    ...
    class OffsetArrayIndexSubTableMixin(object):
        def decompile(self):
            ...
            offsetArray = [
                struct.unpack(dataFormat, self.data[slice(*loc)])[0] for loc in indexingLocations
            ]
...
class eblc_index_sub_table_4(EblcIndexSubTable):
    def decompile(self):
        ...
        indexingLocations = zip(indexingOffsets, indexingOffsets[1:])
        glyphArray = [
            struct.unpack(codeOffsetPairFormat, data[slice(*loc)]) for loc in indexingLocations
        ]
