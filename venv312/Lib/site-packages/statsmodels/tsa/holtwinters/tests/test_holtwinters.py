...
class TestHoltWinters:
    ...
    def test_simple_exp_smoothing(self):
        fit1 = SimpleExpSmoothing(self.oildata_oil, initialization_method="legacy-heuristic").fit(
            0.2, optimized=False
        )
        fit2 = SimpleExpSmoothing(self.oildata_oil, initialization_method="legacy-heuristic").fit(
            0.6, optimized=False
        )
    ...
    def test_holt_damp_r(self):
        ...
        assert_allclose(np.r_[fit.params["initial_trend"], fit.trend], desired, atol=1e-3)
...
def test_valid_bounds(ses):
    ...
    res = ExponentialSmoothing(ses, bounds=bounds, initialization_method="estimated").fit(
...
def test_forecast_index_types(ses, index_typ):
    ...
    with pytest_warns(warning):
        res = ExponentialSmoothing(
            ses, trend="add", seasonal="add", initialization_method="heuristic", **kwargs
        ).fit()
    with pytest_warns(warning):
