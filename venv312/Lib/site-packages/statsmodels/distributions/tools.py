...
class _Grid:
    ...
    def __init__(self, k_grid, eps=0):
        ...
        idx_flat = np.column_stack(np.unravel_index(np.arange(np.prod(k_grid)), k_grid)).astype(
            float
        )
...
def _eval_bernstein_2d(x, fvals):
    ...
    poly_base = stats.binom.pmf(k1[None, :, None], n1, x1[:, None, None]) * stats.binom.pmf(
        k2[None, None, :], n2, x2[:, None, None]
    )
