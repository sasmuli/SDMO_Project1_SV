...
class CP_PLSR:
    ...
    def __init__(self, n_components, tol=1.0e-9, n_iter_max=100, random_state=None, verbose=False):
    ...
    def fit(self, X, Y):
        ...
        for component in range(self.n_components):
            ...
            for iter in range(self.n_iter_max):
                ...
                for mode in range(1, X.ndim):  # Mode 0 of Z collapsed by above tensordot
                    comp_X_factors[mode] = tensor_to_vec(Z_comp[mode - 1])

                comp_X_factors[0] = multi_mode_dot(X, comp_X_factors[1:], range(1, T.ndim(X)))
            ...
            # Deflation
            X -= CPTensor((None, [T.reshape(ff, (-1, 1)) for ff in comp_X_factors])).to_tensor()
            Y -= T.dot(
                T.dot(
                    self.X_factors[0],
    ...
    def predict(self, X):
        ...
        return (
            T.dot(T.dot(T.transpose(scores), estimators), T.transpose(self.Y_factors[1]))
            + self.Y_mean_
        )
