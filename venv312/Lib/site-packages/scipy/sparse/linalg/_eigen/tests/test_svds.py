...
def _check_svds(
    A, k, u, s, vh, which="LM", check_usvh_A=False, check_svd=True, atol=1e-10, rtol=1e-7
):
    n, m = A.shape
...
class SVDSCommonTests:
    ...
    def test_svds_diff0_docstring_example(self):
        ...
        def diff0_func_aslo_def(n):
            return LinearOperator(
                matvec=diff0, matmat=diff0, rmatvec=diff0t, rmatmat=diff0t, shape=(n - 1, n)
            )
    ...
    def test_svds_parameter_tol(self):
        ...
        def err(tol):
            _, s2, _ = svds(A, k=k, v0=np.ones(n), maxiter=1000, solver=self.solver, tol=tol, rng=0)
    ...
    def test_svd_return_singular_vectors(self, rsv, shape):
        ...
        if self.solver == "lobpcg":
            with pytest.warns(UserWarning, match="The problem size"):
                if rsv is False:
                    s2 = svds(A, k, return_singular_vectors=rsv, solver=self.solver, rng=rng)
            ...
            if rsv is False:
                ...
                u2, s2, vh2 = svds(A, k, return_singular_vectors=rsv, solver=self.solver, rng=rng)
                assert_allclose(np.abs(u2), np.abs(u))
                assert_allclose(s2, s)
                assert vh2 is None
            elif rsv == "vh" and respect_vh:
    ...
    def test_svd_linop(self):
        ...
        for n, m, k in nmks:
            ...
            assert_allclose(
                np.dot(U1, np.dot(np.diag(s1), VH1)), np.dot(U2, np.dot(np.diag(s2), VH2))
            )
            ...
            if self.solver == "lobpcg":
                with pytest.warns(UserWarning, match="The problem size"):
                    U1, s1, VH1 = reorder(svds(A, k, which="SM", solver=solver, rng=0, **kwargs))
                    U2, s2, VH2 = reorder(svds(L, k, which="SM", solver=solver, rng=0, **kwargs))
            else:
                U1, s1, VH1 = reorder(svds(A, k, which="SM", solver=solver, rng=0, **kwargs))
                U2, s2, VH2 = reorder(svds(L, k, which="SM", solver=solver, rng=0, **kwargs))
            ...
            if k < min(n, m) - 1:
                ...
                for dt, eps in [(complex, 1e-7), (np.complex64, 3e-3)]:
                    ...
                    if self.solver == "lobpcg":
                        with pytest.warns(UserWarning, match="The problem size"):
                            U1, s1, VH1 = reorder(svds(A, k, which="LM", solver=solver, rng=0))
                            U2, s2, VH2 = reorder(svds(L, k, which="LM", solver=solver, rng=0))
                    else:
                        U1, s1, VH1 = reorder(svds(A, k, which="LM", solver=solver, rng=0))
                        U2, s2, VH2 = reorder(svds(L, k, which="LM", solver=solver, rng=0))
