...
class Git:
    ...
    def __init__(self, path: str, conf=None):
        ...
        if conf is None:
            conf = Conf({"path_to_repo": str(self.path), "git": self})
    ...
    def _discover_main_branch(self, repo):
        try:
            ...
            logger.info(
                "HEAD is a detached symbolic reference, setting main branch to empty string"
            )
    ...
    def get_commits_last_modified_lines(
        ...
        return self._calculate_last_commits(commit, modifications, hashes_to_ignore_path)
    ...
    def diff(self, from_commit_id: str, to_commit_id: str) -> List[ModifiedFile]:
        ...
        diff_index = from_commit.diff(other=to_commit, paths=None, create_patch=True)

        modified_files_list = [ModifiedFile(diff=diff) for diff in diff_index]
        return modified_files_list

    def _calculate_last_commits(
        self,
        commit: Commit,
        ...
        for mod in modifications:
            ...
            try:
                ...
                logger.debug(
                    f"Could not found file {mod.filename} in commit {commit.hash}. Probably a double rename!"
                )
