...
exogenous variables.  Got length %s"
        ...
        sp_exog = sparse.lil_matrix(
            (int(self.nobs * M), int(np.sum(self.df_model + 1)))
        )  # linked lists to build
        self._cols = np.cumsum(np.hstack((0, self.df_model + 1)))
        for i in range(M):
            sp_exog[i * self.nobs : (i + 1) * self.nobs, self._cols[i] : self._cols[i + 1]] = sys[
                1::2
            ][i]
    ...
    def whiten(self, X):
        ...
        if X is self.endog:  # definitely not a robust check
            ...
            return (
                sparse.kron(self.cholsigmainv, sparse.eye(nobs, nobs)) * X
            ).toarray()  # *=dot until cast to array
        ...
        for eq_key in indep_endog:
            try:
                ...
                raise TypeError(
                    ...
                    "iterable. Got type %s for converter %s" % (type(indep_endog[eq_key]), eq_key)
                )
