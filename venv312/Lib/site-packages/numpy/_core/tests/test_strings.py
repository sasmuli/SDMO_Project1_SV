...
class TestMethods:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                30000 * "a" + 100 * "b", 100 * "b", 0, None, 1, id="30000*a+100*b-100*b-0-None-1"
            ),
            pytest.param(30000 * "a", 100 * "b", 0, None, 0, id="30000*a-100*b-0-None-0"),
            pytest.param(
                30000 * "a" + 100 * "ab", "ab", 0, None, 100, id="30000*a+100*ab-ab-0-None-100"
            ),
            pytest.param(
                15000 * "a" + 15000 * "b",
                15000 * "b",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("spam, spam, eggs and spam", "spam", "ham", 0, "spam, spam, eggs and spam"),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("this is the partition method", "ti", "this is the par", "ti", "tion method"),
    ...
    @pytest.mark.parametrize(
        "buf,sep,res1,res2,res3",
        [
            ("this is the partition method", "ti", "this is the parti", "ti", "on method"),
            ("http://www.python.org", "://", "http", "://", "www.python.org"),
            ("http://www.python.org", "?", "", "", "http://www.python.org"),
            ("http://www.python.org", "http://", "", "http://", "www.python.org"),
            ("http://www.python.org", "org", "http://www.python.", "org", ""),
    ...
    def test_slice_unsupported(self, dt):
        ...
        with pytest.raises(
            TypeError, match=r"Cannot cast ufunc '_slice' input .* from .* to dtype\('int(64|32)'\)"
        ):
            np.strings.slice(np.array(["foo", "bar"], dtype=dt), np.array(["foo", "bar"], dtype=dt))

    @pytest.mark.parametrize(
        "int_dt", [np.int8, np.int16, np.int32, np.int64, np.uint8, np.uint16, np.uint32, np.uint64]
    )
    def test_slice_int_type_promotion(self, int_dt, dt):
        ...
        assert_array_equal(
            np.strings.slice(buf, int_dt(2), int_dt(4)), np.array(["ll", "rl"], dtype=dt)
        )
        assert_array_equal(
        ...
        assert_array_equal(
            np.strings.slice(buf, int_dt(0), int_dt(4), int_dt(2)), np.array(["hl", "wr"], dtype=dt)
        )
        assert_array_equal(
            np.strings.slice(
                buf,
...
class TestReplaceOnArrays:
    ...
    def test_replace_broadcasting(self, dt):
        ...
        assert_array_equal(
            r2, np.array([["1,0,0", "1,1,0", "1,1,1"], ["2,0,0", "2,2,0", "2,2,2"]], dtype=dt)
        )
