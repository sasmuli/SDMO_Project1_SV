...
class TestBartHann:
    ...
    def test_basic(self):
        ...
        assert_allclose(
            windows.barthann(7), [0, 0.27, 0.73, 1.0, 0.73, 0.27, 0], rtol=1e-15, atol=1e-15
        )
        assert_allclose(
            windows.barthann(6, False), [0, 0.27, 0.73, 1.0, 0.73, 0.27], rtol=1e-15, atol=1e-15
        )
...
class TestBlackmanHarris:
    ...
    def test_basic(self):
        assert_allclose(
            windows.blackmanharris(6, False), [6.0e-05, 0.055645, 0.520575, 1.0, 0.520575, 0.055645]
        )
        assert_allclose(
...
class TestTaylor:
    ...
    def test_normalized(self):
        ...
        assert_allclose(
            ...
            np.array([0.7504082, 0.86624416, 0.98208011, 0.98208011, 0.86624416, 0.7504082]),
        )
...
class TestBohman:
    ...
    def test_basic(self):
        assert_allclose(
            windows.bohman(6),
            [0, 0.1791238937062839, 0.8343114522576858, 0.8343114522576858, 0.1791238937062838, 0],
        )
        assert_allclose(
...
class TestChebWin:
    ...
    def test_cheb_even_low_attenuation(self):
        cheb_even_low_at_true = array(
            [1.000000, 0.451924, 0.51027, 0.541338, 0.541338, 0.51027, 0.451924, 1.000000]
        )
        with suppress_warnings() as sup:
            sup.filter(UserWarning, "This window is not suitable")
...
exponential_data = {
    ...
    (4, None, 0.2, True): array(
        [0.00055308437014783, 0.0820849986238988, 0.0820849986238988, 0.00055308437014783]
    ),
    (4, None, 1.0, False): array(
        [0.1353352832366127, 0.36787944117144233, 1.0, 0.36787944117144233]
    ),
    (4, None, 1.0, True): array(
        [0.22313016014842982, 0.60653065971263342, 0.60653065971263342, 0.22313016014842982]
    ),
    (4, 2, 0.2, False): array(
        [
            4.53999297624848542e-05,
...
class TestHann:
    ...
    def test_basic(self):
        assert_allclose(
            windows.hann(6, sym=False), [0, 0.25, 0.75, 1.0, 0.75, 0.25], rtol=1e-15, atol=1e-15
        )
        assert_allclose(
...
class TestKaiserBesselDerived:
    ...
    def test_basic(self):
        ...
        assert_allclose(
            windows.kaiser_bessel_derived(4, beta=np.pi / 2)[:2], [0.518562710536, 0.855039598640]
        )
...
class TestGetWindow:
    ...
    def test_general_hamming(self):
        ...
        assert_allclose(
            get_window(("general_hamming", 0.7), 5, fftbins=False), [0.4, 0.7, 1.0, 0.7, 0.4]
        )
