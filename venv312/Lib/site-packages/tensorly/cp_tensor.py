...
def cp_to_tensor(cp_tensor, mask=None):
    ...
    if mask is None:
        full_tensor = T.dot(factors[0] * weights, T.transpose(khatri_rao(factors, skip_matrix=0)))
    else:
...
def cp_permute_factors(ref_cp_tensor, tensors_to_permute):
    ...
    for i in range(n_tensors):
        _, col = congruence_coefficient(ref_cp_tensor.factors, tensors_to_permute[i].factors)
