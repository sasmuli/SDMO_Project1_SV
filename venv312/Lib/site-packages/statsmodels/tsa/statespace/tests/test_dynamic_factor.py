...
class CheckDynamicFactor:
    ...
    def setup_class(
        ...
        **kwargs,
        ...
        cls.model = dynamic_factor.DynamicFactor(
    ...
    def test_predict(self, **kwargs):
        ...
        assert_allclose(
            self.results.predict(end="1982-10-01", **kwargs), self.true["predict"], atol=1e-6
        )
...
class TestDynamicFactor_exog1(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true["dynamic_predict"] = output_results.iloc[1:][
            ["dyn_predict_dfm_exog1_1", "dyn_predict_dfm_exog1_2", "dyn_predict_dfm_exog1_3"]
        ]
        exog = np.ones((75, 1))
...
class TestDynamicFactor_exog2(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true["dynamic_predict"] = output_results.iloc[1:][
            ["dyn_predict_dfm_exog2_1", "dyn_predict_dfm_exog2_2", "dyn_predict_dfm_exog2_3"]
        ]
...
class TestDynamicFactor_general_errors(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true["dynamic_predict"] = output_results.iloc[1:][
            ["dyn_predict_dfm_gen_1", "dyn_predict_dfm_gen_2", "dyn_predict_dfm_gen_3"]
        ]
        super().setup_class(
            true,
            k_factors=1,
    ...
    def test_summary(self):
        ...
        # Make sure we have the right number of tables
        assert_equal(
            len(tables), 2 + self.model.k_endog + self.model.k_factors + self.model.k_endog + 1
        )
        ...
        for i in range(self.model.k_endog):
            ...
            for j in range(self.model.k_endog):
                ...
                pattern = r"L1.e\({}\) +{}".format(name, forg(params[offset + j], prec=4))
                assert re.search(pattern, table)
...
class TestDynamicFactor_ar2_errors(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true["dynamic_predict"] = output_results.iloc[1:][
            ["dyn_predict_dfm_ar2_1", "dyn_predict_dfm_ar2_2", "dyn_predict_dfm_ar2_3"]
        ]
    ...
    def test_mle(self):
        with warnings.catch_warnings(record=True):
            ...
            mod = self.model
            res1 = mod.fit(
                maxiter=10000, optim_score="approx", disp=False, optim_complex_step=False
            )
...
class TestDynamicFactor_scalar_error(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        true["dynamic_predict"] = output_results.iloc[1:][
            ["dyn_predict_dfm_scalar_1", "dyn_predict_dfm_scalar_2", "dyn_predict_dfm_scalar_3"]
        ]
        exog = np.ones((75, 1))
        super().setup_class(true, k_factors=1, factor_order=1, exog=exog, error_cov_type="scalar")
...
class TestSUR(CheckDynamicFactor):
    ...
    def setup_class(cls):
        ...
        exog = np.c_[np.ones((75, 1)), (np.arange(75) + 2)[:, np.newaxis]]
        super().setup_class(
            true, k_factors=0, factor_order=0, exog=exog, error_cov_type="unstructured"
        )
...
def test_misspecification():
    ...
    # Too few endog
    assert_raises(
        ValueError, dynamic_factor.DynamicFactor, endog[:, 0], k_factors=0, factor_order=0
    )
...
def test_miscellaneous():
    ...
    exog = pd.Series(
        np.arange(75), index=pd.date_range(start="1960-04-01", end="1978-10-01", freq="QS")
    )
    mod = CheckDynamicFactor()
    mod.setup_class(true=None, k_factors=1, factor_order=1, exog=exog, filter=False)
...
def test_start_params_nans():
    ...
    dta = (
        np.log(
            pd.DataFrame(results_varmax.lutkepohl_data, columns=["inv", "inc", "consump"], index=ix)
        )
        .diff()
        .iloc[1:]
    )
