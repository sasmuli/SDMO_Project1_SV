...
def test_init_exog_tvtp():
    ...
    assert_raises(
        ValueError, markov_switching.MarkovSwitching, endog, k_regimes=2, exog_tvtp=exog_tvtp
    )
...
def test_transition_matrix():
    ...
    assert_allclose(
        mod.regime_transition_matrix(params)[:, 0, :], transition_matrix[:, 0, :], atol=1e-10
    )
...
def test_partials_logistic():
    ...
    assert_allclose(
        partials_logistic(case), approx_fprime_cs(np.squeeze(case), logistic)[..., None]
    )
