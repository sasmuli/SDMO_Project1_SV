    ...
    def _stats(self, n, a, b, moments="mv"):
        ...
        def kurtosis(n, a, b):
            ...
            term_2 = (
                ...
                + 3.0
                * n**2.0
                * ((a + 5.0) * b**2.0 + (a + 5.0) * (a - 1.0) * b + 2.0 * (a - 1.0) ** 2)
                + 3
                * (a - 1.0)
                * n
                * ((a + 5.0) * b**2.0 + (a + 5.0) * (a - 1.0) * b + 2.0 * (a - 1.0) ** 2.0)
            )
...
class zipf_gen(rv_discrete):
    ...
    def _munp(self, n, a):
        return _lazywhere(
            a > n + 1, (a, n), lambda a, n: special.zeta(a - n, 1) / special.zeta(a, 1), np.inf
        )
...
class zipfian_gen(rv_discrete):
    ...
    def _stats(self, a, n):
        ...
        g2 = (
            Hna**3 * Hna4 - 4 * Hna**2 * Hna1 * Hna3 + 6 * Hna * Hna1**2 * Hna2 - 3 * Hna1**4
        ) / mu2n**2
        g2 -= 3
...
class dlaplace_gen(rv_discrete):
    ...
    def _ppf(self, q, a):
        const = 1 + exp(a)
        vals = ceil(
            np.where(q < 1.0 / (1 + exp(-a)), log(q * const) / a - 1, -log((1 - q) * const) / a)
        )
        vals1 = vals - 1
...
class poisson_binom_gen(rv_discrete):
    ...
    def _rvs(self, *args, size=None, random_state=None):
        ...
        size = p.shape if size is None else (size, 1) if np.isscalar(size) else tuple(size) + (1,)
...
poisson_binom = poisson_binom_gen(name="poisson_binom", longname="A Poisson binomial", shapes="p")
...
class yulesimon_gen(rv_discrete):
    ...
    def _stats(self, alpha):
        ...
        g2 = np.where(
            alpha > 4,
            alpha + 3 + ((11 * alpha**3 - 49 * alpha - 22) / (alpha * (alpha - 4) * (alpha - 3))),
            np.inf,
        )
