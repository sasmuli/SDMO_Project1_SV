...
def forecast(y, coefs, trend_coefs, steps, exog=None):
    ...
    k = len(coefs[0])
    if y.shape[0] < p:
        raise ValueError(f"y must by have at least order ({p}) observations. " f"Got {y.shape[0]}.")
    # initial value
    ...
    def simulate_var(
        ...
        if steps is None:
            ...
            if steps_ is not None and steps != steps_:
                raise ValueError(
                    "if exog or offset are used, then steps must" "be equal to their length or None"
                )
...
class VARResults(VARProcess):
    ...
    def reorder(self, order):
        ...
        if len(order) != len(self.params[0, :]):
            raise ValueError(
                "Reorder specification length should match " "number of endogenous variables"
            )
    ...
    def test_inst_causality(self, causing, signif=0.05):
        ...
        if not all(isinstance(c, allowed_types) for c in causing):
            raise TypeError(
                "causing has to be of type string or int (or a " + "a sequence of these types)."
            )
        causing = [self.names[c] if type(c) is int else c for c in causing]
        causing_ind = [util.get_index(self.names, c) for c in causing]
