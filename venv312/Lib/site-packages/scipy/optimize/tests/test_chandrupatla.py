...
class TestChandrupatlaMinimize:
    ...
    def test_bracket_order(self, xp):
        ...
        xp_test = array_namespace(xp.asarray(1.0))  # need `xp.newaxis`
    ...
    def test_special_cases(self, xp):
        ...
        xp_test = array_namespace(xp.asarray(1.0))  # need `xp.isdtype`
...
@pytest.mark.skip_xp_backends("jax.numpy", reason="JAX arrays do not support item assignment.")
@pytest.mark.skip_xp_backends("cupy", reason="cupy/cupy#8391")
class TestChandrupatla(TestScalarRootFinders):
    ...
    def test_vectorization(self, shape, xp):
        ...
        res = _chandrupatla_root(f, xp.asarray(-5.0), xp.asarray(5.0), args=args_xp)
        ...
        assert xp.all((res.x[finite] == res.xl[finite]) | (res.x[finite] == res.xr[finite]))
        ...
        assert xp.all(
            xp.abs(res.fun[finite]) == xp.minimum(xp.abs(res.fl[finite]), xp.abs(res.fr[finite]))
        )
    ...
    def test_nit_expected(self, case, xp):
        ...
        xp_assert_close(res.fun, xp.asarray(f(root), dtype=xp.float64), rtol=1e-8, atol=2e-3)
