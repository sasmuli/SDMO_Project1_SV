...
class OrderedModel(GenericLikelihoodModel):
    ...
    def from_formula(cls, formula, data, subset=None, drop_cols=None, *args, **kwargs):
        ...
        model = super().from_formula(formula, data=data, drop_cols=["Intercept"], *args, **kwargs)

        if model.endog.ndim == 2:
            if not (
                isinstance(original_endog.dtype, CategoricalDtype) and original_endog.dtype.ordered
            ):
