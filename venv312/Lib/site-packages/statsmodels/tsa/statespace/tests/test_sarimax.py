...
from numpy.testing import assert_, assert_equal, assert_almost_equal, assert_raises, assert_allclose
...
class TestRealGDPARStata:
    ...
    def setup_class(cls):
        ...
        cls.model = sarimax.SARIMAX(
            dlgdp, order=(12, 0, 0), trend="n", hamilton_representation=True
        )
        # Estimated by Stata
        params = [
            0.40725515,
...
class SARIMAXStataTests:
    ...
    def test_hqic(self):
        hqic = (
            -2 * self.result.llf
            + 2 * np.log(np.log(self.result.nobs_effective)) * self.result.params.shape[0]
        )
...
class AdditiveSeasonal(SARIMAXStataTests):
    ...
    def setup_class(cls, true, *args, **kwargs):
        ...
        cls.model = sarimax.SARIMAX(endog, order=(1, 1, (1, 0, 0, 1)), trend="c", *args, **kwargs)
...
class Airline(SARIMAXStataTests):
    ...
    def setup_class(cls, true, *args, **kwargs):
        ...
        cls.model = sarimax.SARIMAX(
            endog, order=(0, 1, 1), seasonal_order=(0, 1, 1, 12), trend="n", *args, **kwargs
        )
    ...
    def test_mle(self):
        with warnings.catch_warnings():
            ...
            result = self.model.fit(disp=-1)
            assert_allclose(result.params, self.result.params, atol=1e-4)
...
class TestAirlineHarvey(Airline):
    ...
    @classmethod
    def setup_class(cls):
        super().setup_class(results_sarimax.air2_stationary, hamilton_representation=False)
...
class Friedman(SARIMAXStataTests):
    ...
    def setup_class(cls, true, exog=None, *args, **kwargs):
        ...
        params = np.r_[
            true["params_exog"], true["params_ar"], true["params_ma"], true["params_variance"]
        ]
...
class TestFriedmanStateRegression(Friedman):
    ...
    def test_regression_parameters(self):
        ...
        assert_almost_equal(
            self.result.filter_results.filtered_state[-2:, -1] / 10.0,
            self.true["mle_params_exog"],
            1,
        )
...
class SARIMAXCoverageTest:
    ...
    def test_transform_untransform(self):
        ...
        model.enforce_stationarity = (
            model.k_ar == 0 or tools.is_invertible(np.r_[1, -par[1:]])
        ) and (len(contracted_psar) <= 1 or tools.is_invertible(np.r_[1, -contracted_psar[1:]]))

        pma = model.polynomial_ma
        psma = model.polynomial_seasonal_ma
        contracted_psma = psma[psma.nonzero()]
        model.enforce_invertibility = (
            model.k_ma == 0 or tools.is_invertible(np.r_[1, pma[1:]])
        ) and (len(contracted_psma) <= 1 or tools.is_invertible(np.r_[1, contracted_psma[1:]]))
    ...
    def test_predict(self):
        ...
        if self.model.k_exog == 0:
            ...
            predict = result.predict(
                start=self.model.nobs, end=self.model.nobs + 10, dynamic=-10, exog=exog
            )
            assert_equal(predict.shape, (11,))
...
class Test_arma_diff_seasonal_diff(SARIMAXCoverageTest):
    ...
    def setup_class(cls, *args, **kwargs):
        kwargs["order"] = (3, 2, 2)
        kwargs["seasonal_order"] = (0, 2, 0, 4)
        super().setup_class(22, *args, **kwargs)
...
class Test_seasonal_ar_trend_polynomial(SARIMAXCoverageTest):
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        kwargs["seasonal_order"] = (3, 0, 0, 4)
        kwargs["trend"] = [1, 0, 0, 1]
        super().setup_class(28, *args, **kwargs)
...
class Test_seasonal_ar_seasonal_diff(SARIMAXCoverageTest):
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        super().setup_class(30, *args, **kwargs)
...
class Test_seasonal_ma_trend_polynomial(SARIMAXCoverageTest):
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        kwargs["trend"] = [1, 0, 0, 1]
        kwargs["decimal"] = 3
        super().setup_class(36, *args, **kwargs)
...
class Test_seasonal_ma_seasonal_diff(SARIMAXCoverageTest):
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        super().setup_class(38, *args, **kwargs)
...
class Test_seasonal_arma_seasonal_diff(SARIMAXCoverageTest):
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        super().setup_class(46, *args, **kwargs)
...
def test_simple_time_varying():
    ...
    exog = 2 * endog
    mod = sarimax.SARIMAX(
        endog, exog=exog, order=(0, 0, 0), time_varying_regression=True, mle_regression=False
    )
...
def test_manual_approximate_diffuse_initialization():
    ...
    # in kwargs
    mod4 = sarimax.SARIMAX(
        endog, order=(3, 0, 0), initialization="approximate_diffuse", initial_variance=1e9
    )
    res4 = mod4.filter([0.5, 0.2, 0.1, 1])
...
def test_misc_exog():
    ...
    models = [
        ...
        sarimax.SARIMAX(endog, exog=exog2, order=(1, 1, 0), simple_differencing=False),
        sarimax.SARIMAX(endog_pd, exog=exog1_pd, order=(1, 1, 0)),
        sarimax.SARIMAX(endog_pd, exog=exog2_pd, order=(1, 1, 0)),
        sarimax.SARIMAX(endog_pd, exog=exog2_pd, order=(1, 1, 0), simple_differencing=False),
    ]
...
def test_arima000():
    ...
    # RLS + TVP
    mod = sarimax.SARIMAX(
        endog, order=(0, 0, 0), exog=exog, mle_regression=False, time_varying_regression=True
    )
    mod.ssm.filter_univariate = True
    ...
    for element in itertools.product(
        ...
        for name in mod_conc.ssm.shapes:
            if name == "obs":
                continue
            assert_allclose(
                getattr(res_conc.filter_results, name), getattr(res_orig.filter_results, name)
            )
        ...
        diffuse_mask = (
            res_orig.filter_results.initial_state_cov.diagonal() == mod_orig.ssm.initial_variance
        )
        ...
        for name in filter_attr_burn:
            ...
            # initialization
            if (
                name in ["predicted_state_cov", "filtered_state_cov"]
                and kwargs["time_varying_regression"]
            ):
        ...
        for name in smoothed_attr:
            ...
            desired = getattr(res_orig.filter_results, name)[..., d:]
            if (
                name in ["smoothed_state_cov", "smoothed_state_autocov"]
                and kwargs["time_varying_regression"]
            ):
                assert_allclose(actual[ix], desired[ix], atol=atol)
            else:
...
def test_extend_by_one():
    ...
    assert_allclose(
        res3.forecast(10, exog=np.ones(10) * 2), res1.forecast(10, exog=np.ones(10) * 2)
    )
