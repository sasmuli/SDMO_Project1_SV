...
class FStringState:
    ...
    def consume_rbrace(self) -> None:
        ...
        if len(self.stack) > 0 and self.stack[-1] == FStringState.in_fstring_expr_modifier:
            self.state = self.stack.pop()
        else:
            self.state = FStringState.at_fstring_middle
...
class TokenIterator:
    ...
    def push_fstring_prefix_quote(self, prefix: str, quote: str) -> None:
        if self.fstring_prefix is not None:
            ...
            self.fstring_prefix_quote_stack.append((self.fstring_prefix, self.fstring_quote))
    ...
    def decimal(self) -> Token:
        ...
        while self.is_in_bounds() and (
            self.source[self.current_index].isdigit() or self.source[self.current_index] == "_"
        ):
            self.advance()
        ...
        if self.current_index + 1 < len(self.source) and (
            ...
            and (self.source[self.current_index] == "e" or self.source[self.current_index] == "E")
            and (
        ...
        if self.is_in_bounds() and (
            (digit_before_decimal or self.source[self.current_index - 1].isdigit())
            and (self.source[self.current_index] == "j" or self.source[self.current_index] == "J")
        ):
            self.advance()
    ...
    def binary(self) -> Token:
        ...
        while self.is_in_bounds() and (
            ...
            self.advance()
        if self.is_in_bounds() and (
            self.source[self.current_index] == "e" or self.source[self.current_index] == "E"
        ):
            self.advance()
            if self.is_in_bounds() and self.source[self.current_index] == "-":
                self.advance()
        ...
        if self.fstring_state.state == FStringState.at_fstring_middle:
            ...
            while self.is_in_bounds():
                ...
                if char == "\\":
                    ...
                    if self.is_in_bounds() and not (self.peek() == "{" or self.peek() == "}"):
    ...
    def __next__(self) -> Token:
        ...
        if not self.issue_128233_handling and current_char == "\r":
            ...
            if current_char != "\n":
                ...
                if self.prev_token is not None and self.prev_token.type == TokenType.comment:
        ...
        if current_char == "#":
            ...
            while self.is_in_bounds() and not self.is_newline():
                if not self.issue_128233_handling and self.source[self.current_index] == "\r":
                    break
                self.advance()
            return self.make_token(TokenType.comment)
        ...
        if current_char == "}":
            self.advance()
            if self.bracket_level == 0 and self.fstring_state.state == FStringState.in_fstring_expr:
