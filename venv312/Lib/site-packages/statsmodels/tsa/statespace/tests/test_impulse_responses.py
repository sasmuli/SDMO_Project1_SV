...
def test_dynamic_factor():
    ...
    mod1 = dynamic_factor.DynamicFactor(
        np.zeros((steps, 2)), k_factors=1, factor_order=2, exog=exog
    )
    mod2 = sarimax.SARIMAX([0], order=(2, 0, 0))
...
class TVSS(mlemodel.MLEModel):
    ...
    def __init__(self, endog, _k_states=None):
        ...
        self["transition", :k_states, :k_states, :] = (Q @ D @ Q.transpose(0, 2, 1)).transpose(
            1, 2, 0
        )
...
def test_time_varying_out_of_sample(reset_randomstate):
    ...
    irfs = mod.impulse_responses(
        [], steps=mod.nobs, design=new_Z[:, :, None], transition=new_T[:, :, None]
    )
    # Compute the same thing, but now with explicit anchor argument
    irfs_anchor = mod.impulse_responses(
        [], steps=mod.nobs, anchor=0, design=new_Z[:, :, None], transition=new_T[:, :, None]
    )

    # Cumulative IRFs
    cirfs = mod.impulse_responses(
        [], steps=mod.nobs, design=new_Z[:, :, None], transition=new_T[:, :, None], cumulative=True
    )
    # Orthogonalized IRFs
...
def test_time_varying_in_sample_anchored(reset_randomstate):
    ...
    # Orthogonalized IRFs
    oirfs = mod.impulse_responses(
        [], steps=mod.nobs - 1 - anchor, anchor=anchor, orthogonalized=True
    )
    # Cumulative, orthogonalized IRFs
    coirfs = mod.impulse_responses(
        [], steps=mod.nobs - 1 - anchor, anchor=anchor, cumulative=True, orthogonalized=True
    )
...
def test_time_varying_out_of_sample_anchored_end(reset_randomstate):
    ...
    # Cumulative IRFs
    cirfs = mod.impulse_responses(
        [], steps=2, anchor="end", design=new_Z, transition=new_T, cumulative=True
    )
    # Orthogonalized IRFs
    oirfs = mod.impulse_responses(
        [], steps=2, anchor="end", design=new_Z, transition=new_T, orthogonalized=True
    )
    # Cumulative, orthogonalized IRFs
    coirfs = mod.impulse_responses(
        [],
