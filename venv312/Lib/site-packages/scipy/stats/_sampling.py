...
def chi_pdf(x, a):
    if x > 0:
        return math.exp(
            (a - 1) * math.log(x) - 0.5 * (x * x) - (a / 2 - 1) * math.log(2) - math.lgamma(0.5 * a)
        )
    else:
        return 0 if a >= 1 else np.inf
...
def chi2_pdf(x, df):
    if x > 0:
        return math.exp(
            (df / 2 - 1) * math.log(x) - 0.5 * x - (df / 2) * math.log(2) - math.lgamma(0.5 * df)
        )
    else:
...
PINV_CONFIG = {
    ...
    "geninvgauss": {
        "pdf": geninvgauss_pdf,
        "check_pinv_params": lambda p, b: (abs(p) <= 1200.0) and (1.0e-10 <= b <= 1200.0),
