...
if Version(sparse.__version__) < _MIN_SPARSE_VERSION:
    raise ImportError("numpy sparse backend requires `sparse` version >= %r" % _MIN_SPARSE_VERSION)
...
class NumpySparseBackend(Backend, backend_name="numpy.sparse"):
    ...
    def tensor(data, dtype=None):
        if is_sparse(data):
            return data.astype(dtype) if dtype is not None and dtype != data.dtype else data
