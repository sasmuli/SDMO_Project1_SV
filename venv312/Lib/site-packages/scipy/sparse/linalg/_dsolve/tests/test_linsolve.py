...
def setup_bug_8278():
    ...
    eyeN = eye_array(N - 1)
    A = kron(eyeN, kron(eyeN, Ah1D)) + kron(eyeN, kron(Ah1D, eyeN)) + kron(Ah1D, kron(eyeN, eyeN))
...
class TestLinsolve:
    ...
    def test_twodiags(self, format: str, idx_dtype: np.dtype):
        A = dia_array(([[1, 2, 3, 4, 5], [6, 5, 8, 9, 10]], [0, 1]), shape=(5, 5)).asformat(format)
        b = array([1, 2, 3, 4, 5])
    ...
    def test_random(self, n, m, lower, format, unit_diagonal, choice_of_A, choice_of_b):
        def random_triangle_matrix(n, lower=True, format="csr", choice_of_A="real"):
            ...
            A = scipy.sparse.random(
                n, n, density=0.1, format="lil", dtype=dtype, random_state=rng, data_rvs=rvs
            )
            if lower:
