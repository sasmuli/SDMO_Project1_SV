...
def explore_directories(start, exclude, ignore):
    ...
    for root, dirs, files in os.walk(start):
        ...
        for filename in filter_out(fullpaths, exclude):
            if not os.path.basename(filename).startswith(".") and _is_python_file(filename):
                yield filename
...
"""
    ...
    for filename, blocks in results.items():
        for block in blocks:
            ...
            name = "{}.{}".format(raw_classname, raw_name) if raw_classname else block["name"]
    ...
    for path in results:
        ...
        if type(info) is dict and info.get("error"):
            ...
            for offender in info:
                ...
                fingerprint = get_fingerprint(path, [offender["type"], offender["name"]])
