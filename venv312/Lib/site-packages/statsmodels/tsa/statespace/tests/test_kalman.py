...
class Clark1987:
    ...
    def setup_class(cls, dtype=float, conserve_memory=0, loglikelihood_burn=0):
        ...
        cls.initial_state_cov = np.asfortranarray(
            np.dot(
                np.dot(cls.transition[:, :, 0], cls.initial_state_cov), cls.transition[:, :, 0].T
            )
        )
    ...
    def init_filter(cls):
        ...
        kfilter = klass(
            model, conserve_memory=cls.conserve_memory, loglikelihood_burn=cls.loglikelihood_burn
        )
    ...
    def test_filtered_state(self):
        assert_almost_equal(
            self.result["state"][0][self.true["start"] :], self.true_states.iloc[:, 0], 4
        )
        assert_almost_equal(
            self.result["state"][1][self.true["start"] :], self.true_states.iloc[:, 1], 4
        )
        assert_almost_equal(
            self.result["state"][3][self.true["start"] :], self.true_states.iloc[:, 2], 4
        )
...
class TestClark1987Single(Clark1987):
    ...
    def test_loglike(self):
        assert_allclose(self.result["loglike"](self.true["start"]), self.true["loglike"], rtol=1e-3)

    def test_filtered_state(self):
        assert_allclose(
            self.result["state"][0][self.true["start"] :], self.true_states.iloc[:, 0], atol=1e-2
        )
        assert_allclose(
            self.result["state"][1][self.true["start"] :], self.true_states.iloc[:, 1], atol=1e-2
        )
        assert_allclose(
            self.result["state"][3][self.true["start"] :], self.true_states.iloc[:, 2], atol=1e-2
        )
...
class TestClark1987Conserve(Clark1987):
    ...
    @classmethod
    def setup_class(cls):
        super().setup_class(dtype=float, conserve_memory=MEMORY_NO_FORECAST | MEMORY_NO_PREDICTED)
        cls.model, cls.filter = cls.init_filter()
        cls.result = cls.run_filter()
...
class Clark1989:
    ...
    def test_filtered_state(self):
        ...
        assert_almost_equal(
            self.result["state"][4][self.true["start"] :], self.true_states.iloc[:, 2], 4
        )
        assert_almost_equal(
            self.result["state"][5][self.true["start"] :], self.true_states.iloc[:, 3], 4
        )
