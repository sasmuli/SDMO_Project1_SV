...
class TestBuiltin:
    ...
    def test_bad_param(self):
        ...
        assert_raises(
            ValueError,
            np.dtype,
            {"names": ["f0", "f1"], "formats": ["i4", "i1"], "offsets": [0, 4], "itemsize": 4},
        )
    ...
    @pytest.mark.parametrize(
        ["type_char", "char_size", "scalar_type"], [["U", 4, np.str_], ["S", 1, np.bytes_]]
    )
...
class TestRecord:
    ...
    def test_different_titles(self):
        # In theory, they may hash the same (collision) ?
        a = np.dtype(
            {"names": ["r", "b"], "formats": ["u1", "u1"], "titles": ["Red pixel", "Blue pixel"]}
        )
        b = np.dtype(
            {"names": ["r", "b"], "formats": ["u1", "u1"], "titles": ["RRed pixel", "Blue pixel"]}
        )
        assert_dtype_not_equal(a, b)
    ...
    def test_aligned_size(self):
        ...
        assert_equal(dt.itemsize, 8)
        dt = np.dtype(
            {"names": ["f0", "f1"], "formats": ["i4", "u1"], "offsets": [0, 4]}, align=True
        )
        assert_equal(dt.itemsize, 8)
        ...
        dt1 = np.dtype(
            [("f0", "i4"), ("f1", [("f1", "i1"), ("f2", "i4"), ("f3", "i1")]), ("f2", "i1")],
            align=True,
        )
    ...
    def test_union_struct(self):
        ...
        dt = np.dtype(
            {"names": ["f0", "f1", "f2"], "formats": ["<u4", "<u2", "<u2"], "offsets": [0, 0, 2]},
            align=True,
        )
        ...
        dt = np.dtype(
            {"names": ["f0", "f1", "f2"], "formats": ["<u4", "<u2", "<u2"], "offsets": [4, 0, 2]},
            align=True,
        )
        assert_equal(dt.itemsize, 8)
        # field name should not matter: assignment is by position
        dt2 = np.dtype(
            {"names": ["f2", "f0", "f1"], "formats": ["<u4", "<u2", "<u2"], "offsets": [4, 0, 2]},
            align=True,
        )
        ...
        assert_raises(
            TypeError, np.dtype, {"names": ["f0", "f1"], "formats": ["O", "i1"], "offsets": [0, 2]}
        )
        assert_raises(
            TypeError, np.dtype, {"names": ["f0", "f1"], "formats": ["i4", "O"], "offsets": [0, 3]}
        )
        assert_raises(
            TypeError,
            np.dtype,
    ...
    def test_multifield_index(self, align_flag):
        ...
        dt_sub = dt[["B"]]
        assert_equal(
            dt_sub, np.dtype({"names": ["B"], "formats": ["<f8"], "offsets": [88], "itemsize": 96})
        )
        assert_equal(dt_sub.isalignedstruct, align_flag)
    ...
    def test_partial_dict(self):
        # 'names' is missing
        assert_raises(
            ValueError, np.dtype, {"formats": ["i4", "i4"], "f0": ("i4", 0), "f1": ("i4", 4)}
        )
...
class TestStructuredDtypeSparseFields:
    ...
    dtype = np.dtype(
        [("a", {"names": ["aa", "ab"], "formats": ["f", "f"], "offsets": [0, 4]}, (2, 3))]
    )
