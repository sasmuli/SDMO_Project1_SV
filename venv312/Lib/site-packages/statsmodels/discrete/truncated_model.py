...
class TruncatedLFGeneric(CountModel):
    ...
    def __init__(
        self, endog, exog, truncation=0, offset=None, exposure=None, missing="none", **kwargs
    ):
...
class TruncatedLFPoisson(TruncatedLFGeneric):
    ...
    def __init__(
        self, endog, exog, offset=None, exposure=None, truncation=0, missing="none", **kwargs
    ):
        super().__init__(
            endog,
            exog,
...
class TruncatedLFNegativeBinomialP(TruncatedLFGeneric):
    ...
    def __init__(
        self, endog, exog, offset=None, exposure=None, truncation=0, p=2, missing="none", **kwargs
    ):
        super().__init__(
            endog,
            exog,
...
class _RCensoredGeneric(CountModel):
    ...
    def loglikeobs(self, params):
        ...
        llf = np.concatenate(
            (llf_main[self.zero_idx], np.log(1 - np.exp(llf_main[self.nonzero_idx])))
        )
            ...
            (
                ...
                (
                    ...
                    * -np.exp(llf_main[self.nonzero_idx])
                    / (1 - np.exp(llf_main[self.nonzero_idx]))
                ).T,
            )
        )
    ...
    def predict(self, params, exog=None, exposure=None, offset=None, which="mean", y_values=None):
        ...
        # not yet supported
            ...
            probs_main = self.model2.predict(
                params_main, exog, np.exp(exposure), offset, which="prob", y_values=y_values
            )
