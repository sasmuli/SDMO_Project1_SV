...
use one of the methods for adjusting data for missing-values."""
    ...
    def _compute_gls_weights(self):
        ...
        if self._ncomp == self._nvar:
            raise ValueError(
                "gls can only be used when ncomp < nvar " "so that residuals have non-zero variance"
            )
...
estimates are based on only {eff_series} (effective) series."""
    ...
    def _fill_missing_em(self):
        ...
        if np.any(col_non_missing < ncomp) or np.any(row_non_missing < ncomp):
            raise ValueError(
                "Implementation requires that all columns and "
    ...
    def _compute_rsquare_and_ic(self):
        ...
        penalties = np.array(
            [
                sum_to_prod * np.log(1.0 / sum_to_prod),
    ...
    def project(self, ncomp=None, transform=True, unweight=True):
        ...
        if ncomp > self._ncomp:
            raise ValueError("ncomp must be smaller than the number of " "components computed.")
        ...
        if transform:
            ...
            if self._standardize or self._demean:
                projection += self._mu
        if self._index is not None:
            projection = pd.DataFrame(projection, columns=self._columns, index=self._index)
        return projection
    ...
    def _to_pandas(self):
        ...
        # Projections
        df = pd.DataFrame(self.projection, columns=self._columns, index=index)
        self.projection = df
        # Weights
    ...
    def plot_scree(self, ncomp=None, log_scale=True, cumulative=False, ax=None):
        ...
        if log_scale:
            ...
            ylim = np.exp(np.array([np.log(ylim[0]) - scale * sp, np.log(ylim[1]) + scale * sp]))
        else:
            sp = ylim[1] - ylim[0]
...
def pca(
    ...
    return (pc.factors, pc.loadings, pc.projection, pc.rsquare, pc.ic, pc.eigenvals, pc.eigenvecs)
