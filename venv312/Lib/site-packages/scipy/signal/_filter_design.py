...
def findfreqs(num, den, N, kind="ba"):
    ...
    hfreq = np.round(np.log10(np.max(3 * np.abs(ez.real + integ) + 1.5 * ez.imag)) + 0.5)
    lfreq = np.round(np.log10(0.1 * np.min(np.abs((ez + integ).real) + 2 * ez.imag)) - 0.5)
...
def iirdesign(wp, ws, gpass, gstop, analog=False, ftype="ellip", output="ba", fs=None):
    ...
    return iirfilter(
        N, Wn, rp=gpass, rs=gstop, analog=analog, btype=btype, ftype=ftype, output=output, fs=fs
    )


def iirfilter(
    N, Wn, rp=None, rs=None, btype="band", analog=False, ftype="butter", output="ba", fs=None
):
    """
...
def ellip(N, rp, rs, Wn, btype="low", analog=False, output="ba", fs=None):
    ...
    """
    return iirfilter(
        N, Wn, rs=rs, rp=rp, btype=btype, analog=analog, output=output, ftype="elliptic", fs=fs
    )
...
def buttord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    try:
        ...
        W0 = 1.0
        warnings.warn("Order is zero...check input parameters.", RuntimeWarning, stacklevel=2)
...
def ellipap(N, rp, rs):
    ...
    if N % 2:
        newp = np.compress(
            abs(p.imag) > EPSILON * np.sqrt(np.sum(p * np.conjugate(p), axis=0).real), p, axis=-1
        )
...
def iircomb(w0, Q, ftype="notch", fs=2.0, *, pass_zero=False):
    ...
    ftype = ftype.lower()
    if not 0 < w0 < fs / 2:
        raise ValueError(f"w0 must be between 0 and {fs / 2}" f" (Nyquist), but given {w0}.")
    if ftype not in ("notch", "peak"):
...
def gammatone(freq, ftype, order=None, numtaps=None, fs=None):
    ...
    if not 0 < freq < fs / 2:
        raise ValueError(
            f"The frequency must be between 0 and {fs / 2}" f" (Nyquist), but given {freq}."
        )
