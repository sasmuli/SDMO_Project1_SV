...
def fontInfoOpenTypeGaspRangeRecordsValidator(value: Any) -> bool:
    ...
    dictPrototype: GenericDict = dict(rangeMaxPPEM=(int, True), rangeGaspBehavior=(list, True))
    ppemOrder = []
...
def fontInfoWOFFMetadataLicenseValidator(value: Any) -> bool:
    ...
    Version 3+.
    """
    dictPrototype: GenericDict = dict(url=(str, False), text=(list, False), id=(str, False))
    if not genericDictValidator(value, dictPrototype):
        return False
    if "text" in value:
        for text in value["text"]:
...
def fontInfoWOFFMetadataExtensionValidator(value: Any) -> bool:
    ...
    Version 3+.
    """
    dictPrototype: GenericDict = dict(names=(list, False), items=(list, True), id=(str, False))
    if not genericDictValidator(value, dictPrototype):
        return False
    if "names" in value:
...
def fontInfoWOFFMetadataExtensionItemValidator(value: Any) -> bool:
    ...
    Version 3+.
    """
    dictPrototype: GenericDict = dict(id=(str, False), names=(list, True), values=(list, True))
    if not genericDictValidator(value, dictPrototype):
        return False
    for name in value["names"]:
        if not fontInfoWOFFMetadataExtensionNameValidator(name):
