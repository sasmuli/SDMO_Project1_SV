...
def fitquantilesgmm(distfn, x, start=None, pquant=None, frozen=None):
    ...
    parest = optimize.fmin(
        lambda params: np.sum(
            momentcondquant(distfn, params, mom2s, (pquant, xqs), shape=None) ** 2
        ),
        start,
    )
    return parest
...
if __name__ == "__main__":
    ...
    mcond = gammamomentcond(
        distfn, [5.0, 10], mom2=stats.gamma.stats(alpha, 0.0, 1.0), quantile=None
    )
    ...
    betaparest_gmmquantile = optimize.fmin(
        lambda params: np.sum(
            momentcondquant(stats.beta, params, mom2s, (pq, xqsb), shape=None) ** 2
        ),
        [10, 10, 0.0, 1.0],
        maxiter=2000,
    )
    ...
    tparest_gmm3quantilefsolve = optimize.fsolve(
        lambda params: momentcondunbound(distfn, params, mom2s, (pq, xqs)), [10, 1.0, 2.0]
    )
    print("tparest_gmm3quantilefsolve", tparest_gmm3quantilefsolve)
    tparest_gmm3quantile = optimize.fmin(
        lambda params: np.sum(momentcondunbound(distfn, params, mom2s, (pq, xqs)) ** 2),
        [10, 1.0, 2.0],
    )
    ...
    print(
        optimize.fsolve(
            lambda params: momentcondunboundls(distfn, params, mom2s, shape=5), [1.0, 2.0]
        )
    )
    print(
        optimize.fmin(
            lambda params: np.sum(momentcondunboundls(distfn, params, mom2s, shape=5) ** 2),
            [1.0, 2.0],
        )
    )
    print(distfn.fit(trvs))
    ...
    print(
        optimize.fsolve(
            lambda params: momentcondunboundls(distfn, params, mom2s, (pq, xqs), shape=5),
            [1.0, 2.0],
        )
    )
    ...
    tparest_gmmquantile = optimize.fmin(
        lambda params: np.sum(momentcondquant(distfn, params, mom2s, (pq, xqs), shape=None) ** 2),
        [10, 1.0, 2.0],
    )
    ...
    print(
        fitquantilesgmm(
            stats.genpareto, p2rvs, start=x0p, pquant=np.linspace(0.01, 0.99, 10), frozen=None
        )
    )
