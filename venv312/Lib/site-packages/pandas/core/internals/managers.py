...
class BaseBlockManager(DataManager):
        ...
        if warn and warn_copy_on_write() and not self._has_no_reference(0):
            ...
            if self.ndim == 2 and isinstance(indexer, tuple):
                ...
                if values.ndim == 2:
                    ...
                    self._iset_split_block(0, blk_loc, values)  # type: ignore[attr-defined]
    ...
    def convert_dtypes(self, **kwargs):
        ...
        return self.apply("convert_dtypes", copy=copy, using_cow=using_copy_on_write(), **kwargs)
...
class SingleBlockManager(BaseBlockManager, SingleDataManager):
    ...
    def __getstate__(self):
        ...
        extra_state = {
            "0.14.1": {
                "axes": axes_array,
                "blocks": [
                    {"values": b.values, "mgr_locs": b.mgr_locs.indexer} for b in self.blocks
                ],
            }
        }
    ...
    def __setstate__(self, state) -> None:
        ...
        if isinstance(state, tuple) and len(state) >= 4 and "0.14.1" in state[3]:
            ...
            ndim = len(self.axes)
            self.blocks = tuple(
                unpickle_block(b["values"], b["mgr_locs"], ndim=ndim) for b in state["blocks"]
            )
        else:
...
def _form_blocks(arrays: list[ArrayLike], consolidate: bool, refs: list) -> list[Block]:
    ...
    for (_, dtype), tup_block in grouper:
        ...
        if isinstance(dtype, np.dtype):
            ...
            dtype_blocks = [
                block_type(x[1], placement=BlockPlacement(x[0]), ndim=2) for x in tup_block
            ]
            nbs.extend(dtype_blocks)

        else:
            dtype_blocks = [
                block_type(ensure_block_shape(x[1], 2), placement=BlockPlacement(x[0]), ndim=2)
                for x in tup_block
            ]
            nbs.extend(dtype_blocks)
    return nbs
