...
def test_optimized_covariance(n_latent):
    ...
    for component in np.arange(n_latent):
        max_cov += abs(np.cov(tl.tensor_to_vec(x_cp.factors[0][:, component]), y, bias=True)[0, 1])
        pls_cov += abs(np.cov(tl.tensor_to_vec(pls.X_factors[0][:, component]), y, bias=True)[0, 1])
