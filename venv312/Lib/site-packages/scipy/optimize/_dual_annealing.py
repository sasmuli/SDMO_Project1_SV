...
class VisitingDistribution:
    ...
    def __init__(self, lb, ub, visiting_param, rng_gen):
        ...
        self._factor4_p = (
            np.sqrt(np.pi) * self._factor2 / (self._factor3 * (3.0 - self._visiting_param))
        )
    ...
    def visit_fn(self, temperature, dim):
        ...
        den = np.exp(
            (self._visiting_param - 1.0) * np.log(np.fabs(y)) / (3.0 - self._visiting_param)
        )
...
class StrategyChain:
    ...
    def __init__(
        self, acceptance_param, visit_dist, func_wrapper, minimizer_wrapper, rand_gen, energy_state
    ):
