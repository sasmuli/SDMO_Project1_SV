...
class Commit:
    ...
    def co_authors(self) -> List[Developer]:
        ...
        for co_author in self._c_object.co_authors:
            d = self._conf.get("developer_factory").get_developer(co_author.name, co_author.email)
            co_authors.append(d)
    ...
    def _stats(self):
        ...
        if len(self.parents) == 0:
            text = self._conf.get("git").repo.git.diff_tree(
                self.hash, "--", numstat=True, root=True
            )
            text2 = ""
            ...
            text = self._conf.get("git").repo.git.diff(
                self._c_object.parents[0].hexsha,
    ...
    def modified_files(self) -> List[ModifiedFile]:
        ...
        if len(self.parents) == 1:
            ...
            # NULL TREE
            diff_index = self._c_object.diff(NULL_TREE, paths=None, create_patch=True, **options)
    ...
    def _parse_diff(self, diff_index: List[Diff]) -> List[ModifiedFile]:
        ...
        for diff in diff_index:
            modified_files_list.append(ModifiedFile(diff=diff))
