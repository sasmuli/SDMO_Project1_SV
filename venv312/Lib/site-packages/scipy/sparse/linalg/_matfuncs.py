...
class _ExpmPadeHelper:
    ...
    def pade5(self):
        ...
        U = _smart_matrix_product(
            self.A, b[5] * self.A4 + b[3] * self.A2 + b[1] * self.ident, structure=self.structure
        )
...
def _expm(A, use_exact_onenorm):
    ...
    if (isinstance(A, np.ndarray) or issparse(A) or is_pydata_spmatrix(A)) and not np.issubdtype(
        A.dtype, np.inexact
    ):
        A = A.astype(float)
