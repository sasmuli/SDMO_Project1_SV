...
class TestMLS:
    ...
    def test_mls_inputs(self):
        ...
        xp_assert_equal(max_len_seq(10, length=0)[0], np.asarray([], dtype=np.int8))
        # unknown taps
        ...
        for nbits in range(2, 8):
            for state in [None, np.round(np.random.rand(nbits))]:
                for taps in [None, alt_taps[nbits]]:
                    ...
                    xp_assert_equal(np.abs(m), np.ones_like(m), err_msg=err_msg)
                    ...
                    xp_assert_close(tester[0], float(out_len), err_msg=err_msg)
                    # steady-state is -1
                    err_msg = "mls steady-state has incorrect value"
                    xp_assert_close(
                        tester[1:], np.full(out_len - 1, -1, dtype=tester.dtype), err_msg=err_msg
                    )
