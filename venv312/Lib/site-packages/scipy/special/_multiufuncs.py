...
from ._gufuncs import legendre_p_all, assoc_legendre_p_all, sph_legendre_p_all, sph_harm_y_all
        ...
        if not isinstance(ufunc_or_ufuncs, np.ufunc):
            ...
            for ufunc in ufuncs_iter:
                if not isinstance(ufunc, np.ufunc):
                    raise ValueError(
                        "All ufuncs must have type `numpy.ufunc`." f" Received {ufunc_or_ufuncs}"
                    )
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._resolve_out_shapes is not None:
            ...
            ufunc_arg_dtypes = tuple(
                ufunc_arg.dtype if hasattr(ufunc_arg, "dtype") else np.dtype(type(ufunc_arg))
            ...
            if self.__force_complex_output:
                ufunc_out_dtypes = tuple(
                    np.result_type(1j, ufunc_out_dtype) for ufunc_out_dtype in ufunc_out_dtypes
                )
...
def _(diff_n):
    ...
    if not 0 <= diff_n <= 2:
        raise ValueError(
            "diff_n is currently only implemented for orders 0, 1, and 2," f" received: {diff_n}."
        )
    return diff_n
...
def _(n, m, z_shape, branch_cut_shape, nout, **kwargs):
    ...
    return (
        (n + 1, 2 * abs(m) + 1) + np.broadcast_shapes(z_shape, branch_cut_shape) + (diff_n + 1,),
    )
