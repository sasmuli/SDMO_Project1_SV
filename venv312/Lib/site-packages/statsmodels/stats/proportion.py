...
def proportion_confint(count, nobs, alpha: float = 0.05, method="normal"):
    ...
    def _check(x: np.ndarray, name: str) -> np.ndarray:
        ...
        if np.any(y != x):
            raise ValueError(
                f"{name} must have an integral dtype. Found data with " f"dtype {x.dtype}"
            )
        return y
    ...
    if method == "goodman":
        ...
        def truncated_poisson_factorial_moment(interval, r, p):
            ...
            return p**r * (
                1
                - (
                    (poisson_interval((a - r + 1, a), p) - poisson_interval((b - r, b - 1), p))
        ...
        def approximated_multinomial_interval(intervals):
            ...
            return np.exp(
                np.sum(
                    np.log(
                        [poisson_interval(interval, p) for (interval, p) in zip(intervals, counts)]
                    )
                )
        ...
        def nu(c):
            ...
            return approximated_multinomial_interval(
                [(np.maximum(count - c, 0), np.minimum(count + c, n)) for count in counts]
            )
        ...
        while not (nuc <= (1 - alpha) < nucp1):
            if c > n:
                raise Exception(
                    "Couldn't find a value for `c` that " "solves nu(c) <= 1 - alpha < nu(c + 1)"
                )
            c += 1
            nuc = nucp1
            nucp1 = nu(c + 1)
...
def confint_proportions_2indep(
    count1, nobs1, count2, nobs2, method=None, compare="diff", alpha=0.05, correction=True
):
    """
    ...
    if compare == "diff":
        if method in ["wald", "agresti-caffo"]:
            ...
            low, upp = _score_confint_inversion(
                count1, nobs1, count2, nobs2, compare=compare, alpha=alpha, correction=correction
            )
...
def power_proportions_2indep(
    ...
    pow_ = normal_power_het(
        diff, nobs1, alpha, std_null=std_null, std_alternative=std_alt, alternative=alternative
    )
...
def _score_confint_inversion(
    ...
    rci0 = confint_proportions_2indep(
        count1, nobs1, count2, nobs2, method=use_method[compare], compare=compare, alpha=alpha
    )
