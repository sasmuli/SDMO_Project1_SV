...
def map(font, location, *, inputNormalized=False, outputNormalized=False, dropZeroes=False):
    if "fvar" not in font:
        return None
    ...
    if not inputNormalized:
        location = {tag: normalizeValue(value, axes[tag]) for tag, value in location.items()}

    if "avar" in font:
        location = font["avar"].renormalizeLocation(location, font, dropZeroes)

    if not outputNormalized:
        location = {tag: _denormalize(value, axes[tag]) for tag, value in location.items()}
...
def main(args=None):
    ...
    parser.add_argument("-f", action="store_true", help="Do not omit axes at default location.")
    parser.add_argument("-i", action="store_true", help="Input coordinates are normalized (-1..1).")
    parser.add_argument("-o", action="store_true", help="Output coordinates as normalized (-1..1).")
    ...
    location = {tag: float(value) for tag, value in (item.split("=") for item in options.coords)}
