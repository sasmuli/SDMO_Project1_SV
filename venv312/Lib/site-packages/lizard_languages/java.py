...
class JavaReader(CLikeReader):
    ...
    def __init__(self, context):
        ...
        self.parallel_states = [JavaStates(context), CLikeNestingStackStates(context)]
...
class JavaStates(CLikeStates):  # pylint: disable=R0903
    ...
    def _state_global(self, token):
        ...
        if self._try_start_a_class(token):
            return
        if (
            not self.in_record_constructor
        ):  # Only process as potential function if not in record constructor
    ...
    def _state_class_declaration(self, token):
        if token == "{":
            ...
            self.sub_state(
                JavaClassBodyStates(self.class_name, self.is_record, self.context), callback, token
            )
