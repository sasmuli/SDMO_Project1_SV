...
def series_small_a_small_b():
    ...
    expression = (
        gamma(b) / sympy.exp(x) * Sum(x**k / factorial(k) / gamma(a * k + b), (k, 0, S.Infinity))
    )
...
def asymptotic_series():
    ...
    class coef_C(sympy.Function):
        ...
        def eval(cls, m, rho, beta):
            ...
            res = res * (
                gamma(m + Rational(1, 2)) / (2 * pi) * (2 / (rho + 1)) ** (m + Rational(1, 2))
            )
            return res
...
def optimal_epsilon_integral():
    ...
    def arclength(eps, a, b, x, epsrel=1e-2, limit=100):
        ...
        """
        return quad(
            lambda phi: np.sqrt(1 + fp(eps, a, b, x, phi) ** 2), 0, np.pi, epsrel=epsrel, limit=100
        )[0]
