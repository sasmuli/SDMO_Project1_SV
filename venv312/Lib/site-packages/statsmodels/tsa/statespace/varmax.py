    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        trend_data = prepare_trend_data(
            self.polynomial_trend, self.k_trend, self.nobs + 1, offset=self.trend_offset
        )
        ...
        if self.error_cov_type == "diagonal":
            ...
            param_names += [
                (
                    ...
                    if i == j
                    else f"sqrt.cov.{endog_names[j]}.{endog_names[i]}"
                )
                for i in range(self.k_endog)
                for j in range(i + 1)
            ]
    ...
    def untransform_params(self, constrained):
        ...
        if self.k_ar > 0 and self.enforce_stationarity:
            ...
            unconstrained_matrices, variance = unconstrain_stationary_multivariate(
                coefficients, state_cov
            )
    ...
    def update(self, params, transformed=True, includes_fixed=False, complex_step=False):
        ...
        if self.k_trend > 0:
            ...
            if self._final_trend is not None and self._idx_state_intercept[-1].stop == -1:
                self.ssm["state_intercept", : self.k_endog, -1:] += np.dot(
                    self._final_trend, trend_params
                ).T
            ...
            if exog is not None:
                ...
                try:
                    ...
                    raise ValueError(
                        ...
                        % (str((self.k_exog,)), str(exog.shape))
                    )
...
class VARMAXResults(MLEResults):
    ...
    def __init__(self, model, params, filter_results, cov_type=None, cov_kwds=None, **kwargs):
        ...
        if self.model.k_ar > 0:
            ...
            self.coefficient_matrices_var = (
                (ar_params.reshape(k_endog * k_ar, k_endog).T).reshape(k_endog, k_endog, k_ar).T
            )
        if self.model.k_ma > 0:
            ...
            self.coefficient_matrices_vma = (
                (ma_params.reshape(k_endog * k_ma, k_endog).T).reshape(k_endog, k_endog, k_ma).T
            )
    ...
    def extend(self, endog, exog=None, **kwargs):
        ...
        mod.ssm.initialization = Initialization(
            mod.k_states, "known", constant=initial_state, stationary_cov=initial_state_cov
        )
    ...
    def _set_final_exog(self, exog):
        ...
        with mod._set_final_exog(exog):
            ...
            self.filter_results.state_intercept[: mod.k_endog, -1] = mod[
                "state_intercept", : mod.k_endog, -1
            ]
            try:
                yield
            finally:
    ...
    def _set_final_predicted_state(self, exog, out_of_sample):
        ...
        if flag:
            ...
            tmp_res = tmp_mod.filter(
                self.params, transformed=True, includes_fixed=True, return_ssm=True
            )
    ...
    def get_prediction(
        ...
        **kwargs,
        ...
        # Handle end (e.g. date)
        _start, _end, out_of_sample, _ = self.model._get_prediction_index(
    ...
    def summary(self, alpha=0.05, start=None, separate_params=True):
        ...
        summary = super().summary(
            alpha=alpha, start=start, model_name=model_name, display_params=not separate_params
        )
        ...
        if separate_params:
            ...
            def make_table(self, mask, title, strip_end=True):
                ...
                return summary_params(
                    res, yname=None, xname=param_names, alpha=alpha, use_t=False, title=title
                )
