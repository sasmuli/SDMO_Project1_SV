...
class MultivariateNormalChol:
    ...
    def logpdf_obs(self, x):
        ...
        llike = 0.5 * (
            np.log(sigma2)
            - 2.0 * np.log(np.diagonal(self.cholsigmainv))
            + (x_whitened**2) / sigma2
            + np.log(2 * np.pi)
        )
...
def ar2transform(x, arcoefs):
    ...
    y[1] = (
        np.sqrt(1 - a2**2) * x[2] - a1 * np.sqrt(1 - a1**2) / (1 - a2) * x[1]
    )  # TODO:wrong index in x
...
print(
    np.log(stats.norm._pdf(x_whitened)).sum() - 0.5 * logdetsigma,
)
print("stats whitened")
print(
    np.log(stats.norm.pdf(x, scale=np.sqrt(np.diag(sigma)))).sum(),
)
print("stats scaled")
print(
    0.5
    * (
        np.dot(linalg.cho_solve((linalg.cho_factor(sigma, lower=False)[0].T, False), x.T), x)
        + nobs * np.log(2 * np.pi)
        - 2.0 * np.log(np.diagonal(cholsigmainv)).sum()
    )
)
print(
    0.5
    * (
        np.dot(linalg.cho_solve((linalg.cho_factor(sigma)[0].T, False), x.T), x)
        + nobs * np.log(2 * np.pi)
        - 2.0 * np.log(np.diagonal(cholsigmainv)).sum()
    )
)
print(
    0.5
    * (
        np.dot(linalg.cho_solve(linalg.cho_factor(sigma), x.T), x)
        + nobs * np.log(2 * np.pi)
        - 2.0 * np.log(np.diagonal(cholsigmainv)).sum()
    )
)
