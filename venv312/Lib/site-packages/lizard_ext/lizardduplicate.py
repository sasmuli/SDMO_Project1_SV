...
class InvolvingScope(object):
    ...
    def _full_inclusive_sequences(self, sequences):
        return any(
            len(dup) == len(sequences)
            and dup[0][0] <= sequences[0][0]
            and dup[0][1] >= sequences[0][1]
...
class DuplicateFinder(object):
    ...
    def find_start_and_ends(self):
        for i, same in enumerate(self.hashed_node_indice.values()):
            ...
            for dup in scope.same_beginning(same, before_same):
                token_count = len(dup) * (dup[0][1] - dup[0][0] + self.sample_size)
    ...
    def duplicate_rate(self):
        try:
            return (
                ...
                * 1.0
                / (len(self.nodes) + (len(self.boundaries) - 1) * (self.sample_size - 2))
            )
...
class LizardExtension(ExtensionBase):
    ...
    def __call__(self, tokens, reader):
        token_unifier = reader.context.decorate_nesting_stack(NestingStackWithUnifiedTokens)
        for token in tokens:
