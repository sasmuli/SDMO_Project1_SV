...
def splprep(
    ...
    if per:
        for i in range(idim):
            if x[i][0] != x[i][-1]:
                if not quiet:
                    warnings.warn(
                        RuntimeWarning("Setting x[%d][%d]=x[%d][0]" % (i, m, i)), stacklevel=2
                    )
                x[i][-1] = x[i][0]
    if not 0 < idim < 11:
    ...
    if ier <= 0 and not quiet:
        warnings.warn(
            RuntimeWarning(
                _iermess[ier][0] + "\tk=%d n=%d m=%d fp=%f s=%f" % (k, len(t), m, fp, s)
            ),
            stacklevel=2,
        )
...
def bisplrep(
    ...
    if ierm <= 0 and not quiet:
        _mess = _iermess2[ierm][0] + "\tkx,ky=%d,%d nx,ny=%d,%d m=%d fp=%f s=%f" % (
            kx,
            ky,
            len(tx),
            len(ty),
