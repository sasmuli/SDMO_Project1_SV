...
def jac_rational(t, y):
    return np.array(
        [
            [0, 1 / t],
            [-2 * y[1] ** 2 / (t * (y[0] - 1) ** 2), (y[0] + 4 * y[1] - 1) / (t * (y[0] - 1))],
        ]
    )
...
def test_duplicate_timestamps():
    ...
    assert_allclose(sol.sol(0.01), np.asarray([-0.00039033, -0.08806632]), rtol=1e-5, atol=1e-8)
...
def test_integration():
    ...
    for vectorized, method, t_span, jac in product(
        ...
        with suppress_warnings() as sup:
            sup.filter(
                UserWarning, "The following arguments have no effect for a chosen " "solver: `jac`"
            )
            res = solve_ivp(
                fun,
                t_span,
...
def test_integration_const_jac():
    ...
    for method, jac in product(["Radau", "BDF"], [J, J_sparse]):
        res = solve_ivp(
            fun_linear, t_span, y0, rtol=rtol, atol=atol, method=method, dense_output=True, jac=jac
        )
        assert_equal(res.t[0], t_span[0])
        assert_(res.t_events is None)
        assert_(res.y_events is None)
    ...
    for method in [RK23, RK45, DOP853, Radau, BDF, LSODA]:
        ...
        for t_span in ([5, 9], [5, 1]):
            ...
            if method is not LSODA:
                solver = method(
                    fun_rational, t_span[0], y0, t_span[1], rtol=rtol, atol=atol, max_step=1e-20
                )
                message = solver.step()
...
def test_t_eval():
    ...
    t_eval = [4, 6]
    assert_raises(
        ValueError, solve_ivp, fun_rational, [5, 9], y0, rtol=rtol, atol=atol, t_eval=t_eval
    )
...
def test_OdeSolution():
    ...
    assert_equal(
        sol([0, 6, -2, 1.5, 4.5, 2.5, 5, 5.5, 2]), np.array([[-1, 1, -1, -1, 1, 1, 1, 1, -1]])
    )
...
def test_tbound_respected_oscillator(method):
    ...
    def reactions_func(t, y):
        ...
        yprime = np.array([1.73307544e-02, 6.49376470e-06, 0.00000000e00, 0.00000000e00])
        return yprime
    ...
    def run_sim2(t_end, n_timepoints=10, shortest_delay_line=10000000):
        ...
        t1 = 200.0
        return solve_ivp(
            reactions_func, (t0, t1), init_state.copy(), dense_output=True, max_step=t1 - t0
        )
