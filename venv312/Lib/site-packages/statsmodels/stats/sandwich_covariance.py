...
def cov_hc2(results):
    ...
    h = np.diag(
        np.dot(results.model.exog, np.dot(results.normalized_cov_params, results.model.exog.T))
    )
...
def cov_cluster_2groups(results, group, group2=None, use_correction=True):
    ...
    if group2 is None:
        if group.ndim != 2 or group.shape[1] != 2:
            raise ValueError(
                "if group2 is not given, then groups needs to be " + "an array with two columns"
            )
        group0 = group[:, 0]
        group1 = group[:, 1]
    else:
