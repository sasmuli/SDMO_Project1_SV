...
assert_type(
    np.average(AR_f8, weights=AR_c16, returned=True), tuple[np.complexfloating, np.complexfloating]
)
...
assert_type(
    np.copy(CHAR_AR_U, "K", subok=True), np.char.chararray[tuple[Any, ...], np.dtype[np.str_]]
)
...
assert_type(
    np.interp(1, [1], [1j]), np.complex128
)  # pyright correctly infers `complex128 | float64`
...
assert_type(
    np.meshgrid(AR_i8, AR_f8, copy=False), tuple[npt.NDArray[np.int64], npt.NDArray[np.float64]]
)
assert_type(np.meshgrid(AR_LIKE_f8, AR_f8), tuple[npt.NDArray[Any], npt.NDArray[np.float64]])
assert_type(
    np.meshgrid(AR_LIKE_f8, AR_i8, AR_c16),
