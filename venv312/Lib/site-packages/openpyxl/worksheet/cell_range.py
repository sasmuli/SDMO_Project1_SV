...
class CellRange(Serialisable):
    ...
    def shift(self, col_shift=0, row_shift=0):
        ...
        if self.min_col + col_shift <= 0 or self.min_row + row_shift <= 0:
            raise ValueError(
                "Invalid shift value: col_shift={0}, row_shift={1}".format(col_shift, row_shift)
            )
    ...
    def union(self, other):
        ...
        return CellRange(
            min_col=min_col, min_row=min_row, max_col=max_col, max_row=max_row, title=self.title
        )
