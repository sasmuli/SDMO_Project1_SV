...
def set_mode(compatibility=None):
    if compatibility:
        raise NotImplementedError("Compatibility mode is only available in" " statsmodels <= 0.9")
...
def concat(series, axis=0, allow_mix=False):
    ...
    if max_ndim > 2:
        raise ValueError("`tools.concat` does not support arrays with 3 or" " more dimensions.")
...
def _compute_coefficients_from_multivariate_pacf_python(
    partial_autocorrelations, error_variance, transform_variance=False, order=None, k_endog=None
):
    """
    ...
    for s in range(order):  # s = 0, ..., p-1
        ...
        forwards.append(
            linalg.solve_triangular(
                backward_factors[s], partial_autocorrelations[s].T, lower=True, trans="T"
            )
        )
...
def constrain_stationary_multivariate_python(
    ...
    if not use_list:
        ...
        unconstrained = [
            unconstrained[:k_endog, i * k_endog : (i + 1) * k_endog] for i in range(order)
        ]
...
def _compute_multivariate_acovf_from_coefficients(
    ...
    if type(coefficients) is list:
        ...
        coefficients = [
            coefficients[:k_endog, i * k_endog : (i + 1) * k_endog] for i in range(order)
        ]
...
def _compute_multivariate_pacf_from_coefficients(
    ...
    autocovariances = [
        autocovariance.T for autocovariance in _acovf(constrained, error_variance, maxlag=order)
    ]
...
def copy_index_matrix(
    A, B, index, index_rows=False, index_cols=False, is_diagonal=False, inplace=False, prefix=None
):
    """
...
def _compute_smoothed_state_weights(
    ...
    weights, state_intercept_weights, prior_weights, _ = func(
        _smoother, _kfilter, _model, compute_t, compute_j, scale, bool(compute_prior_weights)
    )
