...
def test_simple_logit_map():
    ...
    assert_allclose(glmm.logposterior_grad(rslt.params), np.zeros_like(rslt.params), atol=1e-3)
...
def test_simple_poisson_map():
    ...
    rslt1 = glmm1.fit_map()
    assert_allclose(glmm1.logposterior_grad(rslt1.params), np.zeros_like(rslt1.params), atol=1e-3)
...
def test_crossed_logit_map():
    ...
    assert_allclose(glmm.logposterior_grad(rslt.params), np.zeros_like(rslt.params), atol=1e-4)
...
def test_crossed_logit_vb():
    ...
    assert_allclose(
        rslt1.params[0:5],
        np.r_[-5.43073978e-01, -2.46197518e00, -2.36582801e00, -9.64030461e-03, 2.32701078e-03],
        rtol=1e-4,
        atol=1e-4,
    )
    ...
    assert_allclose(
        rslt1.cov_params().flat[0:5],
        np.r_[4.12927123e-02, -2.04448923e-04, 4.64829219e-05, 1.20377543e-04, -1.45003234e-04],
        rtol=1e-4,
        atol=1e-4,
    )
