...
def _background_gradient(
    ...
    with _mpl(Styler.background_gradient) as (_, _matplotlib):
        ...
        def relative_luminance(rgba) -> float:
            ...
            """
            r, g, b = (
                x / 12.92 if x <= 0.04045 else ((x + 0.055) / 1.055) ** 2.4 for x in rgba[:3]
            )
        ...
        def css(rgba, text_only) -> str:
            if not text_only:
                ...
                return (
                    f"background-color: {_matplotlib.colors.rgb2hex(rgba)};" f"color: {text_color};"
                )
            else:
...
def _highlight_between(
    ...
    if inclusive == "both":
        ...
        raise ValueError(
            f"'inclusive' values can be 'both', 'left', 'right', or 'neither' " f"got {inclusive}"
        )
