...
class TestLocalLinearTrendAnalytic:
    ...
    def test_results(self):
        ...
        P3 = sigma2_y * np.array(
            [[5 + 2 * q_mu + q_beta, 3 + q_mu + q_beta], [3 + q_mu + q_beta, 2 + q_mu + 2 * q_beta]]
        )
    ...
    def check_object(self, actual, desired, rtol_diffuse):
        ...
        if rtol_diffuse is not None:
            ...
            if rtol_diffuse != np.inf:
                assert_allclose(
                    actual.T[:d], desired.T[:d], rtol=rtol_diffuse, atol=self.atol_diffuse
                )
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        if init_approx is None:
            init_approx = Initialization(
                cls.ssm.k_states, "approximate_diffuse", approximate_diffuse_variance=kappa
            )
