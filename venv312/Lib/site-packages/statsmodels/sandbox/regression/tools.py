...
def tstd_pdf(x, df):
    ...
    r = np.array(df * 1.0)
    Px = np.exp(special.gammaln((r + 1) / 2.0) - special.gammaln(r / 2.0)) / np.sqrt(
        (r - 2) * np.pi
    )
...
def locscale_grad(y, loc, scale, dlldy, *args):
    ...
    dlldscale = -1.0 / scale - dlldy(yst, *args) * (y - loc) / scale**2
...
if __name__ == "__main__":
    ...
    if verbose:
        ...
        print(
            misc.derivative(llt, 1.5, dx=1e-10, n=1, args=(0, 1, 20), order=3),
        )
        print("ts", locscale_grad(1.5, 0, 1, ts_dlldy, 20))
        print(
            misc.derivative(llt, 1.5, dx=1e-10, n=1, args=(0, 2, 20), order=3),
        )
        print("ts", locscale_grad(1.5, 0, 2, ts_dlldy, 20))
        print(
            misc.derivative(llt, 1.5, dx=1e-10, n=1, args=(1, 2, 20), order=3),
        )
        print("ts", locscale_grad(1.5, 1, 2, ts_dlldy, 20))
        print(
            misc.derivative(lltloc, 1, dx=1e-10, n=1, args=(1.5, 2, 20), order=3),
        )
        ...
        print(
            misc.derivative(lltloc, loc, dx=1e-10, n=1, args=(y, scale, df), order=3),
        )
        ...
        print(
            misc.derivative(llnormloc, loc, dx=1e-10, n=1, args=(y, scale), order=3),
        )
    ...
    for df in [3, 10, 100]:
        for loc, scale in params:
            ...
            assert_almost_equal(
                ts_lls(yt, np.array([loc, scale**2]), df),
                llt(yt, loc, scale, df),
                5,
                err_msg="loglike",
            )
            assert_almost_equal(
                tstd_lls(yt, np.array([loc, scale**2]), df),
                llt(yt, loc, scale * np.sqrt((df - 2.0) / df), df),
                5,
                err_msg="loglike",
            )
