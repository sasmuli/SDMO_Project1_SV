    ...
    def fit(
        ...
        if start_params is None:
            ...
            if start_params.shape[0] != self.exog.shape[1] or start_params.ndim != 1:
                raise ValueError(
                    "start_params must by a 1-d array with {} " "values".format(self.exog.shape[1])
                )
            fake_wls = reg_tools._MinimalWLS(
                self.endog, self.exog, weights=np.ones_like(self.endog), check_weights=False
            )
        ...
        results.fit_options = dict(
            cov=cov.upper(), scale_est=scale_est, norm=self.M.__class__.__name__, conv=conv
        )
