...
BLOCK_FACTOR_ORDERS_KP1 = {"global": 1, "test1": 1, "test2": 1}
BLOCK_FACTOR_ORDERS_KP2 = {"global": 2, "test1": 2, "test2": 2}
BLOCK_FACTOR_MULTIPLICITIES_KP2 = {"global": 2, "test1": 2, "test2": 2}
...
@pytest.mark.parametrize(
    "run",
    ["111", "112", "11F", "221", "222", "22F", "block_111", "block_112", "block_221", "block_222"],
)
def test_known(matlab_results, run):
    ...
    assert_allclose(
        res.forecast(1, original_scale=False).iloc[0], results["smoothed_forecasts"][-1]
    )
...
def test_emstep1(matlab_results, run):
    ...
    # Construct the model
    mod = dynamic_factor_mq.DynamicFactorMQ(
        endog_M.iloc[:, : results1["k_endog_M"]],
        endog_quarterly=endog_Q,
    ...
    init = initialization.Initialization(
        mod.k_states,
        "known",
        constant=results1["initial_state"],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            BLOCK_FACTORS_KP1.copy(),
            BLOCK_FACTOR_ORDERS_KP2.copy(),
...
def test_emstep_methods_missing(
    matlab_results, k_factors, factor_orders, factor_multiplicities, idiosyncratic_ar1
):
...
def test_news(matlab_results, run):
    ...
    res2 = res1.apply(
        updated_M.iloc[:, : results["k_endog_M"]],
