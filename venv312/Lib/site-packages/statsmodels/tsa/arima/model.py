...
class ARIMA(sarimax.SARIMAX):
    ...
    def fit(
        ...
        if method == "statespace":
            required_kwargs = ["enforce_stationarity", "enforce_invertibility", "concentrate_scale"]
        ...
        if has_exog or method == "statespace":
            ...
            if self._spec_arima.is_integrated:
                ...
                if seasonal_order[1] > 0:
                    seasonal_order = (seasonal_order[0], 0, seasonal_order[2], seasonal_order[3])
            if self._has_fixed_params:
            ...
            if method == "yule_walker":
                ...
                p, fit_details = burg(endog, ar_order=order[0], demean=False, **method_kwargs)
                ...
                p, fit_details = innovations_mle(
                    endog, order=order, seasonal_order=seasonal_order, demean=False, **method_kwargs
                )
