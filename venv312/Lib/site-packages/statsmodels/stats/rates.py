...
def tolerance_int_poisson(
    count, exposure, prob=0.95, exposure_new=1.0, method=None, alpha=0.05, alternative="two-sided"
):
...
def confint_quantile_poisson(
    count, exposure, prob, exposure_new=1.0, method=None, alpha=0.05, alternative="two-sided"
):
...
def _invert_test_confint_2indep(
    ...
    def func(r):
        v = (
            test_poisson_2indep(
                count1, exposure1, count2, exposure2, value=r, method=method, compare=compare
            )[1]
            - alpha
        ) ** 2
        return v
...
def power_poisson_ratio_2indep(
    ...
    pow_ = normal_power_het(
        es, nobs1, alpha, std_null=std_null, std_alternative=std_alt, alternative=alternative
    )
...
def _power_equivalence_het_v0(
    es_low, es_upp, nobs, alpha=0.05, std_null_low=None, std_null_upp=None, std_alternative=None
):
...
def power_negbin_ratio_2indep(
    ...
    if method_var == "alt":
        ...
        v0 = _var_cmle_negbin(
            rate1, rate2, nobs_ratio, exposure=exposure, value=value, dispersion=dispersion
        )[0]
    else:
        raise NotImplementedError(f"method_var {method_var} not recognized")
...
def power_equivalence_neginb_2indep(
    ...
    if method_var == "alt":
        ...
        v0_low = _var_cmle_negbin(
            rate1, rate2, nobs_ratio, exposure=exposure, value=low, dispersion=dispersion
        )[0]
        v0_upp = _var_cmle_negbin(
            rate1, rate2, nobs_ratio, exposure=exposure, value=upp, dispersion=dispersion
        )[0]
    else:
        raise NotImplementedError(f"method_var {method_var} not recognized")
