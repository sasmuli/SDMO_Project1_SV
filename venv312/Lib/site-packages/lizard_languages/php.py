...
class PHPReader(CodeReader, CCppCommentsMixin):
    ...
    language_names = ["php"]
    _conditions = set(
        ["if", "elseif", "for", "foreach", "while", "&&", "||", "?", "catch", "case", "match"]
    )
    ...
    def generate_tokens(source_code, addition="", token_class=None):
        ...
        current_pos = 0
        code_block_pattern = re.compile(r"\<\?(?:php)?(.*?)(?:(\?\>)|\Z)", re.M | re.S)
        for match in code_block_pattern.finditer(source_code):
            ...
            for token in CodeReader.generate_tokens(match.group(1), addition, token_class):
                yield token
