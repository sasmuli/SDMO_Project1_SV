...
class GLM(base.LikelihoodModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._check_inputs(
            family, self.offset, self.exposure, self.endog, self.freq_weights, self.var_weights
        )
        if offset is None:
            delattr(self, "offset")
        if exposure is None:
    ...
    def _check_inputs(self, family, offset, exposure, endog, freq_weights, var_weights):
        ...
        if np.shape(self.freq_weights) == () and self.freq_weights > 1:
            self.freq_weights = self.freq_weights * np.ones(endog.shape[0])
    ...
    def loglike_mu(self, mu, scale=1.0):
        ...
        return self.family.loglike(self.endog, mu, self.var_weights, self.freq_weights, scale)
    ...
    def loglike(self, params, scale=None):
        ...
        llf = self.family.loglike(self.endog, expval, self.var_weights, self.freq_weights, scale)
        return llf
    ...
    def hessian_factor(self, params, scale=None, observed=True):
        ...
        score_factor = self.score_factor(params, scale=1.0)
    ...
    def _derivative_exog(
        ...
        lin_pred = self.predict(params, exog, which="linear", offset=offset, exposure=exposure)
    ...
    def _derivative_predict(self, params, exog=None, transform="dydx", offset=None, exposure=None):
        ...
        if (
            ...
            or (getattr(self, "offset", None) is not None)
        ):
            raise NotImplementedError("offset and exposure not supported")
    ...
    def score_test(self, params_constrained, k_constraints=None, exog_extra=None, observed=True):
        ...
        if exog_extra is None:
            if k_constraints is None:
                raise ValueError("if exog_extra is None, then k_constraints" "needs to be given")
            ...
            hessian_factor = self.hessian_factor(params_constrained, observed=observed)
    ...
    def _update_history(self, tmp_result, mu, history):
        ...
        history["deviance"].append(
            self.family.deviance(self.endog, mu, self.var_weights, self.freq_weights, self.scale)
        )
        return history
    ...
    def estimate_scale(self, mu):
        ...
        if not self.scaletype:
            if isinstance(
                self.family, (families.Binomial, families.Poisson, families.NegativeBinomial)
            ):
                return 1.0
            else:
        ...
        if isinstance(self.scaletype, str):
            if self.scaletype.lower() == "x2":
                ...
                raise ValueError(
                    "Scale %s with type %s not understood" % (self.scaletype, type(self.scaletype))
                )
        else:
            raise ValueError(
                "Scale %s with type %s not understood" % (self.scaletype, type(self.scaletype))
        ...
        if method == "brentq":
            ...
            def psi_p(power, mu):
                ...
                return (
                    np.sum(
                        self.iweights
                        * ((self.endog - mu) ** 2 / (scale * (mu**power)) - 1)
                        * np.log(mu)
                    )
                    / self.freq_weights.sum()
                )
    ...
    def get_distribution(
        ...
        kwds = {}
        if np.any(n_trials != 1) and isinstance(self.family, families.Binomial):

            kwds["n_trials"] = n_trials

        distr = self.family.get_distribution(mu, scale, var_weights=var_weights, **kwds)
        return distr
    ...
    def _fit_gradient(
        ...
        **kwargs,
        ...
        glm_results = results_class(
            self, rslt.params, cov_p, scale, cov_type=cov_type, cov_kwds=cov_kwds, use_t=use_t
        )
    ...
    def _fit_irls(
        ...
        **kwargs,
        ...
        for iteration in range(maxiter):
            ...
            wlsendog = (
                lin_pred + self.family.link.deriv(mu) * (self.endog - mu) - self._offset_exposure
            )
            wls_mod = reg_tools._MinimalWLS(
                wlsendog, wlsexog, self.weights, check_endog=True, check_weights=True
            )
    ...
    def fit_regularized(
        ...
        **kwargs
        ...
        result = fit_elasticnet(
            self, method=method, alpha=alpha, start_params=start_params, refit=refit, **defaults
        )
    ...
    def fit_constrained(self, constraints, start_params=None, **fit_kwds):
        ...
        **fit_kwds : keyword arguments
        ...
        params, cov, res_constr = fit_constrained(
            self, R, q, start_params=start_params, fit_kwds=fit_kwds
        )
...
class GLMResults(base.LikelihoodModelResults):
    ...
    def resid_anscombe_scaled(self):
        ...
        return self.family.resid_anscombe(
            self._endog, self.fittedvalues, var_weights=self._var_weights, scale=self.scale
        )
    ...
    def llf_scaled(self, scale=None):
        ...
        if scale is None:
            if (
                isinstance(self.family, families.Gaussian)
                and isinstance(self.family.link, families.links.Power)
                and (self.family.link.power == 1.0)
            ):
    ...
    def get_prediction(
        ...
        if which is None:
            ...
            res_linpred = linpred.get_prediction(
                self, exog=exog, transform=transform, row_labels=row_labels, pred_kwds=pred_kwds
            )

            pred_kwds["which"] = "mean"
            res = pred.get_prediction_glm(
                self,
                exog=exog,
                transform=transform,
    ...
    def get_distribution(
        ...
        if isinstance(
            self.model.family, (families.Binomial, families.Poisson, families.NegativeBinomial)
        ):
            # use scale=1, independent of QMLE scale for discrete
            scale = 1.0
            if self.scale != 1.0:
    ...
    def plot_added_variable(
        self, focus_exog, resid_type=None, use_glm_weights=True, fit_kwargs=None, ax=None
    ):
    ...
    def summary(self, yname=None, xname=None, title=None, alpha=0.05):
        ...
        smry.add_table_2cols(
            self, gleft=top_left, gright=top_right, yname=yname, xname=xname, title=title
        )
