...
class ArrowExtensionArray(
    ...
    def _box_pa_array(
        ...
        if isinstance(value, cls):
            ...
            if (
                ...
                and (pa.types.is_large_binary(pa_type) or pa.types.is_large_string(pa_type))
            ):
    ...
    def reshape(self, *args, **kwargs):
        raise NotImplementedError(
            f"{type(self)} does not support reshape " f"as backed by a 1D pyarrow.ChunkedArray."
        )
    ...
    def to_numpy(
        ...
        if pa.types.is_timestamp(pa_type) or pa.types.is_duration(pa_type):
            ...
            and (
                na_value is np.nan or original_na_value is lib.no_default and is_float_dtype(dtype)
            )
        ):
    ...
    def _accumulate(
        ...
        **kwargs
        ...
        convert_to_int = (pa.types.is_temporal(pa_dtype) and name in ["cummax", "cummin"]) or (
        ...
        if name == "sem":
            ...
            def pyarrow_meth(data, skip_nulls, min_count=0):  # type: ignore[misc]
                ...
                if skip_nulls:
                    ...
                    if mask is not None or check_below_min_count((len(data),), None, min_count):
                        return pa.scalar(None, type=data.type)
    ...
    def _dt_seconds(self):
        return type(self)(
            pa.array(self._to_timedeltaarray().seconds, from_pandas=True, type=pa.int32())
        )
    ...
    def _dt_nanoseconds(self):
        return type(self)(
            pa.array(self._to_timedeltaarray().nanoseconds, from_pandas=True, type=pa.int32())
        )
    ...
    def _dt_time(self):
        unit = (
            self.dtype.pyarrow_dtype.unit if self.dtype.pyarrow_dtype.unit in {"us", "ns"} else "ns"
        )
