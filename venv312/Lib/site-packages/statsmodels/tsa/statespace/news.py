...
class NewsResults:
    ...
    def __init__(
        ...
        self.post_impacted_forecasts = pd.DataFrame(
            news_results.post_impacted_forecasts.T, index=self.row_labels, columns=columns
        ).rename_axis(index="impact date", columns="impacted variable")
        # E[y^i | previous]
        self.prev_impacted_forecasts = pd.DataFrame(
            news_results.prev_impacted_forecasts.T, index=self.row_labels, columns=columns
        ).rename_axis(index="impact date", columns="impacted variable")
        # E[y^i | post] - E[y^i | revisions]
        self.update_impacts = pd.DataFrame(
            news_results.update_impacts, index=self.row_labels, columns=columns
        ).rename_axis(index="impact date", columns="impacted variable")
        ...
        self.revision_grouped_impacts = (
            self.revision_impacts - self.revision_detailed_impacts.fillna(0)
        )
        ...
        self.revisions_iloc = pd.DataFrame(
            list(zip(*news_results.revisions_ix)), index=["revision date", "revised variable"]
        ).T
        ...
        self.updates_iloc = pd.DataFrame(
            list(zip(*news_results.updates_ix)), index=["update date", "updated variable"]
        ).T
        ...
        r_ix_all = pd.MultiIndex.from_arrays(
            [self.revisions_ix["revision date"], self.revisions_ix["revised variable"]]
        )
        ...
        u_ix = pd.MultiIndex.from_arrays(
            [self.updates_ix["update date"], self.updates_ix["updated variable"]]
        )
        ...
        if news_results.revisions_all is None:
            self.revisions_all = pd.Series(
                [], index=r_ix_all, name="revision", dtype=model.params.dtype
            )
        else:
            self.revisions_all = pd.Series(
                news_results.revisions_all, index=r_ix_all, name="revision"
            )
        ...
        if news_results.revised_all is None:
            self.revised_all = pd.Series(
                [], index=r_ix_all, dtype=model.params.dtype, name="revised"
            )
        else:
    ...
    def data_revisions(self):
        ...
        # Save revisions data
        data = pd.concat(
            [self.revised_all.rename("revised"), self.revised_prev_all.rename("observed (prev)")],
            axis=1,
        ).sort_index()
    ...
    def _revision_grouped_impacts(self):
        ...
        df = df.set_index(["revision date", "revised variable"], append=True).reorder_levels(
            [2, 3, 0, 1]
        )
        return df
    ...
    def revision_details_by_impact(self):
        ...
        if self.n_revisions_grouped > 0:
            ...
            df.index = df.index.set_names(
                ["revision date", "revised variable", "impact date", "impacted variable"]
            )
    ...
    def revision_details_by_update(self):
        ...
        details = (
            df.set_index(["observed (prev)", "revised"], append=True)
            .reorder_levels(
                [
                    "revision date",
                    "revised variable",
    ...
    def summary_impacts(
        ...
        if groupby in ["impacted variable", "impacted_variable"]:
            ...
            raise ValueError(
                "Invalid groupby for impacts table. Valid options"
    ...
    def summary_details(
        ...
        if groupby == "update date":
            ...
            raise ValueError(
                "Invalid groupby for details table. Valid options"
        ...
        if sparsify:
            for i in sorted(base_levels)[::-1][:-1]:
                if k_level_values[i] == 1:
                    ...
                    can_drop = (
                        (name == columns["update date"] and update_date is not None)
                        or (name == columns["updated variable"] and updated_variable is not None)
                        or (name == "impact date" and impact_date is not None)
                        or (
                            name == "impacted variable"
                            and (
                                impacted_variable is not None or self.impacted_variable is not None
                            )
                        )
                    )
    ...
    def summary_revisions(self, sparsify=True):
        ...
        """
        data = (
            pd.merge(self.data_revisions, self.revisions_all, left_index=True, right_index=True)
            .sort_index()
            .reset_index()
        )
        data = data[
            [
                ...
                "observed (prev)",
                "revision",
                "detailed impacts computed",
            ]
        ]
        try:
            data[["revision date", "revised variable"]] = data[
    ...
    def summary_news(self, sparsify=True):
        ...
        """
        data = (
            pd.merge(self.data_updates, self.news, left_index=True, right_index=True)
            .sort_index()
            .reset_index()
        )
        try:
            data[["update date", "updated variable"]] = data[
    ...
    def summary(
        ...
        updated_variable=None,
        revision_date=None,
        revised_variable=None,
        impacts_groupby="impact date",
        details_groupby="update date",
        show_revisions_columns=None,
        sparsify=True,
        include_details_tables=None,
        include_revisions_tables=False,
        float_format="%.2f",
    ):
        """
        ...
        # Top summary table
        top_left = [("Model:", [model_name]), ("Date:", None), ("Time:", None)]
        ...
        summary.tables.insert(
            table_ix,
            self.summary_impacts(
                impact_date=impact_date,
                impacted_variable=impacted_variable,
                groupby=impacts_groupby,
                show_revisions_columns=show_revisions_columns,
                sparsify=sparsify,
        ...
        if len(self.updates_iloc) > 0:
            summary.tables.insert(table_ix, self.summary_news(sparsify=sparsify))
            table_ix += 1
        ...
        if include_revisions_tables and self.n_revisions > 0:
            summary.tables.insert(table_ix, self.summary_revisions(sparsify=sparsify))
            table_ix += 1
            ...
            revision_details_tables = self.summary_details(
                ...
                updated_variable=revised_variable,
                groupby=details_groupby,
                sparsify=sparsify,
                float_format=float_format,
                multiple_tables=multiple_tables,
    ...
    def get_details(self, include_revisions=True, include_updates=True):
        details = []
        if include_updates:
            details.append(self.details_by_impact.rename(columns={"forecast (prev)": "previous"}))
        if include_revisions:
            tmp = self.revision_details_by_impact.rename_axis(
                index={"revision date": "update date", "revised variable": "updated variable"}
            )
            tmp = tmp.rename(
                columns={"revised": "observed", "observed (prev)": "previous", "revision": "news"}
            )
            details.append(tmp)
        if not (include_updates or include_revisions):
            details.append(
                self.details_by_impact.rename(columns={"forecast (prev)": "previous"}).iloc[:0]
            )
    ...
    def get_impacts(self, groupby=None, include_revisions=True, include_updates=True):
        details = self.get_details(
            include_revisions=include_revisions, include_updates=include_updates
        )
