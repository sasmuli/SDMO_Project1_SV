...
class TestTransformedDensityRejection:
    ...
    def test_bad_construction_points_array(self):
        ...
        with pytest.raises(
            ValueError,
            match=r"`construction_points` must " r"either be a " r"scalar or a non-empty array.",
        ):
            TransformedDensityRejection(StandardNormal(), construction_points=construction_points)
        ...
        with pytest.warns(RuntimeWarning, match=r"50 : starting point out of " r"domain"):
            TransformedDensityRejection(
                StandardNormal(), domain=(-3, 3), construction_points=construction_points
            )
...
class TestNumericalInversePolynomial:
    ...
    class dist3:
        ...
        def cdf(self, x):
            return (
                x / 10.0 + 0.5 + 0.09 / (2 * np.pi) * (np.cos(10 * np.pi) - np.cos(2 * np.pi * x))
            )
...
class TestRatioUniforms:
    def test_rv_generation(self):
        ...
        gen = RatioUniforms(
            lambda x: np.exp(-x), umax=1, vmin=0, vmax=2 * np.exp(-1), random_state=12345
        )
