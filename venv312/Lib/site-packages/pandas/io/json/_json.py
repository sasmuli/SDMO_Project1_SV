...
class JsonReader(abc.Iterator, Generic[FrameSeriesStrT]):
    ...
    def __init__(
        ...
        if isinstance(filepath_or_buffer, str) and not self.lines and "\n" in filepath_or_buffer:
            warnings.warn(
        ...
        if self.engine == "pyarrow":
            if not self.lines:
                raise ValueError(
                    "currently pyarrow engine only supports " "the line-delimited JSON format"
                )
...
class FrameParser(Parser):
    ...
    def _parse(self) -> None:
        ...
        if orient == "columns":
            self.obj = DataFrame(ujson_loads(json, precise_float=self.precise_float), dtype=None)
        elif orient == "split":
            decoded = {
                str(k): v for k, v in ujson_loads(json, precise_float=self.precise_float).items()
            }
            self.check_keys_split(decoded)
            orig_names = [
                (tuple(col) if isinstance(col, list) else col) for col in decoded["columns"]
            ]
