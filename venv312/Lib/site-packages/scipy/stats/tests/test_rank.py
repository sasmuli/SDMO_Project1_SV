...
class TestRankData:
    ...
    def test_rankdata_object_string(self):
        ...
        rankf = dict(
            min=min_rank, max=max_rank, ordinal=ordinal_rank, average=average_rank, dense=dense_rank
        )
    ...
    def test_nan_policy_propagate(self):
        ...
        data = [0, 2, 3, -2, np.nan, np.nan]
        assert_array_equal(
            rankdata(data, nan_policy="propagate"), [np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]
        )
