...
def test_parafac2(monkeypatch, normalize_factors, init):
    ...
    noisy_slices = [
        slice_ + tl.tensor(0.001 * rng.standard_normal(T.shape(slice_))) for slice_ in slices
    ]
    rec, err = parafac2(
        noisy_slices,
        rank,
...
def test_parafac2_slice_and_tensor_input():
    ...
    slice_rec = parafac2(slices, rank, random_state=1234, normalize_factors=False, n_iter_max=2)
    slice_rec_tensor = parafac2_to_tensor(slice_rec)

    tensor_rec = parafac2(tensor, rank, random_state=1234, normalize_factors=False, n_iter_max=2)
...
def test_parafac2_normalize_factors():
    ...
    normalized_rec = parafac2(slices, rank, random_state=rng, normalize_factors=True, n_iter_max=50)
