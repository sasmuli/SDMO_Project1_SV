...
def linprog(
    ...
    if np.any(integrality) and not meth == "highs":
        integrality = None
        warning_message = (
            "Only `method='highs'` supports integer " "constraints. Ignoring `integrality`."
        )
        warn(warning_message, OptimizeWarning, stacklevel=2)
    ...
    status, message = _check_result(
        x, fun, status, slack, con, lp_o.bounds, tol, message, integrality
    )
