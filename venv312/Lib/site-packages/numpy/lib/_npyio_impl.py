...
def save(file, arr, allow_pickle=True, fix_imports=np._NoValue):
    ...
    if fix_imports is not np._NoValue:
        ...
        warnings.warn(
            "The 'fix_imports' flag is deprecated and has no effect. " "(Deprecated in NumPy 2.1)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    with file_ctx as fid:
        ...
        format.write_array(
            fid, arr, allow_pickle=allow_pickle, pickle_kwargs={"fix_imports": fix_imports}
        )
    ...
    *,
    ...
    if comment is None:
        ...
        # Input validation if there are multiple comment characters
            ...
            raise TypeError(
                f"Comment characters '{comments}' cannot include the " f"delimiter '{delimiter}'"
            )
    ...
    if arr.shape:
        if arr.shape[0] == 0:
            warnings.warn(
                f'loadtxt: input contained no data: "{fname}"', category=UserWarning, stacklevel=3
            )
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    with fid_ctx:
        split_line = LineSplitter(
            delimiter=delimiter, comments=comments, autostrip=autostrip, encoding=encoding
        )
