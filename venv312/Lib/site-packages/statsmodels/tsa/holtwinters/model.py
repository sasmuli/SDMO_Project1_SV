...
class ExponentialSmoothing(TimeSeriesModel):
    ...
    def _check_bounds(self, bounds):
        ...
        for key in bounds:
            if key not in variables:
                ...
                raise KeyError(
                    f"{key} does not match the list of supported variables " f"names: {supported}."
                )
            bound = bounds[key]
            ...
            if ("smoothing" in key or "damp" in key) and (bound[0] < 0.0 or bound[1] > 1.0):
    ...
    def fix_params(self, values):
        ...
        if self._initialization_method in ("estimated", None):
            extra_keys = [
                key.replace("smoothing_", "initial_") for key in valid_keys if "smoothing_" in key
            ]
    ...
    def _initialize_simple(self):
        ...
        lvl, trend, seas = _initialization_simple(self._y, trend, seasonal, self.seasonal_periods)
    ...
    def _update_for_fixed(self, sel, alpha, beta, gamma, phi, l0, b0, s0):
        if self._fixed_parameters:
            ...
            if (~sel[~not_fixed]).any():
                ...
                raise ValueError(
                    "Cannot fix a parameter that is not being " f"estimated: {invalid_names}"
                )
    ...
    def _optimize_parameters(self, data: _OptConfig, use_brute, method, kwargs) -> _OptConfig:
        ...
        (
            ...
            b0,
            s0,
        ) = self._update_for_fixed(sel, init_alpha, init_beta, init_gamma, init_phi, l0, b0, s0)
    ...
    def fit(
        ...
        *,
        ...
        use_basinhopping = bool_like(use_basinhopping, "use_basinhopping", optional=True)
        ...
        if use_boxcox is not None:
            raise ValueError("use_boxcox was set at model initialization and cannot " "be changed")
        ...
        if use_basinhopping is not None:
            raise ValueError(
                "use_basinhopping is deprecated. Set optimization method " "using 'method'."
            )
    ...
    def _predict(
        ...
        if seasonal == "mul":
            for i in range(1, nobs + 1):
                ...
                if has_trend:
                    b[i] = (beta * detrend(lvls[i], lvls[i - 1])) + (betac * dampen(b[i - 1], phi))
            ...
            for i in range(1, nobs + 1):
                lvls[i] = y_alpha[i - 1] + (alphac * trended(lvls[i - 1], dampen(b[i - 1], phi)))
                if has_trend:
                    b[i] = (beta * detrend(lvls[i], lvls[i - 1])) + (betac * dampen(b[i - 1], phi))
            _trend = b[1 : nobs + 1].copy()
            season = s[m : nobs + m].copy()
