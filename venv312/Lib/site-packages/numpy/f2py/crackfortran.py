        ...
        if sourcecodeform == "fix":
            ...
            if cont:
                ...
                if localdolowercase:
                    ...
                    finalline = (
                        ll.lower() if not (is_f2py_directive and iscstyledirective(ll)) else ll
                    )
                else:
                    finalline = ll
                origfinalline = ll
...
# Crack line
beforethisafter = (
    r"\s*(?P<before>%s(?=\s*(\b(%s)\b)))" r"\s*(?P<this>(\b(%s)\b))" r"\s*(?P<after>%s)\s*\Z"
)
##
...
subroutinepattern = (
    re.compile(beforethisafter % (r"[a-z\s]*?", "subroutine", "subroutine", ".*"), re.I),
    "begin",
)
...
endifpattern = re.compile(beforethisafter % (r"[\w]*?", endifs, endifs, ".*"), re.I), "endif"
#
...
dimensionpattern = (
    re.compile(beforethisafter % ("", "dimension|virtual", "dimension|virtual", ".*"), re.I),
    "dimension",
)
...
def crackline(line, reset=0):
    ...
    if not m:
        ...
        if groupcounter <= 0:
            raise Exception(
                "crackline: groupcounter(=%s) is nonpositive. " "Check the blocks." % (groupcounter)
            )
...
def analyzeline(m, case, line):
    ...
    if (
        ...
        outmess(f'analyzeline: no group yet. Creating program group with name "{newname}".\n')
        gotnextfile = 0
        groupcounter = groupcounter + 1
                ...
                for l in ll:
                    if "=" in l:
                        ...
                        if m2:
                            ...
                            outmess(f"analyzeline: Not local=>use pattern found in {repr(l)}\n")
                    else:
                        rl[l] = l
...
def updatevars(typespec, selector, attrspec, entitydecl):
    ...
    for e in el1:
        ...
        if not m:
            outmess(f"updatevars: no name pattern found for entity={repr(e)}. Skipping.\n")
            continue
...
def cracktypespec(typespec, selector):
    ...
    if selector:
        if typespec in ["complex", "integer", "logical", "real"]:
            ...
            if not kindselect:
                outmess(f"cracktypespec: no kindselector pattern found for {repr(selector)}\n")
                return
            ...
            if not charselect:
                outmess(f"cracktypespec: no charselector pattern found for {repr(selector)}\n")
                return
            ...
            if m:
                ...
                if n in block["vars"]:
                    if "attrspec" in block["vars"][n]:
                        block["vars"][n]["attrspec"].append(f"dimension({','.join(dims)})")
                    else:
    ...
    for n in param_names:
        if "=" in vars[n]:
            ...
            if isdouble(vars[n]):
                ...
                outmess(
                    f"get_parameters[TODO]: " f"implement evaluation of complex expression {v}\n"
                )
...
def _eval_scalar(value, params):
    ...
    try:
        ...
        errmess(
            '"%s" in evaluating %r ' "(available names: %s)\n" % (msg, value, list(params.keys()))
        )
    return value
            ...
            if dim and "dimension" not in vars[n]:
                ...
                for d in rmbadname([x.strip() for x in markoutercomma(dim).split("@,@")]):
                    ...
                    try:
                        ...
                        outmess("analyzevars: could not parse dimension for " f"variable {d!r}\n")
            ...
            if block["block"] == "function":
                ...
                if "prefix" in block:
                    ...
                    if m:
                        ...
                        outmess(f"analyzevars: prefix ({repr(block['prefix'])}) were not used\n")
...
def determineexprtype(expr, vars, rules={}):
    ...
    if m:
        if "name" in m.groupdict() and m.group("name"):
            outmess(f"determineexprtype: selected kind types not supported ({repr(expr)})\n")
        return {"typespec": "integer"}
    ...
    if not t:
        outmess(f"determineexprtype: could not determine expressions ({repr(expr)}) type.\n")
    return t
...
def character_backward_compatibility_hook(item, parents, result, *args, **kwargs):
    ...
    if new_value is not None:
        if new_value != value:
            ...
            outmess(
                f"character_bc_hook[{parent_key}.{key}]:" f" replaced `{value}` -> `{new_value}`\n",
                1,
            )
