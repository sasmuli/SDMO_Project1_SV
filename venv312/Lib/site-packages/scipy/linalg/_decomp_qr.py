...
def qr_multiply(
    ...
    if mode not in ["left", "right"]:
        raise ValueError("Mode argument can only be 'left' or 'right' but " f"not '{mode}'")
    ...
    if mode == "left":
        if c.shape[0] != min(M, N + overwrite_c * (M - N)):
            raise ValueError(
                "Array shapes are not compatible for Q @ c" f" operation: {a.shape} vs {c.shape}"
            )
    else:
        if M != c.shape[1]:
            raise ValueError(
                "Array shapes are not compatible for c @ Q" f" operation: {c.shape} vs {a.shape}"
            )
