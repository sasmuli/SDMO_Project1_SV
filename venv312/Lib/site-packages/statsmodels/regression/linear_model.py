...
class GLSAR(GLS):
    ...
    def __init__(self, endog, exog=None, rho=1, missing="none", hasconst=None, **kwargs):
        ...
        if exog is None:
            ...
            super().__init__(
                endog, np.ones((endog.shape[0], 1)), missing=missing, hasconst=None, **kwargs
            )
        else:
    ...
    **kwargs
    ...
    def resid_pearson(self):
        ...
        if np.sqrt(self.scale) < 10 * eps * self.model.endog.mean():
            ...
            warnings.warn("All residuals are 0, cannot compute normed residuals.", RuntimeWarning)
            return self.wresid
        else:
        ...
        **Reminder**: ``use_correction`` in "hac-groupsum" and "hac-panel" is
        ...
        if cov_type in ["fixed scale", "fixed_scale"]:
            ...
            res.cov_params_default = sw.cov_nw_panel(
                self, maxlags, groupidx, weights_func=weights_func, use_correction=use_correction
            )
            ...
            res.cov_params_default = sw.cov_nw_groupsum(
                self, maxlags, time, weights_func=weights_func, use_correction=use_correction
            )
    ...
    def get_prediction(self, exog=None, transform=True, weights=None, row_labels=None, **kwargs):
        ...
        return pred.get_prediction(
            self, exog=exog, transform=transform, weights=weights, row_labels=row_labels, **kwargs
        )
...
class OLSResults(RegressionResults):
    ...
    **kwargs
    ...
    def el_test(
        self, b0_vals, param_nums, return_weights=0, ret_params=0, method="nm", stochastic_exog=1
    ):
        """
