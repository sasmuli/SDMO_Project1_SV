...
class CheckOrdinalModelMixin:
    ...
    def test_basic(self):
        ...
        assert_allclose(
            res1.model.transform_threshold_params(res1.params)[1:-1], res2.thresholds, atol=4e-4
        )
...
class TestLogitModel(CheckOrdinalModelMixin):
    ...
    def test_postestimation(self):
        ...
        dt = dia.test_chisquare_binning(
            y_dummy, fitted, sort_var=sv, bins=10, df=None, ordered=True, sort_method="stable"
        )
...
class TestProbitModel(CheckOrdinalModelMixin):
    ...
    def setup_class(cls):
        ...
        cls.pred_table = np.array(
            [[202, 18, 0, 220], [112, 28, 0, 140], [27, 13, 0, 40], [341, 59, 0, 400]],  # noqa
            dtype=np.int64,
        )
...
def test_nan_endog_exceptions():
    ...
    if hasattr(pd, "CategoricalDtype"):
        df = pd.DataFrame(
            {
                "endog": pd.Series(y, dtype=pd.CategoricalDtype([1, 2, 3], ordered=True)),
                "exog": x[:, 1],
            }
        )
