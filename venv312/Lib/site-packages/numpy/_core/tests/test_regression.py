...
class TestRegression:
    ...
    def test_sign_for_complex_nan(self):
        ...
        with np.errstate(invalid="ignore"):
            ...
            have = np.sign(C)
            want = np.array(
                [-1 + 0j, -0.6 + 0.8j, 0 + 0j, 0.8 - 0.6j, 1 + 0j, complex(np.nan, np.nan)]
            )
    ...
    def test__array_interface__descr(self):
        # gh-17068
        dt = np.dtype({"names": ["a", "b"], "offsets": [0, 0], "formats": [np.int64, np.int64]})
    ...
    def test__array_namespace__(self):
        ...
        with pytest.raises(
            ValueError, match='Version "2025.12" of the Array API Standard ' "is not supported."
        ):
            arr.__array_namespace__(api_version="2025.12")

        with pytest.raises(
            ValueError, match="Only None and strings are allowed as the Array API version"
        ):
    ...
    def test_logspace_base_does_not_determine_dtype(self):
        ...
        expected = np.array(
            [[1.0, 3.1621094, 10.0, 31.625, 100.0], [100.0, 31.625, 10.0, 3.1621094, 1.0]],
            dtype=np.float32,
        )
        ...
        out2 = np.logspace(start, stop, num=5, axis=1, dtype=np.float32, base=np.array([10.0]))
    ...
    def test_repeated_square_consistency(self):
        # gh-26940
        buf = np.array([-5.171866611150749e-07 + 2.5618634555957426e-07j, 0, 0, 0, 0, 0])
