...
def compare_encompassing(results_x, results_z, cov_type="nonrobust", cov_kwargs=None):
    ...
    return pd.DataFrame(
        [x_nested, z_nested], index=["x", "z"], columns=["stat", "pvalue", "df_num", "df_denom"]
    )


def acorr_ljungbox(
    x, lags=None, boxpierce=False, model_df=0, period=None, return_df=True, auto_lag=False
):
    """
    ...
    if auto_lag:
        ...
        if not boxpierce:
            q_sacf = (
                nobs
                * (nobs + 2)
                * np.cumsum(sacf[1 : maxlag + 1] ** 2 / (nobs - np.arange(1, maxlag + 1)))
            )
        else:
    ...
    return pd.DataFrame(
        {"lb_stat": qljungbox, "lb_pvalue": pval, "bp_stat": qboxpierce, "bp_pvalue": pvalbp},
        index=lags,
    )
...
def acorr_lm(
    resid, nlags=None, store=False, *, period=None, ddof=0, cov_type="nonrobust", cov_kwargs=None
):
    """
...
def het_goldfeldquandt(
    y, x, idx=None, split=None, drop=None, alternative="increasing", store=False
):
    """
...
F-statistic ={:8.4f} and p-value ={:8.4f}""".format(
            ordering, fval, fpval
        )
...
def linear_reset(
    res, power=3, test_type="fitted", use_f=False, cov_type="nonrobust", cov_kwargs=None
):
    r"""
...
def spec_white(resid, exog):
    ...
    if x.shape[1] < 2 or not np.any(np.ptp(x, 0) == 0.0):
        raise ValueError(
            "White's specification test requires at least two" "columns where one is a constant."
        )
...
"""
    ...
    # crit = a * np.sqrt(nrr)
    rcusumci = (a * np.sqrt(nrr) + 2 * a * np.arange(0, nobs - skip) / np.sqrt(nrr)) * np.array(
        [[-1.0], [+1.0]]
    )
...
def breaks_hansen(olsresults):
    ...
    crit95 = np.array(
        [(2, 1.01), (6, 1.9), (15, 3.75), (19, 4.52)], dtype=[("nobs", int), ("crit", float)]
    )
