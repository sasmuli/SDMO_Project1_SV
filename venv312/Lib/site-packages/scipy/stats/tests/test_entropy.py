...
class TestEntropy:
    ...
    def test_entropy_base_2d_nondefault_axis(self, xp):
        pk = xp.asarray([[0.1, 0.2], [0.6, 0.3], [0.3, 0.5]])
        xp_assert_close(stats.entropy(pk, axis=1), xp.asarray([0.63651417, 0.63651417, 0.66156324]))
    ...
    def test_entropy_2d_nondefault_axis(self, xp):
        ...
        qk = xp.asarray([[0.2, 0.1], [0.3, 0.6], [0.5, 0.3]])
        xp_assert_close(
            stats.entropy(pk, qk, axis=1), xp.asarray([0.23104906, 0.23104906, 0.12770641])
        )
...
class TestDifferentialEntropy:
    ...
    @pytest.mark.skip_xp_backends("jax.numpy", reason="JAX doesn't support item assignment")
    ...
    def test_rmse_std(self, method, dist, xp):
        ...
        xp_assert_close(xp.sqrt(xp.mean((res - true_entropy) ** 2)), rmse_expected, atol=0.005)
