...
def register_sparse_backend(backend_name):
    ...
    if backend_name in _KNOWN_BACKENDS:
        module = importlib.import_module(f"tensorly.contrib.sparse.backend.{backend_name}_backend")
...
def dispatch_sparse(func):
    @functools.wraps(func, assigned=("__name__", "__qualname__", "__doc__", "__annotations__"))
