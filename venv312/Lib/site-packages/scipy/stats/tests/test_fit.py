...
def test_cont_fit(distname, arg, method):
    ...
    diffthreshold = np.max(
        np.vstack([truearg * thresh_percent, np.full(distfn.numargs + 2, thresh_min)]), 0
    )
...
def _check_loc_scale_mle_fit(name, data, desired, atol=None):
    ...
    actual = d.fit(data)[-2:]
    assert_allclose(actual, desired, atol=atol, err_msg=f"poor mle fit of (loc, scale) in {name}")
...
@pytest.mark.parametrize(
    "dist, params",
    [(stats.norm, (0.5, 2.5)), (stats.binom, (10, 0.3, 2))],  # type: ignore[attr-defined]
...
class TestFit:
    ...
    def test_argus(self, method):
        ...
        assert_nlff_less_or_close(dist, data, res.params, shapes, **self.tols, nlff_name=nlff_name)
    ...
    def test_beta(self):
        ...
        data = dist.rvs(*shapes, size=N, random_state=rng)
        shape_bounds = {"a": (0.1, 10), "b": (0.1, 10), "loc": (0.1, 10), "scale": (0.1, 10)}
        res = stats.fit(dist, data, shape_bounds, method="mle", optimizer=self.opt)
        assert_nlff_less_or_close(dist, data, res.params, shapes, nlff_name="nnlf", **self.tols)
...
class TestGoodnessOfFit:
    ...
    def test_against_ks(self):
        ...
        res = goodness_of_fit(stats.norm, x, known_params=known_params, statistic="ks", rng=rng)
    ...
    def test_against_cvm(self):
        ...
        x = examgrades
        known_params = {"loc": np.mean(x), "scale": np.std(x, ddof=1)}
        res = goodness_of_fit(stats.norm, x, known_params=known_params, statistic="cvm", rng=rng)
    ...
    def test_against_anderson_case_0(self):
        ...
        res = goodness_of_fit(stats.norm, x, known_params=known_params, statistic="ad", rng=rng)
    ...
    def test_custom_statistic(self):
        ...
        result = goodness_of_fit(
            stats.expon, data, known_params={"loc": 0, "scale": 1}, statistic=greenwood, rng=rng
        )
