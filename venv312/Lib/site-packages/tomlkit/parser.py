...
class Parser:
    ...
    def _parse_bare_key(self) -> Key:
        ...
        Parses a bare key.
        """
        while (self._current.is_bare_key_char() or self._current.is_spaces()) and self.inc():
            pass
    ...
    def _parse_string(self, delim: StringType) -> String:
        ...
        while True:
            ...
            if (
                ...
                and (
                    code == CHR_DEL
                    or (code <= CTRL_CHAR_LIMIT and code not in [CTRL_I, CTRL_J, CTRL_M])
                )
            ):
    ...
    def _parse_table(
        ...
        if any(" " in part.key.strip() and part.is_bare() for part in name_parts):
            raise self.parse_error(ParseError, f'Invalid table name "{full_key.as_string()}"')
            ...
            for i, _name in enumerate(name_parts[1:]):
                child = table.get(
                    ...
                    Table(
                        ...
                        name=_name.key,
                        display_name=(full_key.as_string() if i == len(name_parts) - 2 else None),
                    ),
                )
