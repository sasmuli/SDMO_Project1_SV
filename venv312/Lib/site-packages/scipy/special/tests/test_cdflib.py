...
class TestCDFlib:
    ...
    def test_bdtrik(self):
        _assert_inverts(
            sp.bdtrik, _binomial_cdf, 0, [ProbArg(), IntArg(1, 1000), ProbArg()], rtol=1e-4
        )
    ...
    def test_tklmbda_zero_shape(self):
        ...
        one = mpmath.mpf(1)
        assert_mpmath_equal(
            lambda x: sp.tklmbda(x, 0), lambda x: one / (mpmath.exp(-x) + one), [Arg()], rtol=1e-7
        )
...
def test_bdtrik_nbdtrik_inf():
    y = np.array([np.nan, -np.inf, -10.0, -1.0, 0.0, 0.00001, 0.5, 0.9999, 1.0, 10.0, np.inf])
    y = y[:, None]
