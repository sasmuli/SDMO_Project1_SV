...
def _format_str_once(
    ...
    if Preview.normalize_cr_newlines in mode:
        normalized_contents, _, newline_type = decode_bytes(src_contents.encode("utf-8"), mode)

        src_node = lib2to3_parse(normalized_contents.lstrip(), target_versions=mode.target_versions)
    else:
    ...
    if not dst_contents:
        if Preview.normalize_cr_newlines in mode:
            ...
            normalized_content, _, newline = decode_bytes(src_contents.encode("utf-8"), mode)
...
def get_features_used(  # noqa: C901
    ...
    for n in node.pre_order():
        if n.type == token.FSTRING_START:
            ...
            and len(n.children) >= 2
            and (n.children[1].type == token.STAR or n.children[1].type == syms.testlist)
        ):
