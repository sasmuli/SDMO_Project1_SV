...
def test_string_dtype_all_na(string_dtype_no_object, reduction_func, min_count, test_series):
...
def test_string_dtype_empty_sum(string_dtype_no_object, min_count):
    ...
    expected = DataFrame({"b": value}, index=pd.Index(["x"], name="a", dtype=dtype), dtype=dtype)
    tm.assert_frame_equal(result, expected)
