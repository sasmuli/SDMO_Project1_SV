...
from scipy._lib._util import check_random_state, _rename_parameter, rng_integers, _transition_to_rng
...
def _bootstrap_iv(
    ...
    message = (
        "Ignoring the dimension specified by `axis`, arrays in `data` do not "
    ...
    for sample in data:
        if sample.shape[axis_int] <= 1:
            raise ValueError(
                "each sample in `data` must contain two or more " "observations along `axis`."
            )
    ...
    if paired:
        ...
        for sample in data_iv[1:]:
            if sample.shape[-1] != n:
                message = (
                    "When `paired is True`, all samples must have the " "same length along `axis`"
                )
...
def _monte_carlo_test_iv(data, rvs, statistic, vectorized, n_resamples, batch, alternative, axis):
    ...
    if vectorized is None:
        try:
            ...
            message = (
                f"Signature inspection of {statistic=} failed; " "pass `vectorize` explicitly."
            )
            raise ValueError(message) from e
...
def monte_carlo_test(
    ...
    *,
    ...
    (data, rvs, statistic, vectorized, n_resamples, batch, alternative, axis, dtype, xp) = args
...
def _power_iv(rvs, test, n_observations, significance, vectorized, n_resamples, batch, kwargs):
    """Input validation for `monte_carlo_test`."""
...
def power(
    ...
    *alternative* hypothesis and calculate the power.
    ...
    """
    tmp = _power_iv(rvs, test, n_observations, significance, vectorized, n_resamples, batch, kwargs)
    (rvs, test, nobs, significance, vectorized, n_resamples, batch, args, kwds, shape) = tmp
...
def _permutation_test_iv(
    data, statistic, permutation_type, vectorized, n_resamples, batch, alternative, axis, rng
):
...
def permutation_test(
    ...
    *difference* between two paired elements. Accordingly, if ``data`` contains
    ...
    # relative tolerance for detecting numerically distinct but
    # theoretically equal values in the null distribution
    eps = 0 if not np.issubdtype(observed.dtype, np.inexact) else np.finfo(observed.dtype).eps * 100
...
class BootstrapMethod(ResamplingMethod):
    ...
    def __init__(self, n_resamples=9999, batch=None, random_state=None, method="BCa", *, rng=None):
        # Uncomment in SciPy 1.17.0
        # warnings.warn(_rs_deprecation.replace('attribute', 'argument'),
        #               DeprecationWarning, stacklevel=2)
