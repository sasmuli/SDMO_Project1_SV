...
def test_predicted_filtered_smoothed_with_nans_TVSS(reset_randomstate):
    ...
    kwargs = {
        key: mod_oos[key]
        for key in ["obs_intercept", "design", "obs_cov", "transition", "selection", "state_cov"]
    }
...
def test_predicted_filtered_smoothed_TVSS(reset_randomstate):
    ...
    for t in range(mod.nobs):
        assert_allclose(
            s_pred.var_pred_mean[t], Z[t] @ res.smoothed_state_cov[..., t] @ Z[t].T + H[t]
        )
        assert_allclose(
            f_pred.var_pred_mean[t], Z[t] @ res.filtered_state_cov[..., t] @ Z[t].T + H[t]
        )
        assert_allclose(
            p_pred.var_pred_mean[t], Z[t] @ res.predicted_state_cov[..., t] @ Z[t].T + H[t]
        )
...
def test_predicted_filtered_dynamic_varmax(use_exog, trend):
    ...
    p1 = res1.get_prediction(
        start=0, dynamic=20, end=60, exog=x_fcast1, signal_only=True, information_set="filtered"
    )
    ...
    p1 = res1.get_prediction(
        start=2, dynamic=18, end=60, exog=x_fcast1, signal_only=True, information_set="filtered"
    )
    p2 = res2.get_prediction(
    ...
    p1 = res1.get_prediction(
        start=20, dynamic=True, end=60, exog=x_fcast1, signal_only=True, information_set="filtered"
    )
    p2 = res2.get_prediction(
