...
@overload
def item(value: datetime, _parent: Item | None = ..., _sort_keys: bool = ...) -> DateTime: ...
...
@overload
def item(value: Sequence[dict], _parent: Item | None = ..., _sort_keys: bool = ...) -> AoT: ...
...
def item(value: Any, _parent: Item | None = None, _sort_keys: bool = False) -> Item:
    ...
    if isinstance(value, bool):
        ...
        table_constructor = InlineTable if isinstance(_parent, (Array, InlineTable)) else Table
...
class Array(Item, _CustomList):
    ...
    def __init__(self, value: list[Item], trivia: Trivia, multiline: bool = False) -> None:
        super().__init__(trivia)
        list.__init__(
            self,
    ...
    def add_line(
        ...
        *items: Any,
        ...
        if comment:
            ...
            new_values.append(Comment(Trivia(indent=indent, comment=f"# {comment}", trail="")))
    ...
    def insert(self, pos: int, value: Any) -> None:
        ...
        if pos < length:
            ...
            if idx >= 1 and self._value[idx - 1].is_whitespace():
                ...
                idx -= 1
                if self._value[idx].indent is not None and "\n" in self._value[idx].indent.s:
                    default_indent = "\n    "
    ...
    def __delitem__(self, key: int | slice):
        ...
        if isinstance(key, slice):
            indices_to_remove = list(range(key.start or 0, key.stop or length, key.step or 1))
        else:
        ...
        for i in sorted(indices_to_remove, reverse=True):
            try:
                ...
                comma_in_indent = group_rm.indent is not None and "," in group_rm.indent.s
                ...
                if group_rm.indent is not None and "\n" in group_rm.indent.s:
                    ...
                    group = self._value[idx] if len(self._value) > idx else None
                    if group is not None and (group.indent is None or "\n" not in group.indent.s):
...
class Table(AbstractTable):
    ...
    def is_super_table(self) -> bool:
        ...
        for k, child in self.items():
            ...
            if not isinstance(child, (Table, AoT)) or real_key is None or real_key.is_dotted():
                return False
        return True
