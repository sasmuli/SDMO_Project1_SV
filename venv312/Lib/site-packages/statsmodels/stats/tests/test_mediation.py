...
def test_framing_example():
    ...
    outcome_exog = patsy.dmatrix(
        "emo + treat + age + educ + gender + income", data, return_type="dataframe"
    )
    outcome_model = sm.GLM(
        outcome, outcome_exog, family=sm.families.Binomial(link=sm.families.links.Probit())
    )

    mediator = np.asarray(data["emo"])
    mediator_exog = patsy.dmatrix(
        "treat + age + educ + gender + income", data, return_type="dataframe"
    )
    ...
    med = Mediation(
        outcome_model, mediator_model, tx_pos, med_pos, outcome_fit_kwargs={"atol": 1e-11}
    )
...
def test_framing_example_formula():
    ...
    med = Mediation(
        outcome_model, mediator_model, "treat", "emo", outcome_fit_kwargs={"atol": 1e-11}
    )
...
def test_surv():
    ...
    for mt in "full", "partial", "no":
        ...
        df = pd.DataFrame(
            {"ytime": ytime, "ystatus": ystatus, "mtime": mtime, "mstatus": mstatus, "exp": exp}
        )
