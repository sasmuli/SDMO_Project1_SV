...
class GoogleDocstring(UnicodeMixin):
    ...
    def _consume_contiguous(self):
        ...
        lines = []
        while (
            self._line_iter.has_next() and self._line_iter.peek() and not self._is_section_header()
        ):
            lines.append(next(self._line_iter))  # type: ignore
        return lines
...
class NumpyDocstring(GoogleDocstring):
    ...
    def __init__(self, docstring, config=None, app=None, what="", name="", obj=None, options=None):
        ...
        super(NumpyDocstring, self).__init__(docstring, config, app, what, name, obj, options)
    ...
    def _is_section_break(self):
        ...
        return (
            ...
            or ["", ""] == [line1, line2]
            or (
                self._is_in_section and line1 and not self._is_indented(line1, self._section_indent)
            )
        )
    ...
    _name_rgx = re.compile(
        r"^\s*(:(?P<role>\w+):`(?P<name>[a-zA-Z0-9_.-]+)`|" r" (?P<name2>[a-zA-Z0-9_.-]+))\s*", re.X
    )
