...
def test_erf_complex():
    ...
    try:
        ...
        assert_func_equal(
            sc.erf, lambda x: complex(mpmath.erf(x)), points, vectorized=False, rtol=1e-13
        )
        assert_func_equal(
            sc.erfc, lambda x: complex(mpmath.erfc(x)), points, vectorized=False, rtol=1e-13
        )
    finally:
...
def test_gammainc_boundary():
    ...
    with mpmath.workdps(100):
        dataset = [
            (a0, x0, float(mpmath.gammainc(a0, b=x0, regularized=True))) for a0, x0 in zip(a, x)
        ]
    dataset = np.array(dataset)
...
class TestSystematic:
    ...
    def test_airyai_prime(self):
        ...
        assert_mpmath_equal(
            lambda z: sc.airy(z)[1], lambda z: mpmath.airyai(z, derivative=1), [Arg(-1e3, 1e3)]
        )

    def test_airyai_prime_complex(self):
        assert_mpmath_equal(
            lambda z: sc.airy(z)[1], lambda z: mpmath.airyai(z, derivative=1), [ComplexArg()]
        )

    def test_airybi(self):
        # oscillating function, limit range
        assert_mpmath_equal(
            lambda z: sc.airy(z)[2], lambda z: mpmath.airybi(z), [Arg(-1e8, 1e8)], rtol=1e-5
        )
    ...
    def test_airybi_prime(self):
        ...
        assert_mpmath_equal(
            lambda z: sc.airy(z)[3], lambda z: mpmath.airybi(z, derivative=1), [Arg(-1e3, 1e3)]
        )

    def test_airybi_prime_complex(self):
        assert_mpmath_equal(
            lambda z: sc.airy(z)[3], lambda z: mpmath.airybi(z, derivative=1), [ComplexArg()]
        )

    def test_bei(self):
        assert_mpmath_equal(
            sc.bei, exception_to_nan(lambda z: mpmath.bei(0, z, **HYPERKW)), [Arg(-1e3, 1e3)]
        )

    def test_ber(self):
        assert_mpmath_equal(
            sc.ber, exception_to_nan(lambda z: mpmath.ber(0, z, **HYPERKW)), [Arg(-1e3, 1e3)]
        )
    ...
    def test_betainc(self):
        assert_mpmath_equal(
            sc.betainc,
            time_limited()(
                exception_to_nan(lambda a, b, x: mpmath.betainc(a, b, 0, x, regularized=True))
            ),
            [Arg(), Arg(), Arg()],
        )
    ...
    def test_betaincc(self):
        assert_mpmath_equal(
            sc.betaincc,
            time_limited()(
                exception_to_nan(lambda a, b, x: mpmath.betainc(a, b, x, 1, regularized=True))
            ),
            [Arg(), Arg(), Arg()],
            dps=400,
        )
    ...
    @pytest.mark.xfail(
        run=False,
        reason="it's not the same as the mpmath function --- " "maybe different definition?",
    )
    def test_pcfv(self):
        def pcfv(v, x):
    ...
    @pytest.mark.xfail(
        run=False, reason="issues at large arguments (atol OK, rtol not) " "and <eps-close to z=0"
    )
    ...
    @pytest.mark.xfail(
        reason=(
            "see gh-3551 for bad points on 32 bit " "systems and gh-8095 for another bad " "point"
        )
    )
    def test_rf(self):
