...
def test_multivariate_gam_cv():
    ...
    gam_cv = MultivariateGAMCV(
        smoother=bsplines, alphas=alphas, gam=GLMGam, cost=cost, endog=y, exog=None, cv_iterator=cv
    )
...
def test_glm_pirls_compatibility():
    ...
    gam_res_glm = gam_glm.fit(
        start_params=gam_res_glm.params, method="bfgs", max_start_irls=0, disp=1, maxiter=20000
    )
...
def test_cov_params():
    ...
    assert_allclose(res_glm.cov_params(), res_glm_gam.cov_params(), rtol=1e-4, atol=1e-8)
