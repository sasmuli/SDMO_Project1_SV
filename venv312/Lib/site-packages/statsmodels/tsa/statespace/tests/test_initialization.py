...
def test_global_approximate_diffuse():
    ...
    init = Initialization(mod.k_states, "approximate_diffuse", approximate_diffuse_variance=1e10)
...
def test_global_stationary():
    ...
    check_initialization(
        mod, init, [intercept / (1 - phi)], np.diag([0]), np.eye(1) * sigma2 / (1 - phi**2)
    )
...
def test_mixed_stationary():
    ...
    desired_cov = np.diag(
        [cov[0, 0] / (1 - transition[0, 0] ** 2), cov[1, 1] / (1 - transition[1, 1] ** 2)]
    )
    check_initialization(mod, init, desired_intercept, np.diag([0, 0]), desired_cov)
...
def test_nested():
    ...
    init1_1_2 = Initialization(1, "approximate_diffuse", approximate_diffuse_variance=1e9)
    ...
    desired_cov[3, 3] = 2.0
    desired_cov[5, 5] = 1e6
    check_initialization(mod, init, [0, 0, 0, 1, 0, 0], np.diag([0, 0, 0, 0, 1, 0]), desired_cov)
...
def test_invalid():
    ...
    assert_raises(ValueError, Initialization, 5, "stationary", stationary_cov=[1, 2])
