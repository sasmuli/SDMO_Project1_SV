...
def plot_var_forc(
    prior, forc, err_upper, err_lower, index=None, names=None, plot_stderr=True, legend_options=None
):
    import matplotlib.pyplot as plt
    ...
    for j in range(k):
        ...
        p2 = ax.plot(rng_f, np.r_[prior[-1:, j], forc[:, j]], "k--", label="Forecast")

        if plot_stderr:
            p3 = ax.plot(rng_err, err_upper[:, j], "k-.", label="Forc 2 STD err")
...
def plot_with_error(
    y, error, x=None, axes=None, value_fmt="k", error_fmt="k--", alpha=0.05, stderr_type="asym"
):
    """
