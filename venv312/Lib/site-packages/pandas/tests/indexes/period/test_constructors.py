...
class TestPeriodIndex:
    ...
    def test_constructor_pi_nat(self):
        ...
        idx = PeriodIndex(
            np.array([NaT, NaT, Period("2011-01", freq="M"), Period("2011-01", freq="M")])
        )
        tm.assert_index_equal(idx, exp)
    ...
    def test_constructor_freq_mult_dti_compat(self, mult, freq_offset, freq_period):
        ...
        expected = date_range(start="2014-04-01", freq=freqstr_offset, periods=10).to_period(
            freqstr_period
        )
        tm.assert_index_equal(pidx, expected)
    ...
    def test_constructor_freq_mult_dti_compat_month(self, mult):
        ...
        expected = date_range(start="2014-04-01", freq=f"{mult}ME", periods=10).to_period(
            f"{mult}M"
        )
        tm.assert_index_equal(pidx, expected)
