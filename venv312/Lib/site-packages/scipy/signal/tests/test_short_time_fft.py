...
def test_exceptions_properties_methods():
    ...
    with pytest.raises(
        ValueError, match="Attribute mfft=7 needs to be " + "at least the window length.*"
    ):
        SFT.mfft = 7
    ...
    with pytest.raises(ValueError, match=r".*\(k1=100\) <= \(k_max=12\) " + "is false!$"):
...
@pytest.mark.parametrize(
    "scale_to, fac_psd, fac_mag", [(None, 0.25, 0.125), ("magnitude", 2.0, 1), ("psd", 1, 0.5)]
)
...
def test_x_slices_padding():
    ...
    d = {
        "zeros": [[[0, 0, 1, 2, 3], [0, 0, 0, 1, 4]], [[3, 4, 5, 0, 0], [4, 9, 16, 0, 0]]],
        "edge": [[[1, 1, 1, 2, 3], [0, 0, 0, 1, 4]], [[3, 4, 5, 5, 5], [4, 9, 16, 16, 16]]],
        "even": [[[3, 2, 1, 2, 3], [4, 1, 0, 1, 4]], [[3, 4, 5, 4, 3], [4, 9, 16, 9, 4]]],
        "odd": [[[-1, 0, 1, 2, 3], [-4, -1, 0, 1, 4]], [[3, 4, 5, 6, 7], [4, 9, 16, 23, 28]]],
    }
...
def test_fft_func_roundtrip(n: int):
    ...
    for f_typ, mfft, scaling, phase_shift in product(*pp.values()):
        ...
        SFT = ShortTimeFFT(
            w, h_n, fs=n, fft_mode=f_typ, mfft=mfft, scale_to=scaling, phase_shift=phase_shift
        )
        X0 = SFT._fft_func(x0)
...
def test_asymmetric_window_roundtrip(hop: int):
    ...
    xp_assert_close(x1, x1, err_msg="Roundtrip for asymmetric window with " + f" {hop=} failed!")
...
def test_minimal_length_signal(m_num):
    ...
    xp_assert_close(
        x1,
        x,
        err_msg=f"Roundtrip minimal length signal ({n=})" + f" for {m_num} sample window failed!",
    )
    with pytest.raises(ValueError, match=rf"len\(x\)={n-1} must be >= ceil.*"):
        SFT.stft(x[:-1])
    with pytest.raises(
        ValueError,
        match=rf"S.shape\[t_axis\]={Sx.shape[1]-1}" f" needs to have at least {Sx.shape[1]} slices",
    ):
        SFT.istft(Sx[:, :-1], k1=n)
...
def test_tutorial_stft_legacy_stft():
    ...
    SFT = ShortTimeFFT.from_window(
        win, fs, nperseg, noverlap, fft_mode="centered", scale_to="magnitude", phase_shift=None
    )
    Sz1 = SFT.stft(z)
...
def test_roundtrip_multidimensional(fft_mode: FFT_MODE_TYPE):
    ...
    SFT = ShortTimeFFT(get_window("hann", 4), hop=2, fs=1, scale_to="magnitude", fft_mode=fft_mode)
...
def test_roundtrip_two_dimensional(phase_shift: int | None):
    ...
    y = SFT.istft(Sx, k1=n)
    xp_assert_close(
        y, x, atol=2 * np.finfo(SFT.win.dtype).resolution, err_msg="2-dim. roundtrip failed!"
    )
...
def test_roundtrip_windows(window, n: int, nperseg: int, noverlap: int):
    ...
    x32_1_r = x32_1.real
    xp_assert_close(
        x32, x32_1_r.astype(np.float32), err_msg="Roundtrip for 32 Bit float values failed"
    )
    xp_assert_close(
        x32.imag, np.zeros_like(x32.imag), err_msg="Roundtrip for 32 Bit float values failed"
    )
...
def test_roundtrip_complex_window(signal_type):
    ...
    z1 = SFT.istft(Sz, k1=len(z))
    xp_assert_close(
        z.astype(np.complex128), z1, err_msg="Roundtrip for complex-valued window failed"
    )
...
def test_average_all_segments():
    ...
    SFT = ShortTimeFFT.from_window(
        window, fs, nperseg, noverlap, fft_mode="onesided2X", scale_to="psd", phase_shift=None
    )
    # `welch` positions the window differently than the STFT:
    P = SFT.spectrogram(
        x, detr="constant", p0=0, p1=(len(x) - noverlap) // SFT.hop, k_offset=nperseg // 2
    )
...
def test_stft_padding_roundtrip(window, N: int, nperseg: int, noverlap: int, mfft: int, padding):
    ...
    x1 = SFT.istft(Sx, k1=N)
    xp_assert_close(
        x1, x.astype(np.complex128), err_msg=f"Failed real roundtrip with '{padding}' padding"
    )

    Sz = SFT.stft(z, padding=padding)
    z1 = SFT.istft(Sz, k1=N)
    xp_assert_close(z1, z, err_msg="Failed complex roundtrip with " + f" '{padding}' padding")
