...
def _multi_dot_three(A, B, C):
    ...
    # cost1 = cost((AB)C)
    cost1 = (
        A.shape[0] * A.shape[1] * B.shape[1] + A.shape[0] * B.shape[1] * C.shape[1]  # (AB)
    )  # (--)C
    # cost2 = cost((AB)C)
    cost2 = (
        B.shape[0] * B.shape[1] * C.shape[1] + A.shape[0] * A.shape[1] * C.shape[1]  # (BC)
    )  # A(--)
...
class MixedLM(base.LikelihoodModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            exog_re is not None
            and data_tools._is_using_ndarray_type(exog_re, None)
            and exog_re.ndim == 1
        ):
        ...
        **kwargs,
        ...
        if "groups" not in kwargs.keys():
            raise AttributeError(
                "'groups' is a required keyword argument " + "in MixedLM.from_formula"
            )
        ...
        if vc_formula is not None:
            ...
            for vc_name in vcf:
                ...
                for group_ix, group in enumerate(kylist):
                    ii = gb.groups[group]
                    mat = patsy.dmatrix(
                        md, data.loc[ii, :], eval_env=eval_env, return_type="dataframe"
                    )
    ...
    def fit_regularized(
        self, start_params=None, method="l1", alpha=0, ceps=1e-4, ptol=1e-6, maxit=200, **fit_kwargs
    ):
        """
    ...
    def get_fe_params(self, cov_re, vcomp, tol=1e-10):
        ...
        if not hasattr(self, "_endex_li"):
            ...
            for group_ix, _ in enumerate(self.group_labels):
                mat = np.concatenate(
                    (self.exog_li[group_ix], self.endog_li[group_ix][:, None]), axis=1
                )
    ...
    def loglike(self, params, profile_fe=True):
        ...
        if type(params) is not MixedLMParams:
            params = MixedLMParams.from_packed(
                params, self.k_fe, self.k_re, self.use_sqrt, has_fe=False
            )
    ...
    def hessian(self, params):
        ...
        if type(params) is not MixedLMParams:
            params = MixedLMParams.from_packed(
                params, self.k_fe, self.k_re, use_sqrt=self.use_sqrt, has_fe=True
            )
        ...
        for group_ix, group in enumerate(self.group_labels):
            ...
            for jj1, matl1, matr1, vsl1, vsr1, sym1 in self._gen_dV_dPar(ex_r, solver, group_ix):
                ...
                for jj2, matl2, matr2, vsl2, vsr2, sym2 in self._gen_dV_dPar(
                    ex_r, solver, group_ix, jj1
                ):
        ...
        **fit_kwargs
        ...
        if do_cg:
            ...
            for j in range(len(method)):
                rslt = super().fit(
                    start_params=packed, skip_hessian=True, method=method[j], **fit_kwargs
                )
                ...
                if j + 1 < len(method):
                    ...
                    warnings.warn(
                        "Retrying MixedLM optimization with %s" % next_method, ConvergenceWarning
                    )
                else:
...
class MixedLMResults(base.LikelihoodModelResults, base.ResultMixin):
    ...
    def profile_re(
        self, re_ix, vtype, num_low=5, dist_low=1.0, num_high=5, dist_high=1.0, **fit_kwargs
    ):
        """
        ...
        **fit_kwargs
        ...
        if low <= 0:
            raise ValueError(
                "dist_low is too large and would result in a "
        ...
        for x in rvalues:
            ...
            rslt = model.fit(
                start_params=params, free=free, reml=self.reml, cov_pen=self.cov_pen, **fit_kwargs
            )._results
...
class MixedLMResultsWrapper(base.LikelihoodResultsWrapper):
    _attrs = {
        "bse_re": ("generic_columns", "exog_re_names_full"),
