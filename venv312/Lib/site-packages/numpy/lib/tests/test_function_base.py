...
class TestAverage:
    ...
    def test_weights(self):
        ...
        with pytest.raises(
            TypeError, match="Axis must be specified when shapes of a " "and weights differ"
        ):
...
class TestInsert:
    ...
    def test_basic(self):
        ...
        assert_equal(insert(a, np.array([True, False, True, False]), 9), [9, 1, 2, 9, 3])
    ...
    def test_structured_array(self):
        a = np.array([(1, "a"), (2, "b"), (3, "c")], dtype=[("foo", "i"), ("bar", "S1")])
        val = (4, "d")
        b = np.insert(a, 0, val)
    ...
    def test_basic(self, cumprod):
        ...
        for ctype in [
            ...
            if ctype in ["1", "b"]:
                ...
                assert_array_equal(
                    cumprod(a, axis=-1), np.array([1, 2, 20, 220, 1320, 6600, 26400], ctype)
                )
                assert_array_equal(
                    cumprod(a2, axis=0),
...
class TestLeaks:
    ...
    @pytest.mark.skipif(
        NOGIL_BUILD,
        reason=("Functions are immortalized if a thread is " "launched, making this test flaky"),
    )
    @pytest.mark.parametrize(
        "name, incr",
        [
...
class TestDigitize:
    ...
    @pytest.mark.xfail(reason="gh-11022: np._core.multiarray._monoticity loses precision")
...
class TestBincount:
    ...
    def test_empty_list(self, minlength):
        assert_array_equal(np.bincount([], minlength=minlength), np.zeros(minlength, dtype=int))
...
class TestPercentile:
    ...
    @pytest.mark.parametrize(["function", "quantile"], [(np.quantile, 0.4), (np.percentile, 40.0)])
    ...
    def test_linear_interpolation(
        self, function, quantile, method, weighted, expected, input_dtype, expected_dtype
    ):
...
class TestQuantile:
    ...
    def test_quantile_with_weights_and_axis(self, method):
        ...
        for i in range(2):
            for j in range(3):
                q_res[i, j] = np.quantile(y[i, :, j], alpha, method=method, weights=w[i, :, j])
        assert_allclose(q, q_res)
