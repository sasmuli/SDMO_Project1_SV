...
def test_bootstrap_batch(method, axis):
    ...
    res1 = bootstrap(
        (x,), np.mean, batch=None, method=method, random_state=0, axis=axis, n_resamples=100
    )
    np.random.seed(0)
    res2 = bootstrap((x,), np.mean, batch=10, method=method, axis=axis, n_resamples=100)
...
def test_bootstrap_vectorized(method, axis, paired):
    ...
    res1 = bootstrap(
        (x, y, z), my_statistic, paired=paired, method=method, rng=0, axis=0, n_resamples=100
    )
    ...
    res2 = bootstrap(
        (x, y, z), my_statistic, paired=paired, method=method, rng=0, axis=axis, n_resamples=100
    )
...
def test_bootstrap_against_theory(method):
    ...
    config = dict(data=(data,), statistic=np.mean, n_resamples=5000, method=method, rng=rng)
...
def test_multisample_BCa_against_R():
    ...
    res_percent = stats.bootstrap((x, y), statistic, method="percentile", batch=100, rng=rng)
...
def test_bootstrap_vectorized_3samp(method, axis):
    ...
    res1 = bootstrap(
        (x, y, z), statistic, vectorized=True, axis=axis, n_resamples=100, method=method, rng=0
    )
    res2 = bootstrap(
        (x, y, z), statistic_1d, vectorized=False, axis=axis, n_resamples=100, method=method, rng=0
    )
    assert_allclose(res1.confidence_interval, res2.confidence_interval)
    assert_allclose(res1.standard_error, res2.standard_error)
...
def test_bootstrap_gh15678(method):
    ...
    data = (data,)
    res = bootstrap(
        data, stats.skew, method=method, n_resamples=100, rng=np.random.default_rng(9563)
    )
...
def test_bootstrap_min():
    ...
    data = (data,)
    res = bootstrap(data, np.min, method="BCa", n_resamples=100, rng=np.random.default_rng(3942))
    assert true_min == res.confidence_interval.low
    res2 = bootstrap(
        -np.array(data), np.max, method="BCa", n_resamples=100, rng=np.random.default_rng(3942)
    )
...
def test_re_bootstrap(additional_resamples):
    ...
    res = stats.bootstrap(
        (x,), np.mean, n_resamples=n1, rng=rng, confidence_level=0.95, method="percentile"
    )
    res = stats.bootstrap(
        (x,),
        np.mean,
...
def test_vector_valued_statistic(method):
    ...
    counts = np.sum(
        (res.confidence_interval.low.T < params) & (res.confidence_interval.high.T > params), axis=0
    )
...
class TestMonteCarloHypothesisTest:
    ...
    def get_rvs(self, rvs_in, rs, dtype=None, xp=np):
        return lambda *args, **kwds: xp.asarray(rvs_in(*args, random_state=rs, **kwds), dtype=dtype)
    ...
    def test_axis_dtype(self, axis, xp):
        ...
        res = monte_carlo_test(x, rvs, statistic, vectorized=True, n_resamples=20000, axis=axis)
    ...
    def test_against_ks_1samp(self, alternative, a):
        ...
        res = monte_carlo_test(
            x, norm_rvs, statistic1d, n_resamples=1000, vectorized=False, alternative=alternative
        )
    ...
    def test_against_cramervonmises(self, a):
        ...
        norm_rvs = self.get_rvs(stats.norm.rvs, rng)
        res = monte_carlo_test(
            x, norm_rvs, statistic1d, n_resamples=1000, vectorized=False, alternative="greater"
        )
    ...
    def test_against_anderson(self, dist_name, i):
        ...
        with suppress_warnings() as sup:
            sup.filter(RuntimeWarning)
            res = monte_carlo_test(
                x, dist_rvs, statistic1d, n_resamples=1000, vectorized=False, alternative="greater"
            )
    ...
    def test_finite_precision_statistic(self):
        ...
        x = rvs(100)
        res = stats.monte_carlo_test(x, rvs, np.var, alternative="less", n_resamples=n_resamples)
...
class TestPower:
    ...
    def test_vectorization(self):
        ...
        def test(x, alternative, axis=-1):
            ...
            return stats.ttest_1samp(x, popmeans_expanded, alternative=alternative, axis=axis)
        ...
        res = stats.power(test, rng.normal, nx, significance=significance, kwargs=kwargs)
            ...
            for nx_i in nx:
                for alternative_i in kwargs["alternative"]:
                    for popmean_i in popmeans:
                        ...
                        tmp = stats.power(test2, rng.normal, nx_i, significance=significance_i)
    ...
    def test_ttest_1samp_power(self):
        ...
        res = stats.power(
            stats.ttest_1samp, rng.normal, n_observations, significance=significance, kwargs=kwargs
        )
...
class TestPermutationTest:
    ...
    def test_against_ansari(self, alternative):
        ...
        res = permutation_test(
            (x, y), statistic1d, n_resamples=np.inf, alternative=alternative, rng=self.rng
        )
    ...
    def test_against_kendalltau(self):
        ...
        res = permutation_test(
            (x,), statistic1d, permutation_type="pairings", n_resamples=np.inf, rng=self.rng
        )
...
def test_parameter_vectorized(fun_name):
    ...
    fun_options = {
        ...
        "permutation_test": {"data": (sample,), "rng": rng, "permutation_type": "samples"},
