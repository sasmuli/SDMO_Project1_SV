        ...
        for i in range(row):
            if i == 0:
                assisted_tensor = tl.index_update(
                    assisted_tensor,
                    tl.index[i, i:],
                    cum_sum[i:, j] / tl.tensor(tl.arange(row - i) + 1, **tl.context(tensor)),
                )
            else:
                assisted_tensor = tl.index_update(
                    assisted_tensor,
...
def unimodality_prox(tensor):
    ...
    sum_dec = tl.where(
        tl.flip(values, axis=0) == 1,
        sum_dec - tl.abs(tl.flip(tensor, axis=0) - tl.flip(monotone_decreasing, axis=0)),
        tl.tensor(0, **tl.context(tensor)),
    )
        ...
        for k in range(rank):
            if UtU[k, k]:
                if sparsity_coefficient is not None:  # Modifying the function for sparsification
                    deltaV = tl.where(
                        (UtM[k, :] - tl.dot(UtU[k, :], V) - sparsity_coefficient) / UtU[k, k]
                        > -V[k, :],
                        (UtM[k, :] - tl.dot(UtU[k, :], V) - sparsity_coefficient) / UtU[k, k],
                        -V[k, :],
                    )
...
def fista(
    ...
    for iteration in range(n_iter_max):
        if isinstance(UtU, list):
            x_gradient = (
                -UtM + tl.tenalg.multi_mode_dot(x_update, UtU, transpose=False) + sparsity_coef
            )
        else:
...
def admm(
    ...
    for iteration in range(n_iter_max):
        ...
        if tl.norm(dual_residual) < tol * tl.norm(x) and tl.norm(primal_residual) < tol * tl.norm(
            dual_var
        ):
            break
    return x, x_split, dual_var
