...
class UpdateProgress(RemoteProgress):
    ...
    CLONE, FETCH, UPDWKTREE = [
        1 << x for x in range(RemoteProgress._num_op_codes, RemoteProgress._num_op_codes + 3)
    ]
...
class Submodule(IndexObject, TraversableIterableObj):
    ...
    def _to_relative_path(cls, parent_repo: "Repo", path: PathLike) -> PathLike:
        ...
        if osp.isabs(path) and parent_repo.working_tree_dir:
            ...
            if not path:
                raise ValueError(
                    "Absolute submodule path '%s' didn't yield a valid relative path" % path
                )
    ...
    def update(
        ...
        try:
            ...
            if mrepo is not None and to_latest_revision:
                msg_base = (
                    "Cannot update to latest revision in repository at %r as " % mrepo.working_dir
                )
                if not is_detached:
            ...
            if mrepo is not None and mrepo.head.commit.binsha != binsha:
                ...
                if mrepo.head.commit.binsha != self.NULL_BIN_SHA:
                    ...
                    if (
                        may_reset
                        and not force
                        and mrepo.is_dirty(index=True, working_tree=True, untracked_files=True)
                    ):
    ...
    def set_parent_commit(
        ...
        if self.k_modules_file not in pctree:
            raise ValueError(
                "Tree of commit %s did not contain the %s file" % (commit, self.k_modules_file)
            )
        # END handle exceptions
        ...
        if check:
            ...
            if not parser.has_section(sm_section(self.name)):
                ...
                raise ValueError(
                    "Submodule at path %r did not exist in parent commit %s" % (self.path, commit)
                )
