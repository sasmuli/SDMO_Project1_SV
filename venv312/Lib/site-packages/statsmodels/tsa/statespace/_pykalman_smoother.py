...
SMOOTHER_ALL = SMOOTHER_STATE | SMOOTHER_STATE_COV | SMOOTHER_DISTURBANCE | SMOOTHER_DISTURBANCE_COV
...
class _KalmanSmoother:
    ...
    def __next__(self):
        ...
        if missing_partial_obs:
            ...
            obs_cov = np.array(_kfilter.selected_obs_cov[: k_endog**2], copy=True).reshape(
                k_endog, k_endog
            )
        ...
        if smoother_output & (SMOOTHER_STATE_COV | SMOOTHER_DISTURBANCE_COV):
            if missing_entire_obs:
                ...
                scaled_smoothed_estimator_cov[:, :, t - 1] = design.transpose().dot(F_inv).dot(
                    design
                ) + L.transpose().dot(scaled_smoothed_estimator_cov[:, :, t]).dot(L)
