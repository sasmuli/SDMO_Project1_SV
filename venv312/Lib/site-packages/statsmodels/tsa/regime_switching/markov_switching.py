...
class MarkovSwitching(tsbase.TimeSeriesModel):
    ...
    def _regime_transition_matrix_tvtp(self, params, exog_tvtp=None):
        ...
        regime_transition_matrix = np.zeros(
            (self.k_regimes, self.k_regimes, nobs), dtype=np.promote_types(np.float64, params.dtype)
        )
    ...
    def regime_transition_matrix(self, params, exog_tvtp=None):
        ...
        if not self.tvtp:
            ...
            regime_transition_matrix[:-1, :, 0] = np.reshape(
                params[self.parameters["regime_transition"]], (self.k_regimes - 1, self.k_regimes)
            )
    ...
    def _filter(self, params, regime_transition=None):
        ...
        return (
            ...
            initial_probabilities, regime_transition, conditional_loglikelihoods, self.order
        )
    ...
    def filter(
        ...
        return self._wrap_results(
            params, result, return_raw, cov_type, cov_kwds, results_class, results_wrapper_class
        )
    ...
    def _smooth(
        ...
        return cy_kim_smoother_log(
            regime_transition, predicted_joint_probabilities_log, filtered_joint_probabilities_log
        )
    ...
    def _em_regime_transition(self, result):
        ...
        for i in range(self.k_regimes):  # S_{t_1}
            for j in range(self.k_regimes - 1):  # S_t
                regime_transition[i, j] = np.sum(smoothed_joint_probabilities[j, i]) / np.sum(
    ...
    def _untransform_logistic(self, unconstrained, constrained):
        ...
        for i in range(len(unconstrained)):
            resid[i] = (
                unconstrained[i] - np.log(1 + sum_exp - exp[i]) + np.log(1 / constrained[i] - 1)
            )
        return resid
...
class MarkovSwitchingResults(tsbase.TimeSeriesModelResults):
    ...
    def predict(self, start=None, end=None, probabilities=None, conditional=False):
        ...
        return self.model.predict(
            self.params, start=start, end=end, probabilities=probabilities, conditional=conditional
        )
...
wrap.populate_wrapper(MarkovSwitchingResultsWrapper, MarkovSwitchingResults)  # noqa:E305
