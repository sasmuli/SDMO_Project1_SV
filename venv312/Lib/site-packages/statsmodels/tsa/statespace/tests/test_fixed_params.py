...
def test_results_append():
    ...
    res2_fit = res1.append(
        endog2, refit=True, fit_kwargs={"disp": False, "start_params": res_full_fit.params}
    )
...
def test_results_apply():
    ...
    # Test to make sure refit works with fixed parameters
    res2_fit = res.apply(
        endog, refit=True, fit_kwargs={"disp": False, "start_params": res_fit.params}
    )
...
def test_dynamic_factor_validate():
    ...
    mod4 = dynamic_factor.DynamicFactor(
        endog, k_factors=1, factor_order=2, error_cov_type="diagonal", enforce_stationarity=False
    )
    ...
    mod5 = dynamic_factor.DynamicFactor(
        endog, k_factors=2, factor_order=1, error_cov_type="diagonal", enforce_stationarity=False
    )
    ...
    mod6 = dynamic_factor.DynamicFactor(
        endog[["cpi", "realgdp"]], k_factors=1, factor_order=1, error_cov_type="unstructured"
    )
    constraints = {
        "loading.f1.cpi": 1.0,
        "loading.f1.realgdp": 1.0,
        "cov.chol[1,1]": 0.5,
        "cov.chol[2,1]": 0.1,
    }
...
def test_varmax_validate():
    ...
    constraints = {
        "intercept.cpi": 0.5,
        "intercept.realgdp": 1.1,
        "beta.realinv.cpi": 0.2,
        "beta.realinv.realgdp": 0.1,
        "sqrt.var.cpi": 1.2,
        "sqrt.cov.cpi.realgdp": -0.1,
        "sqrt.var.realgdp": 2.3,
        "measurement_variance.cpi": 0.4,
...
def check_results(res1, res2, check_lutkepohl=False, check_params=True):
    ...
    if check_lutkepohl:
        assert_allclose(
            res2.info_criteria("aic", "lutkepohl"), res1.info_criteria("aic", "lutkepohl")
        )
        assert_allclose(
            res2.info_criteria("bic", "lutkepohl"), res1.info_criteria("bic", "lutkepohl")
        )
        assert_allclose(
            res2.info_criteria("hqic", "lutkepohl"), res1.info_criteria("hqic", "lutkepohl")
        )
    ...
    if check_params:
        ...
        assert_allclose(
            res2.cov_params_robust_approx.values[mask_free], res1.cov_params_robust_approx
        )
    ...
    assert_allclose(
        res2.test_heteroskedasticity("breakvar"), res1.test_heteroskedasticity("breakvar")
    )
...
def test_sarimax_nonconsecutive():
    ...
    res2 = mod2.fit_constrained(
        {"ar.L2": 0, "ar.L3": 0}, res1.params, includes_fixed=False, disp=False
    )
...
def test_structural():
    ...
    res1 = mod1.fit(start_params, disp=False)
    res2 = mod2.fit_constrained(
        constraints, start_params=res1.params, includes_fixed=False, disp=False
    )
