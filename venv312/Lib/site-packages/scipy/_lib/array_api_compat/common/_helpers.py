...
def is_numpy_array(x):
    ...
    return isinstance(x, (np.ndarray, np.generic)) and not _is_jax_zero_gradient_array(x)
...
def _check_api_version(api_version):
    if api_version in ["2021.12", "2022.12"]:
        warnings.warn(
            f"The {api_version} version of the array API specification was requested but the returned namespace is actually version 2023.12"
        )
    elif api_version is not None and api_version not in ["2021.12", "2022.12", "2023.12"]:
        raise ValueError(
            "Only the 2023.12 version of the array API specification is currently supported"
        )
