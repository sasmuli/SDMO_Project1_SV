...
def linkify(state: StateCore) -> None:
    ...
    for inline_token in state.tokens:
        if inline_token.type != "inline" or not state.md.linkify.pretest(inline_token.content):
            continue
        ...
        while i >= 1:
            ...
            if currentToken.type == "text" and state.md.linkify.test(currentToken.content):
                ...
                if links and links[0].index == 0 and i > 0 and tokens[i - 1].type == "text_special":
                    links = links[1:]
                ...
                for link in links:
                    ...
                    if not link.schema:
                        urlText = HTTP_RE.sub("", state.md.normalizeLinkText("http://" + urlText))
                    elif link.schema == "mailto:" and TEST_MAILTO_RE.search(urlText):
                        urlText = MAILTO_RE.sub("", state.md.normalizeLinkText("mailto:" + urlText))
                    else:
