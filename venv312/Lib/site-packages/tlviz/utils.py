...
def _alias_mode_axis():
    def decorator(func):
        ...
        if "axis" not in func_sig.parameters or "mode" not in func_sig.parameters:
            raise TypeError(
                f"Function {func} needs both ``mode`` and ``axis`` as possible arguments."
            )
...
def tucker_to_tensor(tucker_tensor):
    ...
    if len(tucker_tensor[1]) > 16:
        raise ValueError(
            "NumPy's einsum function doesn't support forming dense Tucker arrays with more than 16 modes."
        )
