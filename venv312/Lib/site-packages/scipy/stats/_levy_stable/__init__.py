...
def _rvs_Z1(alpha, beta, size=None, random_state=None):
    ...
    def otherwise(alpha, beta, TH, aTH, bTH, cosTH, tanTH, W):
        ...
        res3 = val3 * (
            (np.cos(aTH) + np.sin(aTH) * tanTH - val0 * (np.sin(aTH) - np.cos(aTH) * tanTH)) / W
        ) ** (1.0 / alpha)
        return res3
        ...
        if self._parameterization() == "S0":
            ...
            if np.all(np.reshape(alpha, (1, -1))[0, :] != 1):
                ...
                for pair in uniq_param_pairs:
                    ...
                    data_out[data_mask] = (
                        super().pdf(_x, _alpha, _beta, loc=_delta, scale=gamma).reshape(len(_x), 1)
                    )
                output = data_out.T[0]
                if output.shape == (1,):
                    return output[0]
        ...
        for pair in uniq_param_pairs:
            ...
            if pdf_single_value_method is not None:
                ...
                if fft_grid_spacing is None and fft_n_points_two_power is None:
                    raise ValueError(
                        "One of fft_grid_spacing or fft_n_points_two_power " + "needs to be set."
                    )
        ...
        if self._parameterization() == "S0":
            ...
            if np.all(np.reshape(alpha, (1, -1))[0, :] != 1):
                ...
                for pair in uniq_param_pairs:
                    ...
                    _x = data_in[data_mask, 0]
                    data_out[data_mask] = (
                        super().cdf(_x, _alpha, _beta, loc=_delta, scale=gamma).reshape(len(_x), 1)
                    )
                output = data_out.T[0]
                if output.shape == (1,):
                    return output[0]
...
Cotes = np.array(
    [np.pad(r, (0, len(Cotes_table) - 1 - len(r)), mode="constant") for r in Cotes_table]
)
...
def pdf_from_cf_with_fft(cf, h=0.01, q=9, level=3):
    ...
    if level > 1:
        ...
        s1 = np.sum(
            ...
            * np.exp(
                1j * np.pi * indices / (n - 1) - 2 * 1j * np.pi * indices * steps / (N * (n - 1))
            ),
            axis=0,
        )
    else:
