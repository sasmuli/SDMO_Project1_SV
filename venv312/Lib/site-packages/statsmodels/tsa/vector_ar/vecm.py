...
def _endog_matrices(
    ...
    if "co" in deterministic and "ci" in deterministic:
        raise ValueError(
            "Both 'co' and 'ci' as deterministic terms given. " + "Please choose one of the two."
        )
    y_lag1_stack = [y_lag1]
    ...
    if diff_lags > 0:
        for j in range(delta_x.shape[1]):
            delta_x[:, j] = delta_y[:, j + p - 2 : None if j - 1 < 0 else j - 1 : -1].T.reshape(
                K * (p - 1)
            )
...
def _r_matrices(delta_y_1_T, y_lag1, delta_x):
    ...
    nobs = y_lag1.shape[1]
    m = np.identity(nobs) - (delta_x.T.dot(inv(delta_x.dot(delta_x.T))).dot(delta_x))  # p. 291
...
class CointRankResults:
    ...
    def summary(self):
        ...
        data = [
            [i, self.r_1[i], self.test_stats[i], self.crit_vals[i]] for i in range(num_tests + 1)
        ]
        data_fmt = {
...
def select_coint_rank(endog, det_order, k_ar_diff, method="trace", signif=0.05):
    ...
    if det_order not in [-1, 0, 1]:
        if type(det_order) is int and det_order > 1:
            raise ValueError(
                "A det_order greather than 1 is not supported." "Use a value of -1, 0, or 1."
            )
        else:
...
class VECM(tsbase.TimeSeriesModel):
    ...
    def _lagged_param_names(self):
        ...
        if self.seasons > 0:
            param_names += [
                "season%d.%s" % (s, n) for s in range(1, self.seasons) for n in self.endog_names
            ]
    ...
    def _coint_param_names(self):
        ...
        if "ci" in self.deterministic:
            param_names += [
                "const." + self.load_coef_repr + "%d" % (i + 1) for i in range(self.coint_rank)
            ]

        if "li" in self.deterministic:
            param_names += [
                "lin_trend." + self.load_coef_repr + "%d" % (i + 1) for i in range(self.coint_rank)
            ]
...
class VECMResults:
    ...
    def predict(self, steps=5, alpha=None, exog_fc=None, exog_coint_fc=None):
        ...
        if exog_fc is not None and exog_fc.shape[0] < steps:
            raise ValueError(
                "The argument exog_fc must have at least steps " "elements in its first dimension"
            )
    ...
    def test_granger_causality(self, caused, causing=None, signif=0.05):
        ...
        if not all(isinstance(c, allowed_types) for c in caused):
            raise TypeError(
                "caused has to be of type string or int (or a " "sequence of these types)."
            )
