...
class TestTnc:
    ...
    def g38(self, x):
        ...
        dif[1] = (
            200.0 * (x[1] - pow(x[0], 2)) + 20.2 * (x[1] - 1.0) + 19.8 * (x[3] - 1.0)
        ) * 1.0e-5
        dif[2] = (-360.0 * x[2] * (x[3] - pow(x[2], 2)) - 2.0 * (1.0 - x[2])) * 1.0e-5
        dif[3] = (
            180.0 * (x[3] - pow(x[2], 2)) + 20.2 * (x[3] - 1.0) + 19.8 * (x[1] - 1.0)
        ) * 1.0e-5
        return dif
    ...
    def test_minimize_tnc1c(self):
        ...
        xopt = [1, 1]
        x = optimize.minimize(
            self.fg1, x0, method="TNC", jac=True, bounds=bnds, options=self.opts
        ).x
    ...
    def test_minimize_tnc2(self):
        ...
        x = optimize.minimize(
            self.f1, x0, method="TNC", jac=self.g1, bounds=bnds, options=self.opts
        ).x
        assert_allclose(self.f1(x), self.f1(xopt), atol=1e-8)
    ...
    def test_minimize_tnc3(self):
        ...
        xopt = [0, 0]
        x = optimize.minimize(
            self.f3, x0, method="TNC", jac=self.g3, bounds=bnds, options=self.opts
        ).x
    ...
    def test_minimize_tnc4(self):
        ...
        xopt = [1, 0]
        x = optimize.minimize(
            self.f4, x0, method="TNC", jac=self.g4, bounds=bnds, options=self.opts
        ).x
    ...
    def test_minimize_tnc5(self):
        ...
        x = optimize.minimize(
            self.f5, x0, method="TNC", jac=self.g5, bounds=bnds, options=self.opts
        ).x
    ...
    def test_minimize_tnc38(self):
        ...
        xopt = [1] * 4
        x = optimize.minimize(
            self.f38, x0, method="TNC", jac=self.g38, bounds=bnds, options=self.opts
        ).x
    ...
    def test_minimize_tnc45(self):
        ...
        xopt = [1, 2, 3, 4, 5]
        x = optimize.minimize(
            self.f45, x0, method="TNC", jac=self.g45, bounds=bnds, options=self.opts
        ).x
    ...
    def test_tnc1(self):
        ...
        x, nf, rc = optimize.fmin_tnc(
            fg, x, bounds=bounds, args=(100.0,), messages=optimize._tnc.MSG_NONE, maxfun=200
        )
        ...
        assert_allclose(
            ...
            self.f1(xopt),
            atol=1e-8,
            err_msg="TNC failed with status: " + optimize._tnc.RCSTRINGS[rc],
        )
    ...
    def test_tnc2(self):
        ...
        x, nf, rc = optimize.fmin_tnc(
            fg, x, bounds=bounds, messages=optimize._tnc.MSG_NONE, maxfun=200
        )
    ...
    def test_callback_shouldnt_affect_minimization(self):
        ...
        res2 = optimize.minimize(
            fun, x0, bounds=bounds, method="TNC", options={"maxfun": 1000}, callback=callback
        )
