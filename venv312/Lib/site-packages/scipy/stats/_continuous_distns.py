...
class beta_gen(rv_continuous):
    ...
    def _stats(self, a, b):
        ...
        _beta_skewness = (2 * (b - a) * np.sqrt(a_plus_b + 1)) / ((a_plus_b + 2) * np.sqrt(a * b))
...
class burr_gen(rv_continuous):
    ...
    def _logpdf(self, x, c, d):
        output = _lazywhere(
            ...
            f2=lambda x_, c_, d_: (
                np.log(c_) + np.log(d_) + sc.xlogy(-c_ - 1, x_) - sc.xlog1py(d_ + 1, x_ ** (-c_))
            ),
        )
        if output.ndim == 0:
            return output[()]
    ...
    def _munp(self, n, c, d):
        ...
        return _lazywhere(
            (c > n) & (n == n) & (d == d), (c, d, n), lambda c, d, n: __munp(n, c, d), np.nan
        )
...
class cauchy_gen(rv_continuous):
    ...
    def _logpdf(self, x):
        ...
        y = _lazywhere(
            ...
            f2=lambda absx: (-_LOG_PI - (2 * np.log(absx) + np.log1p((1 / absx) ** 2))),
        )
        return y
...
class chi_gen(rv_continuous):
    ...
    def _entropy(self, df):
        ...
        return _lazywhere(df < 3e2, (df,), regular_formula, f2=asymptotic_formula)
...
class cosine_gen(rv_continuous):
    ...
    def _logpdf(self, x):
        c = np.cos(x)
        return _lazywhere(
            c != -1, (c,), lambda c: np.log1p(c) - np.log(2 * np.pi), fillvalue=-np.inf
        )
...
class f_gen(rv_continuous):
    ...
    def _stats(self, dfn, dfd):
        ...
        g2 = _lazywhere(
            v2 > 8, (g1, v2_6, v2_8), lambda g1, v2_6, v2_8: (8 + g1 * g1 * v2_6) / v2_8, np.nan
        )
        g2 *= 3.0 / 2.0
...
class genpareto_gen(rv_continuous):
    ...
    def _logpdf(self, x, c):
        return _lazywhere(
            (x == x) & (c != 0), (x, c), lambda x, c: -sc.xlog1py(c + 1.0, c * x) / c, -x
        )
...
class genextreme_gen(rv_continuous):
    ...
    def _stats(self, c):
        ...
        def sk1_eval(c, *args):
            ...
            return _lazywhere(c >= -1.0 / 3, (c,) + args, f=sk1_eval_f, fillvalue=np.nan)
        ...
        def ku1_eval(c, *args):
            ...
            return _lazywhere(c >= -1.0 / 4, args, ku1_eval_f, fillvalue=np.nan)
...
class halflogistic_gen(rv_continuous):
    ...
    def _isf(self, q):
        return _lazywhere(
            q < 0.5, (q,), lambda q: -sc.logit(0.5 * q), f2=lambda q: 2 * np.arctanh(1 - q)
        )
...
class invgamma_gen(rv_continuous):
    ...
    def _stats(self, a, moments="mvsk"):
        ...
        if "k" in moments:
            g2 = _lazywhere(
                a > 4, (a,), lambda x: 6.0 * (5.0 * x - 11.0) / (x - 3.0) / (x - 4.0), np.nan
            )
        return m1, m2, g1, g2
...
class invgauss_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        if isinstance(data, CensoredData) or isinstance(self, wald_gen) or method.lower() == "mm":
            return super().fit(data, *args, **kwds)
...
class geninvgauss_gen(rv_continuous):
    ...
    def _logquasipdf(self, x, p, b):
        ...
        return _lazywhere(
            x > 0, (x, p, b), lambda x, p, b: (p - 1) * np.log(x) - b * (x + 1 / x) / 2, -np.inf
        )
    ...
    def _rvs(self, p, b, size=None, random_state=None):
        ...
        if np.isscalar(p) and np.isscalar(b):
            ...
            while not it.finished:
                ...
                idx = tuple(
                    (it.multi_index[j] if not bc[j] else slice(None)) for j in range(-len(size), 0)
                )
...
class landau_gen(rv_continuous):
    ...
    def _rvs(self, size=None, random_state=None):
        ...
        S = 2 / np.pi * ((pi_2 + U) * np.tan(U) - np.log((pi_2 * W * np.cos(U)) / (pi_2 + U)))
        return S
...
class ncx2_gen(rv_continuous):
    ...
    def _logpdf(self, x, df, nc):
        ...
        return _lazywhere(
            cond, (x, df, nc), f=_ncx2_log_pdf, f2=lambda x, df, _: chi2._logpdf(x, df)
        )
    ...
    def _pdf(self, x, df, nc):
        ...
        with np.errstate(over="ignore"):  # see gh-17432
            return _lazywhere(
                cond, (x, df, nc), f=scu._ncx2_pdf, f2=lambda x, df, _: chi2._pdf(x, df)
            )
    ...
    def _cdf(self, x, df, nc):
        ...
        with np.errstate(over="ignore"):  # see gh-17432
            return _lazywhere(
                cond, (x, df, nc), f=scu._ncx2_cdf, f2=lambda x, df, _: chi2._cdf(x, df)
            )
    ...
    def _ppf(self, q, df, nc):
        ...
        with np.errstate(over="ignore"):  # see gh-17432
            return _lazywhere(
                cond, (q, df, nc), f=scu._ncx2_ppf, f2=lambda x, df, _: chi2._ppf(x, df)
            )
    ...
    def _sf(self, x, df, nc):
        ...
        with np.errstate(over="ignore"):  # see gh-17432
            return _lazywhere(
                cond, (x, df, nc), f=scu._ncx2_sf, f2=lambda x, df, _: chi2._sf(x, df)
            )
    ...
    def _isf(self, x, df, nc):
        ...
        with np.errstate(over="ignore"):  # see gh-17432
            return _lazywhere(
                cond, (x, df, nc), f=scu._ncx2_isf, f2=lambda x, df, _: chi2._isf(x, df)
            )
    ...
    def _stats(self, df, nc):
        ...
        _ncx2_skewness = np.sqrt(8.0) * k_plus_cl(df, nc, 3) / np.sqrt(k_plus_cl(df, nc, 2.0) ** 3)
        _ncx2_kurtosis_excess = 12.0 * k_plus_cl(df, nc, 4.0) / k_plus_cl(df, nc, 2.0) ** 2
        return (
            _ncx2_mean,
            _ncx2_variance,
            _ncx2_skewness,
...
class t_gen(rv_continuous):
    ...
    def _pdf(self, x, df):
        return _lazywhere(
            ...
            f2=lambda x, df: (np.exp(self._logpdf(x, df))),
        )
...
class powerlaw_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        if fscale is not None:
            if fscale <= 0:
                raise ValueError(
                    "Negative or zero `fscale` is outside the " "range allowed by the distribution."
                )
...
class skewnorm_gen(rv_continuous):
    ...
    def _pdf(self, x, a):
        return _lazywhere(
            ...
            f2=lambda x, a: 2.0 * _norm_pdf(x) * _norm_cdf(a * x),
        )
    ...
    def _skewnorm_odd_moments(self):
        skewnorm_odd_moments = {
            ...
            15: Polynomial(
                [2027025, -4729725, 8513505, -10135125, 7882875, -3869775, 1091475, -135135]
            ),
            17: Polynomial(
                [
                    34459425,
...
deprmsg = "`trapz` is deprecated in favour of `trapezoid` " "and will be removed in SciPy 1.16.0."
...
class _DeprecationWrapper:
    def __init__(self, method):
        self.msg = (
            f"`trapz.{method}` is deprecated in favour of trapezoid.{method}. "
...
class tukeylambda_gen(rv_continuous):
    ...
    def _get_support(self, lam):
        b = _lazywhere(lam > 0, (lam,), f=lambda lam: 1 / lam, fillvalue=np.inf)
        return -b, b
            ...
            if r == 1:
                ...
                if solve_for_kappa(lower_bound) >= 0:
                    ...
                    root_res = root_scalar(
                        solve_for_kappa, method="brentq", bracket=(lower_bound, upper_bound)
                    )
...
class crystalball_gen(rv_continuous):
    ...
    def _munp(self, n, beta, m):
        ...
        return N * _lazywhere(
            n + 1 < m, (n, beta, m), np.vectorize(n_th_moment, otypes=[np.float64]), np.inf
        )
