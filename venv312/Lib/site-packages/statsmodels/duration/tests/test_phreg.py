...
class TestPHReg:
    ...
    def test_formula_args(self):
        ...
        model2 = PHReg.from_formula(
            "time ~ x1 + x2", status=df.status, offset=df.offset, entry=df.entry, data=df
        )
        result2 = model2.fit()
    ...
    def test_post_estimation(self):
        ...
        assert_allclose(
            np.abs(w_avg[0]).sum(0), np.r_[7.31008415, 9.77608674, 10.89515885, 13.1106801]
        )
...
@pytest.mark.parametrize(
    "fname,ties,entry_f,strata_f", list(itertools.product(fnames, ties, entry_f, strata_f))
)
