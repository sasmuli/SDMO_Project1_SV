...
def spsolve(A, b, permc_spec=None, use_umfpack=True):
    ...
    if not (issparse(A) and A.format in ("csc", "csr")):
        A = csc_array(A)
        warn(
            "spsolve requires A be CSC or CSR matrix format", SparseEfficiencyWarning, stacklevel=2
        )
...
def splu(A, permc_spec=None, diag_pivot_thresh=None, relax=None, panel_size=None, options=None):
    ...
    _options = dict(
        DiagPivotThresh=diag_pivot_thresh, ColPerm=permc_spec, PanelSize=panel_size, Relax=relax
    )
...
def spilu(
    ...
    if not (issparse(A) and A.format == "csc"):
        A = csc_array(A)
        warn("spilu converted its input to CSC format", SparseEfficiencyWarning, stacklevel=2)
...
def spsolve_triangular(A, b, lower=True, overwrite_A=False, overwrite_b=False, unit_diagonal=False):
    ...
    x, info = _superlu.gstrs(
        trans, N, L.nnz, L.data, L.indices, L.indptr, N, U.nnz, U.data, U.indices, U.indptr, b
    )
    if info:
