...
def _broadcast_shapes(shapes, axis=None):
    ...
    if axis is not None:
        ...
        new_shapes = [
            tuple(np.insert(new_shape, new_axis, removed_shape)) for removed_shape in removed_shapes
        ]
        return new_shapes
    else:
...
def _add_reduced_axes(res, reduced_axes, keepdims):
    ...
    return (
        [
            np.expand_dims(output, reduced_axes) if not isinstance(output, int) else output
            for output in res
        ]
        if keepdims
        else res
    ...
    def axis_nan_policy_decorator(hypotest_fun_in):
        ...
        def axis_nan_policy_wrapper(*args, _no_deco=False, **kwds):
            ...
            if axis is None:
                ...
                samples = [
                    sample.reshape(new_shape) for sample, new_shape in zip(samples, new_shapes)
                ]
            ...
            if contains_nan and nan_policy == "omit":
                ...
                def hypotest_fun(x):
                    ...
                    if sentinel:
                        samples = _remove_sentinel(samples, paired, sentinel)
                    if is_too_small(samples, kwds):
                        warnings.warn(too_small_nd_omit, SmallSampleWarning, stacklevel=4)
