...
class TestConvolve(_TestConvolve):
    ...
    def test_convolve_method(self, n=100):
        ...
        args = [
            (t1, t2, mode) for t1 in types for t2 in types for mode in ["valid", "full", "same"]
        ]
...
class TestConvolve2d(_TestConvolve2d):
    ...
    def test_consistency_convolve_funcs(self):
        ...
        for mode in ["full", "valid", "same"]:
            ...
            assert_almost_equal(
                np.squeeze(signal.convolve2d([a], [b], mode=mode)), signal.convolve(a, b, mode=mode)
            )
...
class TestFFTConvolve:
    ...
    @pytest.mark.parametrize(
        "axes", ["", None, [0, 1], [1, 0], [0, -1], [-1, 0], [-2, 1], [1, -2], [-2, -1], [-1, -2]]
    )
...
class TestOAConvolve:
    @pytest.mark.slow()
    @pytest.mark.parametrize(
        "shape_a_0, shape_b_0", gen_oa_shapes_eq(list(range(100)) + list(range(100, 1000, 23)))
    )
    ...
    @pytest.mark.parametrize(
        "shape_a_0, shape_b_0, " "shape_a_1, shape_b_1, mode", gen_oa_shapes_2d([50, 47, 6, 4])
    )
    @pytest.mark.parametrize("is_complex", [True, False])
    def test_2d_noaxes(
...
class TestResample:
    ...
    def test_rfft(self, N, num, window):
        ...
        y = np.cos(-(x**2) / 6.0)
        assert_allclose(
            signal.resample(y, num, window=window), signal.resample(y + 0j, num, window=window).real
        )
...
class _TestLinearFilter:
    ...
    def test_scalar_input(self, a, b):
        ...
        assert_allclose(lfilter(np.array([1.0]), np.array([1.0]), data), lfilter(b, a, data))
...
@pytest.mark.parametrize("dt", [np.csingle, np.cdouble, pytest.param(np.clongdouble, marks=_pmf)])
class TestCorrelateComplex:
    ...
    def _setup_rank1(self, dt, mode):
        ...
        y_r = (correlate(a.real, b.real, mode=mode) + correlate(a.imag, b.imag, mode=mode)).astype(
            dt
        )
    ...
    def test_swap_full(self, dt):
        ...
        y = correlate(d, k)
        assert_equal(
            y, [0.0 + 0.0j, 10.0 - 2.0j, 28.0 - 6.0j, 22.0 - 6.0j, 16.0 - 6.0j, 8.0 - 4.0j]
        )
...
class TestDecimate:
    ...
    def _test_phaseshift(self, method, zero_phase):
        ...
        for rate_to in rates_to:
            ...
            d_tos = np.exp(1j * 2 * np.pi * freqs[:, np.newaxis] * t_to) * signal.windows.tukey(
                t_to.size, 0.1
            )
...
class TestEnvelope:
    ...
    def test_envelope_invalid_parameters(self):
        ...
        parameters."""
        with pytest.raises(ValueError, match=r"Invalid parameter axis=2 for z.shape=.*"):
        ...
        for bp_in in [(0, 1, 2), (0, 2.0), (None, 2.0)]:
            ...
            with pytest.raises(ValueError, match=rf"bp_in=\({ts}\) isn't a 2-tuple of.*"):
                # noinspection PyTypeChecker
                envelope(np.ones(4), bp_in=bp_in)
        with pytest.raises(ValueError, match="n_out=10.0 is not a positive integer or.*"):
            # noinspection PyTypeChecker
            envelope(np.ones(4), n_out=10.0)
        for bp_in in [(-1, 3), (1, 1), (0, 10)]:
            with pytest.raises(ValueError, match=r"`-n//2 <= bp_in\[0\] < bp_in\[1\] <=.*"):
                envelope(np.ones(4), bp_in=bp_in)
    ...
    def test_envelope_verify_parameters(self):
        ...
        self.assert_close(
            sp_fft.rfft(ze2_0),
            np.array([4, 2, 0, 0, 0]).astype(complex),
            msg="Envelope calculation error",
        )
        self.assert_close(
            sp_fft.rfft(zr_0),
            np.array([4, 0, 0, 3, 0]).astype(complex),
            msg="Residual calculation error",
        )

        ze_1, zr_1 = envelope(z, (1, 3), residual="all", squared=False)
        self.assert_close(ze_1**2, ze2_0, msg="Unsquared versus Squared envelope calculation error")
        self.assert_close(zr_1, zr_0, msg="Unsquared versus Squared residual calculation error")

        ze2_2, zr_2 = envelope(z, (1, 3), residual="all", squared=True, n_out=3 * n)
        self.assert_close(ze2_2[::3], ze2_0, msg="3x up-sampled envelope calculation error")
        self.assert_close(zr_2[::3], zr_0, msg="3x up-sampled residual calculation error")

        ze2_3, zr_3 = envelope(z, (1, 3), residual="lowpass", squared=True)
        self.assert_close(ze2_3, ze2_0, msg="`residual='lowpass'` envelope calculation error")
        self.assert_close(
            sp_fft.rfft(zr_3),
            np.array([4, 0, 0, 0, 0]).astype(complex),
            msg="`residual='lowpass'` residual calculation error",
        )

        ze2_4 = envelope(z, (1, 3), residual=None, squared=True)
        self.assert_close(ze2_4, ze2_0, msg="`residual=None` envelope calculation error")

        # compare complex analytic signal to real version
        Z_a = np.copy(Z)
        Z_a[1:] *= 2
        z_a = sp_fft.ifft(Z_a, n=n)  # analytic signal of Z
        self.assert_close(z_a.real, z, msg="Reference analytic signal error")
        ze2_a, zr_a = envelope(z_a, (1, 3), residual="all", squared=True)
        self.assert_close(
            ze2_a,
            ze2_0.astype(complex),  # dtypes must match
            msg="Complex envelope calculation error",
        )
        self.assert_close(
            sp_fft.fft(zr_a),
            np.array(Zr_a).astype(complex),
    ...
    def test_envelope_real_signals(self, Z, bp_in, Ze2_desired, Zr_desired):
        ...
        self.assert_close(Ze2, Ze2_desired, msg="Envelope calculation error (residual='all')")
        self.assert_close(Zr, Zr_desired, msg="Residual calculation error (residual='all')")

        if bp_in[1] is not None:
            Zr_desired[bp_in[1] :] = 0
        self.assert_close(
            Ze2_lp, Ze2_desired, msg="Envelope calculation error (residual='lowpass')"
        )
        self.assert_close(Zr_lp, Zr_desired, msg="Residual calculation error (residual='lowpass')")
    ...
    def test_envelope_complex_signals(self, Z, bp_in, Ze2_desired, Zr_desired):
        ...
        self.assert_close(
            Ze2, np.array(Ze2_desired).astype(complex), msg="Envelope calculation error"
        )
        self.assert_close(
            Zr, np.array(Zr_desired).astype(complex), msg="Residual calculation error"
        )
    ...
    def test_envelope_verify_axis_parameter(self):
        ...
        Ze2_desired = np.array([[4, 2, 0, 0, 0], [16, 8, 0, 0, 0]], dtype=complex)
...
class TestPartialFractionExpansion:
    ...
    def test_residue_general(self):
        ...
        self.assert_rp_almost_equal(
            r, p, [-18.125 - 13.125j, -18.125 + 13.125j, 36.25], [0.5 - 0.2j, 0.5 + 0.2j, 0.7]
        )
        assert_equal(k.size, 0)
