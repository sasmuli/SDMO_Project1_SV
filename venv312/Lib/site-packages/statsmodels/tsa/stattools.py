...
def acovf(x, adjusted=False, demean=True, fft=True, missing="none", nlag=None):
    ...
    missing = string_like(missing, "missing", options=("none", "raise", "conservative", "drop"))
...
def grangercausalitytests(x, maxlag, addconst=True, verbose=None):
    ...
    try:
        ...
        if lags.min() <= 0 or lags.size == 0:
            raise ValueError("maxlag must be a non-empty list containing only " "positive integers")
    ...
    for mlg in lags:
        ...
        if verbose:
            print(
                ...
                " df_num=%d" % (ftres.fvalue, ftres.pvalue, ftres.df_denom, ftres.df_num)
            )
        result["params_ftest"] = (
