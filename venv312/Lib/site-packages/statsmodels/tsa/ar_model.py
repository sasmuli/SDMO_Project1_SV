...
class AutoReg(tsa_model.TimeSeriesModel):
    ...
    def __init__(
        ...
        *,
        ...
        self._trend = cast(
            Literal["n", "c", "t", "ct"],
            string_like(trend, "trend", options=("n", "c", "t", "ct"), optional=False),
        )
    ...
    def predict(
        ...
        if self.exog is None and (exog is not None or exog_oos is not None):
            ...
            if num_oos > 0 and exog_oos is None:
                raise ValueError(
                    "exog_oos must be provided when producing " "out-of-sample forecasts."
                )
    ...
    def apply(self, endog, exog=None, refit=False, fit_kwargs=None):
        ...
        if (mod.exog is None) != (existing.exog is None):
            if existing.exog is not None:
                raise ValueError(
                    "exog must be provided when the original model contained " "exog variables"
                )
            raise ValueError(
                "exog must be None when the original model did not contain " "exog variables"
            )
