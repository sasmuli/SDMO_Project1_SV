...
def _execute_1D(func_str, pocketfft_func, x, n, axis, norm, overwrite_x, workers, plan):
    ...
    if is_numpy(xp):
        x = np.asarray(x)
        return pocketfft_func(
            x, n=n, axis=axis, norm=norm, overwrite_x=overwrite_x, workers=workers, plan=plan
        )
...
def _execute_nD(func_str, pocketfft_func, x, s, axes, norm, overwrite_x, workers, plan):
    ...
    if is_numpy(xp):
        x = np.asarray(x)
        return pocketfft_func(
            x, s=s, axes=axes, norm=norm, overwrite_x=overwrite_x, workers=workers, plan=plan
        )
