...
class _bsr_base(_cs_matrix, _minmax_mixin):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, blocksize=None, *, maxprint=None):
        ...
        if issparse(arg1):
            ...
            if isshape(arg1):
                ...
                if self.data.ndim != 3:
                    raise ValueError(f"BSR data must be 3-dimensional, got shape={self.data.shape}")
                if blocksize is not None:
                    if not isshape(blocksize):
                        raise ValueError(f"invalid blocksize={blocksize}")
                    if tuple(blocksize) != self.data.shape[1:]:
                        raise ValueError(
                            f"mismatching blocksize={blocksize}" f" vs {self.data.shape[1:]}"
                        )
            else:
    ...
    def check_format(self, full_check=True):
        ...
        if self.indptr.dtype.kind != "i":
            warn(f"indptr array has non-integer dtype ({self.indptr.dtype.name})", stacklevel=2)
        if self.indices.dtype.kind != "i":
            warn(f"indices array has non-integer dtype ({self.indices.dtype.name})", stacklevel=2)
    ...
    def count_nonzero(self, axis=None):
        if axis is not None:
            raise NotImplementedError("count_nonzero over axis is not implemented for BSR format.")
