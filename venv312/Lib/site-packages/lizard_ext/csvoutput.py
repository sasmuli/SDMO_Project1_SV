...
def csv_output(result, options):
    ...
    for extension in options.extensions:
        if extension.__class__.__name__ == "LizardExtension":
            if hasattr(extension, "FUNCTION_INFO") and len(list(extension.FUNCTION_INFO)) == 1:
                ...
                extension_caption = (
                    var_extension["caption"] if "caption" in var_extension else "No_caption"
                )
    ...
    if options.verbose:
        ...
        print(
            f"NLOC,CCN,token,PARAM,length,location,file,function,"
            ...
            for source_function in source_file.function_list:
                if source_function:
                    ...
                    for variable in extension_variables:
                        extension_string = (
                            f"{extension_string},{source_function.__getattribute__(variable)}"
                        )
                    print(
                        f"{source_function.nloc},{source_function.cyclomatic_complexity},"
