...
def main(
    ...
    # -2 - 2j, -2 + 2j, 2 - 2j, 2 + 2j
    X, Y = np.meshgrid(
        np.linspace(-box_size, box_size, grid_size), np.linspace(-box_size, box_size, grid_size)
    )
    Z = X + Y * 1j
    Z = Z.flatten().tolist()
    # Add z = 1 + 0j (region 0).
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="hyp2f1 is evaluated in box of side_length 2*box_size centered" " at the origin.",
    )
    parser.add_argument(
        "--grid_size",
        type=int,
