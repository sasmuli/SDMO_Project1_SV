...
class FactorInfo:
    ...
    def __init__(self, factor, type, state, num_columns=None, categories=None):
        ...
        if self.type not in ["numerical", "categorical"]:
            raise ValueError(
                "FactorInfo.type must be " "'numerical' or 'categorical', not %r" % (self.type,)
            )
        self.state = state
                    ...
                    for factor in subterm.factors:
                        ...
                        if fi.type == "numerical":
                            ...
                            if cm.shape[0] != len(fi.categories):
                                raise ValueError(
                                    "Mismatched contrast matrix " "for factor %r" % (factor,)
                                )
...
def test_DesignInfo():
    ...
    term_codings_a_bad_rows = OrderedDict(
        [
            (
                t_a,
                [SubtermInfo([f_a], {f_a: ContrastMatrix(np.ones((3, 2)), ["[1]", "[2]"])}, 2)],
            )
        ]
    )
    pytest.raises(
...
def _format_float_column(precision, col):
    ...
    mask = np.array(
        [simple_float_chars.issuperset(col_str) and "." in col_str for col_str in col_strs]
    )
...
class DesignMatrix(np.ndarray):
    ...
    def _repr_pretty_(self, p, cycle):
        ...
        if min_total_width <= MAX_TOTAL_WIDTH:
            ...
            formatted_cols = [
                _format_float_column(PRECISION, printable_part[:, i]) for i in range(self.shape[1])
            ]
            ...
            column_widths = [
                ...
                for (name_width, num_width) in zip(column_name_widths, column_num_widths)
            ]
