...
class CLikeReader(CodeReader, CCppCommentsMixin):
    ...
    def generate_tokens(source_code, addition="", token_class=None):
        ...
        addition = (
            r"|(?:\d*\.\d+(?:[eE][-+]?\d+)?)" + r"|(?:\d+\.(?:\d+)?(?:[eE][-+]?\d+)?)" + addition
        )
...
class CLikeNestingStackStates(CodeStateMachine):
    ...
    def _read_namespace_name(self, token, saved):
        ...
        self._state = self._state_global
        if token == "{":
            self.context.add_namespace(
                "".join(itertools.takewhile(lambda x: x not in self.__namespace_separators, saved))
            )
...
class CLikeStates(CodeStateMachine):
    ...
    def _state_name_with_space(self, token):
        self._state = self._state_operator if token == "operator" else self._state_function
        self.context.add_to_function_name(token)
