...
from scipy.special._support_alternative_backends import (
    get_array_special_func,
...
@pytest.mark.skipif(not HAVE_ARRAY_API_STRICT, reason="`array_api_strict` not installed")
...
@pytest.mark.parametrize(
    "shapes", [[(0,)] * 4, [tuple()] * 4, [(10,)] * 4, [(10,), (11, 1), (12, 1, 1), (13, 1, 1, 1)]]
)
def test_support_alternative_backends(xp, f_name_n_args, dtype, shapes):
    f_name, n_args = f_name_n_args

    if SCIPY_DEVICE != "cpu" and is_torch(xp) and f_name in {"stdtr", "betaincc", "betainc"}:
        pytest.skip(
            f"`{f_name}` does not have an array-agnostic implementation "
    ...
    if is_jax(xp) and f_name == "gammaincc" or f_name == "chdtrc":  # google/jax#20699  # gh-20972
