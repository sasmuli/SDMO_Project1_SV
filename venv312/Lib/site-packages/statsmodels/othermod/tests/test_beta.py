...
expected_income_precision = pd.read_table(io.StringIO(_income_estimates_precision), sep=r"\s+")

expected_methylation_mean = pd.read_table(io.StringIO(_methylation_estimates_mean), sep=r"\s+")
...
class TestBetaModel:
    ...
    def test_methylation_precision(self):
        ...
        assert_allclose(
            rslt.params[-2:], expected_methylation_precision["Estimate"], atol=1e-5, rtol=1e-10
        )
    ...
    def test_precision_formula(self):
        m = BetaModel.from_formula(
            self.model, methylation, exog_precision_formula="~ age", link_precision=links.Identity()
        )
        rslt = m.fit()
    ...
    def test_scores(self):
        ...
        for link in (links.Identity(), links.Log()):
            ...
            assert_allclose(
                link.inverse(analytical[3:]), link.inverse(numerical[3:]), rtol=5e-7, atol=5e-6
            )
...
class TestBetaMeth:
    ...
    def setup_class(cls):
        ...
        mod = BetaModel.from_formula(
            formula, methylation, exog_precision_formula="~ age", link_precision=links.Log()
        )
