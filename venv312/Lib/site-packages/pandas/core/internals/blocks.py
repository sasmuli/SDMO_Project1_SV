...
class Block(PandasObject, libinternals.Block):
    ...
    def coerce_to_target_dtype(
        ...
        copy = False
        if not using_cow and isinstance(self.dtype, StringDtype) and self.dtype.storage == "python":
            copy = True
    ...
    def _maybe_downcast(
        ...
        if self.dtype == _dtype_obj:
            ...
            nbs = extend_blocks(
                [
                    blk.convert(using_cow=using_cow, copy=not using_cow, convert_string=False)
                    for blk in blocks
                ]
            )
            if caller == "fillna":
        ...
        try:
            ...
            if self.ndim == 1 or self.shape[0] == 1:
                ...
                if not is_list_like(new):
                    ...
                    return self.coerce_to_target_dtype(new, warn_on_upcast=True).putmask(mask, new)
                else:
    ...
    def fillna(
        ...
        if inplace:
            nbs = self.putmask(mask.T, value, using_cow=using_cow, already_warned=already_warned)
        else:
        ...
        return extend_blocks(
            [
                blk._maybe_downcast([blk], downcast=downcast, using_cow=using_cow, caller="fillna")
                for blk in nbs
            ]
        )
