...
def test_basic(using_infer_string):  # TODO: split this test
    ...
    tm.assert_index_equal((desc_result.stack(future_stack=True).index.get_level_values(0)), exp)
    exp = Index(["count", "mean", "std", "min", "25%", "50%", "75%", "max"] * 4)
    tm.assert_index_equal((desc_result.stack(future_stack=True).index.get_level_values(1)), exp)
...
def test_describe_categorical_columns():
    ...
    tm.assert_categorical_equal(result.stack(future_stack=True).columns.values, cats.values)
...
def test_preserve_on_ordered_ops(func, values):
    ...
    expected = DataFrame({"payload": [-2, -1], "col": Series(values, dtype=c.dtype)}).set_index(
        "payload"
    )
    tm.assert_frame_equal(result, expected)
...
def test_get_nonexistent_category():
    ...
    with pytest.raises(KeyError, match="'vau'"):
        df.groupby("var").apply(
            lambda rows: DataFrame({"var": [rows.iloc[-1]["var"]], "val": [rows.iloc[-1]["vau"]]})
        )
