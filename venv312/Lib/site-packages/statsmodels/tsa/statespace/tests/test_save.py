...
from statsmodels.tsa.statespace import sarimax, structural, varmax, dynamic_factor
...
def test_dynamic_factor(temp_filename):
    mod = dynamic_factor.DynamicFactor(
        macrodata[["realgdp", "realcons"]].diff().iloc[1:].values, k_factors=1, factor_order=1
    )
    res = mod.smooth(mod.start_params)
    res.summary()
    res.save(temp_filename)
