...
class TestConvertTemperature:
    def test_convert_temperature(self, xp):
        xp_assert_equal(sc.convert_temperature(xp.asarray(32.0), "f", "Celsius"), xp.asarray(0.0))
        xp_assert_equal(
        ...
        xp_assert_equal(
            sc.convert_temperature(xp.asarray([32.0, 32.0]), "f", "k"), xp.asarray([273.15, 273.15])
        )
        xp_assert_equal(
        ...
        xp_assert_close(
            sc.convert_temperature(xp.asarray([491.67, 491.67], dtype=xp.float64), "Rankine", "C"),
            xp.asarray([0.0, 0.0], dtype=xp.float64),
            rtol=0.0,
            atol=1e-13,
        )
        xp_assert_close(
            sc.convert_temperature(xp.asarray([491.67, 491.67], dtype=xp.float64), "r", "F"),
            xp.asarray([32.0, 32.0], dtype=xp.float64),
            rtol=0.0,
            atol=1e-13,
        )
        xp_assert_close(
            sc.convert_temperature(xp.asarray([32.0, 32.0], dtype=xp.float64), "fahrenheit", "R"),
            xp.asarray([491.67, 491.67], dtype=xp.float64),
            rtol=0.0,
            atol=1e-13,
        )
        xp_assert_close(
            sc.convert_temperature(xp.asarray([273.15, 273.15], dtype=xp.float64), "K", "R"),
            xp.asarray([491.67, 491.67], dtype=xp.float64),
            rtol=0.0,
            atol=1e-13,
        )
...
class TestLambdaToNu:
    def test_lambda_to_nu(self, xp):
        xp_assert_equal(
            sc.lambda2nu(xp.asarray([sc.speed_of_light, 1])), xp.asarray([1, sc.speed_of_light])
        )

    @skip_xp_backends(np_only=True, reason="Python list input uses NumPy backend")
    def test_lambda_to_nu_array_like(self, xp):
        assert_allclose(sc.lambda2nu([sc.speed_of_light, 1]), [1, sc.speed_of_light])


class TestNuToLambda:
    def test_nu_to_lambda(self, xp):
        xp_assert_equal(
            sc.nu2lambda(xp.asarray([sc.speed_of_light, 1])), xp.asarray([1, sc.speed_of_light])
        )

    @skip_xp_backends(np_only=True, reason="Python list input uses NumPy backend")
    def test_nu_to_lambda_array_like(self, xp):
        assert_allclose(sc.nu2lambda([sc.speed_of_light, 1]), [1, sc.speed_of_light])
