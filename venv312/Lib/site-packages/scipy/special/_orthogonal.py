...
def jacobi(n, alpha, beta, monic=False):
    ...
    p = orthopoly1d(
        x, w, hn, kn, wfunc, (-1, 1), monic, lambda x: _ufuncs.eval_jacobi(n, alpha, beta, x)
    )
    return p
...
def genlaguerre(n, alpha, monic=False):
    ...
    kn = (-1) ** n / _gam(n + 1)
    p = orthopoly1d(
        x, w, hn, kn, wfunc, (0, inf), monic, lambda x: _ufuncs.eval_genlaguerre(n, alpha, x)
    )
    return p
...
def laguerre(n, monic=False):
    ...
    kn = (-1) ** n / _gam(n + 1)
    p = orthopoly1d(
        x, w, hn, kn, lambda x: exp(-x), (0, inf), monic, lambda x: _ufuncs.eval_laguerre(n, x)
    )
    return p
...
def chebyc(n, monic=False):
    ...
    kn = 1.0
    p = orthopoly1d(
        x, w, hn, kn, wfunc=lambda x: 1.0 / sqrt(1 - x * x / 4.0), limits=(-2, 2), monic=monic
    )
    if not monic:
        p._scale(2.0 / p(2))
...
def roots_legendre(n, mu=False):
    ...
    def df(n, x):
        return (-n * x * _ufuncs.eval_legendre(n, x) + n * _ufuncs.eval_legendre(n - 1, x)) / (
            1 - x**2
        )
...
def sh_legendre(n, monic=False):
    ...
    if n == 0:
        return orthopoly1d(
            [], [], 1.0, 1.0, wfunc, (0, 1), monic, lambda x: _ufuncs.eval_sh_legendre(n, x)
        )
