...
class TestResize:
    ...
    def test_unsigned_resize(self):
        ...
        for dt_pair in [(np.int32, np.uint32), (np.int64, np.uint64)]:
            ...
            assert_array_equal(np.resize(arr, dt_pair[0](1)), np.resize(arr, dt_pair[1](1)))
...
class TestNonarrayArgs:
    ...
    def test_reshape_shape_arg(self):
        ...
        with pytest.raises(
            TypeError,
            match="You cannot specify 'newshape' and 'shape' " "arguments at the same time.",
        ):
            np.reshape(arr, shape=shape, newshape=shape)
        with pytest.raises(
            TypeError, match=r"reshape\(\) missing 1 required positional " "argument: 'shape'"
        ):
            np.reshape(arr)
    ...
    @pytest.mark.parametrize(
        "val, ndigits",
        [
            pytest.param(2**31 - 1, -1, marks=pytest.mark.skip(reason="Out of range of int32")),
...
class TestSeterr:
    def test_default(self):
        err = np.geterr()
        assert_equal(err, {"divide": "warn", "invalid": "warn", "over": "warn", "under": "ignore"})
...
class TestFloatExceptions:
    def assert_raises_fpe(self, fpeerr, flop, x, y):
        ...
        try:
            ...
            assert_(str(exc).find(fpeerr) >= 0, f"Type {ftype} raised wrong fpe error '{exc}'.")
...
class TestTypes:
    ...
    @pytest.mark.parametrize(
        "dtype", list(np.typecodes["All"]) + ["i,i", "10i", "S3", "S100", "U3", "U100", rational]
    )
    ...
    @pytest.mark.parametrize(
        ["dtype1", "dtype2"],
        itertools.product(
            list(np.typecodes["All"]) + ["i,i", "S3", "S100", "U3", "U100", rational], repeat=2
        ),
    )
...
class TestNonzero:
    ...
    @pytest.mark.parametrize(
        "dtype",
        [bool, np.int8, np.int16, np.int32, np.int64, np.uint8, np.uint16, np.uint32, np.uint64],
    )
...
class TestArrayComparisons:
    @pytest.mark.parametrize("bx,by,equal_nan,expected", _test_array_equal_parametrizations())
...
class TestClip:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (np.zeros(10, dtype=object), 0, -(2**64) + 1, np.full(10, -(2**64) + 1, dtype=object)),
    ...
    @given(
        data=st.data(),
        arr=hynp.arrays(
            dtype=hynp.integer_dtypes() | hynp.floating_dtypes(), shape=hynp.array_shapes()
        ),
    )
    def test_clip_property(self, data, arr):
        ...
        amin = data.draw(
            s | hynp.arrays(dtype=numeric_dtypes, shape=in_shapes[0], elements={"allow_nan": False})
        )
        amax = data.draw(
            s | hynp.arrays(dtype=numeric_dtypes, shape=in_shapes[1], elements={"allow_nan": False})
        )
    ...
    def test_clip_min_max_args(self):
        ...
        with assert_raises_regex(TypeError, "missing 1 required positional " "argument: 'a_max'"):
            np.clip(arr, 2)
        with assert_raises_regex(TypeError, "missing 1 required positional " "argument: 'a_min'"):
            np.clip(arr, a_max=2)
        msg = (
        ...
        for i in b:
            for j in b:
                ...
                with warnings.catch_warnings(record=True) as w:
                    ...
                    assert f"One of rtol or atol is not valid, atol: {i}, rtol: {j}" in str(
                        w[-1].message
                    )
...
class TestLikeFuncs:
    ...
    def check_like_function(self, like_function, value, fill_value=False):
        ...
        for d, dtype in self.data:
            ...
            assert_equal(
                np.array(dz.strides) * d.dtype.itemsize, np.array(d.strides) * dz.dtype.itemsize
            )
            ...
            for s in self.shapes:
                ...
                if d.ndim != len(s):
                    assert_equal(
                        np.argsort(
                            like_function(d, dtype=dtype, shape=s, order="K", **fill_kwarg).strides
                        ),
...
class TestMoveaxis:
    ...
    def test_errors(self):
        ...
        assert_raises_regex(AxisError, "destination.*out of bounds", np.moveaxis, x, 0, 5)
...
class TestBroadcast:
    ...
    def test_shape_mismatch_error_message(self):
        with pytest.raises(
            ValueError, match=r"arg 0 with shape \(1, 3\) and " r"arg 2 with shape \(2,\)"
        ):
