...
class TestFedFundsConst(MarkovRegression):
    ...
    def setup_class(cls):
        ...
        true = {
            ...
            "bse_oim": np.r_[0.0104002, 0.0268434, 0.1767083, 0.2999889, np.nan],
    ...
    def test_filter_output(self, **kwargs):
        res = self.result
        assert_allclose(
            res.filtered_joint_probabilities, fedfunds_const_filtered_joint_probabilities
        )
    ...
    def test_predict(self):
        ...
        for name in ["predicted", "filtered", "smoothed", None]:
            ...
            assert_allclose(actual[0], self.true["predict0"], atol=1e-6)
            assert_allclose(actual[1], self.true["predict1"], atol=1e-6)
        ...
        assert_allclose(
            self.model.predict(self.true["params"], probabilities="filtered"),
            self.true["predict_filtered"],
            atol=1e-5,
        )
...
class TestFedFundsConstShort(MarkovRegression):
    ...
    def test_filter_output(self, **kwargs):
        ...
        # Filtered
        assert_allclose(
            res.filtered_joint_probabilities, fedfunds_const_short_filtered_joint_probabilities
        )
    ...
    def test_smoother_output(self, **kwargs):
        ...
        # Smoothed, last entry
        assert_allclose(
            res.smoothed_joint_probabilities, fedfunds_const_short_smoothed_joint_probabilities
        )
...
class TestFedFundsConstL1(MarkovRegression):
    ...
    def setup_class(cls):
        true = {
            "params": np.r_[
                0.6378175, 0.1306295, 0.724457, -0.0988764, 0.7631424, 1.061174, 0.6915759**2
            ],
            "llf": -264.71069,
            "llf_fit": -264.71069,
            "llf_fit_em": -264.71153,
            "bse_oim": np.r_[
                0.1202616, 0.0495924, 0.2886657, 0.1183838, 0.0337234, 0.0185031, np.nan
            ],
        }
...
class TestFedFundsConstL1Exog(MarkovRegression):
    ...
    def setup_class(cls):
        ...
        super().setup_class(
            true, fedfunds[4:], k_regimes=2, exog=np.c_[fedfunds[3:-1], ogap[4:], inf[4:]]
        )
    ...
    def test_predict(self):
        ...
        for name in ["predicted", "filtered", "smoothed", None]:
            actual = self.model.predict(self.true["params"], probabilities=name, conditional=True)
            assert_allclose(actual[0], self.true["predict0"], atol=1e-5)
            assert_allclose(actual[1], self.true["predict1"], atol=1e-5)
...
class TestFedFundsConstL1Exog3(MarkovRegression):
    ...
    def setup_class(cls):
        ...
        super().setup_class(
            true, fedfunds[4:], k_regimes=3, exog=np.c_[fedfunds[3:-1], ogap[4:], inf[4:]]
        )
...
class TestAreturnsConstL1Variance(MarkovRegression):
    ...
    def setup_class(cls):
        true = {
            ...
            "bse_oim": np.r_[
                0.0634387, 0.0662574, 0.0782852, 0.2784204, 0.0301862, 0.0857841, np.nan, np.nan
            ],
        }
        super().setup_class(
            true, areturns[1:], k_regimes=2, exog=areturns[:-1], switching_variance=True
        )
...
class TestMumpspcNoconstL1Variance(MarkovRegression):
    ...
    def setup_class(cls):
        true = {
            "params": np.r_[0.762733, 0.1473767, 0.420275, 0.9847369, 0.0562405**2, 0.2611362**2],
            "llf": 131.7225,
            "llf_fit": 131.7225,
            "llf_fit_em": 131.7175,
        }
