...
class SubProgram(object):
    ...
    def add_subprog(self, name, **kwd):
        ...
        prog = SubProgram(self._subparsers.add_parser(name, help=help, **kwd), self._signatures)
    ...
    def _generate_command(self, func, name=None, doctype="rest", *args, **kwargs):
        ...
        if doctype == "numpy":
            config = Config(napoleon_google_docstring=False, napoleon_use_rtype=False)
            doc = str(NumpyDocstring(doc, config))
        elif doctype == "google":
            config = Config(napoleon_numpy_docstring=False, napoleon_use_rtype=False)
