...
googlesql_identifiers = (
    _googlesql_builtins.functionnames + _googlesql_builtins.keywords + _googlesql_builtins.types
)
    ...
    tokens = {
        "root": [
            ...
            (
                ...
                + "|".join(
                    s.replace(" ", r"\s+")
                    for s in _postgres_builtins.DATATYPES + _postgres_builtins.PSEUDO_TYPES
                )
                + r")\b",
                Name.Builtin,
            ),
...
class PlPgsqlLexer(PostgresBase, RegexLexer):
    ...
    for i, pattern in enumerate(tokens["root"]):
        if pattern[1] == Keyword:
            tokens["root"][i] = (
                words(
                    _postgres_builtins.KEYWORDS + _postgres_builtins.PLPGSQL_KEYWORDS, suffix=r"\b"
                ),
                Keyword,
            )
            del i
...
re_message = re.compile(
    r"((?:DEBUG|INFO|NOTICE|WARNING|ERROR|" r"FATAL|HINT|DETAIL|CONTEXT|LINE [0-9]+):)(.*?\n)"
)
...
class MySqlLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (words(_mysql_builtins.MYSQL_CONSTANTS, prefix=r"\b", suffix=r"\b"), Name.Constant),
            (words(_mysql_builtins.MYSQL_DATATYPES, prefix=r"\b", suffix=r"\b"), Keyword.Type),
            (words(_mysql_builtins.MYSQL_KEYWORDS, prefix=r"\b", suffix=r"\b"), Keyword),
            (
        ...
        "optimizer-hints": [
            ...
            (words(_mysql_builtins.MYSQL_OPTIMIZER_HINTS, suffix=r"\b"), Comment.Preproc),
            ("[a-z]+", Comment.Special),
            (r"\*", Comment.Special),
        ],
        "multiline-comment": [
