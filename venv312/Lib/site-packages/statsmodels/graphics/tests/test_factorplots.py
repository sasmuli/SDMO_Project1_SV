...
class TestInteractionPlot:
    ...
    def test_plot_both(self, close_figures):
        fig = interaction_plot(
            self.weight, self.duration, self.days, colors=["red", "blue"], markers=["D", "^"], ms=10
        )
    ...
    def test_formatting(self, close_figures):
        fig = interaction_plot(
            self.weight, self.duration, self.days, colors=["r", "g"], linestyles=["--", "-."]
        )
    ...
    def test_formatting_errors(self, close_figures):
        assert_raises(
            ValueError, interaction_plot, self.weight, self.duration, self.days, markers=["D"]
        )
        assert_raises(
            ValueError,
            interaction_plot,
    ...
    def test_plottype(self, close_figures):
        ...
        assert_equal(isinstance(fig, plt.Figure), True)
        assert_raises(
            ValueError, interaction_plot, self.weight, self.duration, self.days, plottype="unknown"
        )
