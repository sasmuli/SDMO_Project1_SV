...
class TestGMMOLS:
    ...
    @pytest.mark.xfail(
        reason="Not asserting anything meaningful", raises=NotImplementedError, strict=True
    )
    def test_other(self):
        res1, res2 = self.res1, self.res2
...
class TestGMMStOneiterOLS_Linear(CheckGMM):
    ...
    @pytest.mark.xfail(
        reason="RegressionResults has no `Q` attribute", raises=AttributeError, strict=True
    )
    def test_other(self):
        super().test_other()
...
class TestGMMSt2:
    ...
    def test_basic(self):
        ...
        bse = np.sqrt(
            np.diag(
                res1._cov_params(has_optimal_weights=True, weights=res1.weights, use_weights=True)
            )
        )
