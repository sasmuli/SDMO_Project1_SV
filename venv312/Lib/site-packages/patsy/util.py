...
if not have_pandas:
    ...
    if hasattr(pandas, "CategoricalDtype"):  # pandas >= 0.25
        ...
        _pandas_is_categorical_dtype = getattr(pandas.core.common, "is_categorical_dtype", None)
...
def test_atleast_2d_column_default():
    ...
    if have_pandas:
        ...
        assert (
            type(atleast_2d_column_default(pandas.DataFrame([[1], [2]]), preserve_pandas=True))
            == pandas.DataFrame
        )
...
def pandas_friendly_reshape(a, new_shape):
    ...
    raise ValueError("cannot reshape a DataFrame with shape %s to shape %s" % (a.shape, new_shape))
