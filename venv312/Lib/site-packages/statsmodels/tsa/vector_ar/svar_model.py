...
class SVAR(tsbase.TimeSeriesModel):
    ...
    def _estimate_svar(
        self, start_params, lags, maxiter, maxfun, trend="c", solver="nm", override=False
    ):
        """
        lags : int
        trend : {str, None}
    ...
    def loglike(self, params):
        ...
        likl = (
            -nobs
            / 2.0
            * (neqs * np.log(2 * np.pi) - np.log(npl.det(A) ** 2) + b_slogdet + np.trace(trc_in))
        )
    ...
    def _solve_AB(self, start_params, maxiter, override=False, solver="bfgs"):
        ...
        retvals = (
            super()
            .fit(start_params=start_params, method=solver, maxiter=maxiter, gtol=1e-20, disp=False)
            .params
        )
...
class SVARResults(SVARProcess, VARResults):
    ...
    def sirf_errband_mc(
        self, orth=False, repl=1000, steps=10, signif=0.05, seed=None, burn=100, cum=False
    ):
        """
