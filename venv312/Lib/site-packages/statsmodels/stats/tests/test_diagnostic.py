...
class TestDiagnosticG:
    ...
    def test_acorr_ljung_box_big_default(self):
        ...
        ljung_box_none = dict(statistic=51.03724531797195, pvalue=0.11334744923390, distr="chi2")

        # bt = Box.test(residuals(fm), type = "Box-Pierce")
        # mkhtest(bt, "ljung_box_bp_none", "chi2")
        ljung_box_bp_none = dict(statistic=45.12238537034000, pvalue=0.26638168491464, distr="chi2")
    ...
    def test_recursive_residuals(self):
        ...
        rr_order = smsdia.recursive_olsresiduals(self.res, skip=3, alpha=0.95, order_by=order)
        assert_allclose(rr[0], rr_order[0], atol=1e-6)

        # regression number, visually checked with graph from gretl
        ub0 = np.array([13.37318571, 13.50758959, 13.64199346, 13.77639734, 13.91080121])
    ...
    def test_influence(self):
        ...
        assert_almost_equal(infl.resid_studentized_internal, lsdiag["std.res"], decimal=12)
        ...
        assert_almost_equal(infl.resid_studentized_external, lsdiag["stud.res"], decimal=12)
...
def test_spec_white_error(reset_randomstate):
    with pytest.raises(ValueError, match="White's specification test "):
        smsdia.spec_white(np.random.standard_normal(100), np.random.standard_normal((100, 1)))
    with pytest.raises(ValueError, match="White's specification test "):
        smsdia.spec_white(np.random.standard_normal(100), np.random.standard_normal((100, 2)))
