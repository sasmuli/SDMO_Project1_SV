...
ARG_RE = re.compile(r"-(?P<long>-)?" r"(?P<key>(?(long)[^ =,]+|.))[ =]?" r"(?P<meta>[^ ,]+)?")
...
def find_param_docs(docstring):
    ...
    for m in SPHINX_RE.finditer(docstring + "\n"):
        if m.group("field") in ["param", "parameter", "arg", "argument", "key", "keyword"]:
            # mando
            ...
            name, opts, meta = get_opts("{0} {1}".format(vname.strip(), vtype or ""))
            name = name.replace("-", "_")
