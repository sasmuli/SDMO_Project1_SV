...
class TryCLogit:
    ...
    def __init__(self, endog, exog_bychoices, ncommon):
        ...
        beta_indices = [
            np.r_[np.array([0, 1]), z[zi[ii] : zi[ii + 1]]] for ii in range(len(zi) - 1)
        ]
    ...
    def xbetas(self, params):
        ...
        for choiceind in range(self.nchoices):
            res[:, choiceind] = np.dot(
                self.exog_bychoices[choiceind], params[self.beta_indices[choiceind]]
            )
        return res
