...
class _ArpackParams:
    def __init__(
        self, n, k, tp, mode=1, sigma=None, ncv=None, v0=None, maxiter=None, which="LM", tol=0
    ):
        if k <= 0:
...
class SpLuInv(LinearOperator):
    ...
    def _matvec(self, x):
        ...
        if self.isreal and np.issubdtype(x.dtype, np.complexfloating):
            return self.M_lu.solve(np.real(x).astype(self.dtype)) + 1j * self.M_lu.solve(
                np.imag(x).astype(self.dtype)
            )
        else:
...
def get_OPinv_matvec(A, M, sigma, hermitian=False, tol=0):
    ...
    if M is None:
        ...
        if (not isdense(A) and not issparse(A) and not is_pydata_spmatrix(A)) or (
            ...
            return IterOpInv(aslinearoperator(A), aslinearoperator(M), sigma, tol=tol).matvec
...
def eigs(
    ...
    if k >= n - 1:
        warnings.warn(
            "k >= N - 1 for N * N square matrix. " "Attempting to use scipy.linalg.eig instead.",
            RuntimeWarning,
            stacklevel=2,
        )
