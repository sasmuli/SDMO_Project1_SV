...
class DynamicFactor(MLEModel):
    ...
    **kwargs
    ...
    def state_names(self):
        ...
        if self.error_order > 0:
            names += [
                (("e(%s)" % endog_names[j]) if i == 0 else ("e(%s).L%d" % (endog_names[j], i)))
                for i in range(self.error_order)
                for j in range(self.k_endog)
            ]
    ...
    def transform_params(self, unconstrained):
        ...
        if self.enforce_stationarity and self.factor_order > 0:
            ...
            unconstrained_matrices = unconstrained[self._params_factor_transition].reshape(
        ...
        if self.enforce_stationarity and self.error_order > 0:
            ...
            if self.error_var:
                unconstrained_matrices = unconstrained[self._params_error_transition].reshape(
    ...
    def untransform_params(self, constrained):
        ...
        if self.enforce_stationarity and self.error_order > 0:
            ...
            # Joint VAR specification
            if self.error_var:
                constrained_matrices = constrained[self._params_error_transition].reshape(
                    self.k_endog, self._error_order
                )
                start = self.k_factors
                end = self.k_factors + self.k_endog
    ...
    def update(self, params, transformed=True, includes_fixed=False, complex_step=False):
        ...
        # 4. Factor transition VAR
        self.ssm[self._idx_factor_transition] = params[self._params_factor_transition].reshape(
            self.k_factors, self.factor_order * self.k_factors
        )

        # 5. Error transition VAR
        if self.error_var:
            self.ssm[self._idx_error_transition] = params[self._params_error_transition].reshape(
                self.k_endog, self._error_order
            )
        else:
...
wrap.populate_wrapper(DynamicFactorResultsWrapper, DynamicFactorResults)  # noqa:E305
