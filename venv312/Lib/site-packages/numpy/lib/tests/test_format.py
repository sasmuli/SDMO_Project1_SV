...
def test_memmap_roundtrip(tmpdir):
    for i, arr in enumerate(basic_arrays + record_arrays):
        ...
        ma = format.open_memmap(
            mfn, mode="w+", dtype=arr.dtype, shape=arr.shape, fortran_order=fortran_order
        )
        ma[...] = arr
        ma.flush()
...
# titles
dt5 = np.dtype(
    {"names": ["a", "b"], "formats": ["i4", "i4"], "offsets": [1, 6], "titles": ["aa", "bb"]}
)
# empty
...
def test_version_2_0_memmap(tmpdir):
    ...
    # 1.0 requested but data cannot be saved this way
    assert_raises(
        ValueError, format.open_memmap, tf1, mode="w+", dtype=d.dtype, shape=d.shape, version=(1, 0)
    )
...
@pytest.mark.parametrize(
    ...
    [
        np.dtype(
            {"names": ["a", "b"], "formats": [float, np.dtype("S3", metadata={"some": "stuff"})]}
        ),
