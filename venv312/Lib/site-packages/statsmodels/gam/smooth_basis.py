        ...
        if knots is not None:
            if len(knots) != n_inner_knots:
                raise ValueError(
                    ...
                    "but %s knots were provided" % (df, degree, n_inner_knots, len(knots))
                )
...
class UnivariateBSplines(UnivariateGamSmoother):
    ...
    def transform(self, x_new, deriv=0, skip_ctransf=False):
        ...
        exog = _eval_bspline_basis(
            x_new, self.knots, self.degree, deriv=deriv, include_intercept=self.include_intercept
        )
...
class AdditiveGamSmoother(with_metaclass(ABCMeta)):
    ...
    def transform(self, x_new):
        ...
        exog = np.hstack(
            list(self.smoothers[i].transform(x_new[:, i]) for i in range(self.k_variables))
        )
        return exog
    ...
    def _make_smoothers_list(self):
        ...
        for v in range(self.k_variables):
            uv_smoother = UnivariatePolynomialSmoother(
                self.x[:, v], degree=self.degrees[v], variable_name=self.variable_names[v]
            )
