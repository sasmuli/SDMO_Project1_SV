...
def _validate_interval(interval):
    ...
    if interval.ndim != 2 or interval.shape[-1] != 2:
        raise ValueError(
            "`interval` must be a two-dimensional array with "
    ...
    if (interval[:, 0] > interval[:, 1]).any():
        raise ValueError(
            "In each row of `interval`, the left value must not" " exceed the right value."
        )
...
class CensoredData:
    ...
    * interval-censored: an observation lies somewhere on an interval between
    ...
    def __repr__(self):
        ...
        return (
            f"CensoredData(uncensored={uncensored_str}, left={left_str}, "
    ...
    def __sub__(self, other):
        return CensoredData(
            uncensored=self._uncensored - other,
    ...
    def _uncensor(self):
        ...
        """
        data = np.concatenate(
            (self._uncensored, self._left, self._right, self._interval.mean(axis=1))
        )
        return data
