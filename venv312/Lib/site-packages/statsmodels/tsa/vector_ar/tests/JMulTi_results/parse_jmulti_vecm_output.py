...
def sublists(lst, min_elmts=0, max_elmts=None):
    ...
    result = itertools.chain.from_iterable(
        itertools.combinations(lst, sublist_len) for sublist_len in range(min_elmts, max_elmts + 1)
    )
    ...
    for dt_s in dataset.dt_s_list:
        ...
        for line in params_file:
            if section == -1 and section_header[section + 1] not in line:
                continue
            if (
                section < len(section_header) - 1 and section_header[section + 1] in line
            ):  # new section
                section += 1
                continue
        ...
        sigmau_file = (
            "vecm_" + dataset.__str__() + "_" + source + "_" + dt_string + "_Sigmau" + ".txt"
        )
        ...
        lagorder_file = (
            "vecm_" + dataset.__str__() + "_" + source + "_" + dt_string + "_lagorder" + ".txt"
        )
        ...
        test_norm_file = (
            "vecm_" + dataset.__str__() + "_" + source + "_" + dt_string + "_diag" + ".txt"
        )
