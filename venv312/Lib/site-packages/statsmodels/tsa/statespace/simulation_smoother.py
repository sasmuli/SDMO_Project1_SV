...
class SimulationSmoother(KalmanSmoother):
    ...
    **kwargs
        ...
        if simulation_output is None:
            ...
            if simulation_output == 0:
                ...
                argument_set = not all(
                    [simulate_state is None, simulate_disturbance is None, simulate_all is None]
                )
                if argument_set:
                    raise ValueError(
    ...
    def _simulate(
        self, nsimulations, simulator=None, random_state=None, return_simulator=False, **kwargs
    ):
    ...
    def simulator(self, nsimulations, random_state=None):
        return self.simulation_smoother(
            simulation_output=0, method="kfs", nobs=nsimulations, random_state=random_state
        )
        ...
        if initial_state_variates is not None:
            if pretransformed_initial_state_variates:
                ...
                self._simulation_smoother.set_initial_state_variates(
                    np.array(initial_state_variates, dtype=self.dtype), pretransformed=False
                )
