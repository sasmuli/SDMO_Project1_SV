...
@nx._dispatchable(graphs={"G1": 0, "G2": 1}, preserve_edge_attrs=True, preserve_node_attrs=True)
...
def optimize_edit_paths(
            ...
            for k, i in enumerate(g_ind):
                ...
                for l, j in enumerate(h_ind):
                    ...
                    if nx.is_directed(G1) or nx.is_directed(G2):
                        ...
                        if any(
                            g in ((p, u), (u, p)) and h in ((q, v), (v, q)) for p, q in matched_uv
                        ):
                            continue
    ...
    def get_edit_ops(matched_uv, pending_u, pending_v, Cv, pending_g, pending_h, Ce, matched_cost):
        ...
        if m <= n:
            candidates = (
                (t, fixed_j) for t in range(m + n) if t != fixed_i and (t < m or t == m + fixed_j)
            )
        else:
            candidates = (
                (fixed_i, t) for t in range(m + n) if t != fixed_j and (t < n or t == n + fixed_i)
            )
...
def panther_similarity(G, source, k=5, path_length=5, c=0.5, delta=0.1, eps=None, weight="weight"):
    ...
    top_k_with_val = dict(zip(node_map[top_k_sorted].tolist(), S[top_k_sorted].tolist()))
...
def generate_random_paths(
    ...
    randint_fn = seed.integers if isinstance(seed, np.random.Generator) else seed.randint
    ...
    for path_index in range(sample_size):
        ...
        for _ in range(path_length):
            ...
            nbr_index = seed.choice(num_nodes, p=transition_probabilities[starting_index])
