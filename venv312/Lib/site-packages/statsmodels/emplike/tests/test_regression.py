...
class TestRegressionPowell(GenRes):
    ...
    def test_hypothesis_beta1(self):
        beta1res = self.res1.el_test([0.5], [1], return_weights=1, method="powell")
    ...
    def test_ci_beta0(self):
        beta0ci = self.res1.conf_int_el(0, lower_bound=-52.9, upper_bound=-24.1, method="powell")
    ...
    def test_ci_beta1(self):
        beta1ci = self.res1.conf_int_el(1, lower_bound=0.418, upper_bound=0.986, method="powell")
        assert_almost_equal(beta1ci, self.res2.test_ci_beta1, 4)

    @pytest.mark.slow
    def test_ci_beta2(self):
        beta2ci = self.res1.conf_int_el(2, lower_bound=0.59, upper_bound=2.2, method="powell")
        assert_almost_equal(beta2ci, self.res2.test_ci_beta2, 5)

    @pytest.mark.slow
    def test_ci_beta3(self):
        beta3ci = self.res1.conf_int_el(3, lower_bound=-0.39, upper_bound=0.01, method="powell")
...
class TestRegressionNM(GenRes):
    ...
    def test_hypothesis_beta1(self):
        beta1res = self.res1.el_test([0.5], [1], return_weights=1, method="nm")
        assert_almost_equal(beta1res[:2], self.res2.test_beta1[:2], 4)
        assert_almost_equal(beta1res[2], self.res2.test_beta1[2], 4)
