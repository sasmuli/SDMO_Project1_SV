...
class ParameterConstraint:
    ...
    def __init__(self, lhs, rhs, exog):
        ...
        if len(rhs) != lhs.shape[0]:
            raise ValueError(
                "The number of rows of the left hand "
...
class GEE(GLM):
        ...
        **kwargs,
        ...
        if family is not None:
            if not isinstance(family.link, tuple(family.safe_links)):
                ...
                warnings.warn(
                    msg.format(family.link.__class__.__name__, family.__class__.__name__),
                    DomainWarning,
                )
        ...
        if self._offset_exposure is None or (
            np.isscalar(self._offset_exposure) and self._offset_exposure == 0.0
        ):
            self.offset_li = None
        else:
    ...
    def estimate_scale(self):
        ...
        if self.scaletype is None:
            if isinstance(
                self.family,
                (families.Binomial, families.Poisson, families.NegativeBinomial, _Multinomial),
            ):
                return 1.0
    ...
    def _starting_params(self):
        ...
        model = GLM(
            self.endog, self.exog, family=self.family, offset=offset, freq_weights=self.weights
        )
        result = model.fit()
    ...
    def fit(
        ...
        if self.constraint is not None:
            ...
            if mean_params is None:
                warnings.warn(
                    "Unable to estimate constrained GEE " "parameters.", ConvergenceWarning
                )
                return None
    ...
    def _handle_constraint(self, mean_params, bcov):
        ...
        score_cov += np.dot(
            amat_12.T, np.dot(np.linalg.solve(amat_11, bmat_11), np.linalg.solve(amat_11, amat_12))
        )
...
class GEEResults(GLMResults):
    ...
    def qic(self, scale=None, n_step=1000):
        ...
        if scale is None:
            warnings.warn(
                "QIC values obtained using scale=None are not " "appropriate for comparing models"
            )
    ...
    def summary(self, yname=None, xname=None, title=None, alpha=0.05):
        ...
        top_left = [
            ...
            ("Dependence structure:", [self.model.cov_struct.__class__.__name__]),
            ("Date:", None),
            (
                "Covariance type: ",
                [
        ...
        smry.add_table_2cols(
            self, gleft=diagn_left, gright=diagn_right, yname=yname, xname=xname, title=""
        )
...
class OrdinalGEE(GEE):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            endog, exog, groups, time, family, cov_struct, missing, offset, dep_data, constraint
        )
    ...
    def fit(
        ...
        rslt = super().fit(
            maxiter, ctol, start_params, params_niter, first_dep_update, cov_type=cov_type
        )
...
class GEEMargins:
    ...
    def summary_frame(self, alpha=0.05):
        ...
        names = [
            _transform_names[self.margeff_options["method"]],
            "Std. Err.",
            "z",
            "Pr(>|z|)",
            "Conf. Int. Low",
        ...
        table = np.column_stack(
            (self.margeff, self.margeff_se, self.tvalues, self.pvalues, self.conf_int(alpha))
        )
