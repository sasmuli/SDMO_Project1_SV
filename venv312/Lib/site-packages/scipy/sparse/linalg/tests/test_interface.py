...
class TestLinearOperator:
    ...
    def test_matmul(self):
        ...
        assert_equal(operator.matmul(b.reshape(1, -1), A.adjoint()), b.reshape(1, -1) * A.adjoint())
        assert_equal(operator.matmul(B, A.H), B @ A.H)
        assert_equal(operator.matmul(B, A.adjoint()), B @ A.adjoint())
...
class TestAsLinearOperator:
    def setup_method(self):
        ...
        self.cases += [
            (interface.aslinearoperator(M).T, A.T) for M, A in make_cases(original.T, np.float64)
        ]
        self.cases += [
        ...
        self.cases += [
            (interface.aslinearoperator(M).T, A.T) for M, A in make_cases(original.T, np.complex128)
        ]
        self.cases += [
            (interface.aslinearoperator(M).H, A.T.conj())
