...
from statsmodels.gam.generalized_additive_model import GLMGam, GLMGamResults, GLMGamResultsWrapper
...
class CheckGAMMixin:
    ...
    def test_params(self):
        ...
        assert_allclose(
            np.asarray(res1.cov_params()), res2.Vp * self.covp_corrfact, rtol=1e-6, atol=1e-9
        )

        assert_allclose(res1.scale, res2.scale * self.covp_corrfact, rtol=1e-8)

        assert_allclose(
            np.asarray(res1.bse), res2.se * np.sqrt(self.covp_corrfact), rtol=1e-6, atol=1e-9
        )
...
class TestTheilPLS5(CheckGAMMixin):
    ...
    def test_cov_robust(self):
        ...
        assert_allclose(np.asarray(res1.cov_params()), res2.Ve * self.covp_corrfact, rtol=1e-4)
...
class TestGLMPenalizedPLS5(CheckGAMMixin):
    ...
    def setup_class(cls):
        ...
        cls.res1 = mod.fit(
            cov_type=cls.cov_type, method="bfgs", maxiter=100, disp=0, trim=False, scale="x2"
        )
        cls.res2 = results_pls.pls5
...
class TestGAMMPG:
    ...
    def setup_class(cls):
        ...
        gam_cc = GLMGam(
            df_autos["city_mpg"], exog=exog, smoother=cc, alpha=(1 / s_scale * sp / 2).tolist()
        )
...
class TestGAMMPGBSPoisson(CheckGAMMixin):
    ...
    def setup_class(cls):
        ...
        alpha0 = 1 / s_scale * sp / 2
        gam_bs = GLMGam(
            df_autos["city_mpg"], exog=cls.exog, smoother=bs, family=family.Poisson(), alpha=alpha0
        )
    ...
    def test_wald(self):
        ...
        wtt = res1.wald_test_terms(
            skip_single=True, combine_terms=["fuel", "drive", "weight", "hp"], scalar=True
        )
...
class TestGAMMPGBSPoissonFormula(TestGAMMPGBSPoisson):
    ...
    def setup_class(cls):
        ...
        gam_bs = GLMGam.from_formula(
            "city_mpg ~ fuel + drive", df_autos, smoother=bs, family=family.Poisson(), alpha=alpha0
        )
