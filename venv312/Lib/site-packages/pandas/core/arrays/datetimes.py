    ...
    def _generate_range(
        ...
        *,
        ...
        if freq is not None:
            ...
            i8values = (
                np.linspace(0, end._value - start._value, periods, dtype="int64") + start._value
            )
...
def _maybe_infer_tz(tz: tzinfo | None, inferred_tz: tzinfo | None) -> tzinfo | None:
    ...
    if tz is None:
        ...
        raise TypeError(
            f"data is already tz-aware {inferred_tz}, unable to " f"set specified tz: {tz}"
        )
    return tz
...
def _validate_dt64_dtype(dtype):
    ...
    if dtype is not None:
        ...
        if (
            isinstance(dtype, np.dtype) and (dtype.kind != "M" or not is_supported_dtype(dtype))
