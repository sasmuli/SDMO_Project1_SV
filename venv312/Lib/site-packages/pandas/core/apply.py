...
class Apply(metaclass=abc.ABCMeta):
    ...
    def compute_dict_like(
        ...
        is_non_unique_col = selected_obj.ndim == 2 and selected_obj.columns.nunique() < len(
            selected_obj.columns
        )
...
class FrameApply(NDFrameApply):
    ...
    def validate_values_for_numba(self):
        ...
        for colname, dtype in self.obj.dtypes.items():
            if not is_numeric_dtype(dtype):
                raise ValueError(
                    f"Column {colname} must have a numeric dtype. " f"Found '{dtype}' instead"
                )
    ...
    def apply_raw(self, engine="python", engine_kwargs=None):
        ...
        if engine == "numba":
            ...
            nb_looper = generate_apply_looper(self.func, **engine_kwargs)  # type: ignore[arg-type]
