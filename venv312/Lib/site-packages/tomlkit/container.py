...
class Container(_CustomDict):
    ...
    def add(self, key: Key | Item | str, item: Item | None = None) -> Container:
        ...
        if item is None:
            if not isinstance(key, (Comment, Whitespace)):
                raise ValueError("Non comment/whitespace items must have an associated key")
    ...
    def append(self, key: Key | str | None, item: Item, validate: bool = True) -> Container:
        ...
        if (
            ...
            if last_index < len(self._body):
                ...
                if not (isinstance(after_item, Whitespace) or "\n" in after_item.trivia.indent):
    ...
    def _render_table(self, key: Key, table: Table, prefix: str | None = None) -> str:
        ...
        for k, v in table.value.body:
            if isinstance(v, Table):
                ...
                cur += self._render_simple_item(k, v, prefix=_key if key.is_dotted() else None)
...
def ends_with_whitespace(it: Any) -> bool:
    ...
    """
    return (isinstance(it, Table) and isinstance(it.value._previous_item(), Whitespace)) or (
