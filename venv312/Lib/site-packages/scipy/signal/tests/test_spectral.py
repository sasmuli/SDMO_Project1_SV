...
class TestWelch:
    ...
    def test_detrend_external_nd_0(self):
        ...
        x = np.moveaxis(x, 2, 0)
        f, p = welch(
            x, nperseg=10, axis=0, detrend=lambda seg: signal.detrend(seg, axis=0, type="l")
        )
        assert_allclose(p, np.zeros_like(p), atol=1e-15)
    ...
    def test_window_correction(self):
        ...
        for window in ["hann", "bartlett", ("tukey", 0.1), "flattop"]:
            ...
            assert_allclose(np.sqrt(trapezoid(p_dens, freq)), A * np.sqrt(2) / 2, rtol=1e-3)
...
class TestCSD:
    ...
    def test_detrend_external_nd_0(self):
        ...
        x = np.moveaxis(x, 2, 0)
        f, p = csd(
            x, x, nperseg=10, axis=0, detrend=lambda seg: signal.detrend(seg, axis=0, type="l")
        )
        assert_allclose(p, np.zeros_like(p), atol=1e-15)
...
class TestLombscargle:
    ...
    def test_floating_mean_false(self):
        ...
        pgram_offset = lombscargle(t, y + offset, f, normalize=True, floating_mean=False)
...
class TestSTFT:
    ...
    def test_average_all_segments(self):
        ...
        f, _, Z = stft(
            x, fs, window, nperseg, noverlap, padded=False, return_onesided=False, boundary=None
        )
        fw, Pw = welch(
            x,
            fs,
    ...
    def test_roundtrip_float32(self):
        ...
        for window, N, nperseg, noverlap in settings:
            ...
            _, _, zz = stft(
                x, nperseg=nperseg, noverlap=noverlap, window=window, detrend=None, padded=False
            )
    ...
    def test_roundtrip_padded_signal(self):
        ...
        for window, N, nperseg, noverlap in settings:
            ...
            _, _, zz = stft(
                x, nperseg=nperseg, noverlap=noverlap, window=window, detrend=None, padded=True
            )
...
class TestSampledSpectralRepresentations:
    ...
    def test_windowed_DFT(self):
        ...
        assert_allclose(
            welch(self.x_ref, scaling="spectrum", **kw)[1], P_mag, atol=float_res * max(P_mag)
        )
        assert_allclose(
            welch(self.x_ref, scaling="density", **kw)[1], P_psd, atol=float_res * max(P_psd)
        )
