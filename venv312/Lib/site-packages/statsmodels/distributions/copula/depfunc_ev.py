...
class AsymLogistic(PickandDependence):
    ...
    def deriv(self, t, a1, a2, theta):
        ...
        d1 = (
            (a1 * (a1 * t) ** (1 / b - 1) - a2 * (a2 * (1 - t)) ** (1 / b - 1))
            * ((a1 * t) ** (1 / b) + (a2 * (1 - t)) ** (1 / b)) ** (b - 1)
            - a1
            + a2
        )
        return d1
...
class TEV(PickandDependence):
    ...
    def evaluate(self, t, rho, df):
        ...
        term1 = np.power(t / (1.0 - t), 1.0 / x) - rho  # for t
        term2 = np.power((1.0 - t) / t, 1.0 / x) - rho  # for 1-t
