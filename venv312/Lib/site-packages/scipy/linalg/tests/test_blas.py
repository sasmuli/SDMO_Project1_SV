...
def test_get_blas_funcs():
    ...
    f1 = get_blas_funcs(
        "axpy", (np.empty((2, 2), dtype=np.float64), np.empty((2, 2), dtype=np.complex64))
    )
    assert_equal(f1.typecode, "z")
...
class TestBLAS3Syr2k:
    ...
    def test_syr2k_wrong_c(self):
        ...
        if f is not None:
            assert_raises(
                Exception, f, **{"a": self.a, "b": self.b, "alpha": 1.0, "c": np.zeros((15, 8))}
            )
        # if C is supplied, it must have compatible dimensions
    ...
    def test_b_overwrites(self):
        ...
        if f is not None:
            for overwr in [True, False]:
                ...
                assert_(
                    bcopy.flags.f_contiguous is False
                    and np.may_share_memory(bcopy, result) is False
                )
