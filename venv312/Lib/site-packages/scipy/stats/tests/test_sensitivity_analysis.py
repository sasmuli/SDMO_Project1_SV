...
class TestSobolIndices:
    ...
    dists = [stats.uniform(loc=-np.pi, scale=2 * np.pi)] * 3  # type: ignore[attr-defined]
    ...
    def test_sample_AB(self):
        ...
        ref = np.array(
            [
                [[101, 104, 107, 110], [2, 5, 8, 11], [3, 6, 9, 12]],
                [[1, 4, 7, 10], [102, 105, 108, 111], [3, 6, 9, 12]],
                [[1, 4, 7, 10], [2, 5, 8, 11], [103, 106, 109, 112]],
            ]
        )
    ...
    def test_ishigami(self, ishigami_ref_indices, func):
        ...
        res = sobol_indices(func=func, n=4096, dists=self.dists, rng=rng)
    ...
    def test_func_dict(self, ishigami_ref_indices):
        ...
        res = sobol_indices(func=func, n=n, dists=dists, rng=rng)
        assert_allclose(res.first_order, ishigami_ref_indices[0], atol=1e-2)

        res = sobol_indices(func=func, n=n, rng=rng)
        assert_allclose(res.first_order, ishigami_ref_indices[0], atol=1e-2)
    ...
    def test_method(self, ishigami_ref_indices):
        ...
        rng = np.random.default_rng(28631265345463262246170309650372465332)
        res = sobol_indices(func=f_ishigami, n=4096, dists=self.dists, method=jansen_sobol, rng=rng)
        ...
        _ = sobol_indices(
            func=f_ishigami, n=8, dists=self.dists, method=jansen_sobol_typed, rng=rng
        )

    def test_normalization(self, ishigami_ref_indices):
        rng = np.random.default_rng(28631265345463262246170309650372465332)
        res = sobol_indices(func=lambda x: f_ishigami(x) + 1000, n=4096, dists=self.dists, rng=rng)
    ...
    def test_constant_function(self, ishigami_ref_indices):
        ...
        rng = np.random.default_rng(28631265345463262246170309650372465332)
        res = sobol_indices(func=f_ishigami_vec_const, n=4096, dists=self.dists, rng=rng)
    ...
    def test_more_converged(self, ishigami_ref_indices):
        rng = np.random.default_rng(28631265345463262246170309650372465332)
        res = sobol_indices(func=f_ishigami, n=2**19, dists=self.dists, rng=rng)  # 524288
