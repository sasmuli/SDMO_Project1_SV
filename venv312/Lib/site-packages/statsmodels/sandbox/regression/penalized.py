...
class TheilGLS(GLS):
    ...
    def __init__(self, endog, exog, r_matrix=None, q_matrix=None, sigma_prior=None, sigma=None):
        ...
        if k_exog != self.exog.shape[1]:
            raise ValueError("r_matrix needs to have the same number of columns" "as exog")
    ...
    def fit(self, pen_weight=1.0, cov_type="sandwich", use_t=True):
        ...
        xpy = np.dot(x.T, y) + sigma2_e * lambd * np.dot(
            r_matrix.T, np.dot(sigma_prior_inv, q_matrix)
        )
        # xpy = xpy[:,None]
        ...
        lfit = TheilRegressionResults(
            self, params, normalized_cov_params=normalized_cov_params, use_t=use_t
        )
