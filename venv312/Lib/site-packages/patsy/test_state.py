...
def check_stateful(cls, accepts_multicolumn, input, output, *args, **kwargs):
    ...
    if have_pandas:
        ...
        test_cases += [
            ...
            (
                [pandas.Series([x], index=[idx]) for (x, idx) in zip(input, pandas_index)],
                output_1d,
            ),
        ]
        if accepts_multicolumn:
...
def test_Standardize():
    ...
    check_stateful(Standardize, True, r20, list((np.arange(20) - 9.5) / 5.7662812973353983), ddof=0)
    check_stateful(Standardize, True, r20, list((np.arange(20) - 9.5) / 5.9160797830996161), ddof=1)
