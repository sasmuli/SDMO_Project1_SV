...
def parse(path, ssm):
    ...
    res["forecasts_error_diffuse_cov"] = np.c_[
        [np.diag(x) for y, x in dta["Finf"].iloc[:n].iterrows()]
    ].T
    ...
    res["smoothed_measurement_disturbance_cov"] = np.c_[
        [np.diag(x) for y, x in dta["V_eps"].iloc[:n].iterrows()]
    ].T
    ...
    # symmetric
    res["scaled_smoothed_diffuse1_estimator_cov"] = (
        dta["N1"].values.reshape(n + 1, m, m, order="F").transpose(1, 2, 0)
    )
    res["scaled_smoothed_diffuse2_estimator_cov"] = (
        dta["N2"].values.reshape(n + 1, m, m, order="F").transpose(1, 2, 0)
    )
