...
def quantiles(
    a, prob=list([0.25, 0.5, 0.75]), alphap=0.4, betap=0.4, axis=None, limit=(), masknan=False
):
    """
...
def plotting_positions_w1d(data, weights=None, alpha=0.4, beta=0.4, method="notnormed"):
    ...
    if weights is None:
        ...
        if weights.shape != x.shape:
            raise ValueError("if weights is given, it needs to be the same" "shape as data")
    n = len(x)
    xargsort = x.argsort()
