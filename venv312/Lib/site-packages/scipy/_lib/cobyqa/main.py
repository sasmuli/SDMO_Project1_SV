...
def minimize(
    ...
    **kwargs,
    ...
    if verbose:
        ...
        print(f"Maximum number of function evaluations: " f"{options[Options.MAX_EVAL]}.")
        ...
        if s_norm <= constants[Constants.SHORT_STEP_THRESHOLD] * framework.resolution:
            ...
            if same_best_point:
                ...
                if (
                    ...
                    > constants[Constants.BYRD_OMOJOKUN_FACTOR] ** 2.0 * framework.radius
                ):
...
def _get_bounds(bounds, n):
    ...
    if bounds is None:
        ...
        if bounds.shape != (n, 2):
            raise ValueError(
                "The shape of the bounds is not compatible with " "the number of variables."
            )
        return Bounds(bounds[:, 0], bounds[:, 1])
    else:
        raise TypeError(
            "The bounds must be an instance of " "scipy.optimize.Bounds or an array-like object."
        )
...
def _set_default_options(options, n):
    ...
    if Options.NPT in options and options[Options.NPT] > ((n + 1) * (n + 2)) // 2:
        raise ValueError(
            f"The number of interpolation points must be at most " f"{((n + 1) * (n + 2)) // 2}."
        )
...
def _set_default_constants(**kwargs):
    ...
    if (
        ...
        raise ValueError(
            "The constant penalty_increase_threshold must be " "greater than or equal to 1."
        )
    if (
