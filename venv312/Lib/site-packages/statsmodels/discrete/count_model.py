...
class GenericZeroInflated(CountModel):
    ...
    def fit_regularized(
        ...
        **kwargs,
        ...
        alpha_p = (
            alpha[: -(self.k_extra - extra)] if (self.k_extra and np.size(alpha) > 1) else alpha
        )
        if start_params is None:
            offset = getattr(self, "offset", 0) + getattr(self, "exposure", 0)
            if np.size(offset) == 1 and offset == 0:
        ...
        for i in range(self.k_exog):
            for j in range(i, -1, -1):
                hess_arr[i, j] = (
                    ...
                    * mu[zero_idx]
                    * (w[zero_idx] - 1)
                    * (1 / coeff - w[zero_idx] * mu[zero_idx] * np.exp(mu[zero_idx]) / coeff**2)
                ).sum() - (
                    mu[nonzero_idx] * self.exog[nonzero_idx, i] * self.exog[nonzero_idx, j]
                ).sum()
...
wrap.populate_wrapper(
    L1ZeroInflatedGeneralizedPoissonResultsWrapper, L1ZeroInflatedGeneralizedPoissonResults
)
