...
def test_unfold():
    ...
    for mode in range(T.ndim(X)):
        ...
        assert_array_equal(T.reshape(unfolding, (-1,)), T.reshape(unfoldings[mode], (-1,)))
...
def test_partial_unfold():
    ...
    ###################################
    tensor = T.tensor(
        np.concatenate([np.arange(24).reshape((1, 3, 4, 2)) + i for i in range(n_samples)])
    )
    t = T.tensor(X)
    ...
    ##################################
    tensor = T.tensor(
        np.concatenate([np.arange(24).reshape((3, 4, 2, 1)) + i for i in range(n_samples)], axis=-1)
    )
    for i in range(T.ndim(X)):  # test for each mode
