...
def _check_order(order: int | Sequence[int] | None, causal: bool) -> bool:
    ...
    if isinstance(order, (int, np.integer)):
        if int(order) < int(causal):
            raise ValueError(
                f"integer orders must be at least {int(causal)} when causal " f"is {causal}."
            )
        return True
    for v in order:
    ...
    if int(causal) and min(order) < 1:
        raise ValueError("sequence orders must be strictly positive when causal is True")
    return True
...
class ARDL(AutoReg):
    ...
    def _construct_regressors(self, hold_back: int | None) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self._hold_back < self._maxlag:
            raise ValueError(
                "hold_back must be >= the maximum lag of the endog and exog " "variables"
            )
    ...
    def predict(
        ...
        def check_exog(arr, name, orig, exact):
            if isinstance(orig, pd.DataFrame):
                if not isinstance(arr, pd.DataFrame):
                    raise TypeError(
                        f"{name} must be a DataFrame when the original exog " "was a DataFrame"
                    )
            ...
            if exact and arr.shape[0] != orig.shape[0]:
                raise ValueError(
                    f"{name} must have the same number of rows as the " f"original data ({n})."
                )
            return arr
        ...
        if num_oos > max_1step:
            ...
            if self._fixed.shape[1] and fixed_oos is None:
                raise ValueError(
                    "fixed_oos must be provided when predicting " "out-of-sample observations"
                )
...
class UECMResults(ARDLResults):
    ...
    def bounds_test(
        ...
        nsim: int = 100_000,
        seed: int | Sequence[int] | np.random.RandomState | np.random.Generator | None = None,
    ):
        r"""
