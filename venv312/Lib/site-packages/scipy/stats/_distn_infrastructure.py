...
def argsreduce(cond, *args):
    ...
    return [
        (arg if np.size(arg) == 1 else np.extract(cond, np.broadcast_to(arg, s))) for arg in newargs
    ]
    ...
    def _construct_doc(self, docdict, shapes_vals=None):
        ...
        for i in range(2):
            ...
            try:
                ...
                raise Exception(
                    "Unable to construct docstring for " f'distribution "{self.name}": {repr(e)}'
                ) from e
    ...
    **Deepcopying / Pickling**
    ...
    def _reduce_func(self, args, kwds, data=None):
        ...
        if method == "mm":
            ...
            raise ValueError(f"Method '{method}' not available; " f"must be one of {methods}")
...
class rv_sample(rv_discrete):
    ...
    def _pmf(self, x):
        return np.select(
            [x == k for k in self.xk], [np.broadcast_arrays(p, x)[0] for p in self.pk], 0
        )
