...
class TestDualAnnealing:
    ...
    def test_high_dim_no_ls(self):
        ret = dual_annealing(self.func, self.hd_bounds, no_local_search=True, rng=self.seed)
    ...
    def test_nb_fun_call_no_ls(self):
        self.nb_fun_call.c = 0
        ret = dual_annealing(self.func, self.ld_bounds, no_local_search=True, rng=self.seed)
        assert_equal(self.nb_fun_call.c, ret.nfev)
    ...
    def test_fun_args_ls(self):
        ret = dual_annealing(self.func, self.ld_bounds, args=((3.14159,)), rng=self.seed)
    ...
    def test_fun_args_no_ls(self):
        ret = dual_annealing(
            self.func, self.ld_bounds, args=((3.14159,)), no_local_search=True, rng=self.seed
        )
    ...
    def test_callback_stop(self):
        ...
        ret = dual_annealing(self.func, self.ld_bounds, callback=self.callback, rng=self.seed)
        assert ret.fun <= 1.0
    ...
    def test_multi_ls_minimizer(self, method, atol):
        ret = dual_annealing(
            self.func, self.ld_bounds, minimizer_kwargs=dict(method=method), rng=self.seed
        )
    ...
    def test_gradient_gnev(self):
        ...
        ret = dual_annealing(rosen, self.ld_bounds, minimizer_kwargs=minimizer_opts, rng=self.seed)
    ...
    def test_callable_jac_hess_with_args_gh11052(self):
        ...
        def hess(x, power):
            ...
            return np.diag(
                power * np.exp(x**power) * x ** (power - 2) * (power * x**power + power - 1)
            )
        ...
        res1 = dual_annealing(
            f, args=(2,), bounds=[[0, 1], [0, 1]], rng=rng, minimizer_kwargs=dict(method="L-BFGS-B")
        )
        res2 = dual_annealing(
            f,
            args=(2,),
