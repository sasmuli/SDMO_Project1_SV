...
class _OptFuncts:
    ...
    def _opt_var(self, nuisance_mu, pval=False):
        ...
        eta_star = self._modif_newton(
            np.array([1.0 / nobs, 1.0 / nobs]), est_vect, np.ones(nobs) * (1.0 / nobs)
        )
    ...
    def _opt_skew(self, nuis_params):
        ...
        eta_star = self._modif_newton(
            np.array([1.0 / nobs, 1.0 / nobs, 1.0 / nobs]), est_vect, np.ones(nobs) * (1.0 / nobs)
        )
    ...
    def _opt_skew_kurt(self, nuis_params):
        ...
        eta_star = self._modif_newton(
            np.array([1.0 / nobs, 1.0 / nobs, 1.0 / nobs, 1.0 / nobs]),
            est_vect,
            np.ones(nobs) * (1.0 / nobs),
        )
        denom = 1.0 + np.dot(eta_star, est_vect.T)
        self.new_weights = 1.0 / nobs * 1.0 / denom
        llr = np.sum(np.log(nobs * self.new_weights))
...
class DescStatUV(_OptFuncts):
    ...
    def ci_mean(
        self, sig=0.05, method="gamma", epsilon=10**-8, gamma_low=-(10**10), gamma_high=10**10
    ):
        """
    ...
    def ci_skew(self, sig=0.05, upper_bound=None, lower_bound=None):
        ...
        if upper_bound is None:
            upper_bound = (
                skew(endog)
                + 2.5
                * ((6.0 * nobs * (nobs - 1.0)) / ((nobs - 2.0) * (nobs + 1.0) * (nobs + 3.0)))
                ** 0.5
            )
        if lower_bound is None:
            lower_bound = (
    ...
    def ci_kurt(self, sig=0.05, upper_bound=None, lower_bound=None):
        ...
        if upper_bound is None:
            upper_bound = kurtosis(endog) + (
                ...
                * (((nobs**2.0) - 1.0) / ((nobs - 3.0) * (nobs + 5.0))) ** 0.5
            )
        if lower_bound is None:
...
class DescStatMV(_OptFuncts):
    ...
    def mv_test_mean(self, mu_array, return_weights=False):
        ...
        if len(mu_array) != endog.shape[1]:
            raise ValueError(
                "mu_array must have the same number of " "elements as the columns of the data."
            )
    ...
    def test_corr(self, corr0, return_weights=0):
        ...
        nuis0 = np.array(
            [endog[:, 0].mean(), endog[:, 0].var(), endog[:, 1].mean(), endog[:, 1].var()]
        )
    ...
    def ci_corr(self, sig=0.05, upper_bound=None, lower_bound=None):
        ...
        if upper_bound is None:
            upper_bound = min(
                0.999, point_est + 2.5 * ((1.0 - point_est**2.0) / (nobs - 2.0)) ** 0.5
            )

        if lower_bound is None:
            lower_bound = max(
                -0.999, point_est - 2.5 * (np.sqrt((1.0 - point_est**2.0) / (nobs - 2.0)))
            )
