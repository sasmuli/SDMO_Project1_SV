...
class TestLocalMaxima1d:
    ...
    def test_flat_maxima(self):
        ...
        x = np.array(
            [-1.3, 0, 1, 0, 2, 2, 0, 3, 3, 3, 2.99, 4, 4, 4, 4, -10, -5, -5, -5, -5, -5, -10]
        )
...
class TestArgrel:
    ...
    def test_basic(self):
        ...
        x = np.array(
            [[1, 2, 2, 3, 2], [2, 1, 2, 2, 3], [3, 2, 1, 2, 2], [2, 3, 2, 1, 2], [1, 2, 3, 2, 1]]
        )
...
class TestPeakWidths:
    ...
    def test_intersection_rules(self):
        ...
        xp_assert_close(peak_widths(x, peaks=[5], rel_height=0), [(0.0,), (3.0,), (5.0,), (5.0,)])
        # width_height == x counts as intersection -> nearest 1 is chosen
        xp_assert_close(
            peak_widths(x, peaks=[5], rel_height=2 / 3), [(4.0,), (1.0,), (3.0,), (7.0,)]
        )
...
class TestFindPeaks:
    ...
    def test_plateau_size(self):
        ...
        xp_assert_equal(props["left_edges"], peaks - (plateau_sizes - 1) // 2, check_dtype=False)
        xp_assert_equal(props["right_edges"], peaks + plateau_sizes // 2, check_dtype=False)

        # Test conditions
        xp_assert_equal(find_peaks(x, plateau_size=4)[0], [11, 18, 33, 100], check_dtype=False)
        xp_assert_equal(find_peaks(x, plateau_size=(None, 3.5))[0], [1, 3, 7], check_dtype=False)
        xp_assert_equal(find_peaks(x, plateau_size=(5, 50))[0], [18, 33], check_dtype=False)
    ...
    def test_height_condition(self):
        ...
        xp_assert_equal(props["peak_heights"], np.array([1 / 3, 2.5, 4.0]), check_dtype=False)
        xp_assert_equal(find_peaks(x, height=0.5)[0], np.array([3, 5]), check_dtype=False)
        xp_assert_equal(find_peaks(x, height=(None, 3))[0], np.array([1, 3]), check_dtype=False)
        xp_assert_equal(find_peaks(x, height=(2, 3))[0], np.array([3]), check_dtype=False)
    ...
    def test_threshold_condition(self):
        ...
        xp_assert_equal(find_peaks(x, threshold=3.5)[0], np.array([], dtype=int), check_dtype=False)
        xp_assert_equal(find_peaks(x, threshold=(None, 5))[0], np.array([1, 3]), check_dtype=False)
        xp_assert_equal(find_peaks(x, threshold=(None, 4))[0], np.array([1]), check_dtype=False)
        xp_assert_equal(
            find_peaks(x, threshold=(2, 4))[0], np.array([], dtype=int), check_dtype=False
        )
    ...
    def test_prominence_condition(self):
        ...
        xp_assert_equal(properties["right_bases"], peaks_true[keep] + 1, check_dtype=False)
    ...
    @pytest.mark.filterwarnings(
        "ignore:some peaks have a prominence of 0", "ignore:some peaks have a width of 0"
    )
...
class TestFindPeaksCwt:
    ...
    def test_find_peaks_window_size(self):
        ...
        found_locs = find_peaks_cwt(
            test_data, widths, gap_thresh=2, min_snr=3, min_length=None, window_size=None
        )
