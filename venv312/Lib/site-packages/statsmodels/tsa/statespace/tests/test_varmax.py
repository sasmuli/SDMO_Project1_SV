    ...
    def test_results(self):
        ...
        if model.k_ar > 0:
            ...
            coefficient_matrices = np.array(
                [
                    coefficients[: model.k_endog, i * model.k_endog : (i + 1) * model.k_endog]
...
class TestVAR(CheckLutkepohl):
    ...
    def setup_class(cls):
        ...
        super().setup_class(true, order=(1, 0), trend="n", error_cov_type="unstructured")
...
class TestVAR_measurement_error(CheckLutkepohl):
    ...
    def setup_class(cls):
        ...
        super().setup_class(
            true, order=(1, 0), trend="n", error_cov_type="diagonal", measurement_error=True
        )
...
class TestVAR_exog(CheckLutkepohl):
    ...
    def test_summary(self):
        ...
        for i in range(self.model.k_endog):
            ...
            assert re.search("L1.dln_consump +%.4f" % params[offset + 2], table)
            assert re.search(
                "beta.x1 +" + forg(params[self.model._params_regression][i], prec=4), table
            )
...
class TestVARMA(CheckFREDManufacturing):
    ...
    def test_summary(self):
        ...
        for i in range(self.model.k_endog):
            ...
            assert re.search(r"L1.e\(dlncaputil\) +" + forg(params[offset_ma + 0], prec=4), table)
            assert re.search(r"L1.e\(dlnhours\) +" + forg(params[offset_ma + 1], prec=4), table)
