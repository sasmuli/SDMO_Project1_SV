...
def test_invalid():
    ...
    assert_raises(
        ValueError, mod.fit, method="statespace", method_kwargs={"enforce_stationarity": False}
    )

    # start_params only valid for MLE methods
    assert_raises(ValueError, mod.fit, method="yule_walker", start_params=[0.5, 1.0])
...
def test_clone():
    ...
    # Concentrated scale
    check_cloned(ARIMA(endog, exog=exog, trend="c", concentrate_scale=True), endog, exog=exog)
...
def test_hannan_rissanen_with_fixed_params(ar_order, ma_order, fixed_params):
    ...
    desired_p, _ = hannan_rissanen(
        endog, ar_order=ar_order, ma_order=ma_order, demean=False, fixed_params=fixed_params
    )
