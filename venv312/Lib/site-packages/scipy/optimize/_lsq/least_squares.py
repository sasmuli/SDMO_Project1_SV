...
def check_tolerance(ftol, xtol, gtol, method):
    ...
    if method == "lm" and (ftol < EPS or xtol < EPS or gtol < EPS):
        raise ValueError(
            "All tolerances must be higher than machine epsilon " f"({EPS:.2e}) for method 'lm'."
        )
    elif ftol < EPS and xtol < EPS and gtol < EPS:
        raise ValueError(
            "At least one of the tolerances must be higher than " f"machine epsilon ({EPS:.2e})."
        )
...
def least_squares(
    ...
    if loss not in IMPLEMENTED_LOSSES and not callable(loss):
        raise ValueError(f"`loss` must be one of {IMPLEMENTED_LOSSES.keys()}" " or a callable.")
    ...
    if f0.ndim != 1:
        raise ValueError("`fun` must return at most 1-d array_like. " f"f0.shape: {f0.shape}")
    ...
    if callable(jac):
        ...
        if method == "lm":
            ...
            if jac != "2-point":
                warn(f"jac='{jac}' works equivalently to '2-point' for method='lm'.", stacklevel=2)
