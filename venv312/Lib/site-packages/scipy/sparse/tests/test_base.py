    ...
    def deco(func, *a, **kw):
        ...
        try:
            for mod in modules:
                ...
                if downcast_maxval is not None:
                    backup.append(
                        (mod, "downcast_intp_index", getattr(mod, "downcast_intp_index", None))
                    )
...
class _TestCommon:
    ...
    def test_round(self):
        ...
        assert_equal(
            np.around(A, decimals=decimal), round(self.spcreator(A), ndigits=decimal).toarray()
        )
    ...
    def test_reshape(self):
        ...
        for order in ["C", "F"]:
            for s in [(12, 1), (1, 12)]:
                assert_array_equal(
                    x.reshape(s, order=order).toarray(), x.toarray().reshape(s, order=order)
                )
    ...
    def test_mean(self):
        ...
        def check(dtype):
            ...
            assert_array_almost_equal(dat.mean(axis=None, keepdims=keep), datsp.mean(axis=None))
            assert_equal(dat.mean(axis=None).dtype, datsp.mean(axis=None).dtype)
            assert_array_almost_equal(dat.mean(axis=0, keepdims=keep), datsp.mean(axis=0))
            assert_equal(dat.mean(axis=0).dtype, datsp.mean(axis=0).dtype)
            assert_array_almost_equal(dat.mean(axis=1, keepdims=keep), datsp.mean(axis=1))
            assert_equal(dat.mean(axis=1).dtype, datsp.mean(axis=1).dtype)
            assert_array_almost_equal(dat.mean(axis=-2, keepdims=keep), datsp.mean(axis=-2))
        ...
        def check():
            ...
            for protocol in range(pickle.HIGHEST_PROTOCOL):
                ...
                for key, val in datsp.__dict__.items():
                    ...
                    elif isinstance(val, tuple) and val and isinstance(val[0], np.ndarray):
                        assert_array_equal(val, sploaded.__dict__[key])
                    else:
...
class _TestFancyIndexing:
    ...
    def test_fancy_indexing(self):
        ...
        assert_equal(
            A[array([[-1], [-3], [-2]]), [2, -4]].toarray(), B[[[-1], [-3], [-2]], [2, -4]]
        )
        assert_equal(
            A[[[-1], [-3], [-2]], array([2, -4])].toarray(), B[[[-1], [-3], [-2]], [2, -4]]
        )
        assert_equal(
            A[array([[-1], [-3], [-2]]), array([2, -4])].toarray(), B[[[-1], [-3], [-2]], [2, -4]]
        )
        ...
        assert_equal(A[1, [[-1, -3]]][[[0, -1]], 1].toarray(), B[1, [[-1, -3]]][[[0, -1]], 1])
        # [:1,[[1,2]]][[[1,2]],:1]
...
class _TestMinMax:
    ...
    def test_minmax_axis(self):
        ...
        for axis in axes_odd + axes_even:
            assert_array_equal(X.max(axis=axis).toarray(), D.max(axis=axis, keepdims=keep))
            assert_array_equal(X.min(axis=axis).toarray(), D.min(axis=axis, keepdims=keep))
...
class TestCOO(
    sparse_test_class(
        getset=False, slicing=False, slicing_assign=False, fancy_indexing=False, fancy_assign=False
    )
):
    spcreator = coo_array
    math_dtypes = [np.int_, np.float64, np.complex128]
