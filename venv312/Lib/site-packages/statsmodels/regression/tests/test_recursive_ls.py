...
def test_ols():
    ...
    scale_alternative = (
        np.sum(
            (res.standardized_forecasts_error[0, 1:] * res.filter_results.obs_cov[0, 0] ** 0.5) ** 2
        )
        / mod.nobs
    )
...
def test_estimates():
    ...
    assert_allclose(
        res.recursive_coefficients.filtered[:, 9:20].T, results_R.iloc[7:18][["beta1", "beta2"]]
    )
    assert_allclose(
        res.recursive_coefficients.filtered[:, 19:].T, results_R.iloc[17:][["beta1", "beta2"]]
    )
...
def test_resid_recursive():
    ...
    assert_allclose(res.resid_recursive[3:], results_stata.iloc[3:]["rr"], atol=1e-5, rtol=1e-5)
