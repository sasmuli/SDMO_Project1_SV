...
class TestStatesAR3:
    ...
    def test_predict_obs(self):
        assert_almost_equal(
            self.results.filter_results.predict().forecasts[0], self.stata.iloc[1:]["dep1"], 4
        )
    ...
    def test_smoothed_measurement_disturbance_cov(self):
        ...
        assert_almost_equal(
            res.smoothed_measurement_disturbance_cov[0].T, self.matlab_ssm[["epsvar"]], 4
        )

    def test_smoothed_state_disturbance(self):
        assert_almost_equal(
            self.results.smoother_results.smoothed_state_disturbance.T, self.matlab_ssm[["eta"]], 4
        )
...
class TestStatesAR3AlternativeSmoothing(TestStatesAR3):
    @classmethod
    def setup_class(cls, *args, **kwargs):
        super().setup_class(smooth_method=SMOOTH_ALTERNATIVE, *args, **kwargs)
    ...
    def test_smoothed_states_cov(self):
        assert_almost_equal(
            self.results.det_smoothed_state_cov.T[1:], self.matlab_ssm.iloc[1:][["detV"]], 4
        )
...
class TestStatesAR3UnivariateSmoothing(TestStatesAR3):
    @classmethod
    def setup_class(cls, *args, **kwargs):
        super().setup_class(filter_method=FILTER_UNIVARIATE, *args, **kwargs)
...
class TestStatesMissingAR3:
    ...
    def test_predicted_states(self):
        assert_almost_equal(
            self.results.predicted_state[:, :-1].T, self.matlab_ssm[["a1", "a2", "a3"]], 4
        )
    ...
    def test_smoothed_measurement_disturbance_cov(self):
        assert_almost_equal(
            self.results.smoothed_measurement_disturbance_cov[0].T, self.matlab_ssm[["epsvar"]], 4
        )
    ...
    def test_smoothed_state_disturbance_cov(self):
        ...
        # )
        assert_almost_equal(
            self.results.smoothed_state_disturbance_cov[0, 0, :], self.R_ssm["detVeta"], 9
        )
...
class TestMultivariateMissing:
    ...
    def test_forecasts_error_cov(self):
        assert_allclose(
            self.results.forecasts_error_cov.diagonal(), self.desired[["F1", "F2", "F3"]]
        )
    ...
    def test_smoothed_states(self):
        assert_allclose(
            self.results.smoothed_state.T, self.desired[["alphahat1", "alphahat2", "alphahat3"]]
        )
    ...
    def test_smoothed_forecasts(self):
        assert_allclose(
            self.results.smoothed_forecasts.T, self.desired[["muhat1", "muhat2", "muhat3"]]
        )
...
class TestMultivariateMissingClassicalSmoothing(TestMultivariateMissing):
    @classmethod
    def setup_class(cls, *args, **kwargs):
        super().setup_class(smooth_method=SMOOTH_CLASSICAL, *args, **kwargs)
...
class TestMultivariateVAR:
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        mod["obs_cov"] = np.array(
            [[0.0000640649, 0.0, 0.0], [0.0, 0.0000572802, 0.0], [0.0, 0.0, 0.0017088585]]
        )
    ...
    def test_scaled_smoothed_estimator(self):
        assert_allclose(
            self.results.scaled_smoothed_estimator.T, self.desired[["r1", "r2", "r3"]], atol=1e-4
        )
    ...
    def test_forecasts(self):
        assert_allclose(self.results.forecasts.T, self.desired[["m1", "m2", "m3"]], atol=1e-6)

    def test_forecasts_error(self):
        assert_allclose(self.results.forecasts_error.T[:, 0], self.desired["v1"], atol=1e-6)

    def test_forecasts_error_cov(self):
        assert_allclose(
            self.results.forecasts_error_cov.diagonal()[:, 0], self.desired["F1"], atol=1e-6
        )

    def test_predicted_states(self):
        assert_allclose(
            self.results.predicted_state[:, 1:].T, self.desired[["a1", "a2", "a3"]], atol=1e-6
        )
    ...
    def test_smoothed_state_disturbance_cov(self):
        assert_allclose(
            self.results.det_smoothed_state_disturbance_cov.T, self.desired[["detVeta"]], atol=1e-18
        )
...
class TestMultivariateVARUnivariate:
    ...
    def test_forecasts(self):
        assert_allclose(self.results.forecasts.T[:, 0], self.desired["m1"], atol=1e-6)

    def test_forecasts_error(self):
        assert_allclose(self.results.forecasts_error.T, self.desired[["v1", "v2", "v3"]], atol=1e-6)
    ...
    def test_predicted_states(self):
        assert_allclose(
            self.results.predicted_state[:, 1:].T, self.desired[["a1", "a2", "a3"]], atol=1e-8
        )
...
class TestVARAutocovariances:
    ...
    def setup_class(cls, which="mixed", *args, **kwargs):
        ...
        mod["design"] = np.eye(3)
        mod["obs_cov"] = np.array(
            [[609.0746647855, 0.0, 0.0], [0.0, 1.8774916622, 0.0], [0.0, 0.0, 124.6768281675]]
        )
        mod["transition"] = np.array(
            [
...
def test_smoothed_state_autocovariances_backwards_oos(missing, filter_univariate, tvp):
    ...
    # mod_oos.nobs = mod.nobs + 5, we need to pass in 5 additional time points,
    # and that is what extend_kwargs, above, does.
    acov1 = res.smoothed_state_autocovariance(1, end=end, extend_kwargs=extend_kwargs).transpose(
        2, 0, 1
    )
    ...
    acov2 = res.smoothed_state_autocovariance(2, end=end, extend_kwargs=extend_kwargs).transpose(
        2, 0, 1
    )
    assert_allclose(acov2[2:, :2, :2], desired_acov2[2:])
    ...
    # cases
    acov3 = res.smoothed_state_autocovariance(3, end=end, extend_kwargs=extend_kwargs).transpose(
        2, 0, 1
    )
    assert_allclose(acov3[3:, :2, :2], desired_acov3[3:])
...
def test_news_invalid(missing, filter_univariate, tvp):
    ...
    if tvp:
        msg = (
            "Cannot compute the impacts of news on periods outside of the"
