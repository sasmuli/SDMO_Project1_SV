...
def linear_sum_assignment_assertions(solver, array_type, sign, test_case):
    ...
    assert_array_equal(row_ind, np.sort(row_ind))
    assert_array_equal(
        np.sort(expected_cost), np.sort(np.array(cost_matrix[row_ind, col_ind])).flatten()
    )
...
def test_min_weight_full_matching_small_inputs(sign, test_case):
    linear_sum_assignment_assertions(min_weight_full_bipartite_matching, csr_array, sign, test_case)
