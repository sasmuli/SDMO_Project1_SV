...
def test_arma_kwargs():
    ...
    assert_equal(
        res1_imle.arma_results[1].minimize_results.message, "Optimization terminated successfully."
    )

    # Now specify a different method (L-BFGS-B)
    arma_estimator_kwargs = {"minimize_kwargs": {"method": "L-BFGS-B"}}
    _, res2_imle = gls(
        endog, exog=exog, order=(1, 0, 1), n_iter=1, arma_estimator_kwargs=arma_estimator_kwargs
    )
