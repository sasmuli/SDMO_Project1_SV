...
class Expr:
    ...
    def __call__(self, *args, **kwargs):
        ...
        return as_apply(self, *map(as_expr, args), **{k: as_expr(v) for k, v in kwargs.items()})
    ...
    def traverse(self, visit, *args, **kwargs):
        ...
        if self.op in (Op.INTEGER, Op.REAL, Op.STRING, Op.SYMBOL):
            ...
            return normalize(
                Expr(self.op, tuple(item.traverse(visit, *args, **kwargs) for item in self.data))
            )
            ...
            kwoperands = {k: v.traverse(visit, *args, **kwargs) for k, v in self.data[2].items()}
...
def as_apply(func, *args, **kwargs):
    ...
    return Expr(
        Op.APPLY, (func, tuple(map(as_expr, args)), {k: as_expr(v) for k, v in kwargs.items()})
    )
...
class _FromStringWorker:
    ...
    def process(self, s, context="expr"):
        ...
        operands = re.split(
            r"(?<=[@\w\d_])\s*([*]|/)",
            (r if self.language is Language.C else r.replace("**", "@__f2py_DOUBLE_STAR@")),
        )
        if len(operands) > 1:
        ...
        m = re.match(
            r"\A({digit_string})({kind}|)\Z".format(digit_string=r"\d+", kind=r"_(\d+|\w[\w\d_]*)"),
            r,
        )
        if m:
