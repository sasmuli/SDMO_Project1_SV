    ...
    if not xties and not yties and method == "exact":
        ...
        if use_ties:
            ...
            if n > 2:
                ...
                ) * np.sum([v * k * (k - 1) * (k - 2) for (k, v) in yties.items()], dtype=float)
                v2 /= 9.0 * n * (n - 1) * (n - 2)
            else:
                v2 = 0
        else:
        ...
        raise ValueError(
            "Unknown method " + str(method) + " specified, please " "use auto, exact or asymptotic."
        )
...
def kendalltau_seasonal(x):
    ...
    for j in range(m):
        ...
        for k in range(j, m, 1):
            K[j, k] = sum(
                msign((x[i:, j] - x[i, j]) * (x[i:, k] - x[i, k])).sum() for i in range(n)
            )
            covmat[j, k] = (
                K[j, k] + 4 * (R[:, j] * R[:, k]).sum() - n * (n_p[j] + 1) * (n_p[k] + 1)
            ) / 3.0
            K[k, j] = K[j, k]
...
def trimmed_stde(a, limits=(0.1, 0.1), inclusive=(1, 1), axis=None):
    ...
    if axis is None:
        ...
        if a.ndim > 2:
            raise ValueError(
                "Array 'a' must be at most two dimensional, " "but got a.ndim = %d" % a.ndim
            )
...
def winsorize(
    a, limits=None, inclusive=(True, True), inplace=False, axis=None, nan_policy="propagate"
):
...
def skewtest(a, axis=0, alternative="two-sided"):
    ...
    if np.min(n) < 8:
        raise ValueError(
            "skewtest is not valid with less than 8 samples; %i samples" " were given." % np.min(n)
        )
