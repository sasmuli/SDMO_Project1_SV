...
def check_update_indices(news, updates_index):
    ...
    # Otherwise, check that the revision indexes are correct
        ...
        desired_iloc = desired_ix.copy()
        desired_iloc["update date"] = [dates.get_loc(date) for date in desired_ix["update date"]]
...
def check_news(
    ...
    # Total impacts
    total_impacts = news.revision_impacts.astype(float).fillna(0) + news.update_impacts.astype(
        float
    ).fillna(0)
    ...
    if updates:
        ...
        actual = (
            news.details_by_impact["observed"].drop_duplicates().reset_index([0, 1])["observed"]
        )
...
def test_sarimax_time_invariant(revisions, updates, revisions_details_start):
    ...
    post_impacted_forecasts = np.r_[endog2.iloc[-2], 0.5 ** np.arange(3) * endog2.iloc[-1]].reshape(
        4, 1
    )
    ...
    if updates:
        ...
        news_desired = [
            update_realized[i] - update_forecasts[i] for i in range(len(update_forecasts))
        ]
...
def test_sarimax_time_varying(revisions, updates, which):
    ...
    news1 = res1.news(
        endog2, exog=exog2, start="2008Q1", end="2009Q3", comparison_type=comparison_type
    )
...
def test_varmax_time_invariant(revisions, updates):
    ...
    if revisions and updates:
        revisions_index = pd.MultiIndex.from_product(
            [endog1.index[-1:], ["realgdp", "unemp"]], names=["revision date", "revised variable"]
        )
        # If we have updates, the revision is to 2009Q2
    ...
    if updates:
        updates_index = pd.MultiIndex.from_product(
            [pd.period_range(start="2009Q3", periods=1, freq="Q"), ["realgdp", "unemp"]],
            names=["update date", "updated variable"],
        )
    ...
    if updates:
        ...
        columns = pd.MultiIndex.from_product(
            [impact_dates, impacted_variables], names=["impact dates", "impacted variables"]
        )
...
def test_detailed_revisions(revisions_details_start):
    ...
    assert_allclose(
        revision_details.loc[key, "impact"], ar_coeff["realgdp"] * revisions[("2009Q2", "realgdp")]
    )
    ...
    for name in ["cpi", "realcons", "realinv"]:
        assert_allclose(
            news.impacts.loc[("2009Q3", name), "estimate (new)"], y_revised.loc["2009Q3", name]
        )
        assert_allclose(
    ...
    assert_allclose(
        news.impacts["impact of revisions"], revision_details.groupby(level=[2, 3]).sum()["impact"]
    )
...
def test_grouped_revisions(revisions_details_start):
    ...
    assert_allclose(
        revision_details.loc[key + ("realcons",), "impact"], revisions[("2009Q3", "realcons")]
    )
