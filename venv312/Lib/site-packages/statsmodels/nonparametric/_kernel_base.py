...
def _compute_subset(
    class_type, data, bw, co, do, n_cvars, ix_ord, ix_unord, n_sub, class_vars, randomize, bound
):
    ...
    if class_type == "KDEMultivariate":
        ...
        sub_model = KDEMultivariateConditional(
            endog, exog, dep_type, indep_type, bw=bw, defaults=EstimatorSettings(efficient=False)
        )
...
class GenericKDE:
    ...
    def _cv_ml(self):
        ...
        bw = optimize.fmin(
            self.loo_likelihood, x0=h0, args=(np.log,), maxiter=1e3, maxfun=1e3, disp=0, xtol=1e-3
        )
