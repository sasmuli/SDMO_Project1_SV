...
def test_cont_basic_fit_cases():
    ...
    assert len(xslow_fit_mle.union(xfail_fit_mle).union(skip_fit_mle)) == len(xslow_fit_mle) + len(
        xfail_fit_mle
    ) + len(skip_fit_mle)
    assert len(xslow_fit_mm.union(xfail_fit_mm).union(skip_fit_mm)) == len(xslow_fit_mm) + len(
        xfail_fit_mm
    ) + len(skip_fit_mm)
...
def cases_test_moments():
    ...
    for distname, arg in distcont[:] + histogram_test_instances:
        ...
        if distname in xslow_test_moments:
            yield pytest.param(
                distname, arg, True, True, True, True, marks=pytest.mark.xslow(reason="too slow")
            )
            continue
...
@pytest.mark.parametrize(
    "distname,arg,normalization_ok,higher_ok,moment_ok," "is_xfailing", cases_test_moments()
)
...
def test_broadcasting_in_moments_gh12192_regression():
    ...
    expected3 = np.array(
        [[np.nan, np.nan, 4.39365368], [np.nan, np.nan, 5.75994241], [np.nan, np.nan, 6.78730736]]
    )
...
def test_kappa3_array_gh13582():
    ...
    moments = "mvsk"
    res = np.array(
        [[stats.kappa3.stats(shape, moments=moment) for shape in shapes] for moment in moments]
    )
...
def test_kappa4_array_gh13582():
    ...
    moments = "mvsk"
    res = np.array(
        [[stats.kappa4.stats(h[i], k[i], moments=moment) for i in range(5)] for moment in moments]
    )
    ...
    res = np.array(
        [[stats.kappa4.stats(h[i], k[i], moments=moment) for i in range(6)] for moment in moments]
    )
    res2 = np.array(stats.kappa4.stats(h, k, moments=moments))
    npt.assert_allclose(res, res2)
