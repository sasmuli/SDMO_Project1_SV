...
class MarkovAutoregression(markov_regression.MarkovRegression):
    ...
    def _em_iteration(self, params0):
        ...
        coeffs = None
        if self._k_exog > 0:
            coeffs = self._em_exog(
                result, self.endog, self.exog, self.parameters.switching["exog"], tmp
            )
            for i in range(self.k_regimes):
        ...
        if self.order > 0:
            if self._k_exog > 0:
                ...
                ar_coeffs = self._em_exog(
                    result, self.endog, self.exog_ar, self.parameters.switching["autoregressive"]
                )
    ...
    def start_params(self):
        ...
        if self.order > 0:
            if np.any(self.switching_ar):
                for i in range(self.k_regimes):
                    params[self.parameters[i, "autoregressive"]] = beta[self._k_exog :] * (
                        i / self.k_regimes
                    )
            else:
        ...
        # Variance
        if self.switching_variance:
            params[self.parameters["variance"]] = np.linspace(
                variance / 10.0, variance, num=self.k_regimes
            )
        else:
    ...
    def param_names(self):
        ...
        param_names = np.array(
            markov_regression.MarkovRegression.param_names.fget(self), dtype=object
        )
...
wrap.populate_wrapper(MarkovAutoregressionResultsWrapper, MarkovAutoregressionResults)  # noqa:E305
