...
class CombineResults:
    ...
    def summary_array(self, alpha=0.05, use_t=None):
        ...
        res = np.column_stack(
            [self.eff, self.sd_eff, ci_low, ci_upp, self.weights_rel_fe, self.weights_rel_re]
        )
    ...
    def plot_forest(self, alpha=0.05, use_t=None, use_exp=False, ax=None, **kwds):
        ...
        fig = dot_plot(
            points=res_df["eff"], intervals=hw, lines=res_df.index, line_order=res_df.index, **kwds
        )
        return fig
...
def combine_effects(
    effect, variance, method_re="iterated", row_names=None, use_t=False, alpha=0.05, **kwds
):
