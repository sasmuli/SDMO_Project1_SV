...
class Conf:
    ...
    def _check_only_one_from_commit(self) -> None:
        if not self.only_one_filter(
            ...
            raise Exception(
                "You can only specify one filter between since, since_as_filter, from_tag and from_commit"
            )

    def _check_only_one_to_commit(self) -> None:
        if not self.only_one_filter([self.get("to"), self.get("to_commit"), self.get("to_tag")]):
    ...
    def sanity_check_filters(self) -> None:
        ...
        if (
            self.get("from_commit")
            and self.get("to_commit")
            and self.get("from_commit") == self.get("to_commit")
        ):
            logger.warning(
                "You should not point from_commit and to_commit to the same "
    ...
    def _is_commit_before(commit_before: Commit, commit_after: Commit) -> bool:
        if commit_before.committer_date < commit_after.committer_date:
            return True
        if (
            commit_before.committer_date == commit_after.committer_date
            and commit_before.author_date < commit_after.author_date
        ):
            return True
        return False
    ...
    def get_starting_commit(self) -> Optional[List[str]]:
        ...
        if from_commit is not None:
            try:
                ...
                raise Exception(
                    "The commit {} defined in the 'from_tag' or 'from_commit' filter does "
    ...
    def get_ending_commit(self) -> Optional[str]:
        ...
        if to_commit is not None:
            try:
                ...
                raise Exception(
                    "The commit {} defined in the 'to_tag' or 'to_commit' filter does "
    ...
    def is_commit_filtered(self, commit: Commit):
        ...
        if self.get("only_commits") is not None and commit.hash not in self.get("only_commits"):
            ...
            return True
        if self.get("filepath_commits") is not None and commit.hash not in self.get(
            "filepath_commits"
        ):
