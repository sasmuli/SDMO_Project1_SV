...
class TestEinsum:
    ...
    def test_einsum_errors(self, do_opt, einsum_fn):
        ...
        assert_raises(ValueError, einsum_fn, "ij->jij", [[0, 0], [0, 0]], optimize=do_opt)

        # dimensions must match when being collapsed
        assert_raises(ValueError, einsum_fn, "ii", np.arange(6).reshape(2, 3), optimize=do_opt)
        assert_raises(ValueError, einsum_fn, "ii->i", np.arange(6).reshape(2, 3), optimize=do_opt)
    ...
    def test_einsum_sorting_behavior(self):
        ...
        assert all(c.isupper() for c in output_indices1), (
            "Output indices for n=26 should use uppercase letters only: " f"{output_indices1}"
        )
        assert_equal(
            output_indices1,
            "".join(sorted(output_indices1)),
            err_msg=(
                "Output indices for n=26 are not lexicographically sorted: " f"{output_indices1}"
            ),
        )
        ...
        assert any(c.islower() for c in output_indices2), (
            "Output indices for n=27 should include uppercase letters: " f"{output_indices2}"
        )
    ...
    def test_einsum_specific_errors(self, do_opt):
        ...
        assert_raises(
            ValueError, np.einsum, "i->i", np.arange(6).reshape(-1, 1), optimize=do_opt, order="d"
        )
    ...
    def test_einsum_object_errors(self):
        ...
        b = np.array([DestructoBox(i, 100) for i in range(27)], dtype="object").reshape(3, 3, 3)
    ...
    def check_einsum_sums(self, dtype, do_opt=False):
        ...
        for n in range(1, 17):
            ...
            assert_equal(
                np.einsum(a, [Ellipsis], b, [Ellipsis], optimize=do_opt), np.multiply(a, b)
            )
        ...
        for n in range(1, 17):
            ...
            assert_equal(
                np.einsum(a, [Ellipsis, 0], b, [Ellipsis, 0], optimize=do_opt), np.inner(a, b)
            )
        ...
        for n in range(1, 11):
            ...
            assert_equal(
                np.einsum(a, [0, Ellipsis], b, [0, Ellipsis], optimize=do_opt), np.inner(a.T, b.T).T
            )
        ...
        with suppress_warnings() as sup:
            ...
            for n in range(1, 17):
                ...
                c[...] = 0
                np.einsum(
                    a.T, [1, 0], b.T, [1], out=c, dtype="f8", casting="unsafe", optimize=do_opt
                )
                assert_equal(c, np.dot(b.T.astype("f8"), a.T.astype("f8")).astype(dtype))
            ...
            for n in range(1, 17):
                ...
                c[...] = 0
                np.einsum(
                    a, [0, 1], b, [1, 2], out=c, dtype="f8", casting="unsafe", optimize=do_opt
                )
                assert_equal(c, np.dot(a.astype("f8"), b.astype("f8")).astype(dtype))
            ...
            if dtype != "f2":
                ...
                assert_equal(
                    np.einsum(a, [0, 1], b, [1, 2], c, [2, 3], optimize=do_opt), a.dot(b).dot(c)
                )
            ...
            if np.dtype(dtype) != np.dtype("f2"):
                ...
                assert_equal(
                    np.einsum("ijk, jil -> kl", a, b), np.tensordot(a, b, axes=([1, 0], [0, 1]))
                )
                assert_equal(
                ...
                assert_equal(
                    c,
                    np.tensordot(a.astype("f8"), b.astype("f8"), axes=([1, 0], [0, 1])).astype(
                        dtype
                    ),
                )
                c[...] = 0
        ...
        for n in range(1, 25):
            ...
            if np.dtype(dtype).itemsize > 1:
                ...
                assert_equal(
                    np.einsum("...,...", a[1:], a[:-1], optimize=do_opt), np.multiply(a[1:], a[:-1])
                )
                assert_equal(
                    np.einsum("i,i", a[1:], a[:-1], optimize=do_opt), np.dot(a[1:], a[:-1])
                )
        ...
        q = np.ones((1, 2))
        assert_array_equal(
            np.einsum("ij,ij->j", p, q, optimize=True), np.einsum("ij,ij->j", p, q, optimize=False)
        )
        ...
        for optimize in (True, False):
            ...
            assert_array_equal(
                np.einsum("...ij,...jk->...ik", p, q, optimize=optimize), np.full((1, 5), 1.25)
            )
