...
def _wrapper_grad(grad, fun=None, args=(), finite_diff_options=None):
    ...
    if callable(grad):
        ...
        def wrapped1(x, f0=None):
            ncalls[0] += 1
            return approx_derivative(fun, x, f0=f0, **finite_diff_options)
...
def _wrapper_hess(hess, grad=None, x0=None, args=(), finite_diff_options=None):
    if callable(hess):
        ...
        def wrapped1(x, f0=None):
            return approx_derivative(grad, x, f0=f0, **finite_diff_options)
...
class ScalarFunction:
    ...
    def __init__(
        self, fun, x0, args, grad, hess, finite_diff_rel_step, finite_diff_bounds, epsilon=None
    ):
        ...
        self._wrapped_grad, self._ngev = _wrapper_grad(
            grad, fun=self._wrapped_fun, args=args, finite_diff_options=finite_diff_options
        )
        self._update_grad()

        # Hessian evaluation
        if callable(hess):
            self._wrapped_hess, self._nhev, self.H = _wrapper_hess(hess, x0=x0, args=args)
            self.H_updated = True
        elif hess in FD_METHODS:
            self._wrapped_hess, self._nhev, self.H = _wrapper_hess(
                hess, grad=self._wrapped_grad, x0=x0, finite_diff_options=finite_diff_options
            )
            self._update_grad()
            ...
            if sparse_jacobian or sparse_jacobian is None and sps.issparse(self.J):
                ...
                def update_jac():
                    ...
                    self.J = sps.csr_matrix(
                        approx_derivative(fun_wrapped, self.x, f0=self.f, **finite_diff_options)
                    )
