...
class Worksheet(_WorkbookChild):
    ...
    def __getitem__(self, key):
        ...
        if min_col is None:
            rows = tuple(
                self.iter_rows(
                    min_col=min_col, min_row=min_row, max_col=self.max_column, max_row=max_row
                )
            )
            if min_row == max_row:
                rows = rows[0]
        ...
        return tuple(
            self.iter_rows(min_row=min_row, min_col=min_col, max_row=max_row, max_col=max_col)
        )
    ...
    def _invalid_row(self, iterable):
        raise TypeError(
            "Value must be a list, tuple, range or generator, or a dict. Supplied value is {0}".format(
                type(iterable)
            )
        )
