...
class IntervalArray(IntervalMixin, ExtensionArray):
    ...
    def _ensure_simple_new_inputs(
        ...
        if isinstance(left, ABCDatetimeIndex) and str(left.tz) != str(right.tz):
            msg = (
                "left and right must have the same time zone, got " f"'{left.tz}' and '{right.tz}'"
            )
            raise ValueError(msg)
    ...
    def is_non_overlapping_monotonic(self) -> bool:
        ...
        return bool(
            (self._right[:-1] <= self._left[1:]).all() or (self._left[:-1] >= self._right[1:]).all()
        )
    ...
    def __arrow_array__(self, type=None):
        ...
        try:
            ...
            raise TypeError(
                f"Conversion to arrow with subtype '{self.dtype.subtype}' " "is not supported"
            ) from err
    ...
    def _combined(self) -> IntervalSide:
        ...
        if needs_i8_conversion(left.dtype):
            ...
            comb = left._concat_same_type([left, right], axis=1)  # type: ignore[union-attr]
        else:
